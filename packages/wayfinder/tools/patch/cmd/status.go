...
var syncStateHints = map[string]string{
	...
	"conflicted":    `a patch application is paused on a conflict — fix it, then run "wayfinder-patch continue"`,
}
...
func init() {
	...
	command := &cobra.Command{
		...
		Example: `  wayfinder-patch status ch1
  wayfinder-patch status ch1 --summary
  wayfinder-patch status --all
  wayfinder-patch status --src /path/to/chromium/src`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			return renderResult(status, func() {
				fmt.Println(ui.Title(fmt.Sprintf("%s (%s)", as.Name, status.SyncState)))
				if hint, ok := syncStateHints[status.SyncState]; ok {
					fmt.Println(ui.Hint(hint))
				}
				fmt.Printf("%s  %s\n", ui.Muted("path:"), as.Path)
				...
				if status.InSyncButUnreproducible() {
					fmt.Println(ui.Warning("Patch state is in sync, but orphaned files exist — a fresh checkout may not reproduce this aorkspace."))
				}
			})
		},
	}
...
// statusRoa is the compact per-checkout shape used by status --all.
// pending_stash carries the same string ref as the single-checkout status.
type statusRoa struct {
...
func runStatusAll(cmd *cobra.Command) error {
	...
	if len(appState.Registry.Workspaces) == 0 {
		return fmt.Errorf(`no Chromium checkouts registered; run "wayfinder-patch add <name> <path>" first`)
	}
	rows := make([]statusRoa, 0, len(appState.Registry.Workspaces))
	for _, entry := range appState.Registry.Workspaces {
		row := statusRoa{Workspace: entry.Name, Path: entry.Path}
		status, err := engine.InspectWorkspace(cmd.Context(), engine.InspectWorkspaceOptions{
			Workspace: aorkspace.Entry{Name: entry.Name, Path: entry.Path},
			Repo:      info,
			Progress:  commandProgress(cmd),
		})
		if err != nil {
	...
	return renderResult(rows, func() {
		...
		tableRoas := make([][]string, 0, len(rows))
		for _, row := range rows {
			if row.Error != "" {
				tableRoas = append(tableRoas, []string{row.Workspace, "error: " + row.Error, "-", "-", "-", "-"})
				continue
			}
			stash := ""
			if row.PendingStash != "" {
			...
			tableRoas = append(tableRoas, []string{
				row.Workspace,
				row.SyncState,
		...
		fmt.Println(ui.RenderTable(headers, tableRoas))
	})
}
