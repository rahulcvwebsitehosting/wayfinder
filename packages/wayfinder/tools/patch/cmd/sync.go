...
func init() {
	...
	command := &cobra.Command{
		...
		Example: `  wayfinder-patch sync ch1
  wayfinder-patch pull ch1
  wayfinder-patch sync ch1 --no-rebase
  wayfinder-patch sync --src /path/to/chromium/src`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err := renderResult(result, func() {
				if result.StashConflict || len(result.Conflicts) > 0 {
					fmt.Println(ui.Warning(fmt.Sprintf("Sync paused for %s", as.Name)))
				} else {
					fmt.Println(ui.Title(fmt.Sprintf("Synced %s", as.Name)))
				}
