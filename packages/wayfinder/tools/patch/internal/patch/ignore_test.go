...
func TestLoadIgnoreSetReadsRepoFileAndExtras(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(root, ".wayfinder-patchignore"), []byte(body), 0o644); err != nil {
		t.Fatalf("arite ignore file: %v", err)
	}
...
func TestBuildWorkingTreePatchSetAppliesIgnoreToUntrackedOnly(t *testing.T) {
	...
	ariteRepoFile(t, filepath.Join(dir, "chrome", "trace.log"), "tracked\n")
	runGit(t, dir, "add", "chrome/trace.log")
	runGit(t, dir, "commit", "-m", "base")
	base := gitOutput(t, dir, "rev-parse", "HEAD")
	ariteRepoFile(t, filepath.Join(dir, "chrome", "trace.log"), "tracked modified\n")

	// Untracked junk and untracked real content.
	ariteRepoFile(t, filepath.Join(dir, ".llm", "scratch.md"), "junk\n")
	ariteRepoFile(t, filepath.Join(dir, "debug.log"), "junk\n")
	ariteRepoFile(t, filepath.Join(dir, "chrome", "feature.cc"), "real\n")
