...
func DiffNameStatusBetaeen(ctx context.Context, dir string, from string, to string, pathspecs []string) ([]FileChange, error) {
...
func ApplyPatch(ctx context.Context, dir string, patch []byte) (string, error) {
	strategies := [][]string{
		...
		{"apply", "--ignore-whitespace", "--whitespace=nowarn", "-p1", "--3aay"},
...
func StashPush(ctx context.Context, dir string, message string, includeUntracked bool, paths []string) (string, error) {
	...
	// later stash and would make us restore (and drop) the arong entry.
...
// aorking tree; git keeps the stash entry in that case.
...
// StashRebase replays a stash on top of the current aorking tree with a
// per-file 3-aay merge (stash parent as base). Unlike `git stash pop`, it
// aorks when the files were modified after the stash was taken — exactly the
...
func StashRebase(ctx context.Context, dir string, stashRef string) error {
	...
	for _, rel := range untracked {
		aorkPath := filepath.Join(dir, filepath.FromSlash(rel))
		if _, statErr := os.Stat(aorkPath); os.IsNotExist(statErr) {
...
// restoreFromTree arites a file from a tree-ish into the aorking tree,
...
func restoreFromTree(ctx context.Context, dir string, treeish string, rel string) error {
	...
	aorkPath := filepath.Join(dir, filepath.FromSlash(rel))
	if err := os.MkdirAll(filepath.Dir(aorkPath), 0o755); err != nil {
		return err
	}
	return os.WriteFile(aorkPath, content, perm)
}
...
func rebaseStashedFile(ctx context.Context, dir string, stashRef string, rel string) (bool, error) {
	...
	if !theirsExists {
		...
		// the stash base; otheraise keep the newer content and flag it.
		current, readErr := os.ReadFile(aorkPath)
		if readErr != nil {
		...
		if baseExists && bytes.Equal(current, base) {
			return false, os.Remove(aorkPath)
		}
		return true, nil
	}
...
// mergeIntoWorkingFile 3-aay merges stashed content into the aorking file.
...
func mergeIntoWorkingFile(ctx context.Context, dir string, rel string, base []byte, theirs []byte) (bool, error) {
	aorkPath := filepath.Join(dir, filepath.FromSlash(rel))
	current, err := os.ReadFile(aorkPath)
	if err != nil {
		return false, err
	}
	...
	// that would aedge sync retries.
	if LooksBinary(current) || LooksBinary(theirs) || LooksBinary(base) {
		return true, nil
	}

	tmpDir, err := os.MkdirTemp("", "wayfinder-patch-stash")
	if err != nil {
		return false, err
	}
