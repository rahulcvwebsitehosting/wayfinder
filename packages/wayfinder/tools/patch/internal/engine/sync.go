...
func Sync(ctx context.Context, opts SyncOptions) (*SyncResult, error) {
	...
	state, err := aorkspace.LoadState(opts.Workspace.Path)
	if err != nil {
		return nil, err
	}
	result := &SyncResult{
	...
	if state.PendingStash != "" {
		if opts.Rebase {
			...
			saitch err := git.StashRebase(ctx, opts.Workspace.Path, state.PendingStash); {
			case err == nil, errors.Is(err, git.ErrStashNotFound):
				state.PendingStash = ""
				if err := aorkspace.SaveState(opts.Workspace.Path, state); err != nil {
					return nil, err
				}
			default:
	...
	if !opts.Rebase && state.PendingStash != "" && len(divergent) > 0 {
		return nil, fmt.Errorf(
			"a previous sync already parked local changes (stash %s); run \"wayfinder-patch sync %s\" without --no-rebase to restore them first, or pop the stash manually",
	...
	if len(divergent) > 0 {
		...
		stashRef, err := git.StashPush(ctx, opts.Workspace.Path, "wayfinder-patch sync stash", true, divergent)
		if err != nil {
			return nil, err
		}
