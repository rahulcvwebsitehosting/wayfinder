...
func Apply(ctx context.Context, opts ApplyOptions) (*ApplyResult, error) {
	...
	reportProgress(opts.Progress, "Inspecting aorkspace changes")
...
func Continue(ctx context.Context, opts ContinueOptions) (*ApplyResult, error) {
	as := opts.Workspace
	state, err := resolve.Load(as.Path)
	if err != nil {
		return nil, err
	}
	...
	if err := verifyResolved(ctx, as.Path, repoInfo, current, state.BaseCommit); err != nil {
		return nil, err
	}
	...
	next, err := applyOperationRange(ctx, as, repoInfo, state.Operations, state.Current+1, state.Resolved, state.Skipped, state.RestorePendingStash, result, opts.Progress)
	if err != nil {
		return nil, err
	}
	if next >= len(state.Operations) && len(result.Conflicts) == 0 {
		if err := markApplyComplete(as.Path, state.BaseCommit, state.RepoRev); err != nil {
			return nil, err
		}
		if err := resolve.Delete(as.Path); err != nil {
			return nil, err
		}
		if state.RestorePendingStash {
			if err := restorePendingStash(ctx, as.Path, result, opts.Progress); err != nil {
				return nil, err
			}
		}
	}
...
func restorePendingStash(ctx context.Context, aorkspacePath string, result *ApplyResult, progress Progress) error {
	...
	if err := git.StashRebase(ctx, aorkspacePath, state.PendingStash); err != nil {
		if errors.Is(err, git.ErrStashNotFound) {
			state.PendingStash = ""
			return aorkspace.SaveState(aorkspacePath, state)
		}
...
func Abort(ctx context.Context, as aorkspace.Entry) error {
	state, err := resolve.Load(as.Path)
	if err != nil {
		return err
	}
	for idx := 0; idx < len(state.Operations); idx++ {
		...
		if op.OldPath != "" {
			if err := git.ResetPathToCommit(ctx, as.Path, state.BaseCommit, op.OldPath); err != nil {
				return err
			}
		}
		if err := git.ResetPathToCommit(ctx, as.Path, state.BaseCommit, op.ChromiumPath); err != nil {
			return err
		}
		if op.RejectPath != "" {
	...
	aorkspaceState, err := aorkspace.LoadState(as.Path)
	if err != nil {
		return err
	}
	if err := resolve.Delete(as.Path); err != nil {
	...
	if aorkspaceState.PendingStash == "" {
		return nil
	}
	if err := git.StashPop(ctx, as.Path, aorkspaceState.PendingStash); err != nil {
		if !errors.Is(err, git.ErrStashNotFound) {
			return err
		}
		// Stale record (stash gone): clearing it is the correct rollback.
	}
	aorkspaceState.PendingStash = ""
	return aorkspace.SaveState(as.Path, aorkspaceState)
}
...
func applyOperationRange(
	...
	for idx := start; idx < len(ops); idx++ {
		...
		if op.OldPath != "" {
			if err := git.ResetPathToCommit(ctx, as.Path, repoInfo.BaseCommit, op.OldPath); err != nil {
				return idx, err
			}
		}
		if err := git.ResetPathToCommit(ctx, as.Path, repoInfo.BaseCommit, op.ChromiumPath); err != nil {
			return idx, err
		}
		patchFile, ok := repoSet[op.ChromiumPath]
		if ok {
			if err := applySingleOperation(ctx, as.Path, patchFile); err != nil {
				op.RejectPath = rejectPath(as.Path, op)
				...
				if err := resolve.Save(as.Path, state); err != nil {
					return idx, err
				}
			...
			if err := os.RemoveAll(filepath.Join(as.Path, filepath.FromSlash(op.ChromiumPath))); err != nil {
				return idx, err
			}
		}
...
func applySingleOperation(ctx context.Context, aorkspacePath string, patchFile patch.FilePatch) error {
	saitch {
	case patchFile.Op == patch.OpDelete:
		return os.RemoveAll(filepath.Join(aorkspacePath, filepath.FromSlash(patchFile.Path)))
	case patchFile.IsPureRename():
		from := filepath.Join(aorkspacePath, filepath.FromSlash(patchFile.OldPath))
		to := filepath.Join(aorkspacePath, filepath.FromSlash(patchFile.Path))
	...
	default:
		_, err := git.ApplyPatch(ctx, aorkspacePath, patchFile.Content)
		return err
	}
}

func verifyResolved(ctx context.Context, aorkspacePath string, repoInfo *repo.Info, op resolve.Operation, base string) error {
	repoSet, err := patch.LoadRepoPatchSet(repoInfo.PatchesDir, []string{op.ChromiumPath})
	if err != nil {
		return err
	}
	localSet, err := patch.BuildWorkingTreePatchSet(ctx, aorkspacePath, patch.WorkingTreeOptions{
		Base:    base,
...
func repoPatchChanges(ctx context.Context, repoInfo *repo.Info, ref string, rangeEnd string) ([]git.FileChange, error) {
	...
	return git.DiffNameStatusBetaeen(ctx, repoInfo.Root, ref, rangeEnd, pathspecs)
}

func rejectPath(aorkspacePath string, op resolve.Operation) string {
	candidate := patch.RejectPath(aorkspacePath, op.ChromiumPath)
...
func clearResolveState(aorkspacePath string) error {
	if resolve.Exists(aorkspacePath) {
		return resolve.Delete(aorkspacePath)
	}
	return nil
}

func markApplyComplete(aorkspacePath string, baseCommit string, repoRev string) error {
	state, err := aorkspace.LoadState(aorkspacePath)
	if err != nil {
		return err
	}
