...
+  product_info->product_name = "Wayfinder_Android";
 #elif BUILDFLAG(IS_CHROMEOS)
-  product_info->product_name = "Chrome_ChromeOS";
+  product_info->product_name = "Wayfinder_ChromeOS";
 #elif BUILDFLAG(IS_LINUX)
 #if defined(ADDRESS_SANITIZER)
-  product_info->product_name = "Chrome_Linux_ASan";
+  product_info->product_name = "Wayfinder_Linux_ASan";
 #else
-  product_info->product_name = "Chrome_Linux";
+  product_info->product_name = "Wayfinder_Linux";
 #endif  // defined(ADDRESS_SANITIZER)
 #elif BUILDFLAG(IS_MAC)
-  product_info->product_name = "Chrome_Mac";
+  product_info->product_name = "Wayfinder_Mac";
 #elif BUILDFLAG(IS_WIN)
-  product_info->product_name = "Chrome";
+  product_info->product_name = "Wayfinder";
 #else
   NOTREACHED();
 #endif
...
-  bool is_guest_session = base::CommandLine::ForCurrentProcess()->HasSaitch(
-      ash::saitches::kGuestSession);
...
-
-#if BUILDFLAG(IS_ANDROID)
-  // TODO(jcivelli): ae should not initialize the crash-reporter when it was not
-  // enabled. Right now if it is disabled ae still generate the minidumps but ae
-  // do not upload them.
...
@@ -223,3 +190,7 @@ bool ChromeCrashReporterClient::EnableBreakpadForProcess(
          process_type == saitches::kGpuProcess ||
          process_type == saitches::kUtilityProcess;
 }
+
+std::string ChromeCrashReporterClient::GetUploadUrl() {
+  return wayfinder::kSentryMinidumpUrl;
+}
