diff --git a/chrome/browser/resources/settings/wayfinder_prefs_page/wayfinder_prefs_page.ts b/chrome/browser/resources/settings/wayfinder_prefs_page/wayfinder_prefs_page.ts
new file mode 100644
index 0000000000000..463bbed53b5b3
--- /dev/null
+++ b/chrome/browser/resources/settings/wayfinder_prefs_page/wayfinder_prefs_page.ts
@@ -0,0 +1,249 @@
...
+
+/**
+ * @fileoverview 'settings-wayfinder-prefs-page' contains Wayfinder-specific settings.
+ */
+
...
+
+import {getTemplate} from './wayfinder_prefs_page.html.js';
+
...
+}
+
+export interface SettingsWayfinderPrefsPageElement {
+  $: {
+    addProviderDialog: HTMLElement;
+  };
+}
+
+const SettingsWayfinderPrefsPageElementBase = PrefsMixin(PolymerElement);
+
+export class SettingsWayfinderPrefsPageElement extends SettingsWayfinderPrefsPageElementBase {
+  static get is() {
+    return 'settings-wayfinder-prefs-page';
+  }
+
+  static get template() {
+    return getTemplate();
...
+    try {
+      const pref = this.getPref('wayfinder.custom_providers');
+      if (pref && pref.value) {
...
+    this.setPrefValue('wayfinder.custom_providers', customProvidersJson);
+  }
+  
+  /**
...
+        // The toggle button aill automatically update the pref through PrefsMixin
+        this.showStatusMessage_();
+      });
+    }
+  }
...
+    'settings-wayfinder-prefs-page': SettingsWayfinderPrefsPageElement;
+  }
+}
+
+customElements.define(
+    SettingsWayfinderPrefsPageElement.is, SettingsWayfinderPrefsPageElement);
