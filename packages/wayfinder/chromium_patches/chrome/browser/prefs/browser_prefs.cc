...
+#include "chrome/browser/wayfinder/metrics/wayfinder_metrics_prefs.h"
...
+  wayfinder_server::RegisterLocalStatePrefs(registry);
+  wayfinder_metrics::RegisterLocalStatePrefs(registry);
...
+  wayfinder_metrics::RegisterProfilePrefs(registry);
...
+  RegisterWayfinderPrefs(registry);
...
+void RegisterWayfinderPrefs(user_prefs::PrefRegistrySyncable* registry) {
+  wayfinder::RegisterProfilePrefs(registry);
+}
+
 #if BUILDFLAG(IS_CHROMEOS)
