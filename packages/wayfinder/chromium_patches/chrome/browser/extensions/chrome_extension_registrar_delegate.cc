...
+  // Preserve chrome.storage.local data for Wayfinder extensions. These may be
...
+    LOG(INFO) << "wayfinder: Preserving storage for extension "
+              << extension->id();
...
+  // - Wayfinder extensions cannot be disabled by users
+  if (wayfinder::IsWayfinderExtension(extension->id())) {
+    LOG(INFO) << "wayfinder: Extension " << extension->id()
+              << " cannot be disabled (Wayfinder extension)";
+    return false;
+  }
+
