...
+SidePanelWayfinderToggleFunction::RunFunction() {
+  EXTENSION_FUNCTION_VALIDATE(extension());
+
+  std::optional<api::side_panel::WayfinderToggle::Params> params =
+      api::side_panel::WayfinderToggle::Params::Create(args());
...
+      service->WayfinderToggleSidePanelForTab(
...
+  }
+
+  api::side_panel::WayfinderToggleResult result;
+  result.opened = toggle_result.value();
+  return RespondNow(WithArguments(result.ToValue()));
+}
+
+ExtensionFunction::ResponseAction
+SidePanelWayfinderIsOpenFunction::RunFunction() {
+  EXTENSION_FUNCTION_VALIDATE(extension());
+
+  std::optional<api::side_panel::WayfinderIsOpen::Params> params =
+      api::side_panel::WayfinderIsOpen::Params::Create(args());
+  EXTENSION_FUNCTION_VALIDATE(params);
+
+  SidePanelService* service = GetService();
+  base::expected<bool, std::string> is_open_result =
+      service->WayfinderIsSidePanelOpenForTab(
+          *extension(), browser_context(), params->options.tab_id,
