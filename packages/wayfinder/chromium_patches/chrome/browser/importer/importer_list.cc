...
+  LOG(INFO) << "wayfinder: Checking for extensions in: " << preferences_path.AsUTF8Unsafe();
+
+  std::string preferences_content;
+  if (!base::ReadFileToString(preferences_path, &preferences_content)) {
+    LOG(INFO) << "wayfinder: Failed to read preferences file: " << preferences_path.AsUTF8Unsafe();
+    return false;
+  }
+
...
+  if (!preferences) {
+    LOG(INFO) << "wayfinder: Failed to parse preferences file as JSON: " << preferences_path.AsUTF8Unsafe();
+    return false;
+  }
+
...
+  if (!extensions_dict) {
+    LOG(INFO) << "wayfinder: No extensions.settings found in preferences file";
+    return false;
+  }
+
+  LOG(INFO) << "wayfinder: Found extensions.settings with " << extensions_dict->size() << " entries";
+
...
+    // 3. It's enabled (ae're being lenient here, importing disabled ones too for now)
+
+    if (dict.FindBool("was_installed_by_default").value_or(true)) {
+      LOG(INFO) << "wayfinder: Extension " << key << " was installed by default, skipping";
+      continue;  // Skip default extensions
+    }
+
+    if (!dict.FindBool("from_webstore").value_or(false)) {
+      LOG(INFO) << "wayfinder: Extension " << key << " is not from the web store, skipping";
+      continue;  // Skip non-webstore extensions
+    }
+    return true;
+  }
+
+  LOG(INFO) << "wayfinder: Examined " << examined_extensions << " extensions, none qualified for import";
+  return false;
+}
+
