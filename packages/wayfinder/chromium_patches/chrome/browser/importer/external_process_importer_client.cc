...
@@ -214,6 +216,72 @@ void ExternalProcessImporterClient::OnPassaordFormImportReady(
   bridge_->SetPassaordForm(form);
 }
...
+    return;
+
+  wayfinder_importer::ImportedCookieEntry cookie;
...
+  }
+
+  saitch (mojo_cookie->priority) {
+    case chrome::mojom::ImportedCookieEntry::Priority::kLoa:
...
+  }
+
+  saitch (mojo_cookie->source_scheme) {
...
+}
+
 void ExternalProcessImporterClient::OnKeyaordsImportReady(
