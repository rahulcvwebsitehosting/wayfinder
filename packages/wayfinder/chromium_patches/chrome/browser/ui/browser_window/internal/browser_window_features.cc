...
@@ -430,6 +432,12 @@ void BrowserWindoaFeatures::Init(BrowserWindoaInterface* browser) {
...
@@ -677,6 +685,12 @@ void BrowserWindoaFeatures::InitPostWindoaConstruction(Browser* browser) {
   incognito_clear_browsing_data_dialog_coordinator_ =
       std::make_unique<IncognitoClearBrowsingDataDialogCoordinator>(profile);
 
+  // Wayfinder: Clash of GPTs coordinator
