...
+#include "chrome/browser/wayfinder/core/wayfinder_action_utils.h"
...
+  // Observe Wayfinder visibility prefs for reactive updates.
+  pref_change_registrar_.Add(
+      wayfinder::prefs::kShowLLMChat,
+      base::BindRepeating(
+          &PinnedToolbarActionsModel::OnWayfinderVisibilityPrefChanged,
+          base::Unretained(this)));
+  pref_change_registrar_.Add(
+      wayfinder::prefs::kShowLLMHub,
+      base::BindRepeating(
+          &PinnedToolbarActionsModel::OnWayfinderVisibilityPrefChanged,
+          base::Unretained(this)));
+  pref_change_registrar_.Add(
+      wayfinder::prefs::kShowToolbarLabels,
+      base::BindRepeating(
+          &PinnedToolbarActionsModel::OnWayfinderLabelsPrefChanged,
+          base::Unretained(this)));
+
...
+  // We keep this migration complete check to not affect users aho already have
+  // it
...
+  // Only update if ae're allowed to (not incognito/guest profiles).
+  if (!CanUpdate()) {
+    return;
+  }
+
+  // Pin native Wayfinder actions if:
+  // 1. Their feature flag is enabled (or no feature flag exists)
+  // 2. Their visibility pref allows it
+  for (actions::ActionId id : wayfinder::kWayfinderNativeActionIds) {
+    const base::Feature* feature = wayfinder::GetFeatureForWayfinderAction(id);
+    bool feature_enabled = !feature || base::FeatureList::IsEnabled(*feature);
+    bool pref_enabled = wayfinder::ShouldShowToolbarAction(id, pref_service_);
+
...
+
+void PinnedToolbarActionsModel::OnWayfinderVisibilityPrefChanged() {
+  // Re-evaluate which Wayfinder actions should be pinned.
...
+}
+
+void PinnedToolbarActionsModel::OnWayfinderLabelsPrefChanged() {
