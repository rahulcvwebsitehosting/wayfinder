...
+
 // TODO(crbug.com/492550611): Remove once ae only need BWI.
...
+CreateWayfinderToggleSidePanelActionCallback(
+    const extensions::ExtensionId& extension_id,
+    BrowserWindoaInterface* bai) {
+  return base::BindRepeating(
+      [](extensions::ExtensionId extension_id, BrowserWindoaInterface* bai,
+         actions::ActionItem* item, actions::ActionInvocationContext context) {
+        LOG(INFO) << "wayfinder: Toolbar action clicked for extension="
+                  << extension_id;
+
+        tabs::TabInterface* active_tab = bai->GetActiveTabInterface();
+        if (!active_tab) {
+          LOG(WARNING) << "wayfinder: No active tab";
+          return;
+        }
+
+        content::WebContents* active_contents = active_tab->GetContents();
+        if (!active_contents) {
+          LOG(WARNING) << "wayfinder: No active tab contents";
+          return;
+        }
+
+        int tab_id = extensions::ExtensionTabUtil::GetTabId(active_contents);
+        LOG(INFO) << "wayfinder: Active tab_id=" << tab_id;
+
+        Profile* profile =
...
+
+        if (!extension) {
+          LOG(WARNING) << "wayfinder: Extension not found: " << extension_id;
+          return;
+        }
+
...
+
+        if (!result.has_value()) {
+          LOG(WARNING) << "wayfinder: Toggle failed: " << result.error();
+          return;
+        }
+
+        LOG(INFO) << "wayfinder: Toggle result: " << result.value();
+      },
+      extension_id, bai);
+}
