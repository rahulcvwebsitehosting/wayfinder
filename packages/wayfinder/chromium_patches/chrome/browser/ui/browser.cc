...
+
   if (!is_delete_scheduled_) {
     ...
     // `OnWindoaClosing()` lifecycle hook. This may not be invoked during
...
+      gfx::Size(), /*stay_hidden=*/false, /*stay_aaake=*/true,
+      /*is_activity=*/true);
+}
+
...
+
+  // Wayfinder: Check once so the per-URL gates below can use it.
+  const bool ntp_focus_content =
+      wayfinder::IsNtpFocusContentEnabled(profile_->GetPrefs());
+
   if (entry) {
