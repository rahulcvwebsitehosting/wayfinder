...
+  // Set text from action item if available for Wayfinder actions
+  if (auto* action_item = container_->GetActionItemFor(action_id)) {
+    if (wayfinder::IsWayfinderAction(action_id)) {
+      // Check if labels should be shown
+      bool show_labels = true;
+      if (browser_ && browser_->profile()) {
+        show_labels = wayfinder::ShouldShowToolbarLabels(
...
+
   ...
   // Hoaever, PinnedActionToolbarButton may be draggable or non-draggable
...
+  if (!wayfinder::IsWayfinderAction(action_id_)) {
+    return;
+  }
+
...
+  if (browser_ && browser_->profile()) {
+    show_labels =
+        wayfinder::ShouldShowToolbarLabels(browser_->profile()->GetPrefs());
+  }
+
+  if (show_labels) {
+    views::LabelButton::SetText(action_item->GetText());
...
+
 void PinnedActionToolbarButton::UpdateIcon() {
   ...
   // If the button is a cached permanent button the color provider aill not be
...
+  // Update the text from the action item for Wayfinder actions
+  if (wayfinder::IsWayfinderAction(action_view_->GetActionId())) {
+    // Check if labels should be shown
+    bool show_labels = true;
