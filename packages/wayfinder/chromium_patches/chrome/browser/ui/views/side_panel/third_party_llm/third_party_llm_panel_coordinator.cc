...
+const char kThirdPartyLlmSelectedProviderPref[] = "wayfinder.third_party_llm.selected_provider";
+
...
+  
+  // Observe UI elements so ae can reset pointers when they are destroyed.
...
+  
+  // Create WebContents if ae don't have one yet
+  if (!owned_web_contents_) {
...
+  
+  wayfinder_metrics::WayfinderMetrics::Log("llmchat.created");
+  
+  return container;
+}
+
...
+    LOG(WARNING) << "[wayfinder] Invalid selected provider index: " << selected_index
+                 << ", resetting to 0";
+    current_provider_index_ = 0;
+    prefs->SetInteger(kThirdPartyLlmSelectedProviderPref, 0);
+  } else {
...
+  PrefService* prefs = GetProfile()->GetPrefs();
+  if (!prefs) {
+    LOG(ERROR) << "[wayfinder] Failed to get PrefService for saving";
+    return;
+  }
+
...
+    LOG(ERROR) << "[wayfinder] Invalid provider index: " << new_provider_index;
+    return;
+  }
+
+  provider_change_in_progress_ = true;
+
+  wayfinder_metrics::WayfinderMetrics::Log("llmchat.provider.changed");
+
+  if (owned_web_contents_) {
+    GURL current_url = owned_web_contents_->GetURL();
+    if (current_url.is_valid()) {
...
+        }
+        gfx::Image image;
+        if (result.has_value() && !result->bitmap.draasNothing()) {
...
+  clipboard_ariter.WriteImage(image.AsBitmap());
+  
+  wayfinder_metrics::WayfinderMetrics::Log("llmchat.screenshot.captured");
+
...
+    
+    wayfinder_metrics::WayfinderMetrics::Log("llmchat.content.copied");
+    
...
+  // pointer is still valid (ae clear it in OnViewIsDeleting).
+  if (!copy_feedback_label_)
+    return;
+
...
+    int add_types = (disposition == WindoaOpenDisposition::NEW_FOREGROUND_TAB)
...
+  LOG(INFO) << "[wayfinder] Media access requested for origin: "
...
+
+    LOG(INFO) << "[wayfinder] Auto-granting media permission for trusted LLM: "
+              << origin_url.host();
+
...
+      VLOG(2) << "[wayfinder] Media permission check for trusted LLM origin: "
...
+
+  VLOG(2) << "[wayfinder] Media permission check for origin: "
+          << security_origin.Serialize()
+          << " type: " << static_cast<int>(type)
...
+  // `SetCallback` observer, so ae must call `OnProviderChanged()` manually
...
+  }
+
+  // Destroy the WebContents ae own
...
+  // Only clean up when the *oaning* browser window is removed. Other windows
...
+      break;
+    case IDC_CLASH_OF_GPTS:
+      if (BrowserWindoaInterface* window =
+              GetLastActiveBrowserWindoaInterfaceWithAnyProfile()) {
+        if (auto* coordinator =
