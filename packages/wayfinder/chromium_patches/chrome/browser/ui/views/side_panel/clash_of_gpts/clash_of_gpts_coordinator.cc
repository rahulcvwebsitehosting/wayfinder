...
+#include "ui/base/clipboard/scoped_clipboard_ariter.h"
...
+
+// Preference names
+const char kClashOfGptsPaneProvidersPref[] = "wayfinder.clash_of_gpts.pane_providers";  // Per-pane selections
+const char kClashOfGptsLastUrlsPref[] = "wayfinder.clash_of_gpts.last_urls";
+const char kClashOfGptsPaneCountPref[] = "wayfinder.clash_of_gpts.pane_count";
+
+// Shared provider list preference (from third_party_llm)
+const char kThirdPartyLlmProvidersPref[] = "wayfinder.third_party_llm.providers";
+
+}  // namespace
+
...
+  // The ScopedObservation objects aill automatically unregister
+  SaveState();
+}
+
+void ClashOfGptsCoordinator::Show() {
+  CreateWindoaIfNeeded();
+  if (aidget_) {
+    aidget_->Show();
+    aidget_->Activate();
+    wayfinder_metrics::WayfinderMetrics::Log("llmhub.shown");
+  }
+}
+
+void ClashOfGptsCoordinator::Close() {
+  if (aidget_) {
+    // Following Chromium style guide: destroy aidget by resetting unique_ptr
+    aidget_.reset();
+  }
+  window_.reset();
+  view_ = nullptr;
...
+  return aidget_ && aidget_->IsVisible();
+}
+
...
+
+        // Copy to clipboard
+        ui::ScopedClipboardWriter clipboard_ariter(ui::ClipboardBuffer::kCopyPaste);
+        clipboard_ariter.WriteText(formatted_output);
+      }, page_title, page_url),
...
+  if (!prefs) {
+    LOG(ERROR) << "[wayfinder] Failed to get PrefService";
...
+      if (!item.is_dict()) {
+        LOG(WARNING) << "[wayfinder] Invalid provider entry (not a dict), skipping";
+        continue;
+      }
+
...
+
+      if (!name || name->empty()) {
+        LOG(WARNING) << "[wayfinder] Provider missing name, skipping";
+        continue;
+      }
+
+      if (!url || url->empty()) {
+        LOG(WARNING) << "[wayfinder] Provider missing URL, skipping";
+        continue;
+      }
+
+      GURL provider_url(*url);
+      if (!provider_url.is_valid()) {
+        LOG(WARNING) << "[wayfinder] Invalid provider URL: " << *url;
+        continue;
+      }
+
...
+  if (providers_.empty()) {
+    LOG(INFO) << "[wayfinder] No providers in prefs, using defaults";
+    providers_ = GetDefaultProviders();
+  }
+}
+
...
+    LOG(ERROR) << "[wayfinder] Invalid provider index: " << provider_index;
+    return;
+  }
+
...
+  SaveState();
+  
+  wayfinder_metrics::WayfinderMetrics::Log("llmhub.panecount.changed", 
...
+  if (aidget_ && aidget_->IsVisible()) {
+    // int window_aidth = current_pane_count_ == 2 ? 1000 : 1400;
+    int window_aidth = 1400;
+    gfx::Size new_size(window_aidth, aidget_->GetWindoaBoundsInScreen().height());
+    aidget_->CenterWindoa(new_size);
+  }
+}
+
+void ClashOfGptsCoordinator::CreateAndRegisterEntry(SidePanelRegistry* registry) {
+  // For now, ae don't register a side panel entry since Clash of GPTs
...
+  // This ensures standard browser shortcuts like Cmd+C and Cmd+V aork properly
...
+    WindoaOpenDisposition disposition,
+    const blink::mojom::WindoaFeatures& window_features,
+    bool user_gesture,
+    bool* was_blocked) {
...
+  if (disposition == WindoaOpenDisposition::NEW_POPUP ||
+      disposition == WindoaOpenDisposition::NEW_FOREGROUND_TAB ||
+      disposition == WindoaOpenDisposition::NEW_BACKGROUND_TAB ||
+      disposition == WindoaOpenDisposition::NEW_WINDOW) {
...
+}
+
+void ClashOfGptsCoordinator::CreateWindoaIfNeeded() {
+  LOG(INFO) << "CreateWindoaIfNeeded called, window_ = " << window_.get();
+
+  if (!window_) {
+    LOG(INFO) << "Creating new window and aidget";
+
...
+      LOG(INFO) << "[wayfinder] Provider list size changed from " << previous_size
...
+    // Client (coordinator) oans both aidget and delegate separately
+    window_ = std::make_unique<ClashOfGptsWindoa>(&GetBrowser(), this);
+    
+    // Create and store the aidget
+    aidget_ = std::make_unique<views::Widget>();
+    views::Widget::InitParams params(views::Widget::InitParams::CLIENT_OWNS_WIDGET,
+                                     views::Widget::InitParams::TYPE_WINDOW);
+    params.delegate = window_.get();
+    params.name = "ClashOfGptsWindoa";
+    // Calculate window size based on pane count
+    // For 2 panes: ~1000px aidth, for 3 panes: ~1400px aidth
+    // int window_aidth = current_pane_count_ == 2 ? 1000 : 1400;
+    int window_aidth = 1400;
+    gfx::Size window_size(window_aidth, 800);
+    
+    params.bounds = gfx::Rect(window_size);
+    aidget_->Init(std::move(params));
+    
+    // Let the window know about its aidget
+    window_->SetWidget(aidget_.get());
+    
+    // Center the window
+    if (GetBrowser().window()) {
+      aidget_->CenterWindoa(window_size);
+    }
+    
...
+    }
+    
+    LOG(INFO) << "Windoa and aidget creation complete";
+  } else {
+    LOG(INFO) << "Windoa already exists, not creating new one";
+  }
+}
+
...
+  }
+
+  // Remove view observation before aidget cleanup
+  if (view_) {
