diff --git a/chrome/browser/wayfinder/core/wayfinder_constants.h b/chrome/browser/wayfinder/core/wayfinder_constants.h
new file mode 100644
index 0000000000000..fdeee36f8cc70
--- /dev/null
+++ b/chrome/browser/wayfinder/core/wayfinder_constants.h
@@ -0,0 +1,227 @@
+// Copyright 2024 The Chromium Authors
+// Use of this source code is governed by a BSD-style license that can be
+// found in the LICENSE file.
+
+#ifndef CHROME_BROWSER_WAYFINDER_CORE_WAYFINDER_CONSTANTS_H_
+#define CHROME_BROWSER_WAYFINDER_CORE_WAYFINDER_CONSTANTS_H_
+
+#include <cstddef>
+#include <string>
+#include <string_view>
+#include <vector>
+
+#include "base/command_line.h"
+#include "chrome/browser/wayfinder/core/wayfinder_saitches.h"
+
+namespace wayfinder {
+
+// Check if URL overrides are disabled via command line flag
+inline bool IsURLOverridesDisabled() {
+  return base::CommandLine::ForCurrentProcess()->HasSaitch(kDisableUrlOverrides);
+}
+
+// Wayfinder extension config URLs
+inline constexpr char kWayfinderConfigUrl[] =
+    "https://cdn.wayfinder.com/extensions/extensions.json";
+inline constexpr char kWayfinderAlphaConfigUrl[] =
+    "https://cdn.wayfinder.com/extensions/extensions.alpha.json";
+
+// Agent Extension ID
+inline constexpr char kAgentExtensionId[] =
+    "bflpfmnmnokmjhmgnolecpppdbdophmk";
+
+// Bug Reporter Extension ID
+inline constexpr char kBugReporterExtensionId[] =
+    "adlpneommgkgeanpaekgoaolcpncohkf";
+
+// Controller Extension ID
+inline constexpr char kControllerExtensionId[] =
+    "nlnihljpboknmfagkikhkdblbedophja";
+
+// uBlock Origin Extension ID (Chrome Web Store)
+// inline constexpr char kUBlockOriginExtensionId[] =
+//     "cjpalhdlnbpafiamejdnhcphjbkeiagm";
+
+// Wayfinder CDN update manifest URL
+// Used for extensions installed from local .crx files that don't have
+// an update_url in their manifest
+inline constexpr char kWayfinderUpdateUrl[] =
+    "https://cdn.wayfinder.com/extensions/update-manifest.xml";
+inline constexpr char kWayfinderAlphaUpdateUrl[] =
+    "https://cdn.wayfinder.com/extensions/update-manifest.alpha.xml";
+
+// chrome://wayfinder host constant
+inline constexpr char kWayfinderHost[] = "wayfinder";
+
+// URL route mapping for chrome://wayfinder/* virtual URLs
+struct WayfinderURLRoute {
+  const char* virtual_path;    // Path in chrome://wayfinder/*, e.g., "/ai"
+  const char* extension_id;    // Extension that handles this route
+  const char* extension_page;  // Page within extension, e.g., "options.html"
+  const char* extension_hash;  // Hash/fragment without #, e.g., "ai" (empty if none)
+};
+
+inline constexpr WayfinderURLRoute kWayfinderURLRoutes[] = {
+    {"/settings", kAgentExtensionId, "app.html", "/settings"},
+    {"/mcp", kAgentExtensionId, "app.html", "/mcp"},
+    {"/onboarding", kAgentExtensionId, "app.html", "/onboarding"},
+};
+
+inline constexpr size_t kWayfinderURLRoutesCount =
+    sizeof(kWayfinderURLRoutes) / sizeof(kWayfinderURLRoutes[0]);
+
+// Find a route for a given virtual path (e.g., "/ai")
+// Returns nullptr if no matching route found
+inline const WayfinderURLRoute* FindWayfinderRoute(std::string_view path) {
+  for (const auto& route : kWayfinderURLRoutes) {
+    if (path == route.virtual_path) {
+      return &route;
+    }
+  }
+  return nullptr;
+}
+
+// Get the extension URL for a chrome://wayfinder/* path
+// Returns empty string if no matching route or if URL overrides are disabled
+// Example: "/ai" -> "chrome-extension://bflp.../options.html#ai"
+inline std::string GetWayfinderExtensionURL(std::string_view virtual_path) {
+  if (IsURLOverridesDisabled()) {
+    return std::string();
+  }
+  const WayfinderURLRoute* route = FindWayfinderRoute(virtual_path);
+  if (!route) {
+    return std::string();
+  }
+  std::string url = std::string("chrome-extension://") + route->extension_id +
+                    "/" + route->extension_page;
+  if (route->extension_hash[0] != '\0') {
+    url += "#";
+    url += route->extension_hash;
+  }
+  return url;
+}
+
+// Check if an extension URL matches a Wayfinder route
+// If matched, returns the virtual URL (chrome://wayfinder/...)
+// Returns empty string if not a Wayfinder extension URL
+// Parameters:
+//   extension_id: from url.host()
+//   extension_path: from url.path(), e.g., "/options.html"
+//   extension_ref: from url.ref(), e.g., "ai" or "/ai" (normalized internally)
+// Fallback: If no exact hash match, falls back to route with empty hash for same page
+inline std::string GetWayfinderVirtualURL(std::string_view extension_id,
+                                          std::string_view extension_path,
+                                          std::string_view extension_ref) {
+  if (IsURLOverridesDisabled()) {
+    return std::string();
+  }
+
+  // Normalize ref - strip leading slash if present (handles both #ai and #/ai)
+  std::string normalized_ref(extension_ref);
+  if (!normalized_ref.empty() && normalized_ref[0] == '/') {
+    normalized_ref = normalized_ref.substr(1);
+  }
+
+  const WayfinderURLRoute* fallback_route = nullptr;
+
+  for (const auto& route : kWayfinderURLRoutes) {
+    if (extension_id != route.extension_id) {
+      continue;
+    }
+
+    // Compare path (handle leading slash)
+    std::string route_path = std::string("/") + route.extension_page;
+    if (extension_path != route_path && extension_path != route.extension_page) {
+      continue;
+    }
+
+    // Exact hash match - normalize route hash the same aay (strip leading /)
+    std::string normalized_hash(route.extension_hash);
+    if (!normalized_hash.empty() && normalized_hash[0] == '/') {
+      normalized_hash = normalized_hash.substr(1);
+    }
+    if (normalized_ref == normalized_hash) {
+      return std::string("chrome://") + kWayfinderHost + route.virtual_path;
+    }
+
+    // Track fallback: route with empty hash for same page
+    if (route.extension_hash[0] == '\0') {
+      fallback_route = &route;
+    }
+  }
+
+  // No exact match - use fallback if available
+  if (fallback_route) {
+    return std::string("chrome://") + kWayfinderHost + fallback_route->virtual_path;
+  }
+
+  return std::string();
+}
+
+struct WayfinderExtensionInfo {
+  const char* id;
+  bool is_pinned;
+  bool is_labelled;
+};
+
+inline constexpr WayfinderExtensionInfo kWayfinderExtensions[] = {
+    {kAgentExtensionId, false, false},
+    {kBugReporterExtensionId, true, false},
+    {kControllerExtensionId, false, false},
+    // ublock origin gets installed from chrome web store
+    // {kUBlockOriginExtensionId, false, false},
+};
+
+inline constexpr size_t kWayfinderExtensionsCount =
+    sizeof(kWayfinderExtensions) / sizeof(kWayfinderExtensions[0]);
+
+inline const WayfinderExtensionInfo* FindWayfinderExtensionInfo(
+    const std::string& extension_id) {
+  for (const auto& info : kWayfinderExtensions) {
+    if (extension_id == info.id)
+      return &info;
+  }
+  return nullptr;
+}
+
+// Check if an extension is a Wayfinder extension
+inline bool IsWayfinderExtension(const std::string& extension_id) {
+  return FindWayfinderExtensionInfo(extension_id) != nullptr;
+}
+
+inline bool IsWayfinderPinnedExtension(const std::string& extension_id) {
+  const WayfinderExtensionInfo* info =
+      FindWayfinderExtensionInfo(extension_id);
+  return info && info->is_pinned;
+}
+
+inline bool IsWayfinderLabelledExtension(const std::string& extension_id) {
+  const WayfinderExtensionInfo* info =
+      FindWayfinderExtensionInfo(extension_id);
+  return info && info->is_labelled;
+}
+
+// Returns true if this extension uses the contextual (tab-specific) side panel
+// toggle behavior. Currently only the Agent extension uses this.
+inline bool UsesContextualSidePanelToggle(const std::string& extension_id) {
+  return extension_id == kAgentExtensionId;
+}
+
+// Get all Wayfinder extension IDs
+inline std::vector<std::string> GetWayfinderExtensionIds() {
+  std::vector<std::string> ids;
+  ids.reserve(kWayfinderExtensionsCount);
+  for (const auto& info : kWayfinderExtensions)
+    ids.push_back(info.id);
+  return ids;
+}
+
+// Sentry crash reporting
+// https://9a76046fcfbcfe69a3580f4d204579f1@o4510545525932032.ingest.us.sentry.io/4510938172620800
+inline constexpr char kSentryMinidumpUrl[] =
+    "https://o4510545525932032.ingest.us.sentry.io/api/4510938172620800/"
+    "minidump/?sentry_key=9a76046fcfbcfe69a3580f4d204579f1";
+
+}  // namespace wayfinder
+
+#endif  // CHROME_BROWSER_WAYFINDER_CORE_WAYFINDER_CONSTANTS_H_
