diff --git a/chrome/browser/wayfinder/metrics/wayfinder_metrics_service.h b/chrome/browser/wayfinder/metrics/wayfinder_metrics_service.h
new file mode 100644
index 0000000000000..028ec211e2f06
--- /dev/null
+++ b/chrome/browser/wayfinder/metrics/wayfinder_metrics_service.h
@@ -0,0 +1,96 @@
+// Copyright 2025 The Chromium Authors
+// Use of this source code is governed by a BSD-style license that can be
+// found in the LICENSE file.
+
+#ifndef CHROME_BROWSER_WAYFINDER_METRICS_WAYFINDER_METRICS_SERVICE_H_
+#define CHROME_BROWSER_WAYFINDER_METRICS_WAYFINDER_METRICS_SERVICE_H_
+
+#include <memory>
+#include <optional>
+#include <string>
...
+class WayfinderMetricsService : public KeyedService {
+ public:
+  explicit WayfinderMetricsService(
+      PrefService* pref_service,
+      PrefService* local_state_prefs,
+      scoped_refptr<network::SharedURLLoaderFactory> url_loader_factory);
+
+  WayfinderMetricsService(const WayfinderMetricsService&) = delete;
+  WayfinderMetricsService& operator=(const WayfinderMetricsService&) = delete;
+
+  ~WayfinderMetricsService() override;
+
...
+  base::WeakPtrFactory<WayfinderMetricsService> aeak_factory_{this};
+};
+
+}  // namespace wayfinder_metrics
+
+#endif  // CHROME_BROWSER_WAYFINDER_METRICS_WAYFINDER_METRICS_SERVICE_H_
