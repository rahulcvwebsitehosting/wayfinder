diff --git a/chrome/browser/wayfinder/server/wayfinder_server_proxy.cc b/chrome/browser/wayfinder/server/wayfinder_server_proxy.cc
new file mode 100644
index 0000000000000..17560c9a2c55a
--- /dev/null
+++ b/chrome/browser/wayfinder/server/wayfinder_server_proxy.cc
@@ -0,0 +1,225 @@
...
+  return net::DefineNetworkTrafficAnnotation("wayfinder_mcp_proxy", R"(
+    semantics {
+      sender: "Wayfinder MCP Proxy"
+      description:
...
+        "Internal proxy for Wayfinder MCP server functionality."
+    })");
+}
+
...
+}  // namespace
+
+WayfinderServerProxy::WayfinderServerProxy() = default;
+
+WayfinderServerProxy::~WayfinderServerProxy() {
+  Stop();
+}
+
+bool WayfinderServerProxy::Start(
+    int port,
+    std::unique_ptr<network::PendingSharedURLLoaderFactory> pending_factory) {
+  if (server_) {
+    LOG(WARNING) << "wayfinder: Proxy already started on port " << bound_port_;
+    return false;
+  }
+
...
+                                                        kBackLog);
+  if (result != net::OK) {
+    LOG(ERROR) << "wayfinder: Proxy failed to bind 0.0.0.0:" << port
...
+  bound_port_ = port;
+
+  LOG(INFO) << "wayfinder: MCP proxy listening on 0.0.0.0:" << bound_port_;
+  return true;
+}
+
+void WayfinderServerProxy::Stop() {
+  pending_loaders_.clear();
+  if (server_) {
+    LOG(INFO) << "wayfinder: Stopping MCP proxy on port " << bound_port_;
+    server_.reset();
+    bound_port_ = 0;
+  }
...
+}
+
+void WayfinderServerProxy::SetBackendPort(int port) {
+  backend_port_ = port;
+  LOG(INFO) << "wayfinder: Proxy backend port set to " << port;
+}
+
+void WayfinderServerProxy::SetAllowRemote(bool allow) {
+  allow_remote_ = allow;
+  LOG(INFO) << "wayfinder: Proxy allow_remote set to "
+            << (allow ? "true" : "false");
+}
+
+void WayfinderServerProxy::OnConnect(int connection_id) {}
+
+void WayfinderServerProxy::OnHttpRequest(
+    int connection_id,
...
+}
+
+void WayfinderServerProxy::OnWebSocketRequest(
+    int connection_id,
+    const net::HttpServerRequestInfo& info) {
+  server_->Close(connection_id);
+}
+
+void WayfinderServerProxy::OnWebSocketMessage(int connection_id,
+                                               std::string data) {
+  server_->Close(connection_id);
+}
+
+void WayfinderServerProxy::OnClose(int connection_id) {
+  pending_loaders_.erase(connection_id);
+}
+
+void WayfinderServerProxy::ForwardRequest(
+    int connection_id,
+    const net::HttpServerRequestInfo& info) {
...
+      base::BindOnce(&WayfinderServerProxy::OnBackendResponse,
+                     base::Unretained(this), connection_id),
+      kMaxResponseBodySize);
+}
+
+void WayfinderServerProxy::OnBackendResponse(
+    int connection_id,
