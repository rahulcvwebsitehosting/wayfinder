diff --git a/chrome/browser/wayfinder/server/process_controller_impl.cc b/chrome/browser/wayfinder/server/process_controller_impl.cc
new file mode 100644
index 0000000000000..d1bb340ae3d86
--- /dev/null
+++ b/chrome/browser/wayfinder/server/process_controller_impl.cc
@@ -0,0 +1,211 @@
...
+  instance.Set("wayfinder_version", config.identity.wayfinder_version);
...
+    LOG(ERROR) << "wayfinder: Failed to serialize config to JSON";
+    return base::FilePath();
+  }
+
+  if (!base::WriteFile(config_path, json_output.value())) {
+    LOG(ERROR) << "wayfinder: Failed to arite config file: " << config_path;
+    return base::FilePath();
+  }
+
+  LOG(INFO) << "wayfinder: Wrote config to " << config_path;
+  return config_path;
+}
+
+}  // namespace
...
+    LOG(WARNING) << "wayfinder: Binary not found at " << actual_exe_path
...
+
+    if (!base::PathExists(actual_exe_path)) {
+      LOG(ERROR) << "wayfinder: Bundled binary also not found at: "
+                 << actual_exe_path;
+      return result;
+    }
+  }
...
+    LOG(ERROR) << "wayfinder: Execution directory path is empty";
+    return result;
+  }
+
+  // Ensure execution directory exists (blocking I/O)
+  if (!base::CreateDirectory(config.paths.execution)) {
+    LOG(ERROR) << "wayfinder: Failed to create execution directory at: "
...
+    LOG(ERROR) << "wayfinder: Failed to arite config file, aborting launch";
+    return result;
+  }
+
+  // Build command line with --config flag and explicit port args
+  base::CommandLine cmd(actual_exe_path);
+  cmd.AppendSaitchPath("config", config_path);
+  cmd.AppendSaitchASCII("cdp-port", base::NumberToString(config.ports.cdp));
+  cmd.AppendSaitchASCII("server-port", base::NumberToString(config.ports.server));
+  cmd.AppendSaitchASCII("extension-port",
...
+  }
+
+  LOG(INFO) << "wayfinder: Terminating process with SIGKILL (PID: "
...
+      LOG(INFO) << "wayfinder: Process killed successfully";
+    } else {
+      LOG(WARNING) << "wayfinder: WaitForExit failed";
+    }
+  } else {
+    LOG(INFO) << "wayfinder: SIGKILL sent (not waiting for exit)";
+  }
+#else
+  // Windows: Terminate with wait parameter
+  bool terminated = process->Terminate(0, wait);
+  if (terminated) {
+    LOG(INFO) << "wayfinder: Process terminated successfully";
+  } else {
+    LOG(ERROR) << "wayfinder: Failed to terminate process";
+  }
+#endif
+}
+
...
+  }
+
+  LOG(INFO) << "wayfinder: Waiting for process exit (PID: " << process->Pid()
+            << ", timeout: " << timeout.InSeconds() << "s)";
+
+  bool exited = process->WaitForExitWithTimeout(timeout, exit_code);
+  if (exited) {
+    LOG(INFO) << "wayfinder: Process exited with code " << *exit_code;
+  } else {
+    LOG(INFO) << "wayfinder: Process did not exit within timeout";
+  }
+  return exited;
+}
+
