diff --git a/chrome/browser/wayfinder/server/wayfinder_appcast_parser_unittest.cc b/chrome/browser/wayfinder/server/wayfinder_appcast_parser_unittest.cc
new file mode 100644
index 0000000000000..ceb483dee35c6
--- /dev/null
+++ b/chrome/browser/wayfinder/server/wayfinder_appcast_parser_unittest.cc
@@ -0,0 +1,389 @@
...
+// =============================================================================
+
+TEST(WayfinderAppcastParserTest, ParsesValidAppcastWithSingleItem) {
...
+  )";
+
+  auto item = WayfinderAppcastParser::ParseLatestItem(kValidXml);
+
...
+}
+
+TEST(WayfinderAppcastParserTest, ParsesMultipleEnclosuresPerItem) {
...
+  )";
+
+  auto item = WayfinderAppcastParser::ParseLatestItem(kMultiPlatformXml);
+
+  ASSERT_TRUE(item.has_value());
...
+}
+
+TEST(WayfinderAppcastParserTest, ParseAllItems_ReturnsMultipleVersions) {
...
+  )";
+
+  auto items = WayfinderAppcastParser::ParseAllItems(kMultiVersionXml);
+
...
+// =============================================================================
+
+TEST(WayfinderAppcastParserTest, ReturnsNulloptForInvalidXml) {
+  auto item = WayfinderAppcastParser::ParseLatestItem("not valid xml at all");
+  EXPECT_FALSE(item.has_value());
+}
+
+TEST(WayfinderAppcastParserTest, ReturnsNulloptForEmptyString) {
+  auto item = WayfinderAppcastParser::ParseLatestItem("");
+  EXPECT_FALSE(item.has_value());
+}
+
+TEST(WayfinderAppcastParserTest, ReturnsNulloptForEmptyChannel) {
...
+  )";
+
+  auto item = WayfinderAppcastParser::ParseLatestItem(kEmptyXml);
+  EXPECT_FALSE(item.has_value());
+}
+
+TEST(WayfinderAppcastParserTest, ReturnsNulloptForMissingVersion) {
...
+  )";
+
+  auto item = WayfinderAppcastParser::ParseLatestItem(kNoVersionXml);
+  EXPECT_FALSE(item.has_value());
+}
+
+TEST(WayfinderAppcastParserTest, HandlesItemWithNoEnclosures) {
...
+  )";
+
+  auto item = WayfinderAppcastParser::ParseLatestItem(kNoEnclosureXml);
+
...
+}
+
+TEST(WayfinderAppcastParserTest, ParseAllItems_ReturnsEmptyForInvalidXml) {
+  auto items = WayfinderAppcastParser::ParseAllItems("invalid xml");
+  EXPECT_TRUE(items.empty());
+}
+
+TEST(WayfinderAppcastParserTest, ParseAllItems_ReturnsEmptyForEmptyChannel) {
+  const char kEmptyXml[] = R"(
+    <rss xmlns:sparkle="http://www.andymatuschak.org/xml-namespaces/sparkle">
+      <channel></channel>
+    </rss>
+  )";
+
+  auto items = WayfinderAppcastParser::ParseAllItems(kEmptyXml);
+  EXPECT_TRUE(items.empty());
+}
+
+// =============================================================================
...
+// =============================================================================
+
+TEST(WayfinderAppcastParserTest, HandlesWhitespaceInVersion) {
...
+  )";
+
+  auto item = WayfinderAppcastParser::ParseLatestItem(kWhitespaceXml);
+
...
+}
+
+TEST(WayfinderAppcastParserTest, HandlesZeroLengthEnclosure) {
...
+  )";
+
+  auto item = WayfinderAppcastParser::ParseLatestItem(kZeroLengthXml);
+
+  ASSERT_TRUE(item.has_value());
+  ASSERT_EQ(1u, item->enclosures.size());
...
+}
+
+TEST(WayfinderAppcastParserTest, HandlesMissingSignature) {
...
+  )";
+
+  auto item = WayfinderAppcastParser::ParseLatestItem(kNoSigXml);
+
+  ASSERT_TRUE(item.has_value());
+  ASSERT_EQ(1u, item->enclosures.size());
