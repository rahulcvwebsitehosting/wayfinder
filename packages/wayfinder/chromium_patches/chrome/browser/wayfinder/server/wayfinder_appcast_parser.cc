diff --git a/chrome/browser/wayfinder/server/wayfinder_appcast_parser.cc b/chrome/browser/wayfinder/server/wayfinder_appcast_parser.cc
new file mode 100644
index 0000000000000..dd30f75d414ec
--- /dev/null
+++ b/chrome/browser/wayfinder/server/wayfinder_appcast_parser.cc
@@ -0,0 +1,202 @@
...
+
+// static
+std::optional<AppcastItem> WayfinderAppcastParser::ParseLatestItem(
...
+
+// static
+std::vector<AppcastItem> WayfinderAppcastParser::ParseAllItems(
+    const std::string& xml) {
+  std::vector<AppcastItem> items;
+
+  XmlReader reader;
+  if (!reader.Load(xml)) {
+    LOG(WARNING) << "wayfinder: Failed to load appcast XML";
+    return items;
+  }
+
...
+        } else {
+          LOG(WARNING) << "wayfinder: Skipping invalid appcast item (no valid "
...
+  }
+
+  VLOG(1) << "wayfinder: Parsed " << items.size() << " appcast items";
+  return items;
+}
+
+}  // namespace wayfinder_server
