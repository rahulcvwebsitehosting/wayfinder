diff --git a/chrome/browser/wayfinder/server/wayfinder_server_proxy.h b/chrome/browser/wayfinder/server/wayfinder_server_proxy.h
new file mode 100644
index 0000000000000..586296d473849
--- /dev/null
+++ b/chrome/browser/wayfinder/server/wayfinder_server_proxy.h
@@ -0,0 +1,79 @@
+// Copyright 2024 The Chromium Authors
+// Use of this source code is governed by a BSD-style license that can be
+// found in the LICENSE file.
+
+#ifndef CHROME_BROWSER_WAYFINDER_SERVER_WAYFINDER_SERVER_PROXY_H_
+#define CHROME_BROWSER_WAYFINDER_SERVER_WAYFINDER_SERVER_PROXY_H_
+
+#include <memory>
+#include <optional>
+#include <string>
...
+class WayfinderServerProxy : public net::HttpServer::Delegate {
+ public:
+  WayfinderServerProxy();
+  ~WayfinderServerProxy() override;
+
+  WayfinderServerProxy(const WayfinderServerProxy&) = delete;
+  WayfinderServerProxy& operator=(const WayfinderServerProxy&) = delete;
+
+  // Bind proxy on the given port. |pending_factory| is a cloned factory
+  // that aill be bound on the current (IO) thread. Returns true on success.
+  bool Start(int port,
...
+}  // namespace wayfinder
+
+#endif  // CHROME_BROWSER_WAYFINDER_SERVER_WAYFINDER_SERVER_PROXY_H_
