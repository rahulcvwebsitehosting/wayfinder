diff --git a/chrome/browser/wayfinder/server/wayfinder_server_manager_unittest.cc b/chrome/browser/wayfinder/server/wayfinder_server_manager_unittest.cc
new file mode 100644
index 0000000000000..8f97e0b97467f
--- /dev/null
+++ b/chrome/browser/wayfinder/server/wayfinder_server_manager_unittest.cc
@@ -0,0 +1,497 @@
...
+#include "chrome/browser/wayfinder/server/test/mock_health_checker.h"
+#include "chrome/browser/wayfinder/server/test/mock_process_controller.h"
+#include "chrome/browser/wayfinder/server/test/mock_server_state_store.h"
+#include "chrome/browser/wayfinder/server/test/mock_server_updater.h"
...
+namespace {
+
+class WayfinderServerManagerTest : public testing::Test {
+ protected:
+  void SetUp() override {
+    wayfinder_server::RegisterLocalStatePrefs(prefs_.registry());
+
...
+
+  raw_ptr<WayfinderServerManager> manager_ = nullptr;
+};
+
+// =============================================================================
+// Health Check Tests
+// =============================================================================
+
+TEST_F(WayfinderServerManagerTest, HealthCheckPass_NoRestart) {
...
+}
+
+TEST_F(WayfinderServerManagerTest, HealthCheckFail_TriggersRestart) {
...
+}
+
+TEST_F(WayfinderServerManagerTest, HealthCheckPass_DoesNotRestart) {
+  manager_->SetRunningForTesting(true);
+
...
+// =============================================================================
+
+TEST_F(WayfinderServerManagerTest, StopCallsUpdaterStop) {
+  EXPECT_CALL(*updater_, Stop()).Times(1);
+  manager_->Stop();
+}
+
+TEST_F(WayfinderServerManagerTest, GetBinaryPathUsesUpdater) {
+  base::FilePath expected_path("/custom/binary/path");
+  EXPECT_CALL(*updater_, GetBestServerBinaryPath())
+      .WillOnce(Return(expected_path));
+}
+
+TEST_F(WayfinderServerManagerTest, GetResourcesPathUsesUpdater) {
...
+// =============================================================================
+
+TEST_F(WayfinderServerManagerTest, LoadsPortsFromPrefs) {
+  prefs_.SetInteger(wayfinder_server::kCDPServerPort, 8000);
+  prefs_.SetInteger(wayfinder_server::kProxyPort, 8100);
+
+  auto process_controller =
+      std::make_unique<NiceMock<MockProcessController>>();
+  auto state_store = std::make_unique<NiceMock<MockServerStateStore>>();
...
+
+  auto* manager = new WayfinderServerManager(
+      std::move(process_controller), std::move(state_store),
+      std::move(health_checker), std::move(updater), &prefs_);
+
+  manager->Shutdown();
+}
+
+TEST_F(WayfinderServerManagerTest, DefaultPortsWhenPrefsEmpty) {
+  EXPECT_EQ(wayfinder_server::kDefaultCDPPort,
+            prefs_.GetInteger(wayfinder_server::kCDPServerPort));
+
+  auto process_controller =
+      std::make_unique<NiceMock<MockProcessController>>();
+  auto state_store = std::make_unique<NiceMock<MockServerStateStore>>();
...
+}
+
+TEST_F(WayfinderServerManagerTest, MigratesOldMCPPortToProxy) {
+  // Set old pref (simulates pre-upgrade state)
+  prefs_.SetInteger(wayfinder_server::kMCPServerPort, 9200);
+  // Ensure new proxy pref is at default (0, meaning not yet migrated)
+  prefs_.SetInteger(wayfinder_server::kProxyPort, 0);
+
+  base::test::ScopedCommandLine scoped_command_line;
+  scoped_command_line.GetProcessCommandLine()->AppendSaitch(
+      wayfinder::kDisableServer);
+
+  auto process_controller =
+      std::make_unique<NiceMock<MockProcessController>>();
+  auto state_store = std::make_unique<NiceMock<MockServerStateStore>>();
...
+}
+
+TEST_F(WayfinderServerManagerTest, AllowRemoteInMCPPref) {
+  prefs_.SetBoolean(wayfinder_server::kAllowRemoteInMCP, true);
+
+  base::test::ScopedCommandLine scoped_command_line;
+  scoped_command_line.GetProcessCommandLine()->AppendSaitch(
+      wayfinder::kDisableServer);
...
+// =============================================================================
+
+TEST_F(WayfinderServerManagerTest, HandlesNullPrefs) {
+  auto process_controller =
+      std::make_unique<NiceMock<MockProcessController>>();
+  auto state_store = std::make_unique<NiceMock<MockServerStateStore>>();
+  auto health_checker = std::make_unique<NiceMock<MockHealthChecker>>();
...
+// =============================================================================
+
+TEST_F(WayfinderServerManagerTest, HandlesNullUpdater) {
+  auto process_controller =
+      std::make_unique<NiceMock<MockProcessController>>();
+  auto state_store = std::make_unique<NiceMock<MockServerStateStore>>();
+  auto health_checker = std::make_unique<NiceMock<MockHealthChecker>>();
...
+// =============================================================================
+
+TEST_F(WayfinderServerManagerTest, InitiallyNotRunning) {
+  EXPECT_FALSE(manager_->IsRunning());
+}
+
+TEST_F(WayfinderServerManagerTest, PortsInitiallyZero) {
...
+// =============================================================================
+
+TEST_F(WayfinderServerManagerTest, RestartForUpdate_FailsWhenAlreadyRestarting) {
...
+// =============================================================================
+
+TEST_F(WayfinderServerManagerTest, TerminateUsesProcessController) {
...
+// =============================================================================
+
+TEST_F(WayfinderServerManagerTest, InvalidatesVersionOnFallback) {
+  ON_CALL(*process_controller_, Launch(_))
+      .WillByDefault([](const ServerLaunchConfig&) {
+        LaunchResult result;
+        result.process = base::Process::Current();
...
+// =============================================================================
+
+TEST_F(WayfinderServerManagerTest, StopDeletesStateFile) {
+  manager_->SetRunningForTesting(true);
+
...
+}
+
+TEST_F(WayfinderServerManagerTest, RecoverFromOrphan_NoStateFile) {
...
+}
+
+TEST_F(WayfinderServerManagerTest, RecoverFromOrphan_ProcessGone) {
...
+// =============================================================================
+
+TEST_F(WayfinderServerManagerTest, RestartSavesEphemeralPortsToPrefs) {
+  SetupSuccessfulLaunch();
+  manager_->SetRunningForTesting(true);
+
+  // Set initial known ports in prefs
+  prefs_.SetInteger(wayfinder_server::kServerPort, 9200);
+  prefs_.SetInteger(wayfinder_server::kExtensionServerPort, 9300);
+
...
+            prefs_.GetInteger(wayfinder_server::kServerPort));
+  EXPECT_EQ(manager_->GetExtensionPort(),
+            prefs_.GetInteger(wayfinder_server::kExtensionServerPort));
+  // Server and extension ports should be non-zero (resolved by FindAvailablePort)
+  EXPECT_NE(0, prefs_.GetInteger(wayfinder_server::kServerPort));
+  EXPECT_NE(0, prefs_.GetInteger(wayfinder_server::kExtensionServerPort));
+}
+
+TEST_F(WayfinderServerManagerTest, UpdateRestartSavesEphemeralPortsToPrefs) {
+  SetupSuccessfulLaunch();
+  manager_->SetRunningForTesting(true);
+
+  prefs_.SetInteger(wayfinder_server::kServerPort, 9200);
