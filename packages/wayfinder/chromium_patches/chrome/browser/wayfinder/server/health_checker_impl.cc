diff --git a/chrome/browser/wayfinder/server/health_checker_impl.cc b/chrome/browser/wayfinder/server/health_checker_impl.cc
new file mode 100644
index 0000000000000..73ef5eaf9e8e3
--- /dev/null
+++ b/chrome/browser/wayfinder/server/health_checker_impl.cc
@@ -0,0 +1,145 @@
...
+      net::DefineNetworkTrafficAnnotation("wayfinder_health_check", R"(
+        semantics {
+          sender: "Wayfinder Server Manager"
+          description:
+            "Checks if the Wayfinder MCP server is healthy by querying its "
+            "/health endpoint."
...
+            "Internal health check for Wayfinder server functionality."
+        })");
+
...
+  // Create network traffic annotation
+  net::NetworkTrafficAnnotationTag traffic_annotation =
+      net::DefineNetworkTrafficAnnotation("wayfinder_shutdown_request", R"(
+        semantics {
+          sender: "Wayfinder Server Manager"
+          description:
+            "Requests graceful shutdown of the Wayfinder server via POST to "
+            "/shutdown endpoint."
...
+          setting: "This feature cannot be disabled by settings."
+          policy_exception_justification:
+            "Internal shutdown request for Wayfinder server functionality."
+        })");
+
+  // Create resource request
+  auto resource_request = std::make_unique<network::ResourceRequest>();
...
+    LOG(WARNING) << "wayfinder: HTTP request failed - HTTP " << response_code
