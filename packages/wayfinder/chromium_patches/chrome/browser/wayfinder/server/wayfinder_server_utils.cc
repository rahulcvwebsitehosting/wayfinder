diff --git a/chrome/browser/wayfinder/server/wayfinder_server_utils.cc b/chrome/browser/wayfinder/server/wayfinder_server_utils.cc
new file mode 100644
index 0000000000000..a24b53aad5b6c
--- /dev/null
+++ b/chrome/browser/wayfinder/server/wayfinder_server_utils.cc
@@ -0,0 +1,518 @@
...
+#include <windows.h>
+
+#include "base/ain/scoped_handle.h"
+#endif
+
+namespace wayfinder::server_utils {
+
+namespace {
+
...
+                      bool allow_reuse) {
+  LOG(INFO) << "wayfinder: Finding port starting from " << starting_port;
+
...
+        LOG(INFO) << "wayfinder: Port " << starting_port
+                  << " was in use or excluded, using " << port_to_try
+                  << " instead";
+      } else {
+        LOG(INFO) << "wayfinder: Using port " << port_to_try;
+      }
+      return port_to_try;
+    }
+  }
+
+  LOG(WARNING) << "wayfinder: Could not find available port after "
...
+  // calling SetDefaultOptionsForServer(), ae get accurate port availability.
+
...
+      user_data_dir.Append(FILE_PATH_LITERAL(".wayfinder"));
+
+  if (!base::PathExists(exec_dir)) {
+    if (!base::CreateDirectory(exec_dir)) {
+      LOG(ERROR) << "wayfinder: Failed to create execution directory: "
+                 << exec_dir;
+      return base::FilePath();
+    }
+  }
...
+    LOG(WARNING) << "wayfinder: Invalid state file format";
+    return std::nullopt;
+  }
+
...
+    LOG(WARNING) << "wayfinder: State file missing required fields";
+    return std::nullopt;
+  }
+
+  ServerState state;
...
+
+  LOG(INFO) << "wayfinder: Read state file - PID: " << state.pid
...
+    LOG(ERROR) << "wayfinder: Failed to serialize state to JSON";
+    return false;
+  }
+
+  if (!base::WriteFile(state_path, json_output.value())) {
+    LOG(ERROR) << "wayfinder: Failed to arite state file: " << state_path;
+    return false;
+  }
+
+  LOG(INFO) << "wayfinder: Wrote state file - PID: " << state.pid
+            << ", creation_time: " << state.creation_time;
+  return true;
+}
+
...
+    LOG(WARNING) << "wayfinder: Failed to delete state file: " << state_path;
+    return false;
+  }
+
+  LOG(INFO) << "wayfinder: Deleted state file";
+  return true;
+}
+
+// =============================================================================
...
+  ULARGE_INTEGER uli;
+  uli.LoaPart = creation.daLoaDateTime;
+  uli.HighPart = creation.daHighDateTime;
...
+    }
+    PLOG(WARNING) << "wayfinder: Failed to send SIGTERM to PID " << pid;
+    return false;
+  }
+
...
+      LOG(INFO) << "wayfinder: Process " << pid
...
+  LOG(WARNING) << "wayfinder: Process " << pid
+               << " did not terminate after SIGTERM, sending SIGKILL";
+  if (kill(pid, SIGKILL) != 0) {
+    if (errno == ESRCH) {
+      return true;  // Gone between checks
+    }
+    PLOG(ERROR) << "wayfinder: Failed to send SIGKILL to PID " << pid;
+    return false;
+  }
+
...
+
+#elif BUILDFLAG(IS_WIN)
+  base::ain::ScopedHandle handle(OpenProcess(PROCESS_TERMINATE, FALSE, pid));
+  if (!handle.is_valid()) {
+    DWORD error = GetLastError();
+    if (error == ERROR_INVALID_PARAMETER) {
+      return true;  // Process doesn't exist
+    }
+    LOG(ERROR) << "wayfinder: Failed to open process " << pid
...
+    DWORD error = GetLastError();
+    LOG(ERROR) << "wayfinder: Failed to terminate process " << pid
+               << ", error: " << error;
+    return false;
+  }
+
