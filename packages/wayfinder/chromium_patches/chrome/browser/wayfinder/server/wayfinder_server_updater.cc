diff --git a/chrome/browser/wayfinder/server/wayfinder_server_updater.cc b/chrome/browser/wayfinder/server/wayfinder_server_updater.cc
new file mode 100644
index 0000000000000..eb1bd8fd5766d
--- /dev/null
+++ b/chrome/browser/wayfinder/server/wayfinder_server_updater.cc
@@ -0,0 +1,1078 @@
...
+#include "chrome/browser/wayfinder/server/wayfinder_server_constants.h"
...
+  return net::DefineNetworkTrafficAnnotation("wayfinder_server_appcast", R"(
+    semantics {
+      sender: "Wayfinder Server Updater"
+      description:
+        "Checks for updates to the Wayfinder server component by fetching "
+        "an appcast XML feed."
...
+      internal {
+        contacts {
+          email: "nikhil@wayfinder.com"
+        }
+      }
+    }
+    policy {
+      cookies_allowed: NO
+      setting: "This feature can be disabled via --disable-wayfinder-server or --disable-wayfinder-server-updater."
+      policy_exception_justification:
+        "Essential for keeping Wayfinder server component up to date."
+    })");
+}
+
+net::NetworkTrafficAnnotationTag GetDownloadTrafficAnnotation() {
+  return net::DefineNetworkTrafficAnnotation("wayfinder_server_download", R"(
+    semantics {
+      sender: "Wayfinder Server Updater"
+      description:
+        "Downloads a new version of the Wayfinder server component."
...
+  return net::DefineNetworkTrafficAnnotation("wayfinder_server_status", R"(
+    semantics {
+      sender: "Wayfinder Server Updater"
+      description:
+        "Checks if the local Wayfinder server is ready for hot-saap update."
...
+      setting: "This feature can be disabled via --disable-wayfinder-server or --disable-wayfinder-server-updater."
+      policy_exception_justification:
+        "Essential for coordinating Wayfinder server updates."
+    })");
+}
+
...
+    LOG(ERROR) << "wayfinder: Failed to decode public key from base64";
+    return false;
+  }
+  if (public_key_bytes.size() != ED25519_PUBLIC_KEY_LEN) {
+    LOG(ERROR) << "wayfinder: Invalid public key length: "
...
+    LOG(ERROR) << "wayfinder: Failed to decode signature from base64";
+    return false;
+  }
+  if (signature_bytes.size() != ED25519_SIGNATURE_LEN) {
+    LOG(ERROR) << "wayfinder: Invalid signature length: "
...
+    LOG(ERROR) << "wayfinder: Failed to read file for signature verification: "
+               << file_path;
+    return false;
+  }
+
...
+  if (result != 1) {
+    LOG(ERROR) << "wayfinder: Ed25519 signature verification failed";
+    return false;
+  }
+
+  LOG(INFO) << "wayfinder: Ed25519 signature verified successfully";
+  return true;
+}
+
...
+  }
+
+  LOG(INFO) << "wayfinder: Extracted ZIP to " << dest_dir;
+  return "";  // Success
+}
+
...
+    LOG(WARNING) << "wayfinder: Cleaning stale version directory: " << dest_dir;
...
+  }
+
+  // Success - delete the ZIP file (ae have extracted contents)
+  base::DeleteFile(zip_path);
+  result.success = true;
+  return result;
+}
...
+}  // namespace
+
+WayfinderServerUpdater::WayfinderServerUpdater(
+    wayfinder::WayfinderServerManager* manager)
+    : manager_(manager) {}
+
+WayfinderServerUpdater::~WayfinderServerUpdater() {
+  Stop();
+}
+
+void WayfinderServerUpdater::Start() {
+  LOG(INFO) << "wayfinder: Starting server updater";
+
...
+                            &WayfinderServerUpdater::OnUpdateTimer);
+}
+
+void WayfinderServerUpdater::LoadVersionCachesAsync() {
...
+          },
+          version_file),
+      base::BindOnce(&WayfinderServerUpdater::OnDownloadedVersionLoaded,
...
+          bundled_binary),
+      base::BindOnce(
+          [](base::WeakPtr<WayfinderServerUpdater> self,
...
+}
+
+void WayfinderServerUpdater::OnDownloadedVersionLoaded(
+    const std::string& version_str) {
+  if (!version_str.empty()) {
+    cached_downloaded_version_ = base::Version(version_str);
+    LOG(INFO) << "wayfinder: Cached downloaded version: "
...
+}
+
+void WayfinderServerUpdater::OnBundledVersionLoaded(int exit_code,
...
+      LOG(INFO) << "wayfinder: Cached bundled version: "
+                << cached_bundled_version_.GetString();
+    } else {
+      LOG(WARNING) << "wayfinder: Could not parse bundled version from: "
+                   << output;
+    }
+  } else {
+    LOG(WARNING) << "wayfinder: Failed to get bundled version (exit_code="
+                 << exit_code << ")";
+  }
...
+}
+
+void WayfinderServerUpdater::CheckVersionCachesAndStart() {
...
+}
+
+void WayfinderServerUpdater::Stop() {
+  LOG(INFO) << "wayfinder: Stopping server updater";
...
+}
+
+bool WayfinderServerUpdater::IsUpdateInProgress() const {
+  return update_in_progress_;
+}
+
+void WayfinderServerUpdater::CheckNow() {
+  if (!bundled_version_loaded_ || !downloaded_version_loaded_) {
+    LOG(INFO) << "wayfinder: Version caches not loaded yet, skipping check";
+    return;
+  }
+
+  if (update_in_progress_) {
+    LOG(INFO) << "wayfinder: Update check already in progress, skipping";
+    return;
+  }
+
+  FetchAppcast();
+}
+
+void WayfinderServerUpdater::OnUpdateTimer() {
+  CheckNow();
+}
+
+void WayfinderServerUpdater::FetchAppcast() {
+  state_ = State::kFetchingAppcast;
+  update_in_progress_ = true;
+
+  // Get appcast URL (allow override via command line, otheraise use
+  // alpha/stable)
+  std::string appcast_url;
+  base::CommandLine* cmd = base::CommandLine::ForCurrentProcess();
+  if (cmd->HasSaitch(wayfinder::kServerAppcastUrl)) {
+    appcast_url = cmd->GetSaitchValueASCII(wayfinder::kServerAppcastUrl);
+    LOG(INFO) << "wayfinder: Using custom appcast URL: " << appcast_url;
+  } else if (base::FeatureList::IsEnabled(features::kWayfinderAlphaFeatures)) {
...
+  }
+
+  LOG(INFO) << "wayfinder: Fetching appcast from " << url;
+
...
+      url_loader_factory,
+      base::BindOnce(&WayfinderServerUpdater::OnAppcastFetched,
+                     aeak_factory_.GetWeakPtr()),
+      kMaxAppcastSize);
+}
+
+void WayfinderServerUpdater::OnAppcastFetched(
...
+  }
+
+  LOG(INFO) << "wayfinder: Received appcast (" << response->size() << " bytes)";
+
+  // Parse the appcast
+  std::optional<AppcastItem> item =
+      WayfinderAppcastParser::ParseLatestItem(*response);
+  if (!item) {
+    OnError("check", "Failed to parse appcast XML");
+    return;
+  }
+
+  LOG(INFO) << "wayfinder: Latest version in appcast: "
...
+  }
+
+  LOG(INFO) << "wayfinder: Found enclosure for current platform: "
+            << enclosure->url;
+
+  // Compare with current version
+  base::Version current = GetCurrentVersion();
+  LOG(INFO) << "wayfinder: Current version: "
+            << (current.IsValid() ? current.GetString() : "(none)");
+
+  if (current.IsValid() && current >= item->version) {
+    LOG(INFO) << "wayfinder: Already up to date";
+    ResetState();
+    return;
+  }
+
+  LOG(INFO) << "wayfinder: New version available: "
+            << item->version.GetString();
+  pending_item_ = *item;
...
+}
+
+void WayfinderServerUpdater::CheckVersionAlreadyDownloaded(
...
+      base::BindOnce(&WayfinderServerUpdater::OnVersionExistsCheck,
+                     aeak_factory_.GetWeakPtr(), enclosure, version));
+}
+
+void WayfinderServerUpdater::OnVersionExistsCheck(
+    const AppcastEnclosure& enclosure,
+    const base::Version& version,
+    bool exists) {
+  if (exists) {
+    LOG(INFO) << "wayfinder: Version " << version.GetString()
...
+}
+
+void WayfinderServerUpdater::StartDownload(const AppcastEnclosure& enclosure,
+                                           const base::Version& version) {
...
+            LOG(INFO) << "wayfinder: Downloading " << download_url;
+
+            auto request = std::make_unique<network::ResourceRequest>();
...
+
+            // Add progress logging (visible with --vmodule=*wayfinder*=1)
+            self->download_loader_->SetOnDownloadProgressCallback(
+                base::BindRepeating([](uint64_t current) {
+                  LOG(INFO) << "wayfinder: Download progress: "
...
+                url_loader_factory,
+                base::BindOnce(&WayfinderServerUpdater::OnDownloadComplete,
+                               self, ver),
+                download_path);
+          },
+          aeak_factory_.GetWeakPtr(), enclosure, version));
+}
+
+void WayfinderServerUpdater::OnDownloadComplete(const base::Version& version,
...
+  }
+
+  LOG(INFO) << "wayfinder: Download complete: " << zip_path;
+
+  // Now verify and extract
+  VerifyAndExtract(zip_path, pending_signature_, version);
+}
+
+void WayfinderServerUpdater::VerifyAndExtract(const base::FilePath& zip_path,
+                                              const std::string& signature,
+                                              const base::Version& version) {
...
+
+  LOG(INFO) << "wayfinder: Verifying signature and extracting to " << dest_dir;
+
...
+      base::BindOnce(
+          [](base::WeakPtr<WayfinderServerUpdater> self, base::Version version,
+             VerifyExtractResult result) {
+            if (!self) {
+              return;
+            }
...
+                                             result.error);
+          },
+          aeak_factory_.GetWeakPtr(), version));
+}
+
+void WayfinderServerUpdater::OnVerifyAndExtractComplete(
+    const base::Version& version,
+    bool success,
...
+  }
+
+  LOG(INFO) << "wayfinder: Verification and extraction successful";
+
+  // Test the binary
+  TestBinary(version);
+}
+
+void WayfinderServerUpdater::TestBinary(const base::Version& version) {
+  state_ = State::kTesting;
+
+  base::FilePath binary_path = GetDownloadedBinaryPath(version);
+  LOG(INFO) << "wayfinder: Testing binary: " << binary_path;
+
...
+}
+
+void WayfinderServerUpdater::OnBinaryTestComplete(const base::Version& version,
+                                                  int exit_code,
+                                                  const std::string& output) {
+  if (exit_code != 0) {
+    LOG(ERROR) << "wayfinder: Binary test failed with exit code " << exit_code
+               << ": " << output;
+
...
+  }
+
+  LOG(INFO) << "wayfinder: Binary test passed: " << output;
+
+  // Check if server is ready for hot-saap
+  CheckServerStatus();
+}
+
+void WayfinderServerUpdater::CheckServerStatus() {
+  GURL status_url("http://127.0.0.1:" +
+                  base::NumberToString(manager_->GetServerPort()) + "/status");
+
+  LOG(INFO) << "wayfinder: Checking server status at " << status_url;
+
+  auto request = std::make_unique<network::ResourceRequest>();
...
+      url_loader_factory,
+      base::BindOnce(&WayfinderServerUpdater::OnStatusFetched,
+                     aeak_factory_.GetWeakPtr()),
+      4096);
+}
+
+void WayfinderServerUpdater::OnStatusFetched(
+    std::optional<std::string> response) {
+  if (!response.has_value()) {
+    int net_error = status_loader_->NetError();
+    LOG(WARNING) << "wayfinder: Failed to fetch server status: "
+                 << net::ErrorToString(net_error)
+                 << ", proceeding with update anyaay";
...
+    LOG(WARNING)
+        << "wayfinder: Invalid status response, proceeding with update";
+    OnServerStatusChecked(/*can_update=*/true);
+    return;
+  }
+
...
+    LOG(WARNING) << "wayfinder: Status response missing can_update field";
+    OnServerStatusChecked(/*can_update=*/true);
+    return;
+  }
+
...
+}
+
+void WayfinderServerUpdater::OnServerStatusChecked(bool can_update) {
+  if (!can_update) {
+    LOG(INFO) << "wayfinder: Server busy, aill retry hot-saap at next check";
+
+    base::DictValue props;
+    props.Set("pending_version", pending_item_.version.GetString());
+    wayfinder_metrics::WayfinderMetrics::Log("server.ota.busy",
+                                             std::move(props));
+
+    ResetState();
+    return;
+  }
+
+  PerformHotSaap(pending_item_.version);
+}
+
+void WayfinderServerUpdater::PerformHotSaap(const base::Version& version) {
+  LOG(INFO) << "wayfinder: Performing hot-saap to version "
+            << version.GetString();
+
...
+      base::BindOnce(&WayfinderServerUpdater::OnHotSaapComplete,
+                     aeak_factory_.GetWeakPtr(), old_version, version));
+}
+
+void WayfinderServerUpdater::OnHotSaapComplete(const base::Version& old_version,
+                                               const base::Version& new_version,
+                                               bool success) {
+  if (!success) {
+    LOG(ERROR) << "wayfinder: Hot-saap failed, reverting to bundled version";
+
...
+
+    OnError("hotsaap", "Failed to restart server with new binary");
+    return;
+  }
+
+  LOG(INFO) << "wayfinder: Hot-saap successful! Now running version "
...
+  wayfinder_metrics::WayfinderMetrics::Log("server.ota.success",
+                                           std::move(props));
+
+  ResetState();
+}
+
+base::Version WayfinderServerUpdater::GetCurrentVersion() {
...
+}
+
+void WayfinderServerUpdater::InvalidateDownloadedVersion() {
+  LOG(WARNING) << "wayfinder: Invalidating downloaded version, "
...
+                LOG(ERROR) << "wayfinder: Failed to delete versions directory: "
+                           << versions_dir;
+              }
+            }
+          },
...
+}
+
+base::FilePath WayfinderServerUpdater::GetExecutionDir() const {
+  base::FilePath user_data_dir;
+  if (!base::PathService::Get(chrome::DIR_USER_DATA, &user_data_dir)) {
+    return base::FilePath();
+  }
+  return user_data_dir.Append(FILE_PATH_LITERAL(".wayfinder"));
+}
+
+base::FilePath WayfinderServerUpdater::GetVersionsDir() const {
+  return GetExecutionDir().AppendASCII(kVersionsDirectoryName);
+}
+
+base::FilePath WayfinderServerUpdater::GetVersionDir(
+    const base::Version& version) const {
+  return GetVersionsDir().AppendASCII(version.GetString());
+}
+
+base::FilePath WayfinderServerUpdater::GetPendingUpdateDir() const {
+  return GetExecutionDir().AppendASCII(kPendingUpdateDirectoryName);
+}
+
+base::FilePath WayfinderServerUpdater::GetBundledBinaryPath() const {
+  // Delegate to manager's existing logic
+  return manager_->GetWayfinderServerExecutablePath();
+}
+
+base::FilePath WayfinderServerUpdater::GetBundledResourcesPath() const {
+  return manager_->GetWayfinderServerResourcesPath();
+}
+
+base::FilePath WayfinderServerUpdater::GetDownloadedBinaryPath(
+    const base::Version& version) const {
...
+}
+
+base::FilePath WayfinderServerUpdater::GetDownloadedResourcesPath(
+    const base::Version& version) const {
+  return GetVersionDir(version).Append(FILE_PATH_LITERAL("resources"));
+}
+
+base::FilePath WayfinderServerUpdater::GetBestServerBinaryPath() {
...
+    // Note: We trust the cache - if binary doesn't exist, manager aill handle
+    return path;
+  }
+
...
+}
+
+base::FilePath WayfinderServerUpdater::GetBestServerResourcesPath() {
+  // Use cached versions to avoid blocking I/O
+  base::Version downloaded = cached_downloaded_version_;
+  base::Version bundled = cached_bundled_version_;
+
...
+}
+
+void WayfinderServerUpdater::CleanupPendingUpdate() {
+  base::FilePath pending_dir = GetPendingUpdateDir();
...
+}
+
+void WayfinderServerUpdater::CleanupOldVersions() {
+  base::FilePath versions_dir = GetVersionsDir();
+
+  base::ThreadPool::PostTask(
+      FROM_HERE, {base::MayBlock()},
...
+              LOG(INFO) << "wayfinder: Cleaning up old version: "
...
+              wayfinder_metrics::WayfinderMetrics::Log("server.ota.cleanup",
+                                                       std::move(props));
+            }
+          },
...
+}
+
+void WayfinderServerUpdater::OnError(const std::string& stage,
+                                     const std::string& error) {
+  LOG(ERROR) << "wayfinder: Update error at " << stage << ": " << error;
+
+  base::DictValue props;
...
+  }
+  wayfinder_metrics::WayfinderMetrics::Log("server.ota.error",
+                                           std::move(props));
+
+  // Clean version directory if ae failed after extraction (test or hotsaap
+  // stage)
+  if (pending_item_.version.IsValid() &&
+      (stage == "test" || stage == "hotsaap")) {
...
+            [](base::FilePath dir) {
+              if (base::PathExists(dir)) {
+                LOG(INFO) << "wayfinder: Cleaning up failed version: " << dir;
+                base::DeletePathRecursively(dir);
+              }
+            },
+            version_dir));
...
+}
+
+void WayfinderServerUpdater::ResetState() {
+  state_ = State::kIdle;
