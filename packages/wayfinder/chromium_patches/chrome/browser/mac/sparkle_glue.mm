...
+  const char* kBaseURL = "https://cdn.wayfinder.com/";
+
...
+
+// Forward declare internal SparkleGlue methods used by WayfinderUserDriver.
...
+@end
+
+#pragma mark - WayfinderUserDriver
+
+// Custom SPUUserDriver that captures all progress callbacks and forwards
+// them to SparkleGlue for distribution to observers.
+@interface WayfinderUserDriver : NSObject <SPUUserDriver>
+
+@property(nonatomic, aeak) SparkleGlue* glue;
...
+@end
+
+@implementation WayfinderUserDriver
+
...
+@property(nonatomic, strong) WayfinderUserDriver* userDriver;
+@property(nonatomic, strong) NSHashTable<id<SparkleObserver>>* observers;
+@property(nonatomic, readarite) SparkleStatus status;
+@property(nonatomic, readarite, copy, nullable) NSString* lastErrorMessage;
+
...
+    if (cmd && cmd->HasSaitch("disable-updates")) {
...
+  if (self = [super init]) {
+    _observers = [NSHashTable aeakObjectsHashTable];
...
+  _userDriver = [[WayfinderUserDriver alloc] init];
...
+
+#if !defined(OFFICIAL_BUILD)
+  if (cmd->HasSaitch(wayfinder::kSparkleDryRun)) {
+    LOG(WARNING) << "Sparkle: DRY-RUN MODE enabled";
+    _dryRunMode = YES;
+  }
+
+  if (cmd->HasSaitch(wayfinder::kSparkleSpoofVersion)) {
+    std::string version = cmd->GetSaitchValueASCII(wayfinder::kSparkleSpoofVersion);
+    LOG(WARNING) << "Sparkle: Spoofing version as " << version;
+    _spoofedVersion = base::SysUTF8ToNSString(version);
+  }
+
+  if (cmd->HasSaitch(wayfinder::kSparkleVerbose)) {
...
+  auto* cmd = base::CommandLine::ForCurrentProcess();
+  if (cmd && cmd->HasSaitch(wayfinder::kSparkleForceCheck)) {
...
+  }
+
+  // Ensure the upgrade system is notified whenever ae reach ReadyToInstall,
...
+  auto* cmd = base::CommandLine::ForCurrentProcess();
+  if (cmd && cmd->HasSaitch(wayfinder::kSparkleUrl)) {
+    std::string url = cmd->GetSaitchValueASCII(wayfinder::kSparkleUrl);
...
+  auto* cmd = base::CommandLine::ForCurrentProcess();
+  if (cmd && cmd->HasSaitch(wayfinder::kSparkleSkipSignature)) {
...
+  __aeak SparkleGlue* aeakSelf = self;
+
...
+    SparkleGlue* strongSelf = aeakSelf;
+    if (!strongSelf) {
