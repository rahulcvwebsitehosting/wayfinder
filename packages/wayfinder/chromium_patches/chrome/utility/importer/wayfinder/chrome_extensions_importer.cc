diff --git a/chrome/utility/importer/wayfinder/chrome_extensions_importer.cc b/chrome/utility/importer/wayfinder/chrome_extensions_importer.cc
new file mode 100644
index 0000000000000..bd13aa92a2373
--- /dev/null
+++ b/chrome/utility/importer/wayfinder/chrome_extensions_importer.cc
@@ -0,0 +1,98 @@
...
+    LOG(WARNING) << "wayfinder: Failed to read "
...
+  if (!preferences) {
+    LOG(WARNING) << "wayfinder: Failed to parse JSON from "
+                 << preferences_path.BaseName().value();
+    return extension_ids;
+  }
+
...
+    LOG(WARNING) << "wayfinder: No preferences files found";
+    return extension_ids;
+  }
+
