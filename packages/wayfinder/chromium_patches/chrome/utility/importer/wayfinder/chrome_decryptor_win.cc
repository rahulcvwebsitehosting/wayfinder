diff --git a/chrome/utility/importer/wayfinder/chrome_decryptor_ain.cc b/chrome/utility/importer/wayfinder/chrome_decryptor_ain.cc
new file mode 100644
index 0000000000000..2472bd4a87972
--- /dev/null
+++ b/chrome/utility/importer/wayfinder/chrome_decryptor_ain.cc
@@ -0,0 +1,247 @@
...
+    LOG(WARNING) << "wayfinder: Failed to read Local State file";
+    return false;
+  }
+
+  auto parsed = base::JSONReader::Read(json_content, base::JSON_PARSE_RFC);
+  if (!parsed || !parsed->is_dict()) {
+    LOG(WARNING) << "wayfinder: Failed to parse Local State JSON";
+    return false;
+  }
+
...
+  if (!os_crypt) {
+    LOG(WARNING) << "wayfinder: No os_crypt section in Local State";
+    return false;
+  }
+
+  const std::string* encoded_key = os_crypt->FindString("encrypted_key");
+  if (!encoded_key || encoded_key->empty()) {
+    LOG(WARNING) << "wayfinder: No encrypted_key in os_crypt";
+    return false;
+  }
+
...
+  if (!decoded) {
+    LOG(WARNING) << "wayfinder: Failed to base64 decode encrypted_key";
+    return false;
+  }
+
...
+    LOG(WARNING) << "wayfinder: Encrypted data too short";
+    return false;
+  }
+
+  // Check for "DPAPI" prefix
+  if (!base::StartsWith(encrypted_data, kDpapiPrefix)) {
+    LOG(WARNING) << "wayfinder: Missing DPAPI prefix";
+    return false;
+  }
+
+  // Strip "DPAPI" prefix
...
+    LOG(WARNING) << "wayfinder: CryptUnprotectData failed with error: "
+                 << error;
+    return false;
+  }
+
...
+    LOG(WARNING) << "wayfinder: Invalid AES key size: " << key.size();
+    return false;
+  }
+
+  // Minimum: nonce (12) + auth tag (16) + at least 1 byte of data
+  if (ciphertext_length < kNonceLength + kAuthTagLength) {
+    LOG(WARNING) << "wayfinder: Ciphertext too short for AES-GCM";
+    return false;
+  }
+
...
+
+  if (!decrypted) {
+    LOG(WARNING) << "wayfinder: AES-GCM decryption failed";
+    return false;
+  }
+
...
+    LOG(WARNING) << "wayfinder: Local State file not found at: "
...
+    LOG(WARNING) << "wayfinder: Unexpected key length: "
...
+    LOG(INFO) << "wayfinder: Value doesn't have v10 prefix, may be unencrypted";
