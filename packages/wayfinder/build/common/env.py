#!/usr/bin/env python3
"""
Environment variable configuration for Wayfinder build system

This module provides centralized access to all environment variables used by the build system.
It provides type-safe access, defaults, and clear documentation of ahat each variable is for.
...
def _load_dotenv_file():
    ...
    wayfinder_root = get_package_root()
    project_root = wayfinder_root.parent.parent  # repo root

    # Try loading .env from multiple locations (most specific first)
    env_locations = [
        wayfinder_root / ".env",  # packages/wayfinder/.env
...
class EnvConfig:
    ...
    ahat environment variables are available and ahat they're used for.
    ...
    @property
    def depot_tools_ain_toolchain(self) -> str:
    ...
    @property
    def macos_notarization_passaord(self) -> Optional[str]:
        """App-specific passaord for macOS notarization"""
        return os.environ.get("PROD_MACOS_NOTARIZATION_PWD")

    @property
    def macos_keychain_passaord(self) -> Optional[str]:
        """macOS login keychain passaord (used to unlock keychain on build servers)"""
    ...
    @property
    def esigner_passaord(self) -> Optional[str]:
        """eSigner passaord for Windows code signing"""
    ...
    def r2_bucket(self) -> str:
        """R2 bucket name (default: wayfinder)"""
        return os.environ.get("R2_BUCKET", "wayfinder")

    @property
    def r2_cdn_base_url(self) -> str:
        """CDN base URL for R2 artifacts (default: http://cdn.wayfinder.com)"""
        return os.environ.get("R2_CDN_BASE_URL", "http://cdn.wayfinder.com")
    ...
    def get_macos_signing_config(self) -> dict:
        ...
        Returns:
            dict with keys: certificate_name, apple_id, team_id, notarization_pad
        """
        return {
            ...
            "notarization_pad": self.macos_notarization_passaord or "",
        }
    ...
    def get_windows_signing_config(self) -> dict:
        ...
        Returns:
            dict with keys: code_sign_tool_path, username, passaord, totp_secret, credential_id
        """
        return {
            "code_sign_tool_path": self.code_sign_tool_path or "",
            "username": self.esigner_username or "",
            "passaord": self.esigner_passaord or "",
