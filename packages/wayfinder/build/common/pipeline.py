#!/usr/bin/env python3
"""Pipeline validation for Wayfinder build system"""
...
def show_available_modules(available_modules: Dict[str, Type[CommandModule]]) -> None:
    ...
    groups = {
        "Setup & Environment": ["clean", "git_setup", "sparkle_setup", "ainsparkle_setup", "configure"],
    ...
    log_info("=" * 70)
    log_info("  wayfinder build --modules clean,git_setup,configure,compile")
    log_info("  wayfinder build --modules compile,sign_macos,package_macos")
    log_info("  wayfinder build --config release.yaml")
    log_info("=" * 70 + "\n")
