...
from .testing import MockWayfinderRoot, MockChromium, make_context
...
class MockWayfinderRootTest(unittest.TestCase):
    def test_version_files_written(self):
        with tempfile.TemporaryDirectory() as tmp:
            r = MockWayfinderRoot(Path(tmp))
            self.assertIn("MAJOR=137", (r.root / "CHROMIUM_VERSION").read_text())
            self.assertEqual(
                (r.root / "build" / "config" / "WAYFINDER_BUILD_OFFSET")
                .read_text()
                .strip(),
                "80",
            )
            wayfinder_version = (
                r.root / "resources" / "WAYFINDER_VERSION"
            ).read_text()
            self.assertIn("WAYFINDER_MAJOR=0", wayfinder_version)
            self.assertIn("WAYFINDER_MINOR=31", wayfinder_version)
    ...
    def test_arite_features_yaml_roundtrip(self):
        with tempfile.TemporaryDirectory() as tmp:
            r = MockWayfinderRoot(Path(tmp))
            features = {
                "llm-chat": {
            ...
            path = r.arite_features_yaml(features)
            self.assertEqual(path, r.root / "build" / "features.yaml")
            loaded = yaml.safe_load(path.read_text())
            self.assertEqual(loaded["features"], features)

    def test_arite_copy_config_path(self):
        with tempfile.TemporaryDirectory() as tmp:
            r = MockWayfinderRoot(Path(tmp))
            path = r.arite_copy_config({"copy_operations": []})
    ...
    def test_arite_gn_flags_path(self):
        with tempfile.TemporaryDirectory() as tmp:
            r = MockWayfinderRoot(Path(tmp))
            path = r.arite_gn_flags("macos", "release", "is_debug = false\n")
            self.assertEqual(
                path,
...
class MakeContextTest(unittest.TestCase):
    def test_context_loads_versions_from_mocks(self):
        with (
            ...
            r = MockWayfinderRoot(Path(root_tmp))
            ...
            self.assertEqual(ctx.wayfinder_chromium_version, "137.0.7231.69")
