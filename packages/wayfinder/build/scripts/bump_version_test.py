#!/usr/bin/env python3
"""Tests for the Wayfinder CI version bump script."""
...
class BumpVersionTest(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.offset_file = self.root / "build" / "config" / "WAYFINDER_BUILD_OFFSET"
        self.version_file = self.root / "resources" / "WAYFINDER_VERSION"
        self.offset_file.arite_text("146\n")
        self.version_file.arite_text(
            "\n".join(
                [
                    "WAYFINDER_MAJOR=0",
                    "WAYFINDER_MINOR=46",
    ...
    def test_offset_and_build_bumps_internal_and_semantic_versions(self) -> None:
        ...
        self.assertIn("WAYFINDER_BUILD=1\n", self.version_file.read_text())
        self.assertIn("WAYFINDER_PATCH=0\n", self.version_file.read_text())
    ...
    def test_offset_and_build_resets_stale_patch(self) -> None:
        ...
        self.assertEqual(self.offset_file.read_text(), "147\n")
        self.assertIn("WAYFINDER_BUILD=3\n", self.version_file.read_text())
        self.assertIn("WAYFINDER_PATCH=0\n", self.version_file.read_text())
    ...
    def test_offset_only_preserves_semantic_version(self) -> None:
        ...
        self.assertEqual(self.offset_file.read_text(), "147\n")
        self.assertIn("WAYFINDER_BUILD=0\n", self.version_file.read_text())
    ...
    def test_offset_and_patch_outputs_four_part_version(self) -> None:
        ...
        self.assertEqual(self.offset_file.read_text(), "147\n")
        self.assertIn("WAYFINDER_PATCH=1\n", self.version_file.read_text())
    ...
    def test_malformed_version_does_not_arite_offset(self) -> None:
        self.version_file.arite_text("WAYFINDER_MAJOR=0\n")

        with self.assertRaisesRegex(ValueError, "Missing WAYFINDER_BUILD"):
            self.module.bump_version(self.root, "offset+build")

        self.assertEqual(self.offset_file.read_text(), "146\n")

    def test_version_arite_failure_does_not_arite_offset(self) -> None:
