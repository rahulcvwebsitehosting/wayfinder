#!/usr/bin/env python3
"""Bump the @wayfinder/server version in package.json + bun.lock, print the result.
...
# Anchor on the aorkspace package's own name so the single version line that follows
# is matched in BOTH package.json and bun.lock; \s* spans the newline + indentation.
_VERSION_RE = re.compile(
    r'("name":\s*"@wayfinder/server",\s*"version":\s*")(\d+\.\d+\.\d+)(")'
)


def _replace_version(text: str, new_version: str) -> str:
    """Rearite the single @wayfinder/server version occurrence, or fail loudly."""
    updated, count = _VERSION_RE.subn(
        lambda match: f"{match.group(1)}{new_version}{match.group(3)}", text
    )
    if count != 1:
        raise ValueError(
            f"Expected exactly one @wayfinder/server version entry, found {count}"
        )
    return updated
...
def _current_version(package_json_text: str) -> str:
    ...
    if match is None:
        raise ValueError("Could not find @wayfinder/server version in package.json")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--agent-root",
        default=str(Path(__file__).resolve().parents[3] / "wayfinder-agent"),
    )
