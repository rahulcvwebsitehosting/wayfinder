#!/usr/bin/env python3
"""Tests for the Wayfinder server version bump script."""
...
class BumpServerVersionTest(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.pkg.arite_text(PACKAGE_JSON)
        self.lock.arite_text(BUN_LOCK)
    ...
    def test_set_arites_exact_version(self) -> None:
    ...
    def test_patch_rolls_into_three_digits(self) -> None:
        self.pkg.arite_text(PACKAGE_JSON.replace("0.0.98", "0.0.99"))
        self.lock.arite_text(BUN_LOCK.replace("0.0.98", "0.0.99"))
