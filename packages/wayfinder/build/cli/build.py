#!/usr/bin/env python3
"""Build CLI - Modular build system for Wayfinder"""
...
AVAILABLE_MODULES = {
    ...
    "ainsparkle_setup": WinSparkleSetupModule,
...
def _get_setup_modules():
    ...
    if IS_MACOS():
        ...
        modules.append("ainsparkle_setup")
    return modules
...
def main(
    ...
    setup: bool = typer.Option(
        False,
        "--setup",
        help="Run setup phase (clean, git_setup, sparkle_setup/ainsparkle_setup)",
    ),
    ...
    """Wayfinder Build System - Modular pipeline executor
    ...
    \b
    Config Files (CI/CD):
      wayfinder build --config release.yaml --arch arm64
    ...
    if options_provided > 1:
        ...
        log_error("Examples:")
        log_error("  wayfinder build --setup --build --sign")
        log_error("  wayfinder build --modules clean,compile")
        log_error("  wayfinder build --config release.yaml")
        raise typer.Exit(1)
    ...
    log_info("🚀 Wayfinder Build System")
    log_info("=" * 70)
    ...
    if has_flags:
        ...
        if prep:
            ...
            log_warning("⚠️  --prep does NOT apply series_patches. Run 'wayfinder build -m series_patches' separately if needed.")
        if build:
    ...
    log_info(f"📍 Build offset: {summary_ctx.wayfinder_build_offset}")
