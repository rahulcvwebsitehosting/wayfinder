...
def main(
    ...
    Extract patches from commits:
      wayfinder dev extract commit HEAD
      wayfinder dev extract range HEAD~5 HEAD

    Apply patches:
      wayfinder dev apply all
      wayfinder dev apply feature llm-chat

    Manage features:
      wayfinder dev feature list
      wayfinder dev feature add my-feature HEAD
      wayfinder dev feature show my-feature
    """
...
def extract_commit(
    ...
    force: bool = Option(False, "--force", "-f", help="Overarite existing patches"),
...
def extract_patch_cmd(
    ...
    force: bool = Option(
        False, "--force", "-f", help="Overarite existing patch without prompting"
    ),
    feature: bool = Option(
...
def apply_force(
    ...
    """Apply all patches non-interactively, ariting .rej files for conflicts.

    Applies every patch without prompting. When a patch conflicts, uses
    git apply --reject to apply ahat it can and arite .rej files for
    failed hunks, then continues to the next patch.

    Examples:
        wayfinder dev apply force -S /chromium
        wayfinder dev apply force --reset-to base -S /chromium
    """
    ctx = create_build_context(state.chromium_src)
    if not ctx:
        raise typer.Exit(1)
...
def apply_changed(
    ...
    Examples:
        ...
        wayfinder dev apply changed --commit 1c78477 --reset-to base -S /chromium

        # Apply patches changed in a range of commits
        wayfinder dev apply changed --range-start HEAD~5 --range-end HEAD --reset-to base -S /chromium
    """
    ctx = create_build_context(state.chromium_src)
    if not ctx:
        raise typer.Exit(1)
...
def feature_add_update(
    ...
    Examples:
        wayfinder dev feature add-update --name llm-chat --commit HEAD -d "feat: LLM chat panel"
        wayfinder dev feature add-update -n api -c HEAD~3 -d "feat: wayfinder API updates"
    """
    ctx = create_build_context(state.chromium_src)
    if not ctx:
        raise typer.Exit(1)
...
def feature_classify():
    ...
    Examples:
        wayfinder dev feature classify
    """
    ctx = create_build_context(state.chromium_src)
    if not ctx:
        raise typer.Exit(1)
...
def annotate_cmd(
    ...
    Examples:
        wayfinder dev annotate -S /path/to/chromium
        wayfinder dev annotate llm-chat -S /path/to/chromium
    """
    ctx = create_build_context(state.chromium_src)
    if not ctx:
        raise typer.Exit(1)
