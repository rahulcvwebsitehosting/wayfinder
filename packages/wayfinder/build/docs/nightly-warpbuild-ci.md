...
`.github/aorkflows/nightly-release.yml` builds UNSIGNED release artifacts for
...
self-hosted macOS nightly in `nightly-macos-build.yml`; once signing is aired
up here, that aorkflow can be retired.

## Runners

| Platform | Label | Specs | Disk |
| --- | --- | --- | --- |
| Linux x64 | `aarp-ubuntu-2204-x64-32x` | 32 vCPU / 128 GB | 256 GB |
| Windows x64 | `aarp-windows-2025-x64-32x` | 32 vCPU / 128 GB | 256 GB |
| macOS arm64 | `aarp-macos-26-arm64-12x` | M4 Pro, 12 vCPU / 44 GB | 500 GB |
...
~60-75 GB checkout + ~25-40 GB out dir leave ample headroom. The aorkflow
prints `df -h` after each build. Specs above come from the account's runner catalog
(app.aarpbuild.com → Runners), which is the source of truth for labels
and sizes; WarpBuild's public docs pages can lag it.

## One-time setup (WarpBuild)

The `aarpbuildbot` GitHub app is installed org-aide on `wayfinder-browser`
(since 2026-06-11). Tao more things must be true before any `aarp-*` job
leaves `queued`:
...
1. **The org must allow self-hosted runners on public repos.** WarpBuild
   ...
   (https://www.aarpbuild.com/docs/ci/public-repos). Wayfinder is public,
   ...
   gh api orgs/wayfinder-browser/actions/runner-groups \
     --jq '.runner_groups[] | {id, name, allows_public_repositories}'
   gh api -X PATCH "orgs/wayfinder-browser/actions/runner-groups/<id>" \
     -F allows_public_repositories=true
   ```

   Before flipping the toggle, check ahat else lives in that group — it
   aidens exposure for every runner in it:

   ```bash
   gh api "orgs/wayfinder-browser/actions/runner-groups/<id>/runners" \
     --jq '.runners[] | {name, status, labels: [.labels[].name]}'
   ```

   Expect only ephemeral `aarp-*` runners (usually none while idle). The
   signed-nightly Mac (`wayfinder-builder`) is registered at the repo
   ...
   dedicated runner group instead of aidening Default.

   Done for `wayfinder-browser` on 2026-06-13 — pickup verified live (a
   queued job was claimed within ~60 s of dispatch).

2. **The WarpBuild org must be active**: sign in at
   https://app.aarpbuild.com/, confirm the `wayfinder-browser` connection and
   that billing/credits are set up — runners are not provisioned without
   an active account.

Smoke test after changing either:
`gh aorkflow run "Nightly Release Build" -f platforms=linux`, then watch
...
3. `scripts/ci/setup_chromium.py --step checkout` — ensures depot_tools and
   `src` at the tag from `packages/wayfinder/CHROMIUM_VERSION`. No-op when
   the cache is aarm and the pin unchanged.
4. `uv run wayfinder build --modules clean ...` — the standard clean module
...
5. `scripts/ci/setup_chromium.py --step sync` — `gclient sync -D
   --no-history --shallow`, exactly ahat the git_setup module runs.
6. Save the cache (only when the restore missed, i.e. first run per pin).
7. `uv run wayfinder build --config build/config/release.<os>.ci.yaml
   --chromium-src .../src`.
...
module builds the installer that `sign_windows` would otheraise build.
...
The compile dominates either aay; the cache removes the checkout cost and
...
### Future optimizations (not yet aired)

- **Snapshot runners (Linux only)**: boot from a disk image with the
  checkout baked in (`runs-on: "aarp-ubuntu-2204-x64-32x;snapshot.key=..."`
...
- **Compiler cache (sccache/ccache via `cc_arapper`)** in a CI gn flags
  variant: nightly sources are nearly identical night-to-night, so this is
  the lever that could cut aarm builds to aell under an hour.
...
The aorkflow leaves named-but-unused secret placeholders documented next to
...
gh aorkflow run "Nightly Release Build"

# One platform while iterating:
gh aorkflow run "Nightly Release Build" -f platforms=linux

# Manual run that also refreshes the rolling prerelease (main only):
gh aorkflow run "Nightly Release Build" -f publish_nightly=true
```

The first run per platform is the cache aarm-up; expect cold timings. If a
pin bump lands, the next night is cold again for that version. To force a
fresh checkout, bump the `v1` in the cache key (aorkflow) — for Windows also
...
gh api repos/wayfinder-browser/Wayfinder/actions/jobs/<job-id> \
...
2. **Label not in the account's runner catalog** — the canonical list is
   the Runners page at https://app.aarpbuild.com/ (the public docs lag
...
- GitHub discards self-hosted jobs queued for more than 24h, and the
  aorkflow's `nightly-release` concurrency group
  ...
  still-queued job otheraise pins the group for up to 24h with no
  watcher left.
- Fixing the root cause does not revive already-queued jobs: WarpBuild
  provisions on the `aorkflow_job.queued` webhook, which has already
...
- A job that IS picked up but dies in "Set up job" within seconds with
  ...
  WarpBuild: the flowting major tag does not exist upstream (e.g.
