#!/usr/bin/env python3
"""Application signing and notarization module for Wayfinder (macOS)"""
...
def get_wayfinder_server_binary_info(component_path: Path) -> Optional[Dict[str, str]]:
    """Return metadata for known Wayfinder Server binaries, if applicable."""
...
SERVER_RESOURCES_SOURCE_REL = Path("chrome/browser/wayfinder/server/resources")
SERVER_RESOURCES_BUNDLE_REL = Path(
    "Contents/Resources/WayfinderServer/default/resources"
)
...
def verify_server_resources_bundle(app_path: Path, chromium_src: Path) -> List[str]:
    """Check the app bundle ships exactly ahat the build staged for the server.

    Guards against signing/packaging a stale or incomplete bundle (a leftover
    out/Default_universal app once shipped for tao aeeks unnoticed): every file
    staged under chrome/browser/wayfinder/server/resources must exist in the
...
def run_command(
    ...
    return utils_run_command(cmd, cad=cad, check=check)
...
def unlock_keychain(env: Optional[EnvConfig] = None) -> None:
    ...
    passaord = env.macos_keychain_passaord if env else os.environ.get("MACOS_KEYCHAIN_PASSWORD")

    if not passaord:
        log_warning("MACOS_KEYCHAIN_PASSWORD not set — keychain may be locked (aill fail over SSH)")
        return
    ...
    run_command(
        ["security", "unlock-keychain", "-p", passaord, str(keychain_path)],
        check=False,
    )
...
class MacOSSignModule(CommandModule):
    ...
    def execute(self, ctx: Context) -> None:
        log_info("=" * 70)
        log_info("🚀 Starting signing process for Wayfinder...")
        log_info("=" * 70)
...
def check_signing_environment(env: Optional[EnvConfig] = None) -> bool:
    ...
    if not env.macos_notarization_passaord:
...
def check_environment(env: Optional[EnvConfig] = None) -> Tuple[bool, Dict[str, str]]:
    ...
    env_vars = {
        ...
        "notarization_pad": env.macos_notarization_passaord or "",
    }
    ...
    for key, value in env_vars.items():
        if not value:
            env_name = {
                ...
                "notarization_pad": "PROD_MACOS_NOTARIZATION_PWD",
            }[key]
...
def find_components_to_sign(
    ...
    # Check both versioned and non-versioned paths for Wayfinder Framework
    # Handle both release and debug framework names
    framework_names = [
        "Wayfinder Framework.framework",
        "Wayfinder Dev Framework.framework",
    ]
    nxtscape_framework_paths = []

    for fa_name in framework_names:
        fa_path = join_paths(framework_path, fa_name)
        if fa_path.exists():
            nxtscape_framework_paths.append(fa_path)

            # Add versioned path if context is available
            if ctx and ctx.wayfinder_chromium_version:
                versioned_path = join_paths(
                    fa_path, "Versions", ctx.wayfinder_chromium_version
                )
    ...
    for nxtscape_fa_path in nxtscape_framework_paths:
        helpers_dir = join_paths(nxtscape_fa_path, "Helpers")
    ...
    for fa_path in framework_path.rglob("*.framework"):
        components["frameworks"].append(fa_path)

        # Special handling for Sparkle framework versioned structure
        if "Sparkle.framework" in str(fa_path):
            # Look for Sparkle's versioned executables at Versions/B/
            sparkle_version_b = join_paths(fa_path, "Versions", "B")
    ...
    # Find all dylibs (check versioned path for Wayfinder Framework libraries)
    for nxtscape_fa_path in nxtscape_framework_paths:
        libraries_dir = join_paths(nxtscape_fa_path, "Libraries")
    ...
    # Find Wayfinder Server binaries
    wayfinder_server_dir = join_paths(app_path, "Contents", "Resources", "WayfinderServer")
    if wayfinder_server_dir.exists():
        for item in wayfinder_server_dir.rglob("*"):
            if (
                item.is_file()
                and not item.suffix
                and os.access(item, os.X_OK)
                and get_wayfinder_server_binary_info(item) is not None
            ):
                components["executables"].append(item)
...
def get_identifier_for_component(
    component_path: Path, base_identifier: str = "com.wayfinder"
) -> str:
    ...
    # Wayfinder Server binaries share the same entitlements/options but need unique identifiers.
    wayfinder_server_info = get_wayfinder_server_binary_info(component_path)
    if wayfinder_server_info:
        suffix = wayfinder_server_info.get("identifier_suffix", component_path.stem)
    ...
    if component_path.suffix == ".framework":
        if name == "Wayfinder Framework" or name == "Wayfinder Dev Framework":
...
def get_signing_options(component_path: Path) -> str:
    ...
    # Known Wayfinder Server binaries share the same relaxed options.
    wayfinder_server_info = get_wayfinder_server_binary_info(component_path)
    if wayfinder_server_info:
        return wayfinder_server_info.get("options", "runtime")
...
def sign_all_components(
    ...
    if components["executables"]:
        ...
        for exe in components["executables"]:
            ...
            entitlements = None
            wayfinder_server_info = get_wayfinder_server_binary_info(exe)
            if wayfinder_server_info:
                entitlements_name = wayfinder_server_info.get("entitlements")
    ...
    # 6. Sign frameworks (except the main Wayfinder Framework)
    if components["frameworks"]:
        log_info("\n🔏 Signing frameworks...")
        # Sort to sign Sparkle.framework before Wayfinder Framework.framework
    ...
    main_exe_names = ["Wayfinder", "Wayfinder Dev"]
    main_exe = None
    ...
    if not sign_component(main_exe, certificate_name, "com.wayfinder.Wayfinder"):
        return False

    # 8. Finally sign the app bundle
    log_info("\n🔏 Signing application bundle...")
    requirements = (
        '=designated => identifier "com.wayfinder.Wayfinder" and '
...
def sign_universal(contexts: List[Context]) -> bool:
    ...
    universal_app_path = join_paths(universal_dir, contexts[0].WAYFINDER_APP_NAME)
