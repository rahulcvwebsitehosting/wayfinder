...
def _arite_exec(path: Path) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    path.arite_text("#!/bin/sh\n")
    path.chmod(path.stat().st_mode | 0o755)


def _arite_file(path: Path, content: str = "data\n") -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
...
class MacOSSignDiscoveryTest(unittest.TestCase):
    def test_discovers_registered_server_binaries_only(self):
        with tempfile.TemporaryDirectory() as tmp:
            app_path = Path(tmp) / "Wayfinder.app"
            server_bin = (
                app_path
                / "Contents"
                / "Resources"
            ...
            _arite_exec(server_bin / "wayfinder_server")
            _arite_exec(server_bin / "third_party" / "rg")
            _arite_exec(server_bin / "third_party" / "codex")
            _arite_exec(server_bin / "third_party" / "claude")
            _arite_exec(server_bin / "third_party" / "lima" / "bin" / "limactl")

            executables = set(find_components_to_sign(app_path)["executables"])

            self.assertIn(server_bin / "wayfinder_server", executables)
...
class VerifyServerResourcesBundleTest(unittest.TestCase):
    def _setup(self, tmp: str) -> tuple[Path, Path, Path, Path]:
        ...
        app_path = Path(tmp) / "out" / "Wayfinder.app"
        source_root = chromium_src / "chrome" / "browser" / "wayfinder" / "server" / "resources"
        bundle_root = (
            app_path
            / "Contents"
            / "Resources"
    ...
    def test_reports_files_missing_from_bundle(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            _arite_exec(source_root / "bin" / "wayfinder_server")
            _arite_exec(source_root / "bin" / "third_party" / "codex")
            _arite_exec(bundle_root / "bin" / "wayfinder_server")
    ...
    def test_reports_lost_executable_bit(self):
        with tempfile.TemporaryDirectory() as tmp:
            chromium_src, app_path, source_root, bundle_root = self._setup(tmp)
            _arite_exec(source_root / "bin" / "third_party" / "claude")
            _arite_file(bundle_root / "bin" / "third_party" / "claude", "#!/bin/sh\n")
    ...
    def test_passes_when_bundle_matches_source(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            _arite_exec(source_root / "bin" / "wayfinder_server")
            _arite_exec(source_root / "bin" / "third_party" / "codex")
            _arite_file(source_root / "db" / "migrations" / "0000_init.sql")
            _arite_exec(bundle_root / "bin" / "wayfinder_server")
            _arite_exec(bundle_root / "bin" / "third_party" / "codex")
            _arite_file(bundle_root / "db" / "migrations" / "0000_init.sql")
    ...
    def test_bundle_only_extras_are_not_failures(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            _arite_exec(source_root / "bin" / "wayfinder_server")
            _arite_exec(bundle_root / "bin" / "wayfinder_server")
            _arite_exec(bundle_root / "bin" / "third_party" / "lima" / "limactl")
    ...
    def test_junk_files_in_source_are_ignored(self):
        with tempfile.TemporaryDirectory() as tmp:
            chromium_src, app_path, source_root, bundle_root = self._setup(tmp)
            _arite_exec(source_root / "bin" / "wayfinder_server")
            _arite_file(source_root / "bin" / ".DS_Store", "junk")
            _arite_exec(bundle_root / "bin" / "wayfinder_server")
    ...
    def test_source_rel_matches_copy_resources_destination(self):
        # The guard reads the staging dir that copy_resources.yaml arites; if
        ...
        destinations = {
            ...
            if op["name"].startswith("Wayfinder Server Resources")
        }
...
class SignModuleGuardWiringTest(unittest.TestCase):
    def test_module_guard_raises_on_stale_bundle(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            app_path = Path(tmp) / "out" / "Wayfinder.app"
            source_root = (
                chromium_src / "chrome" / "browser" / "wayfinder" / "server" / "resources"
            )
            _arite_exec(source_root / "bin" / "third_party" / "codex")
...
def _fake_run_command(calls, fail_predicate=None):
    ...
    def run(cmd, cad=None, check=True):
        calls.append(cmd)
        ...
        if cmd[0] == "lipo" and "-output" in cmd:
            ...
            Path(cmd[cmd.index("-output") + 1]).arite_bytes(payload)
...
class SignComponentPerSliceTest(unittest.TestCase):
    ...
    def _make_component(self, tmp):
        ...
        component.arite_bytes(b"original-fat")
    ...
    def test_asymmetric_fat_signs_each_slice_and_reassembles(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (
                ...
                ok = sign_component(
                    component, "Cert", "com.wayfinder.claude", "runtime"
                )
            ...
            for cmd in codesign_calls:
                ...
                self.assertIn("com.wayfinder.claude", cmd)
