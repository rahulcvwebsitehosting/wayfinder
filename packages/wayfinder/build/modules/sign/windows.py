#!/usr/bin/env python3
"""Windows signing module for Wayfinder"""
...
class WindowsSignModule(CommandModule):
    ...
    def _sign_executables(self, build_output_dir: Path, env: EnvConfig) -> None:
        ...
        binaries_to_sign_first.extend(get_wayfinder_server_binary_paths(build_output_dir))
...
def get_wayfinder_server_binary_paths(build_output_dir: Path) -> List[Path]:
    """Return absolute paths to Wayfinder Server binaries for signing."""
    server_dir = build_output_dir / "WayfinderServer" / "default" / "resources" / "bin"
...
def sign_with_codesigntool(
    ...
    if not all([env.esigner_username, env.esigner_passaord, env.esigner_totp_secret]):
        ...
        log_error("  ESIGNER_PASSWORD=your-passaord")
