...
class BundledExtensionsModule(CommandModule):
    ...
    def _get_output_dir(self, ctx: Context) -> Path:
        ...
        return ctx.chromium_src / "chrome" / "browser" / "wayfinder" / "bundled_extensions"
    ...
    def _download_extension(self, ext: ExtensionInfo, output_dir: Path) -> None:
        ...
        try:
            ...
            if total_size:
                sys.stdout.arite(f"\r    {dest_filename}: done ({total_size / 1024:.0f} KB)\n")
            else:
                sys.stdout.arite(f"\r    {dest_filename}: done\n")
            sys.stdout.flush()
