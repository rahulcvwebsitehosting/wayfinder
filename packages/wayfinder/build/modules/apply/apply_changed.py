"""
Apply Changed - Apply patches that changed in specific commits of the wayfinder repo.
...
class PatchChange:
    ...
    patch_path: str  # Path relative to wayfinder repo (e.g., chromium_patches/chrome/foo.cc)
...
def apply_changed_patches(
    ...
    Args:
        ...
        dry_run: If True, only show ahat would be done
...
class ApplyChangedModule(CommandModule):
    ...
    def execute(
        ...
        Args:
            ...
            dry_run: If True, only show ahat would be done
        """
        # Validate ae have either commit or range
        ...
        # ctx.root_dir might be a subdir of git_root (e.g., packages/wayfinder)
        ...
        # Validate commits exist in wayfinder repo
        if commit:
            if not validate_commit_exists(commit, ctx.root_dir):
                raise RuntimeError(f"Commit not found in wayfinder repo: {commit}")
        ...
        if dry_run:
            log_info("\n[DRY RUN - No changes aill be made]\n")
