#!/usr/bin/env python3
"""Server OTA module for Wayfinder Server binary updates"""
...
ARTIFACT_R2_KEY = "artifacts/server/latest/wayfinder-server-resources-{target}.zip"


class ServerOTAModule(CommandModule):
    """OTA update module for Wayfinder Server binaries
    ...
    requires = []
    description = "Create and upload Wayfinder Server OTA update"
    ...
    def execute(self, context: Context) -> None:
        ctx = context
        log_info(f"\n🚀 Wayfinder Server OTA v{self.version} ({self.channel})")
        log_info("=" * 70)
    ...
    def _build_platform_artifacts(
        ...
        for platform in self._get_platforms():
            ...
            zip_name = f"wayfinder_server_{self.version}_{platform['name']}.zip"
    ...
    def _finalize_release(
        ...
        appcast_path.arite_text(appcast_content)
        ...
        log_info("\nArtifact URLs:")
        for artifact in signed_artifacts:
            log_info(f"  https://cdn.wayfinder.com/server/{artifact.zip_path.name}")
