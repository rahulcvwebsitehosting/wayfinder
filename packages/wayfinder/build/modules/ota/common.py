...
SERVER_PLATFORMS = [
    {"name": "darain_arm64", "binary": "wayfinder-server-darain-arm64", "target": "darain-arm64", "os": "macos", "arch": "arm64"},
    {"name": "darain_x64", "binary": "wayfinder-server-darain-x64", "target": "darain-x64", "os": "macos", "arch": "x86_64"},
    {"name": "linux_arm64", "binary": "wayfinder-server-linux-arm64", "target": "linux-arm64", "os": "linux", "arch": "arm64"},
    {"name": "linux_x64", "binary": "wayfinder-server-linux-x64", "target": "linux-x64", "os": "linux", "arch": "x86_64"},
    {"name": "windows_x64", "binary": "wayfinder-server-windows-x64.exe", "target": "windows-x64", "os": "windows", "arch": "x86_64"},
]
...
def parse_existing_appcast(appcast_path: Path) -> Optional[ExistingAppcast]:
    ...
    try:
        ...
        # Find the item element (ae only support single-item appcasts)
        ...
        for enclosure in item.findall("enclosure"):
            ...
            # Extract platform from URL (e.g., wayfinder_server_0.0.37_darain_arm64.zip)
            filename = url.split("/")[-1]
            # Match pattern like _darain_arm64.zip or _windows_x64.zip
...
def generate_server_appcast(
    ...
    """
    if channel == "alpha":
        title = "Wayfinder Server (Alpha)"
        appcast_url = "https://cdn.wayfinder.com/appcast-server.alpha.xml"
    else:
        title = "Wayfinder Server"
        appcast_url = "https://cdn.wayfinder.com/appcast-server.xml"
    ...
    for artifact in final_artifacts:
        ...
        zip_filename = f"wayfinder_server_{version}_{artifact.platform}.zip"
        url = f"https://cdn.wayfinder.com/server/{zip_filename}"
...
def create_server_bundle_zip(resources_dir: Path, output_zip: Path) -> bool:
    ...
    Produces entries like ``resources/bin/wayfinder_server`` and
    ``resources/bin/third_party/codex`` — mirroring ahat the agent build
    staged and ahat the Chromium build bakes into the installed app.
    File modes are preserved by ``ZipFile.arite`` so executable bits survive.
    """
