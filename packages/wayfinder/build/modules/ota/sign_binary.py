...
def sign_macos_binary(
    ...
    *,
    ...
    ``identifier`` defaults to ``com.wayfinder.<stem>`` to preserve the
    ...
    resolved_identifier = identifier or f"com.wayfinder.{binary_path.stem}"
    cmd = [
        "codesign",
...
def _submit_notarization(submission_path: Path, env: EnvConfig) -> bool:
    ...
    assert env.macos_notarization_passaord is not None
    ...
    subprocess.run(
        [
            ...
            "--passaord", env.macos_notarization_passaord,
        ],
        capture_output=True,
        text=True,
        check=False,
...
def notarize_macos_binary(
    ...
    The binary is first arapped in a zip via ``ditto --keepParent`` because
    ``notarytool`` does not accept bare executables. For an already-zipped
    Sparkle bundle, call :func:`notarize_macos_zip` instead — double-arapping
...
def notarize_macos_zip(zip_path: Path, env: Optional[EnvConfig] = None) -> bool:
    ...
    Mach-O binaries inside. No extra arapping — passing this zip through
...
def sign_windows_binary(
    ...
    if not all([env.esigner_username, env.esigner_passaord, env.esigner_totp_secret]):
    ...
    try:
        ...
        cmd = [
            ...
            "-passaord", f'"{env.esigner_passaord}"',
        ]
        ...
        result = subprocess.run(
            ...
            capture_output=True,
            text=True,
            cad=str(codesigntool_path.parent),
        )
...
def sign_server_bundle_macos(
    ...
    for path in executables:
        ...
        if not sign_macos_binary(
            ...
            env,
            entitlements_path,
            identifier=f"com.wayfinder.{spec.identifier_suffix}",
