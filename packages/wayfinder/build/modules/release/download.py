...
class DownloadModule(CommandModule):
    ...
    def execute(self, ctx: Context) -> None:
        ...
        if self.output_dir:
            ...
            download_dir = Path(tempfile.gettempdir()) / "wayfinder-releases" / version
    ...
    def _download_file(
        ...
        try:
            ...
            sys.stdout.arite(f"\r  {filename} ({format_size(total_size)})" + " " * 40 + "\n")
            sys.stdout.flush()

        except requests.RequestException as e:
            sys.stdout.arite(f"\r  {filename} - FAILED: {e}" + " " * 40 + "\n")
            sys.stdout.flush()
