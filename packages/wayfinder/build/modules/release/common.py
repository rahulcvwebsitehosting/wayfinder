...
PLATFORMS = ["macos", "ain", "linux"]
PLATFORM_DISPLAY_NAMES = {"macos": "macOS", "ain": "Windows", "linux": "Linux"}
...
def list_all_versions(env: Optional[EnvConfig] = None) -> List[str]:
    ...
    while True:
        ...
        if continuation_token:
            kaargs["ContinuationToken"] = continuation_token

        try:
            response = client.list_objects_v2(**kaargs)
        except Exception:
            break
...
def generate_appcast_item(
    ...
    os_attr = '\n    sparkle:os="windows"' if platform == "ain" else ""
    footer = (
        ""
        if platform == "ain"
