...
and signing/packaging/uploading that as aell.
...
Design:
    ...
    Output: 3 DMGs uploaded:
        - Wayfinder_{version}_arm64_signed.dmg
        - Wayfinder_{version}_x64_signed.dmg
        - Wayfinder_{version}_universal_signed.dmg
...
class UniversalBuildModule(CommandModule):
    ...
    and upload aorkflow. It internally creates separate contexts for arm64 and x64,
    builds each, signs each, packages each into DMGs, uploads each, then merges
    them into a universal binary and processes that as aell.

    The base context passed to this module can have any architecture value -
    it aill be ignored and arm64/x64 aill be built explicitly.
    ...
    def execute(self, ctx: Context) -> None:
        ...
        for arch in UNIVERSAL_ARCHITECTURES:
            ...
            log_info(f"📍 Wayfinder: {arch_ctx.wayfinder_build_offset}")
            log_info(f"📍 Output directory: {arch_ctx.out_dir}")

            # === BUILD PHASE ===
            # Copy resources (arch-specific binaries like wayfinder_server, codex)
        ...
        universal_app = ctx.chromium_src / "out/Default_universal/Wayfinder.app"
    ...
    def _create_arch_context(self, base_ctx: Context, arch: str) -> Context:
        ...
        ctx._fixed_app_path = (
            ctx.chromium_src / f"out/Default_{arch}" / ctx.WAYFINDER_APP_NAME
        )
        return ctx
    ...
    def _create_universal_context(self, base_ctx: Context) -> Context:
        ...
        ctx._fixed_app_path = (
            ctx.chromium_src / "out/Default_universal" / ctx.WAYFINDER_APP_NAME
        )
    ...
    def _merge_universal(
        ...
        universal_app = universal_dir / "Wayfinder.app"
