...
class ConfigureValidateTest(unittest.TestCase):
    ...
    def test_passes_with_existing_flags_file(self):
        with (
            ...
            root.arite_gn_flags(get_platform(), "release", "is_debug = false\n")
            ctx = make_context(
...
class ConfigureExecuteTest(unittest.TestCase):
    def _execute(self, build_type: str, architecture: str = "x64"):
        ...
        root = MockWayfinderRoot(Path(root_tmp.name))
        root.arite_gn_flags(get_platform(), build_type, "is_official_build = true\n")
        ctx = make_context(
    ...
    def test_arites_args_gn_with_target_cpu(self):
