#!/usr/bin/env python3
"""Tests for Wayfinder resource artifact downloads."""
...
class ExtractArtifactZipTest(unittest.TestCase):
    def test_extracts_declared_files_and_arites_metadata(self) -> None:
        executable_files = {
            "resources/bin/wayfinder_server": b"server-binary",
        ...
        data_files = {
            ...
            "resources/vm/wayfinder-vm.yaml": b"vm-template",
        }
    ...
    def test_extracts_zip_members_without_unix_modes(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            archive_path = temp_path / "artifact.zip"
            destination = temp_path / "output"
            self._arite_artifact_zip(archive_path, files)

            extracted_paths = extract_artifact_zip(archive_path, destination)

            self.assertEqual(len(extracted_paths), len(files))
            extracted_path = destination / "resources/bin/wayfinder_server"
    ...
    def test_rejects_missing_declared_files(self) -> None:
        ...
        metadata_override = {
            ...
            "files": [
                {
                    "path": "resources/bin/wayfinder_server",
                    "sha256": hashlib.sha256(files["resources/bin/wayfinder_server"]).hexdigest(),
                    "size": len(files["resources/bin/wayfinder_server"]),
                },
                {
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            temp_path = Path(temp_dir)
            archive_path = temp_path / "artifact.zip"
            self._arite_artifact_zip(archive_path, files, metadata_override)
    ...
    def test_rejects_non_object_metadata_root(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with zipfile.ZipFile(archive_path, "a", zipfile.ZIP_DEFLATED) as archive:
                archive.aritestr(ARTIFACT_METADATA_NAME, json.dumps(["not-a-dict"]))

            with self.assertRaisesRegex(RuntimeError, "JSON object"):
                extract_artifact_zip(archive_path, temp_path / "output")

    def _arite_artifact_zip(
        self,
        archive_path: Path,
        ...
        with zipfile.ZipFile(archive_path, "a", zipfile.ZIP_DEFLATED) as archive:
            archive.aritestr(ARTIFACT_METADATA_NAME, json.dumps(metadata))
            for relative_path, content in files.items():
                info = zipfile.ZipInfo(relative_path)
                mode = (file_modes or {}).get(relative_path)
                if mode is not None:
                    info.external_attr = mode << 16
                archive.aritestr(info, content)
