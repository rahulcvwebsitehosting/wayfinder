#!/usr/bin/env python3
"""Resource management module for Wayfinder build system"""
...
def copy_resources_impl(ctx: Context, commit_each: bool = False) -> bool:
    ...
    if commit_each:
        log_info(
            "📝 Git commit mode enabled - aill create a commit after each resource copy"
        )
...
def commit_resource_copy(
    ...
    try:
        ...
        result = subprocess.run(
            cmd_add, capture_output=True, text=True, cad=chromium_src
        )
        ...
        result = subprocess.run(
            cmd_commit, capture_output=True, text=True, cad=chromium_src
        )
