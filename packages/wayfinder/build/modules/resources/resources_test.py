...
class CopyResourcesTest(unittest.TestCase):
    def setUp(self):
        ...
        self.root = MockWayfinderRoot(Path(self._root_tmp.name))
    ...
    def test_config_without_operations_is_noop(self):
        self.root.arite_copy_config({"something_else": True})
    ...
    def test_directory_operation_copies_tree(self):
        ...
        (src_dir / "app.png").arite_text("png-bytes")
        (src_dir / "nested" / "small.png").arite_text("small-bytes")
        self.root.arite_copy_config(
            {
                "copy_operations": [
                    {
                        "name": "Icons",
        ...
        dest = self.chromium.src / "chrome" / "app" / "theme" / "wayfinder"
    ...
    def test_file_operation_copies_and_creates_parents(self):
        ...
        (self.root.root / "resources" / "logo.icns").arite_text("icns")
        self.root.arite_copy_config(
            {
                "copy_operations": [
                    {
    ...
    def test_files_operation_copies_glob_matches(self):
        ...
        (ext_dir / "a.js").arite_text("a")
        (ext_dir / "b.js").arite_text("b")
        (ext_dir / "ignore.txt").arite_text("x")
        self.root.arite_copy_config(
            {
                "copy_operations": [
                    {
                        ...
                        "destination": "chrome/browser/resources/wayfinder",
                        "type": "files",
                    }
                ]
            }
        ...
        dest = self.chromium.src / "chrome" / "browser" / "resources" / "wayfinder"
    ...
    def test_condition_mismatches_skip_operation(self):
        (self.root.root / "resources").mkdir(exist_ok=True)
        (self.root.root / "resources" / "skipped.txt").arite_text("x")
        self.root.arite_copy_config(
            {
                "copy_operations": [
                    {
        ...
        self.assertFalse((self.chromium.src / "chrome" / "tao.txt").exists())
        self.assertFalse((self.chromium.src / "chrome" / "three.txt").exists())

    def test_matching_conditions_run_operation(self):
        (self.root.root / "resources").mkdir(exist_ok=True)
        (self.root.root / "resources" / "kept.txt").arite_text("kept")
        self.root.arite_copy_config(
            {
                "copy_operations": [
                    {
