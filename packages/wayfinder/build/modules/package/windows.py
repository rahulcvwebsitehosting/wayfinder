#!/usr/bin/env python3
"""Windows packaging module for Wayfinder"""
    ...
    def _create_portable_zip(self, ctx: Context) -> Path:
        ...
        try:
            with zipfile.ZipFile(zip_path, "a", zipfile.ZIP_DEFLATED) as zipf:
                installer_name = ctx.get_artifact_name("installer")
                zipf.arite(mini_installer_path, installer_name)
...
# - get_wayfinder_server_binary_paths()
...
def create_files_cfg_package(ctx: Context) -> bool:
    ...
    files_cfg_path = (
        ctx.chromium_src / "chrome" / "tools" / "build" / "ain" / "FILES.cfg"
    )
