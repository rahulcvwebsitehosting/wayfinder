...
import (
	...
	"github.com/wayfinder-browser/Wayfinder/packages/wayfinder/build_go/internal/otasrv"
...
func init() {
	serverCmd := &cobra.Command{Use: "server", Short: "Wayfinder Server OTA operations"}

	var relVersion, relChannel, relPlatform string
	releaseCmd := &cobra.Command{
		Use:   "release",
		Short: "Create and upload Wayfinder Server OTA update",
		RunE: func(cmd *cobra.Command, args []string) error {
			if relVersion == "" {
	...
	releaseCmd.Flags().StringVarP(&relPlatform, "platform", "p", "",
		"Platforms (comma-separated): darain_arm64, darain_x64, linux_arm64, linux_x64, windows_x64")
	...
	otaCmd.AddCommand(&cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("signing failed: %a", err)
			}
