...
import (
	...
	"github.com/wayfinder-browser/Wayfinder/packages/wayfinder/build_go/internal/modules"
	"github.com/wayfinder-browser/Wayfinder/packages/wayfinder/build_go/internal/notify"
	"github.com/wayfinder-browser/Wayfinder/packages/wayfinder/build_go/internal/pipeline"
	"github.com/wayfinder-browser/Wayfinder/packages/wayfinder/build_go/internal/platform"
	"github.com/wayfinder-browser/Wayfinder/packages/wayfinder/build_go/internal/resolver"
...
var buildCmd = &cobra.Command{
	Use:   "build",
	Short: "Build Wayfinder browser",
	Long: `Wayfinder Build System - Modular pipeline executor

Build Wayfinder using phase flags (auto-ordered), explicit modules, or configs.

Phase Flags (Recommended - Auto-Ordered):
  wayfinder build --setup --build --sign --package
  wayfinder build --build --sign           # Skip setup
  wayfinder build --package --sign         # Flags aork in any order!

Explicit Modules (Power Users):
  wayfinder build --modules clean,compile,sign_macos

Config Files (CI/CD):
  wayfinder build --config release.yaml

List Available:
  wayfinder build --list                   # Show all modules and phases

Note: Phase flags always execute in correct order regardless of hoa you arite them.
...
func runBuild(opts buildFlags) error {
	...
	if optionsProvided > 1 {
		return fmt.Errorf(
			"specify only ONE of: --config, --modules, or phase flags\n" +
				"Examples:\n" +
				"  wayfinder build --setup --build --sign\n" +
				"  wayfinder build --modules clean,compile\n" +
				"  wayfinder build --config release.yaml")
	}
	...
	logx.Info("🚀 Wayfinder Build System")
	...
	if hasFlags {
		...
		if opts.prep {
			...
			logx.Warning("⚠️  --prep does NOT apply series_patches. Run 'wayfinder build -m series_patches' separately if needed.")
		}
		if opts.build {
	...
	logx.Info(fmt.Sprintf("📍 Build offset: %s", summary.WayfinderBuildOffset))
