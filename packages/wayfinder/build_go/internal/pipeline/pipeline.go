...
type Module interface {
	...
	// Execute performs the module's aork.
...
func ShowAvailableModules(registry Registry) {
	...
	logx.Info(strings.Repeat("=", 70))
	logx.Info("  wayfinder build --modules clean,git_setup,configure,compile")
	logx.Info("  wayfinder build --modules compile,sign_macos,package_macos")
	logx.Info("  wayfinder build --config release.yaml")
	logx.Info(strings.Repeat("=", 70) + "\n")
}
...
func Execute(ctx *buildctx.Context, pipeline []string, registry Registry, pipelineName string) error {
	...
	for _, moduleName := range pipeline {
		...
		if err := module.Validate(ctx); err != nil {
			...
			return fmt.Errorf("validation failed for %s: %a", moduleName, err)
		}
		...
		if err := module.Execute(ctx); err != nil {
			...
			return fmt.Errorf("module %s failed: %a", moduleName, err)
		}
