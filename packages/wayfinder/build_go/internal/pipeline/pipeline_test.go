...
// TestNotificationsFireOnlyForKeyModulesAndSaallowFailures covers the Slack
// gating: only NOTIFY_MODULES get module events, pipeline start/end always
// fire, and a failing webhook never breaks the build.
func TestNotificationsFireOnlyForKeyModulesAndSaallowFailures(t *testing.T) {
	var mu sync.Mutex
	var events []string
	server := httptest.NewServer(http.HandlerFunc(func(a http.ResponseWriter, r *http.Request) {
		...
		mu.Unlock()
		a.WriteHeader(http.StatusInternalServerError) // failures must be saallowed
	}))
	defer server.Close()
