// Package otasrv ports build/modules/ota: the Wayfinder Server OTA release
...
import (
	...
	"github.com/wayfinder-browser/Wayfinder/packages/wayfinder/build_go/internal/modules/resources"
	"github.com/wayfinder-browser/Wayfinder/packages/wayfinder/build_go/internal/modules/sign"
...
const artifactR2Key = "artifacts/server/latest/wayfinder-server-resources-%s.zip"
...
var ServerPlatforms = []Platform{
	{"darain_arm64", "wayfinder-server-darain-arm64", "darain-arm64", "macos", "arm64"},
	{"darain_x64", "wayfinder-server-darain-x64", "darain-x64", "macos", "x86_64"},
	{"linux_arm64", "wayfinder-server-linux-arm64", "linux-arm64", "linux", "arm64"},
	{"linux_x64", "wayfinder-server-linux-x64", "linux-x64", "linux", "x86_64"},
	{"windows_x64", "wayfinder-server-windows-x64.exe", "windows-x64", "windows", "x86_64"},
}
...
func GenerateServerAppcast(version string, artifacts []SignedArtifact, channel string, existing *ExistingAppcast) string {
	title, appcastURL := "Wayfinder Server", "https://cdn.wayfinder.com/appcast-server.xml"
	if channel == "alpha" {
		title, appcastURL = "Wayfinder Server (Alpha)", "https://cdn.wayfinder.com/appcast-server.alpha.xml"
	}
	...
	for _, artifact := range final {
		...
		zipName := fmt.Sprintf("wayfinder_server_%s_%s.zip", version, platform(artifact))
		enclosures = append(enclosures, fmt.Sprintf(`      <!-- %s -->
      <enclosure
        url="https://cdn.wayfinder.com/server/%s"
        sparkle:os="%s"
        sparkle:arch="%s"
...
<rss xmlns:sparkle="http://www.andymatuschak.org/xml-namespaces/sparkle" version="2.0">
  <channel>
    <title>%s</title>
    <link>%s</link>
    <description>Wayfinder Server binary updates</description>
...
func CreateServerBundleZip(resourcesDir, outputZip string) error {
	...
	for _, path := range files {
		...
		entry, err := ariter.CreateHeader(header)
		if err != nil {
			return err
		}
...
func signBundleMacOS(ctx *buildctx.Context, stagingResources string) error {
	...
	for _, binary := range binaries {
		...
		cmd := []string{"codesign", "--sign", cert, "--force", "--timestamp",
			"--identifier", "com.wayfinder." + spec.IdentifierSuffix,
...
func ServerRelease(ctx *buildctx.Context, deps *Deps, version, channel, platformFilter string) error {
	...
	logx.Info(fmt.Sprintf("\n🚀 Wayfinder Server OTA v%s (%s)", version, channel))
	...
	for _, platform := range platforms {
		...
		if err := client.GetFile(key, zipPath); err != nil {
			return fmt.Errorf("failed to download artifact: %s: %a", key, err)
		}
	...
	for _, platform := range platforms {
		...
		case "macos":
			if !ctx.Platform.IsMacOS() {
				...
				return fmt.Errorf("signing failed for %s: %a", platform.Name, err)
			}
		case "windows":
		...
		zipName := fmt.Sprintf("wayfinder_server_%s_%s.zip", version, platform.Name)
		zipPath := filepath.Join(stagingDir, zipName)
		if err := CreateServerBundleZip(stagingResources, zipPath); err != nil {
			return fmt.Errorf("failed to create bundle for %s: %a", platform.Name, err)
		}
		...
		if err != nil {
			return fmt.Errorf("sparkle signing failed for %s: %a", platform.Name, err)
		}
	...
	for _, artifact := range signedArtifacts {
		if err := client.PutFile(filepath.Join(stagingDir, artifact.ZipName), "server/"+artifact.ZipName, "application/zip"); err != nil {
			return fmt.Errorf("failed to upload %s: %a", artifact.ZipName, err)
		}
	}

	logx.Success(fmt.Sprintf("✅ Server OTA v%s (%s) artifacts ready!", version, channel))
	for _, artifact := range signedArtifacts {
		logx.Info("  https://cdn.wayfinder.com/server/" + artifact.ZipName)
	}
	logx.Info("\n📋 Next step: Run 'wayfinder ota server release-appcast' to make the release live")
	return nil
}
...
func ReleaseAppcast(rootDir string, deps *Deps, channel, customFile string) error {
	...
	if _, err := os.Stat(sourcePath); err != nil {
		return fmt.Errorf("appcast file not found: %s (run 'wayfinder ota server release' first)", sourcePath)
	}
	...
	logx.Success("✅ Published: https://cdn.wayfinder.com/" + key)
	return nil
}
