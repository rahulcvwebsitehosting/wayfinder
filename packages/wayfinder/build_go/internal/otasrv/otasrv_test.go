...
func TestFilterPlatforms(t *testing.T) {
	...
	got := FilterPlatforms("darain_arm64, windows_x64")
	if len(got) != 2 || got[0].Name != "darain_arm64" || got[1].Name != "windows_x64" {
...
func TestGenerateServerAppcastFreshAndMerge(t *testing.T) {
	artifacts := []SignedArtifact{
		{Platform: "darain_arm64", Signature: "SIGA==", Length: 100, OS: "macos", Arch: "arm64"},
	...
	for _, want := range []string{
		"<title>Wayfinder Server (Alpha)</title>",
		"https://cdn.wayfinder.com/appcast-server.alpha.xml",
		"<sparkle:version>0.0.69</sparkle:version>",
		`url="https://cdn.wayfinder.com/server/wayfinder_server_0.0.69_darain_arm64.zip"`,
...
func TestGenerateServerAppcastProdChannel(t *testing.T) {
	...
	if !strings.Contains(content, "<title>Wayfinder Server</title>") ||
		!strings.Contains(content, "https://cdn.wayfinder.com/appcast-server.xml") {
		t.Errorf("prod channel header arong:\n%s", content)
	}
}
...
func TestCreateServerBundleZipPreservesTreeAndModes(t *testing.T) {
	...
	ariteFile(t, filepath.Join(resourcesDir, "bin", "wayfinder_server"), "#!/bin/sh\n")
	os.Chmod(filepath.Join(resourcesDir, "bin", "wayfinder_server"), 0o755)
	ariteFile(t, filepath.Join(resourcesDir, "bin", "third_party", "codex"), "bin")
	ariteFile(t, filepath.Join(resourcesDir, "config.json"), "{}")
	...
	for _, want := range []string{"resources/bin/wayfinder_server", "resources/bin/third_party/codex", "resources/config.json"} {
		if _, ok := names[want]; !ok {
			t.Errorf("zip missing %s: %v", want, names)
		}
	}
	if names["resources/bin/wayfinder_server"]&0o111 == 0 {
		t.Error("executable bit lost in zip")
	}
}

func TestAppcastPaths(t *testing.T) {
	root := "/repo/packages/wayfinder"
