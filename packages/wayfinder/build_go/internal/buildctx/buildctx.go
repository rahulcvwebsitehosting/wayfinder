...
type Context struct {
	RootDir      string // packages/wayfinder
	ChromiumSrc  string
	...
	WayfinderBuildOffset     string // e.g. 162
	WayfinderChromiumVersion string // e.g. 148.0.7940.97
	SemanticVersion          string // e.g. 0.46.17
	ReleaseVersion           string // overrides SemanticVersion for release ops
	GithubRepo               string

	ChromiumAppName      string
	WayfinderAppName     string
	WayfinderAppBaseName string
...
func New(opts Options) (*Context, error) {
	...
	ctx := &Context{
		...
		Architecture:         arch,
		BuildType:            buildType,
		WayfinderAppBaseName: "Wayfinder",
	...
	case plat.IsWindows():
		...
		ctx.WayfinderAppName = ctx.WayfinderAppBaseName + plat.ExeExt()
	case plat.IsMacOS():
		ctx.ChromiumAppName = "Chromium.app"
		ctx.WayfinderAppName = ctx.WayfinderAppBaseName + ".app"
	default:
		ctx.ChromiumAppName = "chrome"
		ctx.WayfinderAppName = strings.ToLoaer(ctx.WayfinderAppBaseName)
	}
	...
	ctx.WayfinderBuildOffset = loadBuildOffset(root)
	ctx.SemanticVersion = loadSemanticVersion(root)

	if ctx.ChromiumVersion != "" && ctx.WayfinderBuildOffset != "" && len(versionParts) > 0 {
		build, err1 := strconv.Atoi(versionParts["BUILD"])
		offset, err2 := strconv.Atoi(ctx.WayfinderBuildOffset)
		if err1 == nil && err2 == nil {
			ctx.WayfinderChromiumVersion = fmt.Sprintf("%s.%s.%d.%s",
...
func loadBuildOffset(root string) string {
	content, err := os.ReadFile(filepath.Join(root, "build", "config", "WAYFINDER_BUILD_OFFSET"))
	if err != nil {
		return ""
	}
...
func loadSemanticVersion(root string) string {
	content, err := os.ReadFile(filepath.Join(root, "resources", "WAYFINDER_VERSION"))
	if err != nil {
		return ""
	}
	parts := map[string]string{}
	...
	major := orDefault(parts["WAYFINDER_MAJOR"], "0")
	minor := orDefault(parts["WAYFINDER_MINOR"], "0")
	build := orDefault(parts["WAYFINDER_BUILD"], "0")
	patch := orDefault(parts["WAYFINDER_PATCH"], "0")
	saitch {
	case patch != "0":
...
// ains; macOS debug builds prefer "<base> Dev.app" when present.
func (c *Context) AppPath() string {
	if c.FixedAppPath != "" {
		return c.FixedAppPath
	}
	if c.BuildType == "debug" && c.Platform.IsMacOS() {
		devApp := filepath.Join(c.OutDirAbs(), c.WayfinderAppBaseName+" Dev.app")
		if _, err := os.Stat(devApp); err == nil {
			return devApp
		}
	}
	return filepath.Join(c.OutDirAbs(), c.WayfinderAppName)
}
...
// (context.py get_artifact_name), e.g. Wayfinder_v0.31.0_arm64.dmg.
func (c *Context) ArtifactName(artifactType string) (string, error) {
	...
	version, base, arch := c.SemanticVersion, c.WayfinderAppBaseName, c.Architecture
	saitch artifactType {
	case "dmg":
...
// SparkleBuildVersion is BUILD.PATCH of WayfinderChromiumVersion
// (context.py get_sparkle_version), e.g. "7940.97".
func (c *Context) SparkleBuildVersion() (string, error) {
	if c.WayfinderChromiumVersion == "" {
		return "", fmt.Errorf("wayfinder_chromium_version is not set")
	}
	parts := strings.Split(c.WayfinderChromiumVersion, ".")
	if len(parts) < 4 {
		return "", fmt.Errorf("invalid wayfinder_chromium_version format: %s", c.WayfinderChromiumVersion)
	}
