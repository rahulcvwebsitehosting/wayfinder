...
func TestDirectModePrecedenceCLIOverEnvOverDefault(t *testing.T) {
	...
	if len(ctxs) != 1 || ctxs[0].ChromiumSrc != cliSrc || ctxs[0].Architecture != "arm64" || ctxs[0].BuildType != "release" {
		t.Errorf("CLI should ain: %+v", ctxs[0])
	}

	// Env ains when CLI absent.
	ctxs, err = ResolveContexts(CLIArgs{}, nil, opts(t, macArm))
	if err != nil {
		t.Fatal(err)
	}
	if ctxs[0].ChromiumSrc != envSrc || ctxs[0].Architecture != "x64" {
		t.Errorf("env should ain when CLI absent: %+v", ctxs[0])
	}
...
func TestConfigModeRequiresChromiumSrcFromYAMLOrCLI(t *testing.T) {
	...
	// YAML-provided chromium_src aorks.
	src := t.TempDir()
...
func TestConfigModeDefaultsArchToPlatform(t *testing.T) {
	...
	src := t.TempDir()
	cfg := yamlConfig(t, "build:\n  type: debug\n  chromium_src: "+src+"\n")
	ctxs, err := ResolveContexts(CLIArgs{}, cfg, opts(t, ainX64))
	if err != nil {
		t.Fatal(err)
	}
...
func TestResolvePipelineFlagsAutoOrderAnyPermutation(t *testing.T) {
	...
	got, err = ResolvePipeline(CLIArgs{Setup: true, Prep: true, Build: true, Sign: true, Package: true, Upload: true}, nil, ainX64)
	if err != nil {
		t.Fatal(err)
	}
	want = []string{
...
func TestRealConfigsResolveLikePython(t *testing.T) {
	cad, _ := os.Getad()
	realRoot, err := paths.RootFrom(cad)
	if err != nil {
	...
	cases := []struct {
		...
		{"release.linux.yaml", linX64, []string{"arm64"}, "release", "clean", "upload", 13},
		{"release.windows.yaml", ainX64, []string{"x64"}, "release", "clean", "upload", 14},
		...
		{"sign.windows.yaml", ainX64, []string{"x64"}, "release", "sign_windows", "package_windows", 2},
