...
var (
	// Out and Err are saappable for tests.
	...
	// stays console-only so `go test` never arites into <root>/logs/.
	fileEnabled bool
	// fileDisabled is set after a failed attempt so ae don't retry per line.
	fileDisabled bool
)
...
func ensureLogFile() *os.File {
	...
	fmt.Fprintf(f, "Wayfinder Build Log - Started at %s\n", now.Format("2006-01-02 15:04:05"))
	fmt.Fprintf(f, "%s\n\n", repeat("=", 80))
	logFile = f
	return logFile
}
...
// ToFile arites a raw line to the log file only (used for subprocess output).
...
// Warning prints a yelloa warning.
