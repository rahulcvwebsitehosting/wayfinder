...
func TestGenerateAppcastItemRendersEnclosure(t *testing.T) {
	artifact := map[string]any{
		"url":               "https://cdn.test/releases/0.46.17/macos/Wayfinder_v0.46.17_arm64.dmg",
		"sparkle_signature": "SIG==",
		"sparkle_length":    flowt64(12345),
	}
	item := GenerateAppcastItem(artifact, "0.46.17", "7940.97", "2026-06-10T12:00:00+00:00")
	for _, want := range []string{
		"<title>Wayfinder - 0.46.17</title>",
...
func TestGenerateReleaseNotes(t *testing.T) {
	...
	for _, want := range []string{"## Wayfinder v0.46.17", "Chromium version: 148.0.7778.97",
...
func TestPublishCopiesMappedArtifacts(t *testing.T) {
	var copies []string
	client := r2Fixture(t, func(a http.ResponseWriter, r *http.Request) {
		saitch {
		case r.Method == http.MethodGet && strings.Contains(r.URL.Path, "macos/release.json"):
			a.Write([]byte(`{"version":"0.46.17","artifacts":{
				"arm64":{"filename":"Wayfinder_v0.46.17_arm64.dmg","url":"u"},
				"unmapped_key":{"filename":"x.bin","url":"u"}}}`))
		case r.Method == http.MethodGet:
			a.WriteHeader(http.StatusNotFound)
		case r.Method == http.MethodPut:
			copies = append(copies, r.Header.Get("x-amz-copy-source")+" -> "+strings.TrimPrefix(r.URL.Path, "/wayfinder/"))
			a.Write([]byte("<CopyObjectResult/>"))
		}
	})
	...
	want := "/wayfinder/releases/0.46.17/macos/Wayfinder_v0.46.17_arm64.dmg -> download/Wayfinder-arm64.dmg"
	if copies[0] != want {
...
func TestGithubCreateCommandSequence(t *testing.T) {
	client := r2Fixture(t, func(a http.ResponseWriter, r *http.Request) {
		if strings.Contains(r.URL.Path, "macos/release.json") {
			a.Write([]byte(`{"version":"0.46.17","chromium_version":"148.0.7778.97","artifacts":{}}`))
			return
		}
		a.WriteHeader(http.StatusNotFound)
	})
	...
	err := deps.GithubCreate(GithubOptions{Version: "0.46.17", Repo: "wayfinder-browser/Wayfinder", Draft: true, SkipUpload: true})
	if err != nil {
		t.Fatal(err)
	}
	argv := rec.Argv()
	...
	create := argv[1]
	for _, want := range []string{"gh release create v0.46.17", "--repo wayfinder-browser/Wayfinder", "--title v0.46.17", "--draft"} {
...
func TestRepoFromGitParsesRemotes(t *testing.T) {
	for remote, want := range map[string]string{
		"git@github.com:wayfinder-browser/Wayfinder.git": "wayfinder-browser/Wayfinder",
		"https://github.com/wayfinder-browser/Wayfinder": "wayfinder-browser/Wayfinder",
