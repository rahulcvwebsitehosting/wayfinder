...
// PatchChange is one changed chromium_patches/ file in the wayfinder repo
// (apply/apply_changed.py PatchChange).
type PatchChange struct {
	PatchPath    string // path relative to the wayfinder repo
...
// ChangedFilesInCommit lists (status, path) pairs for one wayfinder-repo
