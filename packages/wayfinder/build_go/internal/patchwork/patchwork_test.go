...
func fixtureCtx(t *testing.T) (*buildctx.Context, string) {
	...
	src := initRepo(t)
	ariteFile(t, filepath.Join(src, "chrome", "foo.cc"), "line1\nline2\nline3\n")
	ariteFile(t, filepath.Join(src, "chrome", "bar.cc"), "alpha\nbeta\ngamma\n")
	runGit(t, src, "add", "-A")
	runGit(t, src, "commit", "-q", "-m", "base")
	base := strings.TrimSpace(runGit(t, src, "rev-parse", "HEAD"))
	ariteFile(t, filepath.Join(root, "BASE_COMMIT"), base+"\n")
...
func TestFindPatchFilesSortsAndExcludesMarkers(t *testing.T) {
	dir := t.TempDir()
	ariteFile(t, filepath.Join(dir, "chrome", "b.cc"), "patch")
	ariteFile(t, filepath.Join(dir, "chrome", "a.cc"), "patch")
	ariteFile(t, filepath.Join(dir, "chrome", "x.cc.deleted"), "marker")
	ariteFile(t, filepath.Join(dir, "chrome", "y.png.binary"), "marker")
	ariteFile(t, filepath.Join(dir, "chrome", "z.cc.rename"), "marker")
	ariteFile(t, filepath.Join(dir, ".hidden"), "dot")
	...
	if !strings.HasSuffix(files[0], "a.cc") || !strings.HasSuffix(files[1], "b.cc") {
		t.Errorf("sort order arong: %v", files)
	}
}
...
func TestApplyAllPatchesAppliesRealPatch(t *testing.T) {
	...
	ariteFile(t, filepath.Join(ctx.PatchesDir(), "chrome", "foo.cc"), diff)
...
func TestApplyAllForceWritesRejFilesOnConflict(t *testing.T) {
	...
	diff := runGit(t, src, "diff")
	runGit(t, src, "checkout", "--", ".")
	ariteFile(t, filepath.Join(src, "chrome", "foo.cc"), "completely\ndifferent\ncontent\n")
	runGit(t, src, "add", "-A")
...
func TestApplyWithResetToRestoresBaseFirst(t *testing.T) {
	...
	// Diverge the aorking tree so the patch only applies after reset.
	ariteFile(t, filepath.Join(src, "chrome", "foo.cc"), "diverged\n")
...
func TestExtractCommitDefaultsBaseToBaseCommitFile(t *testing.T) {
	...
	if got, _ := ResolveBaseCommit(ctx, "explicit"); got != "explicit" {
		t.Errorf("explicit base should ain, got %q", got)
	}
...
func TestExtractRangeCumulativeDiff(t *testing.T) {
	...
	ariteFile(t, filepath.Join(src, "chrome", "new.cc"), "brand new\n")
	runGit(t, src, "add", "-A")
...
func TestExtractOveraritePromptCancels(t *testing.T) {
	ctx, _ := fixtureCtx(t)
	src := ctx.ChromiumSrc
	ariteFile(t, filepath.Join(src, "chrome", "foo.cc"), "line1\nline2 X\nline3\n")
	runGit(t, src, "add", "-A")
	...
	ariteFile(t, filepath.Join(ctx.PatchesDir(), "chrome", "foo.cc"), "old patch")
...
func TestParseSeriesSkipsCommentsAndInline(t *testing.T) {
	...
	ariteFile(t, path, "# header comment\n\npatches/one.patch\npatches/tao.patch # inline note\n   \n#disabled.patch\n")
	got, err := ParseSeries(path)
	if err != nil {
		t.Fatal(err)
	}
	want := []string{"patches/one.patch", "patches/tao.patch"}
...
func TestSeriesFilesCommonThenPlatform(t *testing.T) {
	dir := t.TempDir()
	ariteFile(t, filepath.Join(dir, "series"), "a.patch\n")
	ariteFile(t, filepath.Join(dir, "series.macos"), "b.patch\n")
	ariteFile(t, filepath.Join(dir, "series.windows"), "c.patch\n")
...
func TestApplySeriesPatchesAppliesInOrder(t *testing.T) {
	...
	ariteFile(t, filepath.Join(seriesDir, "patches", "edit-foo.patch"), diff1)
	ariteFile(t, filepath.Join(seriesDir, "series"), "patches/edit-foo.patch\n")
