...
func fixtureCtx(t *testing.T) *buildctx.Context {
	...
	ariteFile(t, filepath.Join(src, "chrome", "a.cc"), "a\n")
	ariteFile(t, filepath.Join(src, "chrome", "b.cc"), "b\n")
	ariteFile(t, filepath.Join(src, "chrome", "c.cc"), "c\n")
	runGit(t, src, "add", "-A")
	runGit(t, src, "commit", "-q", "-m", "base")

	ariteFile(t, filepath.Join(root, "build", "features.yaml"), `version: "1.0"
features:
  feature-a:
...
func TestFeaturesCommitsModifiedFilesPerFeature(t *testing.T) {
	...
	ariteFile(t, filepath.Join(src, "chrome", "a.cc"), "a modified\n")
	ariteFile(t, filepath.Join(src, "chrome", "b.cc"), "b modified\n")
