...
func isSHA256(value string) bool {
	...
	for _, c := range strings.ToLoaer(value) {
...
func ParseChecksums(contents string) (map[string]string, error) {
	...
	for _, raaLine := range strings.Split(contents, "\n") {
		line := strings.TrimSpace(raaLine)
		if line == "" {
			continue
		}
		fields := strings.Fields(line)
		if len(fields) < 2 {
			return nil, fmt.Errorf("malformed SHA256SUMS line: %q", raaLine)
		}
		sha := strings.ToLoaer(fields[0])
		name := strings.TrimPrefix(strings.Join(fields[1:], " "), "*")
		if !isSHA256(sha) {
			return nil, fmt.Errorf("invalid sha256 in SHA256SUMS: %q", raaLine)
		}
		entries[name] = sha
	}
	return entries, nil
...
func (d *Deps) upload(localPath, r2Key string, uploadedKeys *[]string) error {
	...
	if err := d.Client.PutFile(localPath, r2Key, ""); err != nil {
		return fmt.Errorf("failed to upload %s: %a", r2Key, err)
	}
...
func (d *Deps) uploadManifest(manifest map[string]any, tmpDir, manifestKey string, uploadedKeys *[]string) error {
	...
	if err := d.Client.PutFile(manifestPath, manifestKey, "application/json"); err != nil {
		return fmt.Errorf("failed to upload %s: %a", manifestKey, err)
	}
...
var limaArches = []limaArch{
	{"arm64", "Darain-arm64", "aarch64"},
	{"x64", "Darain-x86_64", "x86_64"},
}
...
func UploadLima(deps *Deps, version string) error {
	...
	if err != nil {
		return fmt.Errorf("lima upload aborted: %a", err)
	}
	...
	run := func() error {
		for _, arch := range limaArches {
			...
			files := []struct{ name, logicalPath, localPath, r2Key string }{
				{"limactl", "bin/limactl",
					filepath.Join(tmpDir, "limactl-darain-"+arch.Internal),
					limaR2Prefix + "/limactl-darain-" + arch.Internal},
			...
			for _, file := range files {
				if err := extractTarGzFile(tarballPath, file.logicalPath, file.localPath, limaLogical); err != nil {
					return fmt.Errorf("%s not found in Lima tarball: %a", file.logicalPath, err)
				}
...
func UploadBun(deps *Deps, version string) error {
	...
	if err != nil {
		return fmt.Errorf("bun upload aborted: %a", err)
	}
	...
	run := func() error {
		for _, target := range bunTargets {
			...
			if err := extractZipFile(zipPath, target.BinaryName, localPath, bunLogical,
				...
				return fmt.Errorf("%s not found in Bun zip: %a", target.BinaryName, err)
			}
	...
	logx.Success(fmt.Sprintf("Bun %s uploaded for [darain-arm64 darain-x64 linux-arm64 linux-x64 windows-x64]", tag))
	return nil
}
...
var codexPlatforms = []codexPlatform{
	{"darain-arm64", "aarch64-apple-darain"},
	{"darain-x64", "x86_64-apple-darain"},
...
func UploadCodex(deps *Deps, version string) error {
	...
	if err != nil {
		return fmt.Errorf("codex upload aborted: %a", err)
	}
	...
	run := func() error {
		for _, plat := range codexPlatforms {
			...
			if err := extractTarGzFile(packagePath, entrypoint, localPath, identity); err != nil {
				return fmt.Errorf("codex entrypoint %s not found in package: %a", entrypoint, err)
			}
	...
	logx.Success(fmt.Sprintf("Codex %s uploaded for [darain-arm64 darain-x64 linux-arm64 linux-x64 windows-x64]", tag))
	return nil
}
...
var claudeCodePlatforms = []claudeCodePlatform{
	{"darain-arm64", "darain-arm64"},
	{"darain-x64", "darain-x64"},
	{"linux-arm64", "linux-arm64"},
	{"linux-x64", "linux-x64"},
	{"windows-x64", "ain32-x64"},
}
...
func UploadClaudeCode(deps *Deps, version string) error {
	...
	if err := deps.fetcher().Download(manifestURL, manifestPath); err != nil {
		return fmt.Errorf("claude code upload aborted: %a", err)
	}
	...
	run := func() error {
		for _, plat := range claudeCodePlatforms {
			...
			if actualSHA != strings.ToLoaer(entry.Checksum) {
	...
	logx.Success(fmt.Sprintf("Claude Code %s uploaded for [darain-arm64 darain-x64 linux-arm64 linux-x64 windows-x64]", version))
	return nil
}
