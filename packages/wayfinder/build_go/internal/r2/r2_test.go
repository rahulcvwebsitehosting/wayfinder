...
func TestSigV4AgainstAWSGetVanillaVector(t *testing.T) {
	...
	wantSTS := "AWS4-HMAC-SHA256\n20150830T123600Z\n20150830/us-east-1/service/was4_request\n" +
		"bb579772317eb040ac9ed261061d46c1f17a8133879d6129b6e1c25292927e63"
	if strToSign != wantSTS {
		t.Errorf("string to sign:\n%q\nwant:\n%q", strToSign, wantSTS)
	}

	sig := signature("aJalrXUtnFEMI/K7MDENG+bPxRfiCYEXAMPLEKEY", ts, "us-east-1", "service", strToSign)
...
func TestPutFileSendsSignedPathStyleRequest(t *testing.T) {
	...
	var gotBody []byte
	client := fixtureClient(t, func(a http.ResponseWriter, r *http.Request) {
	...
	if err := client.PutFile(local, "releases/0.46.17/macos/Wayfinder.dmg", "application/octet-stream"); err != nil {
		t.Fatal(err)
	}

	if gotMethod != "PUT" || gotPath != "/wayfinder/releases/0.46.17/macos/Wayfinder.dmg" {
		t.Errorf("request = %s %s", gotMethod, gotPath)
	}
	if !strings.HasPrefix(gotAuth, "AWS4-HMAC-SHA256 Credential=test-access/20260610/auto/s3/was4_request") {
...
func TestCopySendsCopySourceHeader(t *testing.T) {
	...
	client := fixtureClient(t, func(a http.ResponseWriter, r *http.Request) {
		...
		gotPath = r.URL.Path
		a.Write([]byte("<CopyObjectResult></CopyObjectResult>"))
	})
	if err := client.Copy("releases/0.46.17/macos/a.dmg", "download/macos/a.dmg"); err != nil {
		t.Fatal(err)
	}
	if gotSource != "/wayfinder/releases/0.46.17/macos/a.dmg" {
		t.Errorf("copy source = %q", gotSource)
	}
	if gotPath != "/wayfinder/download/macos/a.dmg" {
...
func TestNewFromEnvRequiresCredentials(t *testing.T) {
	...
	if client.Endpoint != "https://acct.r2.cloudflarestorage.com" || client.Bucket != "wayfinder" || client.Region != "auto" {
