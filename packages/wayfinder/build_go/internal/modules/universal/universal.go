...
import (
	...
	compilemod "github.com/wayfinder-browser/Wayfinder/packages/wayfinder/build_go/internal/modules/compile"
...
func (Build) Execute(ctx *buildctx.Context) error {
	...
	universalApp := filepath.Join(universalDir, "Wayfinder.app")
...
func archContext(base *buildctx.Context, arch string) (*buildctx.Context, error) {
	...
	ctx.FixedAppPath = filepath.Join(ctx.ChromiumSrc, "out", "Default_"+arch, ctx.WayfinderAppName)
	return ctx, nil
}
...
func mergeUniversal(ctx *buildctx.Context, arm64App, x64App, outputApp string) error {
	...
	if err != nil {
		return fmt.Errorf("failed to merge architectures into universal binary: %a", err)
	}
	return nil
}
