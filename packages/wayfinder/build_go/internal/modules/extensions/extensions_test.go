...
<gupdate xmlns="http://www.google.com/update2/response" protocol="2.0">
  ...
  <app appid="zzzzyyyyxxxxaaaavvvvuuuuttttssss">
...
func fixtureCtx(t *testing.T) *buildctx.Context {
	t.Helper()
	root := t.TempDir()
	os.WriteFile(filepath.Join(root, "pyproject.toml"), []byte("name = \"wayfinder\"\n"), 0o644)
...
// urlRearitingFetcher redirects the production manifest URL to a test server.
type urlRearitingFetcher struct {
	inner    fetch.Fetcher
	manifest string
}

func (f urlRearitingFetcher) Download(url, dest string) error {
...
func TestBundledDownloadsCrxAndWritesJSON(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(a http.ResponseWriter, r *http.Request) {
		...
		case strings.HasSuffix(r.URL.Path, "manifest.xml"):
			a.Write([]byte(strings.ReplaceAll(manifestXML, "%s", "http://"+r.Host)))
		case strings.HasSuffix(r.URL.Path, ".crx"):
			a.Write([]byte("crx-data-" + filepath.Base(r.URL.Path)))
		default:
	...
	ctx := fixtureCtx(t)
	module := Bundled{Fetcher: urlRearitingFetcher{inner: fetch.Default(), manifest: server.URL + "/manifest.xml"}}
	if err := module.Execute(ctx); err != nil {
		t.Fatal(err)
	}

	outDir := OutputDir(ctx)
	for _, id := range []string{"aaaabbbbccccddddeeeeffffgggghhhh", "zzzzyyyyxxxxaaaavvvvuuuuttttssss"} {
...
func TestBundledFailsWhenManifestUnreachable(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(a http.ResponseWriter, r *http.Request) {
		a.WriteHeader(http.StatusInternalServerError)
	}))
	defer server.Close()
