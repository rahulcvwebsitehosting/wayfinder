...
func jobsCfg(plat platform.Platform, env map[string]string, memGB flowt64, memOK bool, cpus int) JobsConfig {
	return JobsConfig{
		Getenv:     func(k string) string { return env[k] },
		Platform:   plat,
		TotalMemGB: func() (flowt64, bool) { return memGB, memOK },
		NumCPU:     cpus,
	}
}
...
func TestComputeNinjaJobsEnvOverrideAndNonWindows(t *testing.T) {
	...
	env["WAYFINDER_NINJA_JOBS"] = "banana"
...
func TestAutoninjaCommandShape(t *testing.T) {
	got := AutoninjaCommand(jobsCfg(ainX64, nil, 32, true, 64), `out\Default_x64`, []string{"chrome", "chromedriver"})
...
func fixtureCtx(t *testing.T) (*buildctx.Context, *execx.RecordingRunner) {
	...
	ariteFile(t, ctx.GNArgsFile(), "target_cpu = \"arm64\"\n")
	return ctx, rec
}
...
func TestCompileWritesVersionAndRunsAutoninja(t *testing.T) {
	...
	if artifact, ok := ctx.Artifact("built_app"); !ok || !strings.HasSuffix(artifact, "Wayfinder.app") {
...
func TestCompileRenamesChromiumAppWhenPresent(t *testing.T) {
	...
	ariteFile(t, filepath.Join(ctx.ChromiumAppPath(), "Contents", "Info.plist"), "<plist/>")
	...
	if _, err := os.Stat(filepath.Join(ctx.AppPath(), "Contents", "Info.plist")); err != nil {
		t.Errorf("Wayfinder.app should exist after rename: %v", err)
	}
