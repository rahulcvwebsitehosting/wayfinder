...
// ComputeNinjaJobs resolves the -j value: WAYFINDER_NINJA_JOBS override, else
...
func ComputeNinjaJobs(cfg JobsConfig) int {
	if override := cfg.Getenv("WAYFINDER_NINJA_JOBS"); override != "" {
		jobs, err := strconv.Atoi(override)
		if err == nil && jobs > 0 {
			logx.Info(fmt.Sprintf("Ninja parallelism: -j %d (WAYFINDER_NINJA_JOBS override)", jobs))
			return jobs
		}
		logx.Warning(fmt.Sprintf("Ignoring invalid WAYFINDER_NINJA_JOBS=%q", override))
	}
	...
	logx.Info(fmt.Sprintf(
		"Ninja parallelism: -j %d (capped by %d GB RAM / %d GB per job; override with WAYFINDER_NINJA_JOBS)",
		jobs, int(totalGB), gbPerCompileJob))
	return jobs
}
...
func (Compile) Description() string { return "Build Wayfinder using autoninja" }
...
func (Compile) Validate(ctx *buildctx.Context) error {
	...
	if ctx.WayfinderChromiumVersion == "" {
		return fmt.Errorf("Wayfinder chromium version not set")
	}
...
func (m Compile) Execute(ctx *buildctx.Context) error {
	...
	// Rename Chromium.app → Wayfinder.app when the build produced the
...
func createVersionFile(ctx *buildctx.Context) error {
	parts := strings.Split(ctx.WayfinderChromiumVersion, ".")
	if len(parts) != 4 {
		logx.Warning(fmt.Sprintf("Invalid version format: %s", ctx.WayfinderChromiumVersion))
		return nil
	}
	...
	logx.Info(fmt.Sprintf("Created VERSION file: %s", ctx.WayfinderChromiumVersion))
	return nil
}
