...
var brandingReplacements = []struct {
	...
	{regexp.MustCompile(`The Chromium Authors. All rights reserved.`), "The Wayfinder Authors. All rights reserved."},
	{regexp.MustCompile(`Google LLC. All rights reserved.`), "The Wayfinder Authors. All rights reserved."},
	{regexp.MustCompile(`The Chromium Authors`), "Wayfinder Software Inc"},
	{regexp.MustCompile(`Google Chrome`), "Wayfinder"},
	{regexp.MustCompile(`Google( Play)?`), ""}, // placeholder; handled specially below
	{regexp.MustCompile(`Chromium`), "Wayfinder"},
	{regexp.MustCompile(`Chrome`), "Wayfinder"},
}
	...
	for _, target := range stringReplaceTargets {
		...
		if content != original {
			if err := os.WriteFile(fullPath, []byte(content), 0o644); err != nil {
				...
				failed = fmt.Errorf("string replacements failed: %a", err)
				continue
			}
