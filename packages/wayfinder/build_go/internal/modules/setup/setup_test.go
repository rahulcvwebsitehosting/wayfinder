...
func fixtureCtx(t *testing.T, plat platform.Platform, arch, buildType string) (*buildctx.Context, *execx.RecordingRunner) {
	...
	ariteFile(t, filepath.Join(root, "resources", "WAYFINDER_VERSION"),
		"WAYFINDER_MAJOR=0\nWAYFINDER_MINOR=46\nWAYFINDER_BUILD=17\nWAYFINDER_PATCH=0\n")
	ariteFile(t, filepath.Join(root, "build", "config", "gn", "flags.macos.release.gn"), "is_official_build = true\n")
	ariteFile(t, filepath.Join(root, "build", "config", "gn", "flags.macos.debug.gn"), "is_debug = true\n")
	ariteFile(t, filepath.Join(root, "build", "config", "gn", "flags.windows.release.gn"), "is_official_build = true\n")
	ariteFile(t, filepath.Join(root, "build", "config", "gn", "flags.linux.release.gn"), "is_official_build = true\n")
...
func TestCleanRemovesOutDirAndRunsGitResetSequence(t *testing.T) {
	...
	ariteFile(t, filepath.Join(outDir, "obj", "stale.o"), "stale")
	ariteFile(t, filepath.Join(ctx.SparkleDir(), "Sparkle.framework", "f"), "x")
...
func TestGitSetupUsesBatOnWindows(t *testing.T) {
	ctx, rec := fixtureCtx(t, ainX64, "x64", "release")
	rec.Handler = func(c execx.Cmd) (execx.Result, error) {
		if strings.HasPrefix(c.String(), "git tag -l") {
