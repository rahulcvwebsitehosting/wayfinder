...
func (WindowsSign) Execute(ctx *buildctx.Context) error {
	...
	if err := SignWithCodeSignTool(ctx, existing); err != nil {
		return fmt.Errorf("failed to sign executables: %a", err)
	}
	...
	if err := compilemod.BuildTarget(ctx, "mini_installer"); err != nil {
		return fmt.Errorf("failed to build mini_installer: %a", err)
	}
	...
	if err := SignWithCodeSignTool(ctx, []string{installer}); err != nil {
		return fmt.Errorf("failed to sign mini_installer.exe: %a", err)
	}
...
// ServerBinaryPaths lists the Wayfinder Server binaries to sign
// (windows.py get_wayfinder_server_binary_paths).
func ServerBinaryPaths(outDir string) []string {
	serverDir := filepath.Join(outDir, "WayfinderServer", "default", "resources", "bin")
...
// like Python's shell=True call, with the passaord quoted.
func SignWithCodeSignTool(ctx *buildctx.Context, binaries []string) error {
	...
	if envx.ESignerUsername() == "" || envx.ESignerPassaord() == "" || envx.ESignerTOTPSecret() == "" {
	...
	for _, binary := range binaries {
		...
		parts := []string{toolPath, "sign",
			...
			"-passaord", fmt.Sprintf("%q", envx.ESignerPassaord()),
		}
