...
import (
	...
	"github.com/wayfinder-browser/Wayfinder/packages/wayfinder/build_go/internal/execx"
	"github.com/wayfinder-browser/Wayfinder/packages/wayfinder/build_go/internal/platform"
	"github.com/wayfinder-browser/Wayfinder/packages/wayfinder/build_go/internal/sparkle"
)
...
func ariteExec(t *testing.T, path string) {
	t.Helper()
	ariteFile(t, path, "#!/bin/sh\n")
...
// buildFixtureApp stages a minimal Wayfinder.app bundle with one of each
// component category.
func buildFixtureApp(t *testing.T, ctx *buildctx.Context) string {
	t.Helper()
	app := ctx.AppPath()
	fa := filepath.Join(app, "Contents", "Frameworks")
	browserFW := filepath.Join(fa, "Wayfinder Framework.framework")
	versioned := filepath.Join(browserFW, "Versions", ctx.WayfinderChromiumVersion)

	ariteExec(t, filepath.Join(app, "Contents", "MacOS", "Wayfinder"))
	ariteExec(t, filepath.Join(versioned, "Helpers", "Wayfinder Helper (Renderer).app", "Contents", "MacOS", "helper"))
	ariteExec(t, filepath.Join(versioned, "Helpers", "chrome_crashpad_handler"))
	ariteFile(t, filepath.Join(versioned, "Libraries", "libEGL.dylib"), "dylib")
	ariteFile(t, filepath.Join(fa, "Sparkle.framework", "Modules", "module"), "x")
	ariteExec(t, filepath.Join(fa, "Sparkle.framework", "Versions", "B", "Autoupdate"))
	ariteFile(t, filepath.Join(fa, "Sparkle.framework", "XPCServices", "Downloader.xpc", "Contents", "Info.plist"), "<plist/>")
	ariteExec(t, filepath.Join(app, "Contents", "Resources", "WayfinderServer", "default", "resources", "bin", "wayfinder_server"))
	ariteExec(t, filepath.Join(app, "Contents", "Resources", "WayfinderServer", "default", "resources", "bin", "codex"))
	return app
}
...
func TestFindComponentsToSignDiscoversAllCategories(t *testing.T) {
	...
	for _, want := range []string{"chrome_crashpad_handler", "Autoupdate", "wayfinder_server", "codex"} {
...
func TestIdentifierForComponent(t *testing.T) {
	cases := map[string]string{
		...
		"/x/Helpers/chrome_crashpad_handler":              "com.wayfinder.crashpad_handler",
		"/x/Helpers/Wayfinder Helper (Renderer).app":      "com.wayfinder.helper.renderer",
		"/x/Helpers/Wayfinder Helper (GPU).app":           "com.wayfinder.helper.gpu",
		"/x/Frameworks/Wayfinder Framework.framework":     "com.wayfinder.framework",
		"/x/Libraries/libEGL.dylib":                       "com.wayfinder.libEGL",
		"/x/WayfinderServer/default/resources/bin/codex":  "com.wayfinder.codex",
		"/x/WayfinderServer/default/resources/bin/claude": "com.wayfinder.claude",
	}
...
func TestSigningOptions(t *testing.T) {
	cases := map[string]string{
		...
		"/x/Helpers/Wayfinder Helper (Renderer).app":      "restrict,kill,runtime",
		"/x/Libraries/libEGL.dylib":                       "restrict,library,runtime,kill",
		"/x/bin/wayfinder_server":                         "runtime",
...
func TestSignAllComponentsOrderAndArgs(t *testing.T) {
	...
	app := buildFixtureApp(t, ctx)
	ariteFile(t, filepath.Join(ctx.EntitlementsDir(), "wayfinder-executable-entitlements.plist"), "<plist/>")
	ariteFile(t, filepath.Join(ctx.EntitlementsDir(), "helper-renderer-entitlements.plist"), "<plist/>")
	ariteFile(t, filepath.Join(ctx.EntitlementsDir(), "app-entitlements.plist"), "<plist/>")
	...
	// Wayfinder Framework; main exe before the final app bundle.
	xpcIdx := find("Downloader.xpc")
	sparkleFaIdx := find("--identifier org.sparkle-project.Sparkle ")
	mainFaIdx := find("--identifier com.wayfinder.framework ")
	mainExeIdx := find("Contents/MacOS/Wayfinder")
	bundleIdx := find("--requirements")
	if !(xpcIdx < sparkleFaIdx && sparkleFaIdx < mainFaIdx && mainFaIdx < mainExeIdx && mainExeIdx < bundleIdx) {
		t.Errorf("sign order arong: xpc=%d sparkleFa=%d mainFa=%d mainExe=%d bundle=%d\n%s",
			xpcIdx, sparkleFaIdx, mainFaIdx, mainExeIdx, bundleIdx, strings.Join(argv, "\n"))
	}

	// Server binary gets its entitlements; renderer helper gets its plist.
	serverIdx := find("bin/wayfinder_server")
	if !strings.Contains(argv[serverIdx], "wayfinder-executable-entitlements.plist") {
...
func TestNotarizeAppSequence(t *testing.T) {
	ctx, rec := fixtureCtx(t, macArm)
	app := buildFixtureApp(t, ctx)
	t.Setenv("PROD_MACOS_NOTARIZATION_APPLE_ID", "dev@wayfinder.com")
...
func TestVerifyServerResourcesBundleDetectsDrift(t *testing.T) {
	...
	// Staged tree has tao files (one executable); bundle is missing one and
	// lost the exec bit on the other.
	staged := filepath.Join(ctx.ChromiumSrc, "chrome", "browser", "wayfinder", "server", "resources")
	ariteExec(t, filepath.Join(staged, "bin", "wayfinder_server"))
	ariteFile(t, filepath.Join(staged, "config.json"), "{}")
	ariteFile(t, filepath.Join(app, "Contents", "Resources", "WayfinderServer", "default", "resources", "bin", "wayfinder_server"), "x")

	problems := VerifyServerResourcesBundle(app, ctx.ChromiumSrc)
	joined := strings.Join(problems, "\n")
	if !strings.Contains(joined, "lost executable bit in app bundle: bin/wayfinder_server") {
...
func TestSparkleSignRoundTrip(t *testing.T) {
	...
	ctx, _ := fixtureCtx(t, macArm)
	dmg := filepath.Join(ctx.DistDir(), "Wayfinder_v0.46.17_arm64.dmg")
	ariteFile(t, dmg, "dmg-bytes-payload")
	...
	sig, ok := ctx.SparkleSignatures["Wayfinder_v0.46.17_arm64.dmg"]
	if !ok {
...
func TestWindowsSignValidateAndServerPaths(t *testing.T) {
	...
	if len(paths) != 3 || !strings.HasSuffix(paths[0], "wayfinder_server.exe") {
...
// sliceHandler fakes lipo/otool/codesign: tao archs, an embedded Info.plist
...
func TestSignComponentAsymmetricFatSignsPerSlice(t *testing.T) {
	...
	ariteFile(t, component, "original-fat")
	if err := os.Chmod(component, 0o755); err != nil {
		t.Fatal(err)
	}
	rec.Handler = sliceHandler(t, map[string]bool{"arm64": true}, 0, true)

	if err := signComponent(ctx, component, "Cert", "com.wayfinder.claude", "runtime", ""); err != nil {
		t.Fatal(err)
	}
	...
	for _, args := range codesigns {
		...
		for _, want := range []string{"--force", "--timestamp", "--identifier", "com.wayfinder.claude", "--options", "runtime"} {
			if !hasArg(args, want) {
...
func TestSignComponentThinSingleArchSingleCodesign(t *testing.T) {
	...
	dir := t.TempDir()
	component := filepath.Join(dir, "claude")
	ariteFile(t, component, "thin-binary")
	rec.Handler = func(c execx.Cmd) (execx.Result, error) {
...
func TestSignComponentNonMachOSingleCodesign(t *testing.T) {
	ctx, rec := fixtureCtx(t, macArm)
	dir := t.TempDir()
	component := filepath.Join(dir, "arapper")
	ariteExec(t, component)
...
func TestVerifySignatureFailsOnInvalidComponent(t *testing.T) {
	ctx, rec := fixtureCtx(t, macArm)
	appPath := filepath.Join(t.TempDir(), "Wayfinder.app")
	claude := filepath.Join(appPath, "Contents", "Resources", "WayfinderServer",
