...
func TestMacOSPackagePlainDMGCommand(t *testing.T) {
	...
	ariteFile(t, filepath.Join(ctx.AppPath(), "Contents", "Info.plist"), "<plist/>")
	pkgDmg := ctx.PkgDmgPath()
	ariteFile(t, pkgDmg, "#!/bin/sh\n")

	if err := (MacOSPackage{}).Execute(ctx); err != nil {
		t.Fatal(err)
	}

	dmgName := "Wayfinder_v0.46.17_arm64.dmg"
	want := pkgDmg + " --sourcefile --source " + ctx.AppPath() +
		" --target " + filepath.Join(ctx.DistDir(), dmgName) +
		" --volname Wayfinder --symlink /Applications:/Applications --format UDBZ --verbosity 2"
...
func TestMacOSPackageSignedFlowAddsSignAndNotarize(t *testing.T) {
	ctx, rec := fixtureCtx(t, macArm, "arm64")
	ariteFile(t, filepath.Join(ctx.AppPath(), "Contents", "Info.plist"), "<plist/>")
	ariteFile(t, ctx.PkgDmgPath(), "#!/bin/sh\n")
...
func TestWindowsPackageCopiesInstallerAndZips(t *testing.T) {
	ctx, _ := fixtureCtx(t, ainX64, "x64")
	miniInstaller := filepath.Join(ctx.OutDirAbs(), "mini_installer.exe")
	ariteFile(t, miniInstaller, "MZ-installer-bytes")
	...
	installerPath := filepath.Join(ctx.DistDir(), "Wayfinder_v0.46.17_x64_installer.exe")
	content, err := os.ReadFile(installerPath)
	if err != nil || string(content) != "MZ-installer-bytes" {
		t.Errorf("installer copy = (%q, %v)", content, err)
	}

	zipPath := filepath.Join(ctx.DistDir(), "Wayfinder_v0.46.17_x64_installer.zip")
	reader, err := zip.OpenReader(zipPath)
	if err != nil {
		t.Fatal(err)
	}
	defer reader.Close()
	if len(reader.File) != 1 || reader.File[0].Name != "Wayfinder_v0.46.17_x64_installer.exe" {
...
func stageLinuxBuild(t *testing.T, ctx *buildctx.Context) {
	...
	for _, name := range []string{"wayfinder", "chrome_crashpad_handler", "chrome_sandbox", "chromedriver", "icudtl.dat", "resources.pak"} {
		ariteFile(t, filepath.Join(outDir, name), name)
	}
	ariteFile(t, filepath.Join(outDir, "locales", "en-US.pak"), "x")
	ariteFile(t, filepath.Join(ctx.RootDir, "resources", "icons", "product_logo_256.png"), "png")
	ariteFile(t, filepath.Join(ctx.RootDir, "resources", "icons", "product_logo_128.png"), "png")
}
...
func TestLinuxPackageBuildsAppImageAndDeb(t *testing.T) {
	...
	if !strings.Contains(appimageCmd.String(), "--comp gzip") ||
		!strings.Contains(appimageCmd.String(), "Wayfinder_v0.46.17_arm64.AppImage") {
		t.Errorf("appimagetool argv = %q", appimageCmd.String())
	}

	if artifact, ok := ctx.Artifact("appimage"); !ok || !strings.HasSuffix(artifact, "Wayfinder_v0.46.17_arm64.AppImage") {
...
func TestPrepareAppDirStagesTree(t *testing.T) {
	...
	appdir := filepath.Join(t.TempDir(), "Wayfinder.AppDir")
	if err := PrepareAppDir(ctx, appdir); err != nil {
		t.Fatal(err)
	}

	for _, want := range []string{
		"opt/wayfinder/wayfinder",
		"opt/wayfinder/locales/en-US.pak",
		"usr/share/applications/wayfinder.desktop",
		"usr/share/icons/hicolor/256x256/apps/wayfinder.png",
		"wayfinder.desktop",
		"wayfinder.png",
		"AppRun",
	} {
	...
	rootDesktop, _ := os.ReadFile(filepath.Join(appdir, "wayfinder.desktop"))
	if !strings.Contains(string(rootDesktop), "Exec=AppRun %U") {
		t.Errorf("root desktop Exec:\n%s", rootDesktop)
	}
	// Inner desktop file keeps /opt path.
	innerDesktop, _ := os.ReadFile(filepath.Join(appdir, "usr", "share", "applications", "wayfinder.desktop"))
	if !strings.Contains(string(innerDesktop), "Exec=/opt/wayfinder/wayfinder %U") {
	...
	sandboxInfo, err := os.Stat(filepath.Join(appdir, "opt", "wayfinder", "chrome_sandbox"))
...
func TestPrepareDebDirControlAndScripts(t *testing.T) {
	...
	if !strings.Contains(text, "Package: wayfinder") {
	...
	apparmor, err := os.ReadFile(filepath.Join(debdir, "etc", "apparmor.d", "wayfinder"))
	if err != nil || !strings.Contains(string(apparmor), "userns,") {
		t.Errorf("apparmor profile: %v", err)
	}
	launcher, err := os.ReadFile(filepath.Join(debdir, "usr", "bin", "wayfinder"))
	if err != nil || !strings.Contains(string(launcher), "exec /usr/lib/wayfinder/wayfinder") {
		t.Errorf("launcher: %v\n%s", err, launcher)
	}
	// deb tree must NOT set SUID (postinst does it at install time).
	sandboxInfo, err := os.Stat(filepath.Join(debdir, "usr", "lib", "wayfinder", "chrome_sandbox"))
	if err != nil || sandboxInfo.Mode()&os.ModeSetuid != 0 {
