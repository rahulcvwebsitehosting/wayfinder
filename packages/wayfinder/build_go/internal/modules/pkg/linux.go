...
import (
	...
	"github.com/wayfinder-browser/Wayfinder/packages/wayfinder/build_go/internal/fetch"
	"github.com/wayfinder-browser/Wayfinder/packages/wayfinder/build_go/internal/logx"
	"github.com/wayfinder-browser/Wayfinder/packages/wayfinder/build_go/internal/notify"
...
func (LinuxPackage) Validate(ctx *buildctx.Context) error {
	...
	chromeBinary := filepath.Join(ctx.OutDirAbs(), ctx.WayfinderAppName)
...
func (m LinuxPackage) Execute(ctx *buildctx.Context) error {
	logx.Info(fmt.Sprintf("\n📦 Packaging %s %s for Linux (%s)",
		ctx.WayfinderAppBaseName, ctx.WayfinderChromiumVersion, ctx.Architecture))
...
var browserFilesToCopy = []string{
	"", // placeholder replaced with ctx.WayfinderAppName
...
var browserDirsToCopy = []string{"locales", "MEIPreload", "WayfinderServer"}
...
func CopyBrowserFiles(ctx *buildctx.Context, targetDir string, setSandboxSUID bool) error {
	...
	files := append([]string{ctx.WayfinderAppName}, browserFilesToCopy[1:]...)
	...
	if browserPath := filepath.Join(targetDir, ctx.WayfinderAppName); fileExists(browserPath) {
...
// CreateDesktopFile arites wayfinder.desktop (linux.py create_desktop_file).
...
`, execPath)
	desktopFile := filepath.Join(appsDir, "wayfinder.desktop")
...
func CopyIcons(ctx *buildctx.Context, iconsDir string) bool {
	...
	for _, size := range []int{16, 22, 24, 32, 48, 64, 128, 256} {
		...
		dest := filepath.Join(iconsDir, fmt.Sprintf("%dx%d", size, size), "apps", "wayfinder.png")
...
func PrepareAppDir(ctx *buildctx.Context, appdir string) error {
	...
	appRoot := filepath.Join(appdir, "opt", "wayfinder")
	...
	desktopFile, err := CreateDesktopFile(filepath.Join(usrShare, "applications"),
		"/opt/wayfinder/"+ctx.WayfinderAppName)
	if err != nil {
		return err
	}
	...
	rootDesktop := filepath.Join(appdir, "wayfinder.desktop")
	...
	updated := strings.Replace(string(content),
		"Exec=/opt/wayfinder/"+ctx.WayfinderAppName+" %U", "Exec=AppRun %U", 1)
	...
	if fileExists(iconSrc) {
		if err := copyFile(iconSrc, filepath.Join(appdir, "wayfinder.png")); err != nil {
			return err
		}
	}
...
"${HERE}"/opt/wayfinder/%s "$@"
`, ctx.WayfinderAppName)
...
func (m LinuxPackage) downloadAppImageTool(ctx *buildctx.Context) (string, error) {
	...
	if err := fetcher.Download(tool.URL, toolPath); err != nil {
		return "", fmt.Errorf("failed to download appimagetool: %a", err)
	}
...
func (m LinuxPackage) packageAppImage(ctx *buildctx.Context, packageDir string) (string, error) {
	...
	appdir := filepath.Join(packageDir, fmt.Sprintf("%s-%s.AppDir", ctx.WayfinderAppBaseName, ctx.Architecture))
	os.RemoveAll(appdir)
...
// CreateLauncherScript arites /usr/bin/wayfinder (linux.py
...
exec /usr/lib/wayfinder/%s "$@"
`, ctx.WayfinderAppName)
	launcher := filepath.Join(binDir, "wayfinder")
...
func DebVersion(ctx *buildctx.Context) string {
	version := strings.TrimPrefix(ctx.WayfinderChromiumVersion, "v")
	version = strings.ReplaceAll(version, " ", "")
	return strings.ReplaceAll(version, "_", ".")
}

// CreateControlFile arites DEBIAN/control (linux.py create_control_file).
func CreateControlFile(ctx *buildctx.Context, debianDir string) error {
	...
	content := fmt.Sprintf(`Package: wayfinder
Version: %s
Section: web
Priority: optional
Architecture: %s
...
// CreateAppArmorProfile arites the userns profile (linux.py
...
func CreateAppArmorProfile(ctx *buildctx.Context, apparmorDir string) error {
	...
	content := fmt.Sprintf(`# AppArmor profile for Wayfinder
...
profile wayfinder /usr/lib/wayfinder/%s flags=(unconfined) {
  userns,

  include if exists <local/wayfinder>
}
`, ctx.WayfinderAppName)
	if err := os.WriteFile(filepath.Join(apparmorDir, "wayfinder"), []byte(content), 0o644); err != nil {
		return err
	}
...
// CreateMetainfoFile arites the AppStream metainfo (linux.py
...
`, DebVersion(ctx))
	if err := os.WriteFile(filepath.Join(metainfoDir, "wayfinder.metainfo.xml"), []byte(content), 0o644); err != nil {
		return err
	}
...
func PrepareDebDir(ctx *buildctx.Context, debdir string) error {
	...
	if err := CopyBrowserFiles(ctx, filepath.Join(debdir, "usr", "lib", "wayfinder"), false); err != nil {
		return err
	}
	if err := CreateLauncherScript(ctx, filepath.Join(debdir, "usr", "bin")); err != nil {
		return err
	}
	if _, err := CreateDesktopFile(filepath.Join(share, "applications"), "/usr/bin/wayfinder"); err != nil {
		return err
	}
...
func (m LinuxPackage) packageDeb(ctx *buildctx.Context, packageDir string) (string, error) {
	...
	debdir := filepath.Join(packageDir, fmt.Sprintf("%s_%s_deb", ctx.WayfinderAppBaseName, ctx.Architecture))
	os.RemoveAll(debdir)
