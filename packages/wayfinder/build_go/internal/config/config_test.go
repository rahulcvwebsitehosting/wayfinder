...
func realConfigDir(t *testing.T) string {
	...
	if err != nil {
		t.Fatalf("not running inside the Wayfinder repo: %v", err)
	}
...
func TestLoadRealWindowsAndDebugConfigs(t *testing.T) {
	ain, err := Load(filepath.Join(realConfigDir(t), "release.windows.yaml"))
	if err != nil {
		t.Fatal(err)
	}
	if got := ain.Build.Architectures(); !slices.Equal(got, []string{"x64"}) {
		t.Errorf("windows architectures = %v", got)
	}
	if !slices.Contains(ain.Modules, "sign_windows") || !slices.Contains(ain.Modules, "package_windows") {
		t.Errorf("windows modules = %v", ain.Modules)
	}
