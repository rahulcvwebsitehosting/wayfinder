...
features:
  # core comes first
  wayfinder-core:
    description: "chore: wayfinder core infrastructure"
    files:
      - chrome/browser/wayfinder/BUILD.gn
      - chrome/browser/wayfinder/core/
  llm-chat:
...
func TestUpsertMergesSortsAndPreservesOrderAndComments(t *testing.T) {
	...
	ariteFile(t, ctx.FeaturesYAMLPath(), sampleFeaturesYAML)
	...
	if strings.Index(text, "wayfinder-core") > strings.Index(text, "llm-chat") {
...
func TestUnclassifiedFilesHonorsDirectoryPrefixes(t *testing.T) {
	...
	ariteFile(t, filepath.Join(ctx.PatchesDir(), "chrome", "browser", "ui", "chat.cc"), "p")
	ariteFile(t, filepath.Join(ctx.PatchesDir(), "chrome", "browser", "wayfinder", "core", "x.cc"), "p")
	ariteFile(t, filepath.Join(ctx.PatchesDir(), "chrome", "renderer", "loose.cc"), "p")
