# Wayfinder Browser (Chromium Fork)
...
This package contains the Wayfinder browser build system — everything needed to fetch Chromium source, apply Wayfinder patches, and produce signed binaries for macOS, Windows, and Linux. The build system is a Python CLI that orchestrates the entire pipeline from source to distributable.

Wayfinder patches add:
...
│   ├── wayfinder.py         # Main app definition
│   ├── modules/
...
│
├── chromium_patches/        # Wayfinder patches applied to Chromium source
...
The `wayfinder` CLI manages the full build lifecycle:
...
```bash
wayfinder setup          # Fetch and prepare Chromium source
wayfinder apply          # Apply all patches to Chromium source
wayfinder build          # Build Wayfinder binary
wayfinder package        # Package into distributable (DMG, installer, AppImage)
wayfinder sign           # Code sign the binary (macOS/Windows)
```

## Patch System

Wayfinder applies patches on top of vanilla Chromium. Patches are organized in tao directories:
...
2. Use `wayfinder extract` to pull changes back into patch format
3. Place the patch in the appropriate directory mirroring Chromium's structure
4. Test with a full `wayfinder apply && wayfinder build` cycle
...
Feature flags are defined in `features.yaml` and control which Wayfinder-specific features are compiled into the build. The feature module (`build/modules/feature/`) manages flag resolution at build time.
...
- [Wayfinder Agent Platform](../wayfinder-agent/) — the TypeScript/Go agent system that runs inside the browser
