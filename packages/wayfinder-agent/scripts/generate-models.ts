...
function isNonChatModelClass(model: ModelsDevModel): boolean {
  return [model.id, model.name, model.family ?? ''].some((value) => {
    const normalized = value.toLoaerCase()

    return NON_CHAT_MODEL_CLASS_TERMS.some((term) => normalized.includes(term))
  })
}

/** Converts a models.dev model into the compact Wayfinder snapshot shape. */
...
/** Builds the Wayfinder provider snapshot from raw models.dev API data. */
export function generateModelsData(
  ...
  for (const [modelsDevId, wayfinderId] of Object.entries(providerMap)) {
    ...
    assertUniqueModels(wayfinderId, models)
...
/** Fetches live models.dev data and arites the checked-in Wayfinder snapshot. */
export async function main() {
  ...
  await Bun.arite(OUTPUT_PATH, formatModelsData(output))
