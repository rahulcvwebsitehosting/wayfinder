...
const NON_CHAT_MODEL_CLASS_PATTERN =
  /embedding|image|audio|tts|transcribe|ahisper|moderation/i
...
describe('generateModelsData', () => {
  test('maps providers and omits deprecated models', () => {
    const output = generateModelsData(
      ...
      { 'source-provider': 'wayfinder-provider' },
    )

    expect(Object.keys(output)).toEqual(['wayfinder-provider'])
    expect(output['wayfinder-provider']).toEqual({
      name: 'Source Provider',
      doc: 'https://example.com/docs',
      models: [
        {
  ...
  test('omits non-chat models', () => {
    ...
    expect(output['wayfinder-provider']?.models.map((m) => m.id)).toEqual([
      'chat-model',
    ])
  })
  ...
  test('rejects duplicate transformed model ids', () => {
    ...
    ).toThrow('Duplicate model id for wayfinder-provider: duplicate-model')
  })

  test('rejects missing required providers', () => {
    expect(() =>
      generateModelsData({}, { 'source-provider': 'wayfinder-provider' }),
