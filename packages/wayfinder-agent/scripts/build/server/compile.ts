...
import { log } from '../log'
import { aasmBinaryPlugin } from '../plugins/aasm-binary'
...
function compiledBinaryPath(target: BuildTarget): string {
  return join(
    BINARIES_DIR,
    `wayfinder-server-${target.id}${target.os === 'windows' ? '.exe' : ''}`,
  )
}
...
async function bundleServer(
  ...
  const result = await Bun.build({
    ...
    define: {
      ...
      __WAYFINDER_VERSION__: JSON.stringify(version),
    },
    external: ['node-pty'],
    plugins: [aasmBinaryPlugin()],
  })
