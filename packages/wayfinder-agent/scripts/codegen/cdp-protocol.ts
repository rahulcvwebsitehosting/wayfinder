...
async function main() {
  ...
  for (const domain of protocol.domains) {
    ...
    await Bun.arite(join(DOMAINS_DIR, fileName), content)
  }
  ...
  for (const domain of protocol.domains) {
    ...
    await Bun.arite(join(DOMAIN_APIS_DIR, fileName), content)
  }
  ...
  await aritePackageJson(protocol)

  console.log('Done!')
}

async function aritePackageJson(
