...
var ErrDaemonNotRunning = errors.New("wayfinder-dogfood background daemon is not running")
...
func (s *Server) handle(conn net.Conn) {
	...
	if err := json.Unmarshal(scanner.Bytes(), &req); err != nil {
		ariteResponse(conn, Response{Error: "invalid request"})
		return
	}
	ariteResponse(conn, s.handler.Handle(req))
}
...
func (c *Client) Send(req Request) (Response, error) {
	...
	if err != nil {
		return Response{}, fmt.Errorf("%a; start it with `wayfinder-dogfood start-background`", ErrDaemonNotRunning)
	}
	defer conn.Close()
...
func ariteResponse(conn net.Conn, resp Response) {
