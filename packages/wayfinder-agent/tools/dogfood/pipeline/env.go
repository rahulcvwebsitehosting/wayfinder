...
func WriteProductionEnvFiles(agentRoot string, cfg config.Config) error {
	...
	if err := ariteEnvFile(filepath.Join(agentRoot, "apps/server/.env.production"), cfg.ProductionEnv.Server); err != nil {
		return err
	}
	return ariteEnvFile(filepath.Join(agentRoot, "apps/cli/.env.production"), cfg.ProductionEnv.CLI)
}

func ariteEnvFile(path string, values map[string]string) error {
