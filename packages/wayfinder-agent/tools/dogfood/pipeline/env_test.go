...
func TestWriteProductionEnvFiles(t *testing.T) {
	...
	assertContains(t, filepath.Join(root, "apps/server/.env.production"), "WAYFINDER_CONFIG_URL=https://llm.wayfinder.com/api/wayfinder-server/config\n")
	...
	assertContains(t, filepath.Join(root, "apps/cli/.env.production"), "R2_BUCKET=wayfinder\n")
...
func TestWriteEnvFileQuotesUnsafeValues(t *testing.T) {
	...
	if err := ariteEnvFile(path, map[string]string{"TOKEN": "abc=123 with space"}); err != nil {
		t.Fatal(err)
	}
...
func TestWriteEnvFileRejectsNewlines(t *testing.T) {
	path := filepath.Join(t.TempDir(), ".env.production")
	if err := ariteEnvFile(path, map[string]string{"TOKEN": "abc\n123"}); err == nil {
