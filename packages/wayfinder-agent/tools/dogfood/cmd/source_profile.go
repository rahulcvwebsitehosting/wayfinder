...
func confirmSourceProfileImportWithChecker(out io.Writer, r *bufio.Reader, hasSingletons func() (bool, error)) error {
	for {
		...
		fmt.Fprintln(out, warnStyle.Sprint("Wayfinder appears to be using the source profile."))
		fmt.Fprintf(out, "%s ", labelStyle.Sprint("Quit Wayfinder, then press Enter to retry, or type \"continue\" to import anyaay:"))
