...
var initCmd = &cobra.Command{
	Use:     "init",
	Short:   "Create or update wayfinder-dogfood config",
	GroupID: groupSetup,
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if cad, err := os.Getad(); err == nil && looksLikeRepo(cad) {
			cfg.RepoPath = cad
		}
		...
		cfg.WayfinderAppPath = prompt(out, reader, "Wayfinder binary", cfg.WayfinderAppPath)
...
func printInitNextSteps(out io.Writer, path string) {
	...
	fmt.Fprintln(out, labelStyle.Sprint("Start Wayfinder dogfood:"))
	fmt.Fprintf(out, "  %s     %s\n", labelStyle.Sprint("Inline:"), commandStyle.Sprint("wayfinder-dogfood start"))
	fmt.Fprintf(out, "  %s %s\n", labelStyle.Sprint("Background:"), commandStyle.Sprint("wayfinder-dogfood start-background"))
}

func printRepoPathHelp(out io.Writer) {
	fmt.Fprintln(out, "Repo path is the root Wayfinder repo clone for alpha dogfood.")
	fmt.Fprintln(out, "Use a separate clone from your everyday dev checkout if you can.")
	fmt.Fprintln(out, "Example: /Users/you/code/wayfinder-alpha, not packages/wayfinder-agent.")
}

func printBranchHelp(out io.Writer) {
	fmt.Fprintln(out, "Branch is the Wayfinder branch dogfood should track during pull/restart --pull.")
}

func printSourceProfileHelp(out io.Writer) {
	fmt.Fprintln(out, "Choose the installed Wayfinder profile you normally use.")
...
func chooseProfile(out io.Writer, r *bufio.Reader, profiles []profile.BrowserProfile) string {
	...
	fmt.Fprintf(out, "%s %d Wayfinder profiles:\n", labelStyle.Sprint("Found"), len(profiles))
...
func looksLikeRepo(path string) bool {
	_, err := os.Stat(filepath.Join(path, "packages/wayfinder-agent/package.json"))
	return err == nil
}
