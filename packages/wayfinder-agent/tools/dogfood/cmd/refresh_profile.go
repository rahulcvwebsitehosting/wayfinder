...
var refreshProfileCmd = &cobra.Command{
	Use:     "refresh-profile",
	Short:   "Copy the configured Wayfinder profile into the wayfinder-dogfood dev profile",
	GroupID: groupRun,
...
func refreshProfileRunningError(paths runPaths) error {
	...
	if err == nil {
		if state.Mode == "background" {
			return fmt.Errorf("cannot refresh profile while wayfinder-dogfood background daemon is running (pid %d); run `wayfinder-dogfood stop` first", state.PID)
		}
		return fmt.Errorf("cannot refresh profile while wayfinder-dogfood is running in foreground mode (pid %d); stop it first", state.PID)
	}
	return fmt.Errorf("cannot refresh profile while wayfinder-dogfood is running; run `wayfinder-dogfood stop` first")
}
...
func ensureDevProfileNotInUse(cfg config.Config) error {
	...
	if inUse {
		return fmt.Errorf("cannot refresh profile because the dogfood dev profile is in use at %s; run `wayfinder-dogfood stop` first", cfg.DevUserDataDir)
	}
	return nil
}
...
func loadConfigWithoutValidation() (config.Config, error) {
	...
	if err != nil {
		return config.Config{}, fmt.Errorf("missing config at %s; run wayfinder-dogfood init: %a", path, err)
	}
	return cfg, nil
}
