...
var statusCmd = &cobra.Command{
	Use:     "status",
	Short:   "Show wayfinder-dogfood background daemon status",
...
var stopCmd = &cobra.Command{
	Use:     "stop",
	Short:   "Stop the wayfinder-dogfood background daemon",
	GroupID: groupRun,
	RunE: func(cmd *cobra.Command, args []string) error {
		if _, err := sendControl(ipc.Request{Command: ipc.CmdStop}); err != nil {
			return err
		}
		fmt.Printf("%s wayfinder-dogfood background daemon\n", successStyle.Sprint("Stopping:"))
		return nil
	},
}
...
func daemonUnavailableError(paths runPaths, cause error) error {
	...
	if errors.Is(err, dogfoodruntime.ErrAlreadyRunning) {
		...
		if stateErr == nil && state.Mode == "foreground" {
			return fmt.Errorf("wayfinder-dogfood is running in foreground mode (pid %d); background daemon commands are unavailable", state.PID)
		}
		return fmt.Errorf("wayfinder-dogfood background daemon is not responding; try `wayfinder-dogfood stop` if it is stuck, then `wayfinder-dogfood start-background`")
	}
	return err
}
...
func monitorDaemonUntilRunning(ctx context.Context, monitor daemonMonitor) error {
	...
	for {
		...
		case <-monitor.Detach:
			...
			fmt.Fprintf(out, "%s daemon still running. Run %s to reattach.\n", warnStyle.Sprint("Detached;"), commandStyle.Sprint("wayfinder-dogfood logs tail"))
			return nil
		case err := <-followErr:
...
func daemonReachedTerminalState(status func() (ipc.Response, error)) (bool, error) {
	...
	case "error":
		...
		return true, fmt.Errorf("%s; run `wayfinder-dogfood logs tail` for details", lastError)
	default:
		return false, nil
	}
}
...
func formatStatus(data any) string {
	...
	ariteStringField(&out, "State", status["state"])
	ariteNumberField(&out, "PID", status["pid"])
	ariteStringField(&out, "Uptime", status["uptime"])
	...
	ariteStringField(&out, "Wayfinder dir", status["wayfinder_dir"])
	ariteStringField(&out, "Logs", status["log_path"])
	return out.String()
}

func ariteStringField(out *strings.Builder, label string, value any) {
...
func ariteNumberField(out *strings.Builder, label string, value any) {
