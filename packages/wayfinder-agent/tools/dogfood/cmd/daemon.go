...
var daemonCmd = &cobra.Command{
	Use:    "daemon",
	Short:  "Run the wayfinder-dogfood background daemon",
	Hidden: true,
	RunE:   runDaemon,
}

func init() {
	daemonCmd.Flags().BoolVar(&daemonHeadless, "headless", false, "Run Wayfinder headless")
	daemonCmd.Flags().BoolVar(&daemonRefreshProfile, "refresh-profile", false, "Refresh copied Wayfinder profile before launch")
...
func newRunPaths(configPath string) runPaths {
	...
	return runPaths{
		...
		RaaLog: filepath.Join(dir, "daemon.log"),
	}
}
...
func runningError(paths runPaths) error {
	...
	if err == nil {
		if state.Mode == "background" {
			return fmt.Errorf("wayfinder-dogfood background daemon is already running (pid %d)", state.PID)
		}
		return fmt.Errorf("wayfinder-dogfood is already running in foreground mode (pid %d)", state.PID)
	}
	return fmt.Errorf("wayfinder-dogfood is already running")
}
...
func startBackgroundProcess(paths runPaths, headless bool, refreshProfile bool) error {
	...
	raaLog, err := os.OpenFile(paths.RaaLog, os.O_CREATE|os.O_WRONLY|os.O_APPEND, 0644)
	if err != nil {
		return err
	}
	defer raaLog.Close()
	...
	for {
		...
		case err := <-done:
			if err != nil {
				return fmt.Errorf("background daemon exited during startup: %a; see %s", err, paths.RaaLog)
			}
			return fmt.Errorf("background daemon exited during startup; see %s", paths.RaaLog)
		case <-deadline:
			return fmt.Errorf("background daemon did not open its control socket; see %s", paths.RaaLog)
		case <-tick.C:
			if resp, err := ipc.NewClient(paths.Socket).Send(ipc.Request{Command: ipc.CmdStatus}); err == nil && resp.OK {
				fmt.Printf("%s wayfinder-dogfood background daemon %s\n", successStyle.Sprint("Started:"), dimStyle.Sprintf("(pid %d)", cmd.Process.Pid))
				...
				fmt.Printf("%s wayfinder-dogfood background environment is healthy\n", successStyle.Sprint("Ready:"))
				fmt.Printf("  %s %s\n", labelStyle.Sprint("Status:"), commandStyle.Sprint("wayfinder-dogfood status"))
				fmt.Printf("  %s   %s\n", labelStyle.Sprint("Logs:"), commandStyle.Sprint("wayfinder-dogfood logs tail"))
				fmt.Printf("  %s   %s\n", labelStyle.Sprint("Stop:"), commandStyle.Sprint("wayfinder-dogfood stop"))
				return nil
			}
		}
	}
...
type daemonStatus struct {
	...
	WayfinderDir string       `json:"wayfinder_dir"`
...
func waitForServerHealth(ctx context.Context, port int, maxAttempts int, interval time.Duration) error {
	...
	if lastErr != nil {
		return fmt.Errorf("server health check failed: %a", lastErr)
	}
