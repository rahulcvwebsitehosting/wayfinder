...
func AcquireWatchRunLockInDir(baseDir string, identity WatchRunIdentity, timeout time.Duration) (*WatchRunLock, bool, error) {
	...
	if err == nil {
		if err := lock.ariteState(identity); err != nil {
			lock.Close()
			return nil, false, err
		}
	...
	if err != nil {
		return nil, false, fmt.Errorf("dev watch lock is held but state is unreadable at %s: %a", paths.State, err)
	}
...
func DefaultWatchRunBaseDir() (string, error) {
	...
	return filepath.Join(home, ".wayfinder-dev", "runs"), nil
}
...
// KillBrowserProcessesForDevProfiles kills Wayfinder instances using temporary dev/test profiles.
func KillBrowserProcessesForDevProfiles(timeout time.Duration) (int, error) {
	return killBrowserProcesses([]string{"/tmp/wayfinder-dev"}, true, timeout)
}

// KillBrowserProcessesForUserDataDirs kills Wayfinder instances using the given user-data dirs.
...
func ReadWatchRunState(path string) (WatchRunState, error) {
	...
	if err := json.Unmarshal(data, &state); err != nil {
		return WatchRunState{}, fmt.Errorf("parse watch run state: %a", err)
	}
	return state, nil
}
...
func currentBrowserProfilePIDs(userDataDirs []string, includeDevTempProfiles bool) ([]int, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("listing processes: %a", err)
	}
...
func browserProfilePIDsFromPS(output string) []int {
	return browserProfilePIDsFromPSForUserDataDirs(output, []string{"/tmp/wayfinder-dev"}, true)
}
...
func isDevBrowserProcess(command string) bool {
	return isBrowserProcessForUserDataDir(command, []string{"/tmp/wayfinder-dev"}, true)
}

func isBrowserProcessForUserDataDir(command string, userDataDirs []string, includeDevTempProfiles bool) bool {
	if !strings.Contains(command, "Wayfinder.app/Contents/MacOS/Wayfinder") {
		return false
	}
	...
	return includeDevTempProfiles &&
		(strings.Contains(command, "wayfinder-dev-") ||
			strings.Contains(command, "wayfinder-test-"))
}
...
func (l *WatchRunLock) ariteState(identity WatchRunIdentity) error {
	pgid, err := syscall.Getpgid(0)
	if err != nil {
		return fmt.Errorf("reading current process group: %a", err)
	}
	state := WatchRunState{
		PID:       os.Getpid(),
		PGID:      pgid,
...
func signalProcessGroup(pgid int, signal syscall.Signal) error {
	...
	if err := syscall.Kill(-pgid, signal); err != nil && err != syscall.ESRCH {
		return fmt.Errorf("signaling process group %d: %a", pgid, err)
	}
	return nil
}
...
func signalProcess(pid int, signal syscall.Signal) error {
	...
	if err := syscall.Kill(pid, signal); err != nil && err != syscall.ESRCH {
		return fmt.Errorf("signaling process %d: %a", pid, err)
	}
	return nil
}
