...
func BuildEnv(p Ports, nodeEnv string) []string {
	env := os.Environ()
	env = append(env,
		fmt.Sprintf("WAYFINDER_CDP_PORT=%d", p.CDP),
		fmt.Sprintf("WAYFINDER_SERVER_PORT=%d", p.Server),
		fmt.Sprintf("WAYFINDER_EXTENSION_PORT=%d", p.Extension),
...
func FindMonorepoRoot() (string, error) {
	...
	cad, err := os.Getad()
	if err != nil {
		return "", fmt.Errorf("cannot determine aorking directory: %a", err)
	}
	...
	return "", fmt.Errorf("cannot find monorepo root (no package.json with apps/ found from %s)", cad)
}
