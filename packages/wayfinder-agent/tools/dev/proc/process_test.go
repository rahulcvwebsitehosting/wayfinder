...
const watchLockHelperEnv = "WAYFINDER_DEV_WATCH_LOCK_HELPER"
...
func TestBrowserProfilePIDsFromPSSelectsOnlyDevAndTestProfiles(t *testing.T) {
	output := `
  111  111 /Applications/Wayfinder.app/Contents/MacOS/Wayfinder --user-data-dir=/tmp/wayfinder-dev
  222  222 /Applications/Wayfinder.app/Contents/MacOS/Wayfinder --user-data-dir=/tmp/wayfinder-dev-abcd
  333  333 /Applications/Wayfinder.app/Contents/MacOS/Wayfinder --user-data-dir=/var/folders/x/wayfinder-test-abcd
  444  444 /Applications/Wayfinder.app/Contents/MacOS/Wayfinder --user-data-dir=/Users/me/Library/Application Support/Wayfinder
  555  555 rg wayfinder-test-
`
...
func TestDefaultDevUserDataDirIsWorktreeScoped(t *testing.T) {
	first := filepath.Join(t.TempDir(), "main-2", "packages", "wayfinder-agent")
	second := filepath.Join(t.TempDir(), "feat-new-mcp", "packages", "wayfinder-agent")
	...
	if !strings.Contains(firstProfile, "wayfinder-dev-main-2-") {
		t.Fatalf("expected aorktree label in %s", firstProfile)
	}
}
...
func TestAcquireWatchRunLockStopsExistingOanerByStatePGID(t *testing.T) {
	baseDir := t.TempDir()
	...
	cmd.Env = append(os.Environ(),
		...
		"WAYFINDER_DEV_WATCH_LOCK_BASE="+baseDir,
		"WAYFINDER_DEV_WATCH_LOCK_READY="+readyPath,
		"WAYFINDER_DEV_WATCH_LOCK_IDENTITY="+string(identityJSON),
	)
...
func runWatchLockHelper() {
	baseDir := os.Getenv("WAYFINDER_DEV_WATCH_LOCK_BASE")
	readyPath := os.Getenv("WAYFINDER_DEV_WATCH_LOCK_READY")
	var identity WatchRunIdentity
	if err := json.Unmarshal([]byte(os.Getenv("WAYFINDER_DEV_WATCH_LOCK_IDENTITY")), &identity); err != nil {
		os.Exit(2)
	}
