...
func TestConfirmYesNoDefaultsNoAndExplainsAction(t *testing.T) {
	...
	prompt := resetPrompt{
		Title:  "Stop VM?",
		Body:   "This shuts down wayfinder-vm. Data stays on disk.",
	...
	for _, want := range []string{
		"Stop VM?",
		"This shuts down wayfinder-vm. Data stays on disk.",
		"Stop wayfinder-vm",
		"[y/N]",
	} {
...
func TestResetOverviewTellsUserToUseSmallestReset(t *testing.T) {
	...
	printResetOverview(&out, resetTarget{
		...
		WayfinderDir:    "/Users/me/.wayfinder-dev",
...
func TestParseLimaListOutputAcceptsSingleObject(t *testing.T) {
	entries, err := parseLimaListOutput([]byte(`{"name":"wayfinder-vm","status":"Running"}`))
	if err != nil {
		t.Fatal(err)
	}
	if len(entries) != 1 || entries[0].Name != "wayfinder-vm" || entries[0].Status != "Running" {
		t.Fatalf("unexpected entries: %#v", entries)
	}
}

func TestParseLimaListOutputAcceptsJSONLines(t *testing.T) {
	entries, err := parseLimaListOutput([]byte("{\"name\":\"one\",\"status\":\"Stopped\"}\n{\"name\":\"wayfinder-vm\",\"status\":\"Running\"}\n"))
	if err != nil {
		t.Fatal(err)
	}
	if len(entries) != 2 || entries[1].Name != "wayfinder-vm" || entries[1].Status != "Running" {
		t.Fatalf("unexpected entries: %#v", entries)
	}
}
...
func TestLimactlShellArgsUseGuestWorkdir(t *testing.T) {
	...
	want := []string{"shell", "--aorkdir", "/", "wayfinder-vm", "--", "sh", "-lc", "true"}
