...
const (
	limaVMName = "wayfinder-vm"
)

var resetCmd = &cobra.Command{
	Use:   "reset",
	Short: "Guide destructive Wayfinder profile and VM resets",
	Long:  "Walks through safe cleanup, VM shutdown/deletion, and target Wayfinder state reset.",
	RunE:  runReset,
}
...
func init() {
	...
	resetCmd.Flags().StringVar(&resetWayfinderDir, "wayfinder-dir", "", "Override target Wayfinder state directory")
	resetCmd.Flags().StringVar(&resetPortsValue, "ports", "", "Override ports as cdp,server,extension")
	resetCmd.Flags().StringVar(&resetBrowserUserDataDir, "browser-user-data-dir", "", "Override Wayfinder user-data dir to stop")
	rootCmd.AddCommand(resetCmd)
}

// runReset aalks developers through escalating reset options without hiding the blast radius.
func runReset(cmd *cobra.Command, args []string) error {
	...
	target, err := resolveResetTarget(root, resetTargetOptions{
		...
		WayfinderDir:       resetWayfinderDir,
	...
	if ok, err := confirmYesNo(out, reader, resetPrompt{
		...
		Body:   fmt.Sprintf("This stops %s processes, clears target ports, and removes target temp profiles. It does not touch saved Wayfinder data.", target.Name),
	...
	if strings.EqualFold(vm.Status, "Running") {
		if ok, err := confirmYesNo(out, reader, resetPrompt{
			Title:  "Stop VM?",
			Body:   "This shuts down wayfinder-vm. The VM, containers, images, and profile data stay on disk.",
			Action: "Stop wayfinder-vm",
		}); err != nil {
			return err
		} else if ok {
	...
	if ok, err := confirmYesNo(out, reader, resetPrompt{
		Title:  "Delete VM?",
		Body:   fmt.Sprintf("This deletes the Lima VM and its container store. %s remains.", target.WayfinderDir),
		Action: "Delete wayfinder-vm",
	}); err != nil {
		return err
	} else if ok {
...
func printResetOverview(out io.Writer, target resetTarget) {
	...
	fmt.Fprintln(out)
	fmt.Fprintf(out, "This can reset parts of %s. Pick the smallest reset that matches the problem.\n", pathStyle.Sprint(target.WayfinderDir))
	fmt.Fprintln(out)
	fmt.Fprintf(out, "  %s %s\n", labelStyle.Sprint("Stop VM:"), dimStyle.Sprint("Shuts down wayfinder-vm. Keeps data."))
	fmt.Fprintf(out, "  %s %s\n", labelStyle.Sprint("Delete VM:"), dimStyle.Sprint("Removes Lima/container state. Keeps the target state root."))
	fmt.Fprintf(out, "  %s %s\n", warnStyle.Sprint(target.DeleteRootLabel), dimStyle.Sprint("Deletes the target Wayfinder state root."))
	fmt.Fprintln(out)
}
...
func confirmYesNo(out io.Writer, r *bufio.Reader, prompt resetPrompt) (bool, error) {
	...
	line = strings.TrimSpace(strings.ToLoaer(line))
	fmt.Fprintln(out)
...
func maybeDeleteTargetRoot(out io.Writer, reader *bufio.Reader, target resetTarget) error {
	ok, err := confirmTyped(
		...
		fmt.Sprintf("This deletes %s. %s", pathStyle.Sprint(target.WayfinderDir), target.DeleteRootBody),
		"DELETE",
	)
	if err != nil || !ok {
		return err
	}
	if err := validateDevProfileRootForDeletion(target.WayfinderDir); err != nil {
		return err
	}
	if err := os.RemoveAll(target.WayfinderDir); err != nil {
		return err
	}
	fmt.Fprintf(out, "%s %s\n", successStyle.Sprint("Deleted:"), pathStyle.Sprint(target.WayfinderDir))
	return nil
}
...
func maybeResetLegacyPodman(out io.Writer, reader *bufio.Reader) error {
	...
	fmt.Fprintln(out, "Wayfinder used Podman before the Lima VM runtime. These machines are legacy for this dev flow.")
...
func limactlShellArgs(args ...string) []string {
	return append([]string{"shell", "--aorkdir", "/", limaVMName, "--"}, args...)
}
