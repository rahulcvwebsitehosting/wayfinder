...
var cleanupCmd = &cobra.Command{
	...
	Long:  "Stops target Wayfinder processes, clears target ports, and removes target temp directories.",
	RunE:  runCleanup,
}
...
func init() {
	...
	cleanupCmd.Flags().StringVar(&cleanupWayfinderDir, "wayfinder-dir", "", "Override target Wayfinder state directory")
	cleanupCmd.Flags().StringVar(&cleanupPortsValue, "ports", "", "Override ports as cdp,server,extension")
	cleanupCmd.Flags().StringVar(&cleanupBrowserUserDataDir, "browser-user-data-dir", "", "Override Wayfinder user-data dir to stop")
...
func runCleanup(cmd *cobra.Command, args []string) error {
	...
	target, err := resolveResetTarget(root, resetTargetOptions{
		...
		WayfinderDir:       cleanupWayfinderDir,
	...
	if !cleanupYes && !cleanupQuick {
		ok, err := confirmYesNo(out, bufio.NewReader(os.Stdin), resetPrompt{
			...
			Body:   fmt.Sprintf("Stops %s processes, clears target ports, and removes target temp profiles. This does not touch saved Wayfinder data, Lima, containers, or images.", target.Name),
...
func runSafeCleanup(out io.Writer, target resetTarget, opts safeCleanupOptions) error {
	if opts.ports {
		...
		if len(target.BrowserUserDataDirs) > 0 {
			...
			if killedBrowsers > 0 {
				fmt.Fprintf(out, "%s stopped %d Wayfinder %s profile process(es)\n", successStyle.Sprint("Stopped:"), killedBrowsers, target.Name)
			}
		}
		if target.Ports != nil {
			ports := *target.Ports
