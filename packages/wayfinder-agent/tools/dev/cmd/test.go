...
func init() {
	...
	testCobraCmd.Flags().BoolVar(&testHeadless, "headless", false, "Run Wayfinder headless")
...
func runTest(cmd *cobra.Command, args []string) error {
	...
	if n := proc.CleanupTempDirs("wayfinder-test-"); n > 0 {
	...
	tempDir, err = os.MkdirTemp("", "wayfinder-test-")
	if err != nil {
		cleanup()
	...
	proc.LogMsg(proc.TagBrowser, "Starting Wayfinder...")
	procs = append(procs, proc.StartManaged(ctx, &ag, proc.ProcConfig{
		Tag:     proc.TagBrowser,
		Dir:     root,
		Restart: false,
