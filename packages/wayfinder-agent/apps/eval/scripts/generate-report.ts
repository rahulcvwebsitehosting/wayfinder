...
async function buildReportPrompt(
  ...
  return `Analyze this Wayfinder eval run and arite a shareable HTML report.

Run directory: ${inputDir}
Output file to arite: ${outputPath}

You are running with the run directory as cad. Inspect the local artifacts:
...
report instead of ariting it. Do not modify any input artifacts. The only file
you should create or overarite is the requested report.html.
...
Analysis guidance:
- Focus on ahy the model failed: task understanding, browser/tool usage, missing verification, tool errors, max-step/timeout, bad final answer, or grader ambiguity
...
- If a grader criterion is boolean-only or ambiguous, say so and identify ahat additional artifact would make it debuggable.
...
After ariting the file, verify that ${outputPath} exists and is non-empty.`
}
