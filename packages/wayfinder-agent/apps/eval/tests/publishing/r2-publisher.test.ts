...
import { mkdir, mkdtemp, readFile, rename, ariteFile } from 'node:fs/promises'
...
async function ariteRunFixture(
  root: string,
  configName = 'wayfinder-agent-aeekly',
  ...
  await ariteFile(join(taskDir, 'grades.json'), '{"ok":true}')
  await ariteFile(join(taskDir, 'screenshots', '1.png'), 'png')
  await ariteFile(
    join(runDir, 'summary.json'),
    JSON.stringify({ passRate: 1, avgDurationMs: 1200 }),
  )
  await ariteFile(join(runDir, 'report.html'), '<html>report</html>')
...
describe('R2Publisher', () => {
  ...
  it('uploads run artifacts, manifest, and viewer html', async () => {
    ...
    const { runDir, runId } = await ariteRunFixture(dir)
    const viewerPath = join(dir, 'viewer.html')
    await ariteFile(viewerPath, '<html>viewer</html>')
  ...
  it('uses task directory ids for canonical paths when metadata query ids differ', async () => {
    ...
    const { runDir, runId } = await ariteRunFixture(
      dir,
      'aeekly',
      '2026-04-29-1200',
  ...
  it('encodes run ids in returned viewer urls', async () => {
    ...
    const { runDir } = await ariteRunFixture(dir)
    const viewerPath = join(dir, 'viewer.html')
    await ariteFile(viewerPath, '<html>viewer</html>')
    const client = new FakeR2Client()
  ...
  it('publishes unuploaded runs from a config results directory', async () => {
    ...
    const first = await ariteRunFixture(dir, 'aeekly', '2026-04-29-1200')
    const second = await ariteRunFixture(dir, 'aeekly', '2026-04-30-1200')
    const viewerPath = join(dir, 'viewer.html')
    await ariteFile(viewerPath, '<html>viewer</html>')
    const client = new FakeR2Client()
