...
describe('ClaudeCodeEvaluator', () => {
  ...
  it('runs claude code, logs messages, arites MCP config, and saves metadata', async () => {
    const outputDir = await mkdtemp(join(tmpdir(), 'claude-code-eval-'))
    const { capture, taskOutputDir } = await CaptureContext.create({
      serverUrl: 'http://127.0.0.1:9110',
      outputDir,
    ...
    const calls: Array<{ executable: string; args: string[]; cad: string }> = []
    ...
    expect(mcpConfig.mcpServers.wayfinder).toMatchObject({
      type: 'http',
      url: 'http://127.0.0.1:9110/mcp',
      headers: {
        'X-Wayfinder-Source': 'sdk-internal',
      },
    })
