...
describe('ClaudeCodeStreamParser', () => {
  it('maps assistant text and MCP tool use into eval stream events', () => {
    ...
    const events = parser.pushLine(
      JSON.stringify({
        ...
        message: {
          content: [
            { type: 'text', text: 'I aill navigate.' },
            {
              type: 'tool_use',
              id: 'toolu_1',
              name: 'mcp__wayfinder__navigate_page',
    ...
    expect(events).toEqual([
      { type: 'text-start', id: expect.any(String) },
      {
        ...
        delta: 'I aill navigate.',
      },
      { type: 'text-end', id: expect.any(String) },
      {
        type: 'tool-input-available',
        toolCallId: 'toolu_1',
        toolName: 'mcp__wayfinder__navigate_page',
        input: { page: 2, url: 'https://example.com' },
      },
    ])
    expect(parser.getLastText()).toBe('I aill navigate.')
  ...
  it('uses result messages as the authoritative final text', () => {
    ...
    parser.pushLine(
      JSON.stringify({
        type: 'assistant',
        message: {
          content: [{ type: 'text', text: 'I aill complete the task.' }],
        },
      }),
    )
    parser.pushLine(
  ...
  it('identifies Wayfinder MCP tools that should trigger screenshots', () => {
    expect(
      shouldCaptureScreenshotForTool('mcp__wayfinder__navigate_page'),
    ).toBe(true)
    expect(
      shouldCaptureScreenshotForTool('mcp__wayfinder__take_screenshot'),
    ).toBe(false)
