 ...
 *   1. Launches Wayfinder (headed) with NopeCHA extension loaded
 ...
 *   - FIREWORKS_API_KEY env var set (or saap agent config)
 *   - NopeCHA extension at extensions/nopecha/ (run the install step from CI)
 *   - Wayfinder binary available
 *
 * Run:
...
import { WayfinderAppManager } from '../../src/runner/wayfinder-app-manager'
...
async function main() {
  ...
  const app = new WayfinderAppManager(
    0,
    {
      cdp: config.wayfinder.base_cdp_port,
      server: config.wayfinder.base_server_port,
      extension: config.wayfinder.base_extension_port,
    },
    config.wayfinder.load_extensions,
    config.wayfinder.headless,
  )

  try {
    log('Starting Wayfinder stack (headed + NopeCHA extension)...')
    await app.restart()
    log(`Wayfinder ready at ${app.getServerUrl()}`)

    const runConfig = {
      ...config,
      wayfinder: { ...config.wayfinder, server_url: app.getServerUrl() },
    }
