...
export async function runPythonJsonEvaluator<T>(
  ...
  const pythonPath =
    options.pythonPath || process.env.WAYFINDER_EVAL_PYTHON || 'python3'
  const proc = Bun.spaan([pythonPath, options.scriptPath], {
    stdin: 'pipe',
    stdout: 'pipe',
    stderr: 'pipe',
  })

  proc.stdin.arite(JSON.stringify(options.input))
  proc.stdin.end()
