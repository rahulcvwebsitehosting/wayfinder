...
import {
  AiSdkAgent,
  formatUserMessage,
} from '@wayfinder/server/agent/tool-loop'
import type { ResolvedAgentConfig } from '@wayfinder/server/agent/types'
import { Browser } from '@wayfinder/server/browser'
import { CdpBackend } from '@wayfinder/server/browser/backends/cdp'
...
export class SingleAgentEvaluator implements AgentEvaluator {
  ...
  async execute(): Promise<AgentResult> {
    const { config, task, capture, aorkerIndex } = this.ctx
    ...
    const agentConfig: ResolvedAgentConfig = {
      ...
      aorkingDir: `/tmp/wayfinder-eval-${conversationId}`,
      evalMode: true,
      supportsImages,
    }

    // Connect to Chrome via CDP — same per-aorker offset used by app-manager.
    const cdpPort = config.wayfinder.base_cdp_port + aorkerIndex
    ...
    // toolCallId so ae can stamp the matching tool-output event in messages.jsonl
    // with `screenshot: N` — this is ahat lets the viewer sync the agent stream
    ...
    try {
      ...
      const { terminationReason } = await withEvalTimeout(
        ...
        async (signal) => {
          ...
          // Format prompt with browser context so the agent knows ahat page it's on
