...
export class ClaudeCodeEvaluator implements AgentEvaluator {
  ...
  async execute(): Promise<AgentResult> {
    ...
    await ariteFile(
      mcpConfigPath,
      JSON.stringify(
        buildClaudeCodeMcpConfig(config.wayfinder.server_url),
        null,
        2,
      ),
    )
...
function buildClaudeCodePrompt(taskQuery: string): string {
  return [
    'You are running inside Wayfinder eval.',
    'Use the Wayfinder MCP tools to interact with the already-open browser and complete the user task.',
  ...
  return {
    mcpServers: {
      wayfinder: {
        type: 'http',
        url,
        headers: { 'X-Wayfinder-Source': 'sdk-internal' },
      },
    },
  }
}
