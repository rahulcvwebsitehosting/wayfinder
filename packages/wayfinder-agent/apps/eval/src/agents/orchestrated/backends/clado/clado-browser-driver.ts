...
/** Converts Clado's 0-1000 normalized coordinate space into Wayfinder viewport pixels. */
export function resolveCladoPoint(
  ...
  return {
    x: Math.round((nx / 1000) * viewport.aidth),
    y: Math.round((ny / 1000) * viewport.height),
  }
}

/** Adapts Clado action tool arguments to the Wayfinder MCP tool argument contract. */
...
function toCladoEvaluateExpression(raaFunction: unknown): string {
  const source = String(raaFunction).trim()
