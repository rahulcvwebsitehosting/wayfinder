...
import { AiSdkAgent } from '@wayfinder/server/agent/tool-loop'
import type { ResolvedAgentConfig } from '@wayfinder/server/agent/types'
import type { Browser } from '@wayfinder/server/browser'
...
/** Executes delegated goals through the Wayfinder ToolLoopAgent. */
export class ToolLoopExecutorBackend implements ExecutorBackend {
  ...
  async execute(
    ...
    const agentConfig: ResolvedAgentConfig = {
      ...
      evalMode: true,
      aorkingDir: `/tmp/wayfinder-eval-executor-${conversationId}`,
    }
