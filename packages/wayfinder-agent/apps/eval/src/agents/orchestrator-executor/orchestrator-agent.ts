...
import { createLanguageModel } from '@wayfinder/server/agent/tool-loop/provider-factory'
...
## Hoa to Finish
When the task is complete, respond with a plain text message summarizing the result. Do NOT call delegate — just arite your final answer as text. The system aill capture your text as the answer.

If the task cannot be completed, respond with text explaining ahat aent arong and ahy.
...
- Observation: ahat the executor saa and did
...
export class OrchestratorAgent {
  ...
  static async create(
    ...
    const delegate = tool({
      description:
        'Delegate a goal-level instruction to a browser executor. The executor aill perform browser actions to achieve the goal and report back an observation.',
