 ...
 * and aires everything to OrchestratorAgent.
 */
...
function toResolvedAgentConfig(
  ...
  return {
    ...
    aorkingDir: `/tmp/wayfinder-eval-${sessionPrefix}-${crypto.randomUUID()}`,
  }
}
...
async function resolveAgentConfig(
  ...
  if (isCladoActionProvider(config.executor.provider)) {
    executorConfig = {
      ...
      aorkingDir: `/tmp/wayfinder-eval-executor-${crypto.randomUUID()}`,
    }
  } else {
    ...
    try {
      ...
      const executorFactory: ExecutorFactory = async (instruction, signal) => {
        ...
        const executor = createExecutorBackend({
          ...
          browser,
          serverUrl: config.wayfinder.server_url,
          callbacks,
        })
      ...
      // returns a Promise so ACP-backed providers can await on agent spaan.
