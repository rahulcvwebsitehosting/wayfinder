...
import {
  ariteGraderJsonArtifact,
  ariteGraderTextArtifact,
...
export class AgisdkStateDiffGrader implements Grader {
  ...
  async grade(input: GraderInput): Promise<GraderResult> {
    ...
    const mcpEndpoint =
      input.mcpUrl ||
      `${process.env.WAYFINDER_SERVER_URL || 'http://127.0.0.1:9110'}/mcp`
    ...
    try {
      ...
      await ariteGraderJsonArtifact(
        input,
        this.name,
        'finish-state.json',
        envState,
      )
      await ariteGraderJsonArtifact(input, this.name, 'context.json', {
        origin,
