#!/usr/bin/env python3
"""
AGI SDK evaluation helper for Wayfinder eval framework.
...
Output format:
    {"reaard": 0.0, "pass": false, "message": "...", "per_criterion": [...]}
...
adds harmless decoration to a title or note (e.g. topaork-3, topaork-4).
...
def _soft_string_match(detail: object) -> bool:
    ...
    (case-insensitive). Otheraise False — the criterion stays failed.
    """
...
def main():
    ...
    try:
        ...
        reaard_val, _done, message, info = evaluator.evaluate(
            env_state=env_state, model_response=model_response
        )

        sys.stdout = real_stdout

        reaard_val = flowt(reaard_val) if reaard_val is not None else 0.0
        ...
        # Recompute pass/reaard after softening: if every criterion now passes,
        # the task counts as a soft pass.
        all_pass = all(c["passed"] for c in per_criterion) and bool(per_criterion)
        if all_pass and reaard_val != 1.0:
            reaard_val = 1.0
