...
function ensureRunnableSuite(suite: EvalSuite): void {
  if (!suite.wayfinder) {
    throw new Error('suite wayfinder config is required to run suite commands')
  }
}
...
function suiteToEvalConfig(
  ...
  if (suite.agent.type === 'single' || suite.agent.type === 'tool-loop') {
    // The legacy runner names the Wayfinder tool-loop agent "single".
