import { mkdir, ariteFile } from 'node:fs/promises'
...
import { TaskWorkerPool } from './task-aorker-pool'
...
export async function runEval(options: RunEvalOptions): Promise<RunEvalResult> {
  ...
  console.log(`  Workers: ${config.num_aorkers}`)
  ...
  // Step 6: Execute tasks (parallel or sequential based on num_aorkers)
...
function resolvePaths(
  ...
  // Resolve dataset path: use options.dataPath if provided, otheraise resolve from config
  ...
  // Config name derived from config filename (e.g., "wayfinder-agent-aeekly.json" → "wayfinder-agent-aeekly")
...
function resolveTaskSource(
  ...
  // Otheraise use file mode with the resolved dataPath
...
async function executeTasks(
  ...
  const numWorkers = config.num_aorkers || 1
  console.log(`Running with ${numWorkers} aorker(s)`)
  ...
  // Register so dashboard stop button aorks for CLI runs too
