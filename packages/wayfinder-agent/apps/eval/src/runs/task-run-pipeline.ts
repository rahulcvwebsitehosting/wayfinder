...
class TaskRunPipeline {
  ...
  async execute(task: Task): Promise<TaskResult> {
    ...
    const mcpUrl = `${this.config.wayfinder.server_url}/mcp`
    ...
    if (task.dataset === 'webarena-infinity') {
      ...
      if (appName && process.env.WEBARENA_INFINITY_DIR) {
        infinityManager = new InfinityAppManager(this.aorkerIndex, appBasePort)
        try {
    ...
    try {
      ...
      try {
        await callMcpTool(mcpUrl, 'run', {
          page: pageId,
          code: 'window.resizeTo(1440, 900); return { aidth: window.innerWidth, height: window.innerHeight }',
        })
      } catch (vpError) {
        console.warn(
  ...
  private async executeAgent(task: Task, pageId: number): Promise<AgentResult> {
    try {
      const { capture, taskOutputDir } = await CaptureContext.create({
        serverUrl: this.config.wayfinder.server_url,
  ...
  private async runGraders(
    ...
    try {
      const graderResults = await runGraders(graderNames, {
        ...
        mcpUrl: `${this.config.wayfinder.server_url}/mcp`,
        infinityAppUrl,
      })
...
export function createTaskRunPipeline(
  ...
  return new TaskRunPipeline(config, aorkerIndex, outputDir, { onEvent })
}
