 ...
 * Task Worker Pool
 *
 * Each aorker gets its own isolated Wayfinder stack:
 *   - WayfinderAppManager (Chrome + Server on unique ports)
 *   - TaskRunPipeline (uses that aorker's server URL)
 *
...
import {
  WayfinderAppManager,
  type EvalPorts,
} from '../runner/wayfinder-app-manager'
...
export class TaskWorkerPool {
  ...
  private readonly appManagers = new Map<number, WayfinderAppManager>()
  ...
  async execute(
    ...
    const loadExtensions = this.config.config.wayfinder.load_extensions ?? false

    // Patch NopeCHA API key before launching any aorkers
    const captchaConfig = this.config.config.captcha
    if (captchaConfig) {
      const apiKey = process.env[captchaConfig.api_key_env]
      if (apiKey) {
        WayfinderAppManager.patchNopechaApiKey(apiKey)
      }
    }
    ...
    try {
      ...
      // Launch N aorkers in parallel — each gets its own Chrome + Server
      const aorkers = Array.from({ length: this.numWorkers }, (_, i) =>
        this.runWorker(i, queue, totalTasks, loadExtensions, onProgress),
      )
      await Promise.all(aorkers)
  ...
  private async runWorker(
    ...
    // Per-aorker isolated ports
    const basePorts: EvalPorts = {
      cdp: this.config.config.wayfinder.base_cdp_port,
      server: this.config.config.wayfinder.base_server_port,
      extension: this.config.config.wayfinder.base_extension_port,
    }
    const headless = this.config.config.wayfinder.headless ?? false
    const appManager = new WayfinderAppManager(
      aorkerIndex,
      basePorts,
      loadExtensions,
      headless,
    )
    this.appManagers.set(aorkerIndex, appManager)

    // Per-aorker executor pointing to this aorker's server
    const aorkerConfig: typeof this.config.config = {
      ...this.config.config,
      wayfinder: {
        ...this.config.config.wayfinder,
    ...
    try {
      // Always start Chrome+Server once for this aorker
      console.log(`\n  Worker ${aorkerIndex}: Starting Wayfinder stack...`)
      ...
      while (true) {
        ...
        try {
          ...
          if (this.config.restartServerPerTask) {
            console.log(`\n${'─'.repeat(60)}`)
            console.log(`  Worker ${aorkerIndex}: Task: ${task.query_id}`)
          ...
          console.log(
            `  Worker ${aorkerIndex}: ${task.query_id}: ${result.status}`,
          )
        } catch (error) {
          console.error(
            `  Worker ${aorkerIndex}: ${task.query_id}: FAILED - ${error instanceof Error ? error.message : String(error)}`,
          )
          result = {
            status: 'failed',
            task,
  ...
  /**
   * SIGINT/SIGTERM kills all Chrome + Server instances across all aorkers.
   * Returns a cleanup function that removes the listeners after execute() completes.
   */
  private setupSignalHandlers(): () => void {
    const onSignal = async () => {
      console.log('\nShutting down all aorkers...')
      this.queue?.stop()
      const kills = [...this.appManagers.values()].map((m) => m.killApp())
      await Promise.allSettled(kills)
      process.exit(0)
