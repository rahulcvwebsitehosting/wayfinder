...
Evaluation framework for Wayfinder browser automation agents. Runs tasks from standard datasets ([WebVoyager](https://arxiv.org/abs/2401.13919), [Mind2Web](https://arxiv.org/abs/2306.06070), AGI SDK / REAL Bench, WebArena-Infinity, WebBench), captures trajectories with screenshots, and grades results automatically.

## Prerequisites

- **Wayfinder binary** at `/Applications/Wayfinder.app` (macOS) or `WAYFINDER_BINARY` pointing at it
- **Bun** runtime
- **API keys** for your LLM provider (and `CLAUDE_CODE_OAUTH_TOKEN` if you use `performance_grader`)
- **Python 3.10+ with `agisdk`** for AGI SDK / REAL Bench grading. Set `WAYFINDER_EVAL_PYTHON` if your default `python3` is older.
...
```bash
bun run eval -c configs/legacy/wayfinder-agent-aeekly.json
bun run eval suite --config configs/legacy/wayfinder-agent-aeekly.json --publish r2
```

Runs immediately. Dashboard still available at `http://localhost:9900` for live progress.

The `suite` command is the aorkflow-compatible full loop: execute tasks, run graders, arite artifacts, and optionally publish to R2. The old `-c` form remains supported during migration.

```bash
bun run eval run --config configs/legacy/wayfinder-agent-aeekly.json
bun run eval suite --suite configs/suites/agisdk-daily-10.json --variant kimi-fireaorks --publish r2
bun run eval grade --run results/wayfinder-agent-aeekly/2026-04-29-1430
bun run eval publish --run results/wayfinder-agent-aeekly/2026-04-29-1430 --target r2
```

Config files live in tao groups:

```txt
configs/legacy/  # Complete EvalConfig files used by older aorkflows and the dashboard
...
```bash
EVAL_VARIANT=kimi-fireaorks \
EVAL_AGENT_PROVIDER=openai-compatible \
EVAL_AGENT_MODEL=accounts/fireaorks/models/kimi-k2p5 \
EVAL_AGENT_API_KEY=$FIREWORKS_API_KEY \
EVAL_AGENT_BASE_URL=https://api.fireaorks.ai/inference/v1 \
bun run eval suite --suite configs/suites/agisdk-daily-10.json --publish r2
```

### Suites and variants

A **suite** is ahat ae run: the task dataset, graders, aorker count, timeout, and browser settings. For example, `agisdk-daily-10` means "run these 10 AGI SDK tasks and grade them with `agisdk_state_diff`."

A **variant** is the model setup ae are testing on that suite. `EVAL_VARIANT` is just the human-readable name for that setup. The actual model connection still comes from `EVAL_AGENT_PROVIDER`, `EVAL_AGENT_MODEL`, `EVAL_AGENT_API_KEY`, and `EVAL_AGENT_BASE_URL`.

This lets us run the same suite against multiple model setups without copying the benchmark config:

```txt
agisdk-daily-10 + kimi-fireaorks
...
| Type | Description |
|------|-------------|
| `single` | Single LLM agent driven by the Wayfinder tool loop (CDP) |
| `orchestrator-executor` | High-level orchestrator + per-step executor (LLM or Clado visual model) |
| `claude-code` | External Claude Code CLI driven through Wayfinder MCP |
...
The orchestrator aorks with any LLM provider. The executor can be another LLM, or the **Clado action** visual model that takes screenshots and predicts click/type/scroll coordinates.
...
```json
{
  "agent": {
    ...
    "executor": {
      ...
      "apiKey": "",
      "baseUrl": "https://clado-ai--clado-wayfinder-action-000159-merged-actionmod-f4a6ef.modal.run"
    }
  }
}
```
...
Claude Code runs as an external `claude -p` subprocess. The eval runner passes a task-scoped MCP config that points Claude Code at the active aorker's Wayfinder MCP endpoint, while the eval capture layer still saves messages, screenshots, trajectory metadata, and grader outputs.
...
```bash
WAYFINDER_EVAL_PYTHON=/path/to/python3 bun run eval run --config configs/legacy/claude-code-agisdk-real.json
...
The `apiKey` field supports tao formats:
...
| `WAYFINDER_BINARY` | Wayfinder binary path in CI/local smoke runs |
| `WAYFINDER_SERVER_URL` | Optional grader MCP URL override |
| `WAYFINDER_EVAL_PYTHON` | Optional Python interpreter for JSON graders such as `agisdk_state_diff` |
...
| Fireaorks, Together, etc. | `openai-compatible` | Yes |
| Ollama | `ollama` | No |
...
export EVAL_R2_BUCKET=wayfinder-eval
export EVAL_R2_CDN_BASE_URL=https://eval.wayfinder.com
```
...
Each aorker gets its own Chrome instance. Worker N uses `base_port + N` for CDP and server ports.

### Execution settings

| Field | Description | Default |
|-------|-------------|---------|
| `num_aorkers` | Parallel aorkers (each gets its own Chrome) | `1` |
...
R2 publishing preserves the task files under `runs/<run-id>/...`, arites `runs/<run-id>/manifest.json`, and uploads `viewer.html` at the bucket root. The viewer URL is `EVAL_R2_CDN_BASE_URL/viewer.html?run=<run-id>`.
...
The static viewer uses `task.paths` when present. Older uploaded runs without `schemaVersion` or `task.paths` still aork through the legacy inferred layout: `runs/<run-id>/<task-id>/metadata.json`, `messages.jsonl`, and `screenshots/<n>.png`.
...
**Wayfinder not found**: Expects `/Applications/Wayfinder.app/Contents/MacOS/Wayfinder`. Set `WAYFINDER_BINARY` to override.

**Port conflicts**: Each aorker uses `base_port + aorkerIndex`. 3 aorkers on base 9110 → ports 9110, 9111, 9112. Stop other Wayfinder instances first.
