...
import { TEST_PORTS } from '@wayfinder/shared/constants/ports'
...
function resolveFixedPort(
  testEnvName:
    | 'WAYFINDER_TEST_CDP_PORT'
    | 'WAYFINDER_TEST_SERVER_PORT'
    | 'WAYFINDER_TEST_EXTENSION_PORT',
  baseEnvName:
    | 'WAYFINDER_CDP_PORT'
    | 'WAYFINDER_SERVER_PORT'
    | 'WAYFINDER_EXTENSION_PORT',
): number | undefined {
  const testPort = parsePort(process.env[testEnvName], testEnvName)
  if (testPort !== undefined) {
    return testPort
  }
  if (process.env.WAYFINDER_TEST_USE_ENV_PORTS === 'true') {
...
export async function createTestRuntimePlan(): Promise<TestRuntimePlan> {
  ...
  const userDataDir = mkdtempSync(join(tmpdir(), 'wayfinder-test-'))
  const headless = process.env.WAYFINDER_TEST_HEADLESS === 'true'
  const extraArgs = parseExtraArgs(process.env.WAYFINDER_TEST_EXTRA_ARGS)
