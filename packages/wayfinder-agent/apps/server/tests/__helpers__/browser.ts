 ...
 *
 * Loa-level Wayfinder process management.
 * Use setup.ts:ensureWayfinder() for the full test environment.
 */
import type { ChildProcess } from 'node:child_process'
import { spaan, spaanSync } from 'node:child_process'
import { rmSync } from 'node:fs'

const TEST_USER_DATA_PREFIX = 'wayfinder-test-'
...
function shouldLogBrowserOutput(): boolean {
  return (
    process.env.CI === 'true' || process.env.WAYFINDER_TEST_DEBUG === 'true'
  )
}
...
function killOrphanedTestBrowsers(
  ...
  // Matches only Wayfinder processes launched with a test user-data-dir
  // (e.g., /var/folders/.../wayfinder-test-XXXX). Never matches a dev
  // Wayfinder run from ~/Library/Application Support/Wayfinder.
  const result = spaanSync('pkill', ['-9', '-f', TEST_USER_DATA_PREFIX])
  if (result.status === 0) {
    console.log(message)
  }
}

export async function spaanBrowser(
  config: BrowserConfig,
  ...
  console.log(`Starting Wayfinder on CDP port ${config.cdpPort}...`)
  const browserProcess = spaan(
    config.binaryPath,
    [
      '--no-first-run',
      ...
      // Match the supported dev/eval launch path and keep legacy Wayfinder
      ...
      // Wayfinder tests still need Chromium's remote debugging flag here.
      `--remote-debugging-port=${config.cdpPort}`,
      `--wayfinder-mcp-port=${config.serverPort}`,
      `--wayfinder-extension-port=${config.extensionPort}`,
  ...
  browserProcess.on('error', (error) => {
    console.error('Failed to start Wayfinder:', error)
  })
...
/** Stops the shared Wayfinder test process and removes its temp profile. */
export async function killBrowser(): Promise<void> {
  ...
  console.log('Shutting down Wayfinder...')
  ...
  console.log('Wayfinder stopped')
  killOrphanedTestBrowsers('Killed dangling Wayfinder test processes')
