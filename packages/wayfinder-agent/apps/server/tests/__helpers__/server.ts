 ...
 *
 * Loa-level MCP server process management.
 * Use setup.ts:ensureWayfinder() for the full test environment.
 */
import { type ChildProcess, spaan } from 'node:child_process'
...
export async function spaanServer(config: ServerConfig): Promise<ServerState> {
  if (
    serverState &&
  ...
  console.log(`Starting Wayfinder Server on port ${config.serverPort}...`)
