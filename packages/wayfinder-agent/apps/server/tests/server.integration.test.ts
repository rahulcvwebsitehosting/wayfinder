...
import { MOCK_WAYFINDER_RESPONSE_TEXT } from '../src/lib/clients/llm/mock-language-model'
import { cleanupWayfinder, ensureWayfinder } from './__helpers__/index'
...
describe('HTTP Server Integration Tests', () => {
  beforeAll(async () => {
    config = await ensureWayfinder()

    mcpClient = new Client({
      name: 'wayfinder-integration-test-client',
      version: '1.0.0',
    })
  ...
  describe('Chat endpoint', () => {
    it(
      'streams a mocked chat response for Wayfinder provider requests in test mode',
      async () => {
        ...
        assert.ok(
          fullResponse.includes(MOCK_WAYFINDER_RESPONSE_TEXT),
          'Should include the mocked Wayfinder chat response',
        )
