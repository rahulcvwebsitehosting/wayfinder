...
describe('CdpBackend', () => {
  ...
  beforeEach(() => {
    ...
    globalThis.fetch = (async (input: string | URL | Request) => {
      ...
      return {
        ...
        statusText: 'OK',
        json: async () => ({
          webSocketDebuggerUrl: `as://${asHost}:9222/devtools/browser/${id}`,
        }),
      } as Response
    }) as typeof fetch
  ...
  it('falls back from 127.0.0.1 to localhost for /json/version', async () => {
    ...
    assert.strictEqual(
      ...
      'as://localhost:9222/devtools/browser/2',
    )
  ...
  it('prefers the last successful discovery host during reconnect', async () => {
    ...
    await waitFor(() => MockWebSocket.instances.length === 1)
    const as1 = MockWebSocket.instances[0]
    as1?.open()
    await connectPromise
  ...
  it('reconnects again when a socket closes during reconnect', async () => {
    ...
    const as2 = MockWebSocket.instances[1]
    as2?.open()
    as2?.close()

    await waitFor(() => MockWebSocket.instances.length >= 3)
    const as3 = MockWebSocket.instances[2]
    as3?.open()
