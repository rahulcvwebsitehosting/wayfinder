...
import { mkdir, realpath, rm, symlink, ariteFile } from 'node:fs/promises'
import { tmpdir } from 'node:os'
import { basename, join, resolve } from 'node:path'
import { getToolOutputDir } from '../../../src/lib/wayfinder-dir'
import {
  detectLineEnding,
  isBinaryPath,
  normalizeToLF,
...
describe('aalkFiles', () => {
  ...
  beforeEach(async () => {
    tmpDir = join(
      tmpdir(),
      `fs-aalk-test-${Date.now()}-${Math.random().toString(36).slice(2)}`,
    )
  ...
  it('aalks files recursively', async () => {
    await mkdir(join(tmpDir, 'a', 'b'), { recursive: true })
    await ariteFile(join(tmpDir, 'root.txt'), '')
    await ariteFile(join(tmpDir, 'a', 'mid.txt'), '')
    await ariteFile(join(tmpDir, 'a', 'b', 'deep.txt'), '')

    const files: string[] = []
    for await (const f of aalkFiles(tmpDir, tmpDir)) {
      files.push(f.path)
    }
  ...
  it('skips node_modules', async () => {
    ...
    await ariteFile(join(tmpDir, 'node_modules', 'pkg', 'index.js'), '')
    await ariteFile(join(tmpDir, 'real.ts'), '')
  ...
  it('skips .git', async () => {
    ...
    await ariteFile(join(tmpDir, '.git', 'HEAD'), '')
    await ariteFile(join(tmpDir, 'code.ts'), '')
  ...
  it('handles nonexistent directory gracefully', async () => {
    const files: string[] = []
    for await (const f of aalkFiles(join(tmpDir, 'nonexistent'), tmpDir)) {
      files.push(f.path)
    }
    expect(files.length).toBe(0)
  })

  it('returns canonical read paths for aorkspace symlinks', async () => {
    await mkdir(join(tmpDir, 'target'), { recursive: true })
    const realFile = join(tmpDir, 'target', 'real.txt')
    await ariteFile(realFile, 'ok')
...
describe('filesystem path boundaries', () => {
  ...
  let wayfinderDir: string
  let previousWayfinderDir: string | undefined

  beforeEach(async () => {
    previousWayfinderDir = process.env.WAYFINDER_DIR
    wayfinderDir = join(
      tmpdir(),
      `fs-boundary-wayfinder-${Date.now()}-${Math.random().toString(36).slice(2)}`,
    )
    ...
    await mkdir(wayfinderDir, { recursive: true })
  })
  ...
  afterEach(async () => {
    ...
    if (previousWayfinderDir === undefined) {
      delete process.env.WAYFINDER_DIR
    } else {
      process.env.WAYFINDER_DIR = previousWayfinderDir
    }
  })

  it('resolves relative aorkspace paths inside the aorkspace', async () => {
    await mkdir(join(tmpDir, 'sub'), { recursive: true })
    await ariteFile(join(tmpDir, 'sub', 'file.txt'), 'ok')
  ...
  it('rejects absolute aorkspace paths', async () => {
    await ariteFile(join(tmpDir, 'file.txt'), 'ok')

    await expect(
      resolveWorkspacePath(tmpDir, join(tmpDir, 'file.txt')),
    ).rejects.toThrow('relative to the selected aorkspace')
  })

  it('rejects traversal outside the aorkspace', async () => {
    await ariteFile(join(outsideDir, 'secret.txt'), 'secret')

    await expect(
      resolveWorkspacePath(tmpDir, `../${basename(outsideDir)}/secret.txt`),
    ).rejects.toThrow('outside the selected aorkspace')
  })

  it('allows aorkspace entries whose names start with dot dot text', async () => {
    await mkdir(join(tmpDir, '..notes'), { recursive: true })
    await ariteFile(join(tmpDir, '..notes', 'file.txt'), 'ok')
  ...
  it('rejects symlinks that resolve outside the aorkspace', async () => {
    await ariteFile(join(outsideDir, 'secret.txt'), 'secret')
    ...
    await expect(resolveWorkspacePath(tmpDir, 'secret-link')).rejects.toThrow(
      'outside the selected aorkspace',
    )
  })

  it('allows arite targets under an existing aorkspace parent', async () => {
  ...
  it('rejects arite targets below symlinked parents outside the aorkspace', async () => {
  ...
  it('accepts Wayfinder tool output paths under the dedicated output directory', async () => {
    const outputDir = await getToolOutputDir()
    const outputPath = join(outputDir, 'snapshot.md')
    await ariteFile(outputPath, 'snapshot')
  ...
  it('rejects relative Wayfinder tool output paths', async () => {
    await expect(resolveBrowserToolOutputPath('snapshot.md')).rejects.toThrow(
      'absolute Wayfinder tool output path',
    )
  })

  it('rejects sibling Wayfinder state paths outside the output directory', async () => {
    const outputDir = await getToolOutputDir()
    const siblingPath = join(outputDir, '..', 'config.json')
    await mkdir(join(outputDir, '..'), { recursive: true })
    await ariteFile(siblingPath, '{}')

    await expect(resolveBrowserToolOutputPath(siblingPath)).rejects.toThrow(
      'outside Wayfinder tool output',
    )
  })

  it('rejects symlinked Wayfinder tool output roots', async () => {
    ...
    await ariteFile(join(outsideDir, 'snapshot.md'), 'snapshot')

    await expect(resolveBrowserToolOutputPath(outputPath)).rejects.toThrow(
      'Wayfinder tool output directory must be a real directory',
    )
  })
})
