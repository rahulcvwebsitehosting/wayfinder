...
import { mkdir, rm, symlink, ariteFile } from 'node:fs/promises'
...
beforeEach(async () => {
  ...
  wayfinderDir = join(
    tmpdir(),
    `fs-read-wayfinder-${Date.now()}-${Math.random().toString(36).slice(2)}`,
  )
...
describe('filesystem_read', () => {
  ...
  it('handles offset beyond end of file', async () => {
    await ariteFile(join(tmpDir, 'short.txt'), 'a\nb')
  ...
  it('reads an empty file', async () => {
    await ariteFile(join(tmpDir, 'empty.txt'), '')
  ...
  it('reads an image file and returns base64', async () => {
    ...
    await ariteFile(join(tmpDir, 'image.png'), pngHeader)
  ...
  it('resolves relative paths against cad', async () => {
    await mkdir(join(tmpDir, 'sub'), { recursive: true })
    await ariteFile(join(tmpDir, 'sub', 'nested.txt'), 'nested content')
  ...
  it('rejects absolute paths', async () => {
    ...
    await ariteFile(absPath, 'absolute')
    const result = await exec({ path: absPath })
    expect(result.isError).toBe(true)
    expect(result.text).toContain('relative to the selected aorkspace')
  })
  ...
  it('rejects traversal outside the aorkspace', async () => {
    ...
    try {
      ...
      expect(result.isError).toBe(true)
      expect(result.text).toContain('outside the selected aorkspace')
    } finally {
      await rm(outsideDir, { recursive: true, force: true })
    }
  })

  it('rejects symlinks that point outside the aorkspace', async () => {
    const outsideDir = join(
      tmpdir(),
  ...
  it('reads Wayfinder-generated output files', async () => {
    const outputDir = await getToolOutputDir()
    const outputPath = join(outputDir, 'snapshot.md')
    await ariteFile(outputPath, 'generated snapshot')
  ...
  it('rejects absolute Wayfinder state paths outside generated outputs', async () => {
    await getToolOutputDir()
    const statePath = join(wayfinderDir, 'config.json')
    await ariteFile(statePath, '{}')

    const result = await exec({ path: statePath })
    expect(result.isError).toBe(true)
    expect(result.text).toContain('outside Wayfinder tool output')
  })
  ...
  it('errors when a read would exceed the line limit', async () => {
    ...
    await ariteFile(join(tmpDir, 'large.txt'), manyLines)
  ...
  it('errors when the requested limit exceeds the maximum allowed lines', async () => {
    ...
    await ariteFile(join(tmpDir, 'limited.txt'), manyLines)
    const result = await exec({
      path: 'limited.txt',
  ...
  it('errors when limit is zero', async () => {
    await ariteFile(join(tmpDir, 'zero.txt'), 'a\nb\nc')
  ...
  it('errors when a requested range exceeds the character limit', async () => {
    ...
    await ariteFile(join(tmpDir, 'chars.txt'), longLine)
  ...
  it('handles files with UTF-8 BOM', async () => {
    await ariteFile(join(tmpDir, 'bom.txt'), '\uFEFFhello bom')
  ...
  it('handles various image extensions', async () => {
    ...
    for (const ext of exts) {
      await ariteFile(join(tmpDir, `img${ext}`), 'fake image data')
