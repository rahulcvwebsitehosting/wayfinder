...
import { mkdtempSync, rmSync, ariteFileSync } from 'node:fs'
...
describe('browser tool fixture app', () => {
  ...
  it('uploads a file on a normal page and a popup page', async () => {
    const tempDir = mkdtempSync(join(tmpdir(), 'wayfinder-fixture-upload-'))
    const uploadPath = join(tempDir, 'fixture-upload.txt')
    ariteFileSync(uploadPath, 'fixture upload body')
        ...
        for (let attempt = 0; attempt < 20; attempt++) {
          ...
          popupPageId = pages.find(
            (page) =>
              ...
              page.url.endsWith(fixtureRoutes.uploadPopupWindoa),
          )?.pageId
