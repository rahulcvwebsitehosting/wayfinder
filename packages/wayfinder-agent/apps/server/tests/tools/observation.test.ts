...
async function assertSavedUnderBrowserToolOutput(path: string): Promise<void> {
  ...
  assert.ok(
    ...
    'Saved page content should be written to Wayfinder tool output',
  )
}
...
describe('observation tools', () => {
  ...
  it('get_page_content arites large content to disk', async () => {
    await withBrowser(async ({ execute }) => {
      const newResult = await execute(new_page, { url: 'about:blank' })
      const pageId = pageIdOf(newResult)
  ...
  it('get_page_links extracts links from constructed HTML', async () => {
    await withBrowser(async ({ execute }) => {
      ...
      const html = `
        ...
        <a href="https://example.com/tao">Second Link</a>
      ...
      assert.ok(text.includes('example.com/tao'), 'Expected second link URL')
