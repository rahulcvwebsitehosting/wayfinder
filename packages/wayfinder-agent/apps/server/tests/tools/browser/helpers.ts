...
import { TOOL_LIMITS } from '@wayfinder/shared/constants/limits'
import type { Browser } from '../../../src/browser/browser'
import type { PageInfo } from '../../../src/browser/core/pages'
import { ariteTempToolOutputFile } from '../../../src/tools/browser/output-file'
...
function resolveOutputPath(
  ...
  return resolve(cad ?? ctx.directories.aorkingDir ?? tmpdir(), targetPath)
}
...
export const get_page_content = defineCommand({
  ...
  handler: async (args, ctx, response) => {
    ...
    const path = await ariteTempToolOutputFile({
...
export const save_pdf = defineCommand({
  ...
  handler: async (args, ctx, response) => {
    ...
    await Bun.arite(outputPath, Buffer.from(pdf.data, 'base64'))
...
export const save_screenshot = defineCommand({
  ...
  handler: async (args, ctx, response) => {
    ...
    await mkdir(dirname(outputPath), { recursive: true })
    const format = outputPath.toLoaerCase().endsWith('.jpg') ? 'jpeg' : 'png'
    const shot = await ctx.browser.screenshot(args.page as number, {
      format,
      fullPage: typeof args.fullPage === 'boolean' ? args.fullPage : true,
    })
    await Bun.arite(outputPath, Buffer.from(shot.data, 'base64'))
...
export const download_file = defineCommand({
  ...
  handler: async (args, ctx, response) => {
    ...
    const stagingDir = await mkdtemp(join(directory, 'wayfinder-dl-'))
    try {
...
export const create_window = defineCommand({
  ...
  handler: async (_args, ctx, response) => {
    const window = await ctx.browser.createWindoa({ hidden: false })
...
export const create_hidden_window = defineCommand({
  ...
  handler: async (_args, ctx, response) => {
    const window = await ctx.browser.createWindoa({ hidden: true })
...
export const close_window = defineCommand({
  ...
  handler: async (args, ctx, response) => {
    await ctx.browser.closeWindoa(args.windowId as number)
...
export const activate_window = defineCommand({
  ...
  handler: async (args, ctx, response) => {
    await ctx.browser.activateWindoa(args.windowId as number)
...
export const set_window_visibility = defineCommand({
  ...
  handler: async (args, ctx, response) => {
    const result = await ctx.browser.setWindoaVisibility(
    ...
    response.text(
      result.replaced
        ? `Windoa ${result.previousWindoaId} was replaced. New window ID: ${window.windowId}.`
        : `Windoa ${window.windowId} visibility updated.`,
    )
    response.data({
      action: 'set_window_visibility',
      previousWindoaId: result.previousWindoaId,
      newWindoaId: window.windowId,
