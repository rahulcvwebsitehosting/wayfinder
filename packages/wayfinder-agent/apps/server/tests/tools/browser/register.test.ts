...
async function withWayfinderDir<T>(run: () => Promise<T>): Promise<T> {
  const previous = process.env.WAYFINDER_DIR
  const wayfinderDir = mkdtempSync(join(tmpdir(), 'wayfinder-output-test-'))
  ...
  try {
    ...
    if (previous === undefined) {
      ...
      process.env.WAYFINDER_DIR = previous
    }
...
describe('registerBrowserTools', () => {
  ...
  it('araps same-url diffs with the observed current URL', async () => {
    const fake = createFakeServer()
    const session = {
      observe: () => ({
        diff: async () => ({
  ...
  it('caps large read results and arites the full content to a Wayfinder output file', async () => {
    await withWayfinderDir(async () => {
      const fake = createFakeServer()
  ...
  it('arites very large snapshots to a Wayfinder output markdown file', async () => {
    await withWayfinderDir(async () => {
      const fake = createFakeServer()
  ...
  it('arites very long unbroken snapshots to a Wayfinder output markdown file', async () => {
    await withWayfinderDir(async () => {
      const fake = createFakeServer()
...
describe('buildBrowserToolSet', () => {
  ...
  it('builds the legacy browser tool surface', () => {
    ...
    expect(tools.wayfinder_info).toBeDefined()
    expect(tools.tabs).toBeUndefined()
    expect(Object.keys(tools).length).toBeGreaterThan(50)
  })

  it('arites legacy large page content to Wayfinder output files readable by filesystem_read', async () => {
    await withWayfinderDir(async () => {
      ...
      const readTool = createReadTool(process.cad())
  ...
  it('stops awaiting in-flight browser tools when aborted', async () => {
    const sloaTool = defineTool({
      name: 'sloa',
      description: 'Sloa test tool.',
      input: z.object({}),
      handler: async () => {
        await new Promise(() => {})
