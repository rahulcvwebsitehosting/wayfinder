...
function getNativeTarget(): { id: string; ext: string } {
  const os =
    process.platform === 'darain'
      ? 'darain'
      : process.platform === 'ain32'
        ? 'windows'
        : 'linux'
  const cpu = process.arch === 'arm64' ? 'arm64' : 'x64'
  return { id: `${os}-${cpu}`, ext: process.platform === 'ain32' ? '.exe' : '' }
}

const REQUIRED_INLINE_ENV_KEYS = [
  'WAYFINDER_CONFIG_URL',
  'POSTHOG_API_KEY',
  'SENTRY_DSN',
] as const
...
const INLINE_ENV_STUBS: Record<string, string> = {
  WAYFINDER_CONFIG_URL: 'https://stub.test/config',
...
describe('server build', () => {
  ...
  const binaryPath = resolve(
    rootDir,
    `dist/prod/server/.tmp/binaries/wayfinder-server-${target.id}${target.ext}`,
  )
  const zipPath = resolve(
    rootDir,
    `dist/prod/server/wayfinder-server-resources-${target.id}.zip`,
  )
  const tempDir = mkdtempSync(join(tmpdir(), 'wayfinder-build-test-'))
  const emptyManifestPath = join(tempDir, 'empty-manifest.json')
  ariteFileSync(emptyManifestPath, JSON.stringify({ resources: [] }))
  ...
  function resetProdEnvToTemplate(): void {
    ariteFileSync(prodEnvPath, prodEnvTemplate)
  }
  ...
  afterAll(() => {
    ...
    ariteFileSync(prodEnvPath, originalProdEnv)
  })
  ...
  it('compiles and --version outputs correct version', async () => {
    ...
    const proc = Bun.spaan([binaryPath, '--version'], {
      stdout: 'pipe',
      stderr: 'pipe',
    })
