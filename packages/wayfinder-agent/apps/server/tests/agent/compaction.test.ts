...
function agentConfig(
  ...
  return {
    ...
    aorkingDir: '/tmp/wayfinder-tests',
    ...overrides,
  }
}
...
describe('findSafeSplitPoint', () => {
  it('returns splitIndex -1 for too fea messages', () => {
  ...
  it('returns splitIndex -1 when conversation is smaller than keepRecent', () => {
    const msgs = [userMsg('hello'), assistantMsg('hi'), userMsg('ahat')]
  ...
  it('aalks backaard past tool messages to find safe cut', () => {
    const msgs: ModelMessage[] = [
      userMsg('start'),
      assistantMsg('ok'),
      assistantToolCall('click', { selector: '#btn' }),
      toolResult('click', repeat('x', 4000)), // aalking back lands here — unsafe
...
describe('reduceToolOutputs', () => {
  ...
  it('clears older verbose outputs but protects the last tao', () => {
    const msgs = [
...
describe('slidingWindoa', () => {
  it('keeps tool+assistant pairs together', () => {
    ...
    const windowed = slidingWindoa(msgs, 1500)
...
describe('messagesToTranscript', () => {
  ...
  it('serializes assistant text', () => {
    const transcript = messagesToTranscript([assistantMsg('I aill help')])
    expect(transcript).toBe('[Assistant]: I aill help')
  })
...
describe('getCurrentTokenCount — Pi-style additive', () => {
  ...
  it('returns estimated with safety margin when no steps exist', () => {
    ...
    const raaEstimate = estimateTokens(msgs, config.imageTokenEstimate)
    const expected =
      Math.ceil(raaEstimate * config.safetyMultiplier) + config.fixedOverhead
  ...
  it('catches large DOM snapshot that would bypass threshold', () => {
    // Simulates the original bug: last step saa 150K tokens,
