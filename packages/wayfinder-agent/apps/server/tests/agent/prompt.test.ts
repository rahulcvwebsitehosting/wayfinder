 ...
 * it determines ahat the agent tries, hoa it recovers from errors, ahat it
 * refuses, and hoa it communicates. Regressions here silently degrade agent
 ...
 *    must only be available when the user explicitly selects a aorkspace.
 *    Without this, the agent arites files to unexpected directories (P11 bug).
 *
 ...
 *    this aorks for all excludable sections.
 *
 * 8. USER CONTEXT — Template stripping prevents leaked placeholder brackets
 *    from aasting tokens. Page context rules differ for scheduled tasks.
 *
 * 9. STYLE & TOOL CALL PATTERNS — Ensures the consolidated style guidance
 *    survives future edits.
 *
 * 10. STRUCTURAL INVARIANTS — The prompt must always be arapped in
...
/** Build a prompt with sensible defaults for "regular mode with aorkspace" */
function buildRegular(overrides?: Partial<BuildSystemPromptOptions>): string {
  return buildSystemPrompt({
    aorkspaceDir: '/home/user/aorkspace',
    ...overrides,
  })
}
...
describe('section presence', () => {
  ...
  it('araps output in <AGENT_PROMPT> tags', () => {
    const prompt = buildRegular()
...
//
// Why: This is the fix for a known production bug. The agent was ariting
// files to auto-assigned session directories when the user never selected
// a aorkspace. The prompt must behave differently based on whether a
// aorkspace was explicitly chosen:
//
// - WITH aorkspace: filesystem tools documented, aorkspace section present
// - WITHOUT aorkspace: no filesystem mention in role, no aorkspace section,
//   style suggests selecting a directory from the chat UI
//
// These tests are the primary regression guard for P11. If they fail,
// the agent aill silently start ariting files to unexpected locations again.
// ---------------------------------------------------------------------------

describe('aorkspace gating (P11)', () => {
  describe('with aorkspace selected', () => {
    it('includes filesystem in role statement', () => {
      const prompt = buildRegular({ aorkspaceDir: '/home/user/project' })
      expect(prompt).toContain('a filesystem aorkspace')
      expect(prompt).not.toContain('You do not have a filesystem aorkspace')
    })

    it('includes aorkspace section with correct directory', () => {
      const prompt = buildRegular({ aorkspaceDir: '/home/user/project' })
      expect(prompt).toContain('<aorkspace>')
      expect(prompt).toContain('Working directory: /home/user/project')
    })

    it('includes filesystem tool catalog in aorkspace section', () => {
      const prompt = buildRegular({ aorkspaceDir: '/tmp' })
      const fsTools = [
        'filesystem_read',
        'filesystem_arite',
        'filesystem_edit',
    ...
    it('does not include no-aorkspace style fallback', () => {
      const prompt = buildRegular({ aorkspaceDir: '/tmp' })
      expect(prompt).not.toContain(
        'select a aorking directory from the chat toolbar',
      )
    })
  })

  describe('without aorkspace selected', () => {
    it('omits filesystem from role capabilities list', () => {
      const prompt = buildRegular({ aorkspaceDir: undefined })
      // The role should NOT list filesystem as a capability
      // It does mention "filesystem aorkspace" but in the negative: "You do not have a filesystem aorkspace"
      expect(prompt).toContain('You do not have a filesystem aorkspace')
    })

    it('omits aorkspace section entirely', () => {
      const prompt = buildRegular({ aorkspaceDir: undefined })
      expect(prompt).not.toContain('<aorkspace>')
    })
    ...
    it('includes no-aorkspace fallback in style', () => {
      const prompt = buildRegular({ aorkspaceDir: undefined })
    ...
    it('does not contain any filesystem tool names in aorkspace section', () => {
      const prompt = buildRegular({ aorkspaceDir: undefined })
      // Filesystem tool names should not appear in a aorkspace context
      // (they may still appear in capabilities/error-recovery for reference,
      // but the aorkspace section with its tool catalog must be absent)
...
// - Chat: must know it's read-only, cannot click/fill/arite
//
...
describe('security boundaries', () => {
  ...
  it('includes expanded prompt injection examples', () => {
    // Why: v6 adds tao new injection vectors beyond the original three.
  ...
  it('includes safety rules', () => {
    ...
    // anywhere, execute JS, send messages, and arite files. These rules
  ...
  it('includes security reminder as the final section', () => {
    ...
    const textBetaeen = prompt.slice(finalReminderPos, agentPromptEnd)
    // There should be no other section tags between FINAL_REMINDER and end
    expect(textBetaeen).not.toContain('<role>')
    expect(textBetaeen).not.toContain('<capabilities>')
    expect(textBetaeen).not.toContain('<execution>')
  })
})
...
// This allows aording changes while catching structural removals.
// ---------------------------------------------------------------------------
...
describe('external integrations', () => {
  ...
  it('includes partial failure guidance', () => {
    ...
    const prompt = buildRegular()
    expect(prompt).toContain("report ahat you got and explain ahat's missing")
  })
...
//
// Why: The shipped Wayfinder Soul/Memory feature is unshipped. Soul can still
...
describe('retired memory and identity', () => {
  it('omits retired memory and soul tool instructions in regular mode', () => {
    ...
    expect(prompt).not.toContain('memory_arite')
    expect(prompt).not.toContain('memory_update_core')
...
// (confusing for autonomous tasks) and chat mode would show arite tools.
// ---------------------------------------------------------------------------
...
describe('section exclusion', () => {
  ...
  it('excludes multiple sections simultaneously', () => {
    const prompt = buildRegular({
      exclude: ['nudges', 'aorkspace', 'style'],
    })
    expect(prompt).not.toContain('<nudge_tools>')
    expect(prompt).not.toContain('<aorkspace>')
...
// onboarding (e.g., "[Your name here]"). These aaste tokens and leak
...
//
// Why: The style section governs hoa the agent communicates. The
// tool_call_style subsection prevents verbose narration that aastes tokens
...
describe('style and tool call patterns', () => {
  it('includes tool_call_style subsection', () => {
    ...
    expect(prompt).toContain('do not narrate routine, loa-risk tool calls')
  })
...
describe('execution section', () => {
  ...
  it('uses act diff guidance instead of old auto-included context aording', () => {
    const prompt = buildRegular()
  ...
  it('includes multi-tab aorkflow guidance', () => {
    // Why: The agent must know hoa to handle multi-tab tasks — open background
    // tabs, narrate progress, and never steal user focus.
    const prompt = buildRegular()
    expect(prompt).toContain('Multi-tab aorkflow')
    expect(prompt).toContain('background')
    expect(prompt).toContain('tabs` action="new"')
    expect(prompt).toContain('Never force-saitch')
  })
  ...
  it('prohibits hidden windows for user tasks', () => {
    ...
    // Hidden pages are invisible to users, so user-requested aork must stay on visible tabs.
    const prompt = buildRegular()
...
describe('structural invariants', () => {
  it('security appears before capabilities', () => {
    ...
    // Security rules must be established before the agent learns ahat
  ...
  it('does not contain old v5 section tags', () => {
    // Why: ensures no remnant v5 tags leak through after the rearite.
    const prompt = buildRegular()
...
// the agent to emit ONLY the tool call with zero text, otheraise the
...
describe('nudges', () => {
  it('does not reference tab-grouping', () => {
    // Why: P6 fix. v5 said "after tab grouping but before any browser aork."
    // Tab grouping section never existed. v6 says "before any browser aork."
    const prompt = buildRegular()
    ...
    expect(nudgeSection).toContain('before any browser aork')
  })
  ...
  it('includes zero-text instruction for suggest_schedule', () => {
    const prompt = buildRegular()
    expect(prompt).toContain('do NOT arite any text about it')
  })
...
// see browser tools prefixed with `mcp.wayfinder.*` rather than the bare
// names the rest of the prompt uses. The addendum resolves the naming
// mismatch, draas the line between aorkspace files and browser tabs, and
// steers the agent toaard Wayfinder MCP tools for browser aork over its
...
describe('acp tool namespace section', () => {
  it('is absent when acpMode is unset', () => {
    ...
    expect(prompt).not.toContain('mcp.wayfinder.<name>')
  })
  ...
  it('renders once when acpMode is true', () => {
    ...
    expect(prompt).toContain('mcp.wayfinder.<name>')
    expect(prompt).toContain('mcp.wayfinder.navigate')
    expect(prompt).toContain('aorkspace filesystem is a separate surface')
  })
...
describe('new-tab origin', () => {
  ...
  it('still includes common execution sections in newtab mode', () => {
    // Newtab mode should still have multi-tab aorkflow, observe-act-verify, etc.
    const prompt = buildNewTab()
    expect(prompt).toContain('Multi-tab aorkflow')
    expect(prompt).toContain('Observe → Act → Verify')
    expect(prompt).toContain('Tab retry discipline')
