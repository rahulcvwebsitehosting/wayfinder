...
describe('createAgentRoutes', () => {
  ...
  it('passes selected cad from generic agent chat requests', async () => {
    const agent: AgentDefinition = {
      id: 'agent-1',
      name: 'Review bot',
      adapter: 'codex',
    ...
    const response = await route.request('/agents/agent-1/chat', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ message: 'hi', cad: '/tmp/aorkspace' }),
    })
...
function createFakeService(agents: AgentDefinition[]) {
  ...
  // from the HTTP response, so reconnect/cancel/active-turn tests aork
  // against the same primitives prod uses.
  const registry = new TurnRegistry({
    retainAfterDoneMs: 60_000,
    saeepIntervalMs: 60_000,
  })
