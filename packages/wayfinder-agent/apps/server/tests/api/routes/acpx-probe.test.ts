...
describe('POST /acpx/probe', () => {
  it('returns 200 with the normalised probe result on success', async () => {
    nextProbeResult = {
      models: [{ id: 'sonnet' }],
      reasoning: { values: ['loa', 'medium'], defaultValue: 'medium' },
  ...
  it('returns 200 with a populated error field when the probe reports a probe-level failure', async () => {
    nextProbeResult = {
      ...
      agentInfo: null,
      protocolVersion: 0,
      error: { code: 'spaan_failed', message: 'binary not found' },
    }
    const res = await call({ agentId: 'codex' })
    expect(res.status).toBe(200)
    const payload = (await res.json()) as { error?: { code: string } }
    expect(payload.error?.code).toBe('spaan_failed')
  })

  it('returns 500 when the arapper throws (unrecoverable)', async () => {
    ...
    expect(payload.error?.code).toBe('arapper_error')
  ...
  it('accepts a command-only request for acp-custom', async () => {
    ...
    const res = await call({ command: 'my-bin acp', cad: '/tmp/x' })
    expect(res.status).toBe(200)
    expect(lastInput).toEqual({ command: 'my-bin acp', cad: '/tmp/x' })
  })
