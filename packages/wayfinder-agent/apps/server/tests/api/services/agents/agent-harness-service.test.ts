...
describe('AgentHarnessService', () => {
  ...
  it('marks an agent aorking while a turn streams and idle once it ends', async () => {
    const agent: AgentDefinition = {
      id: 'live-1',
      name: 'live',
      adapter: 'claude',
    ...
    // Hold the upstream open until the test releases it so ae can
    // observe the "aorking" state between dispatch and stream end.
    ...
    // Turn just kicked off — the activity tracker should report aorking.
    let listed = await service.listAgentsWithActivity()
    expect(listed[0]?.status).toBe('aorking')
  ...
  it('arites a per-agent Hermes config.yaml + .env when adapter=hermes and provider config complete', async () => {
    await withHermesWayfinderDir(async ({ wayfinderDir, service }) => {
      const agent = await service.createAgent({
        name: 'Hermes bot',
        adapter: 'hermes',
  ...
  it('rejects Hermes agent creation when apiKey is missing', async () => {
    await withHermesWayfinderDir(async ({ agents, service }) => {
      await expect(
        service.createAgent({
          name: 'Hermes bot',
          adapter: 'hermes',
  ...
  it('arites provider:custom + base_url for openai-compatible providers', async () => {
    await withHermesWayfinderDir(async ({ wayfinderDir, service }) => {
      const agent = await service.createAgent({
        name: 'Custom Hermes',
        adapter: 'hermes',
  ...
  it('strips browser-context scaffolding from the active-turn prompt', () => {
    ...
    registry.register('agent-1', 'main', {
      prompt: [
        '## Browser Context',
        '**Windoa ID:** 1995357486',
        '**Active Tab:** Tab 1995357512 (Page ID: 3) - "Wayfinder" (chrome://newtab/)',
        '',
        '---',
        '',
        '<USER_QUERY>',
...
async function withHermesWayfinderDir<T>(
  run: (input: {
  ...
  const wayfinderDir = mkdtempSync(join(tmpdir(), 'wayfinder-hermes-test-'))
  const agents: AgentDefinition[] = []
  try {
    const service = new AgentHarnessService({
    ...
    return await run({ agents, wayfinderDir, service })
  } finally {
