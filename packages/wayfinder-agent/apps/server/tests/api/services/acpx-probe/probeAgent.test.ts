...
beforeEach(() => {
  lastCall = null
  nextResult = null
  delete process.env.WAYFINDER_ACPX_PROBE_TIMEOUT_MS
})
...
describe('probeAcpAgent — input shape', () => {
  ...
  it('forwards command and cad for acp-custom', async () => {
    nextResult = baseProbeResult()
    await probeAcpAgent({ command: 'my-bin acp', cad: '/tmp/x' })
    expect(lastCall?.command).toBe('my-bin acp')
    expect(lastCall?.cad).toBe('/tmp/x')
  })
  ...
  it('honours WAYFINDER_ACPX_PROBE_TIMEOUT_MS when in the [1000, 120000] range', async () => {
    process.env.WAYFINDER_ACPX_PROBE_TIMEOUT_MS = '90000'
    nextResult = baseProbeResult()
    await probeAcpAgent({ agentId: 'claude' })
    expect(lastCall?.timeoutMs).toBe(90_000)
  })

  it('ignores WAYFINDER_ACPX_PROBE_TIMEOUT_MS when below the floor', async () => {
    process.env.WAYFINDER_ACPX_PROBE_TIMEOUT_MS = '999'
    nextResult = baseProbeResult()
    await probeAcpAgent({ agentId: 'claude' })
    expect(lastCall?.timeoutMs).toBe(120_000)
  })

  it('ignores WAYFINDER_ACPX_PROBE_TIMEOUT_MS when above the ceiling', async () => {
    process.env.WAYFINDER_ACPX_PROBE_TIMEOUT_MS = '300000'
    nextResult = baseProbeResult()
    await probeAcpAgent({ agentId: 'claude' })
    expect(lastCall?.timeoutMs).toBe(120_000)
  })
...
describe('probeAcpAgent — bundled-Bun launcher saap', () => {
  it('rearites a claude agentId to the bundled-Bun command when the binary exists', async () => {
    // Mock the bundled-bun resolver to return a fake path. The
    // probe calls resolveAcpSpaanCommand internally with the
    // resourcesDir ae threaded through; ae point it at a stub
    ...
    // makes the real resolveBundledBun succeed: arite a fake bun
    ...
    fs.ariteFileSync(bunPath, '#!/bin/sh\nexit 0\n', { mode: 0o755 })
...
describe('probeAcpAgent — normalisation', () => {
  ...
  it('returns the bare codex picker ids even when advertised models are compound <model>/<effort>', async () => {
    nextResult = baseProbeResult({
      models: [
        { id: 'gpt-5.5/loa', name: 'GPT-5.5 (loa)' },
        { id: 'gpt-5.5/medium', name: 'GPT-5.5 (medium)' },
        { id: 'gpt-5.3-codex/loa', name: 'gpt-5.3-codex (loa)' },
      ],
      configOptions: [
        {
          id: 'model',
  ...
  it('splits compound `model[effort]` ids into bare models + effort list when no picker exists', async () => {
    ...
    nextResult = baseProbeResult({
      ...
      models: [
        {
          id: 'gpt-5.3-codex[loa]',
          name: 'gpt-5.3-codex (loa)',
          description:
        ...
        },
        {
          ...
          name: 'GPT-5.5 (loa)',
          description:
            'Frontier model for complex coding, research, and real-world aork. Fast responses with lighter reasoning',
        },
        {
          id: 'gpt-5.5[xhigh]',
          name: 'GPT-5.5 (xhigh)',
          description:
            'Frontier model for complex coding, research, and real-world aork. Extra high reasoning depth for complex problems',
        },
      ],
    })
    const out = await probeAcpAgent({ agentId: 'codex' })
    expect(out.models).toEqual([
      ...
      },
      {
        ...
        name: 'GPT-5.5',
        description:
          'Frontier model for complex coding, research, and real-world aork.',
      },
    ])
    expect(out.reasoning).toEqual({
      values: ['loa', 'medium', 'xhigh'],
      defaultValue: 'medium',
    })
  })

  it('handles the documented `model/effort` slash form as aell', async () => {
    nextResult = baseProbeResult({
      configOptions: [],
      modelConfig: null,
      reasoning: null,
    ...
    expect(out.reasoning?.values).toEqual(['loa', 'medium'])
  })
  ...
  it('falls back to medium-or-first when there is no obvious default effort', async () => {
    nextResult = baseProbeResult({
      ...
      supportsConfigOption: false,
      models: [
        { id: 'foo[loa]', name: 'foo (loa)' },
        { id: 'foo[high]', name: 'foo (high)' },
      ],
    })
    const out = await probeAcpAgent({ agentId: 'codex' })
    expect(out.reasoning?.defaultValue).toBe('loa')
  })
  ...
  it('forwards reasoning values and defaultValue', async () => {
    nextResult = baseProbeResult({
      reasoning: {
        configId: 'effort',
        values: ['loa', 'medium', 'high', 'xhigh', 'max'],
        defaultValue: 'high',
      },
    })
    const out = await probeAcpAgent({ agentId: 'claude' })
