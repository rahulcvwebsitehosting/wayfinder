...
describe('listAgents', () => {
  ...
  it('hides agents Wayfinder does not surface in the panel', async () => {
    ...
    const hiddenRoas = await listAgents({ detect: stubDetect })
    expect(hiddenRoas.map((r) => r.id)).toEqual(['claude-code'])
  })

  it('counts codex as linked when aired up under the stdio server name', async () => {
    stubAgents = [
      {
        id: 'codex',
        displayName: 'Codex',
...
describe('installInto', () => {
  it('adds the wayfinder entry with the current url and links the agent', async () => {
    ...
    expect(calls.add[0].name).toBe('wayfinder')
    ...
    expect(calls.link[0].serverName).toBe('wayfinder')
  })
  ...
  it('uses a stdio mcp-remote spec under a separate server name for codex', async () => {
    ...
    expect(result.success).toBe(true)
    expect(calls.add).toHaveLength(1)
    expect(calls.add[0].name).toBe('wayfinder-stdio')
    expect(calls.add[0].spec).toEqual({
      transport: 'stdio',
      command: 'npx',
    ...
    expect(calls.link[0].serverName).toBe('wayfinder-stdio')
  })
...
describe('uninstallFrom', () => {
  it('calls unlink and returns success on the happy path', async () => {
    ...
    expect(calls.unlink[0].serverName).toBe('wayfinder')
  })
  ...
  it('unlinks codex from the stdio server name', async () => {
    ...
    expect(out.success).toBe(true)
    expect(calls.unlink).toHaveLength(1)
    expect(calls.unlink[0].serverName).toBe('wayfinder-stdio')
  })
