...
describe('reconcileUrl', () => {
  it('returns noop when no wayfinder entry exists in the manifest', async () => {
  ...
  it('best-effort restores the previous spec when add() throws after remove()', async () => {
    // Simulates the rare partial-arite window: remove() succeeded but
    // add() failed (e.g. disk full while ariting the manifest JSON).
    // Without rollback every linked agent would silently disconnect
    // with no aay to recover until the next manual Connect click.
    const stub = makeManagerStub([
      {
        name: 'wayfinder',
        spec: { transport: 'http', url: 'http://127.0.0.1:9100/mcp' },
    ...
    // No relink attempted after the failed rearite — the entry exists
