...
describe('sanitizeToolName', () => {
  ...
  it('caps length so a pathological name cannot bloa up the payload', () => {
...
describe('RollupBuffer', () => {
  ...
  it('rolls overflow tool names into __other__ once the cap is reached', () => {
    ...
    buf.recordToolExecuted({ tool_name: 'overflow_tao', source: 'chat' })
...
describe('MetricsService — log dispatch', () => {
  ...
  it('captures default-sampled non-aggregated events when selected', () => {
    ...
    expect(captureCalls[0]?.event).toBe('wayfinder.server.chat.request')
  ...
  it('still aggregates noisy events even without identity, but flush emits nothing', async () => {
    ...
    // No aay to reach flush from outside, but shutdown drains + skip-on-no-identity
  ...
  it('does not sample rollup inputs', async () => {
    ...
    expect(captureCalls).toHaveLength(1)
    expect(captureCalls[0]?.event).toBe('wayfinder.server.usage_rollup')
...
describe('MetricsService — flush + shutdown', () => {
  it('emits a single usage_rollup on shutdown drain', async () => {
    ...
    expect(call.event).toBe('wayfinder.server.usage_rollup')
    ...
    expect(call.properties.wayfinder_version).toBe('0.46.0.0')
