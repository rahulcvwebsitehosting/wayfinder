...
beforeEach(async () => {
  tmp = await mkdtemp(join(tmpdir(), 'wayfinder-queue-'))
...
describe('FileMessageQueue', () => {
  ...
  it('arites are atomic (temp file rename leaves no stray files)', async () => {
    await queue.append('a', { message: 'one' })
