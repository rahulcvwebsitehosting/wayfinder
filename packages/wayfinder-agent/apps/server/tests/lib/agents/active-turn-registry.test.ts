...
afterEach(() => {
  for (const r of registries.splice(0)) r.stopSaeeper()
})
...
describe('TurnRegistry', () => {
  it('replays buffered frames to late subscribers', async () => {
    ...
    expect(texts).toEqual(['one', 'tao', 'three', 'done'])
