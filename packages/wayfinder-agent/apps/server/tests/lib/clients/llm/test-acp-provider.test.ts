...
describe('testAcpProvider — happy path', () => {
  ...
  it('forwards acpCommand and cad for acp-custom', async () => {
    nextProbeResult = probeOK()
    await runTest({
      provider: 'acp-custom',
      model: 'sonnet',
    ...
    expect(lastProbeInput?.cad).toBe('/tmp/x')
  })
})
...
describe('testAcpProvider — failure modes', () => {
  ...
  it('reports spaan_failed with a recovery hint', async () => {
    nextProbeResult = probeOK({
      error: { code: 'spaan_failed', message: 'no such file' },
    })
    const result = await runTest({
      provider: 'codex',
      model: 'gpt-5.5',
