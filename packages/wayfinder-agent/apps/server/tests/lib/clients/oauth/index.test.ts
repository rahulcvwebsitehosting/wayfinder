...
describe('OAuth client setup', () => {
  ...
  it('initializes a process token manager backed by the Wayfinder database', () => {
    const dir = mkdtempSync(join(tmpdir(), 'wayfinder-oauth-index-test-'))
    tempDirs.push(dir)
    const handle = initializeDb({
      dbPath: join(dir, 'db', 'wayfinder.sqlite'),
    })

    const manager = initializeOAuth(handle.db, 'wayfinder-1')
  ...
  it('stops and clears the current process token manager', () => {
    ...
    const firstManager = initializeOAuth(handle.db, 'wayfinder-1')
    const stopFirst = spyOn(firstManager, 'stopCallbackServer')

    const secondManager = initializeOAuth(handle.db, 'wayfinder-2')
