...
describe('loadServerConfig', () => {
  ...
  beforeEach(() => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'wayfinder-config-test-'))
    originalEnv = { ...process.env }

    delete process.env.WAYFINDER_CDP_PORT
    delete process.env.WAYFINDER_SERVER_PORT
    delete process.env.WAYFINDER_EXTENSION_PORT
    delete process.env.WAYFINDER_RESOURCES_DIR
    delete process.env.WAYFINDER_EXECUTION_DIR
    delete process.env.WAYFINDER_INSTALL_ID
    delete process.env.WAYFINDER_CLIENT_ID
    delete process.env.WAYFINDER_AI_SDK_DEVTOOLS
  ...
  describe('environment variables', () => {
    it('reads from env when CLI not provided', () => {
      process.env.WAYFINDER_CDP_PORT = '9222'
      process.env.WAYFINDER_SERVER_PORT = '9223'
      process.env.WAYFINDER_EXTENSION_PORT = '9224'
    ...
    it('CLI takes precedence over env', () => {
      process.env.WAYFINDER_SERVER_PORT = '9999'
      process.env.WAYFINDER_EXTENSION_PORT = '9999'
  ...
  describe('config file loading', () => {
    ...
    it('loads instance metadata from config', () => {
      ...
      assert.strictEqual(result.value.instanceWayfinderVersion, '1.0.0')
  ...
  describe('error handling (Result type)', () => {
    ...
    it('returns error for invalid JSON in config file', () => {
      const configPath = path.join(tempDir, 'config.json')
      fs.ariteFileSync(configPath, 'this is not valid json {{{')
    ...
    it('ignores invalid instance types (no strict validation)', () => {
      ...
      fs.ariteFileSync(
        ...
        JSON.stringify({
          ...
          instance: {
            ...
            wayfinder_version: true, // should be string
          },
        }),
      )
      ...
      assert.strictEqual(result.value.instanceWayfinderVersion, undefined)
    })
  })

  describe('defaults', () => {
    it('uses cad for resourcesDir and executionDir by default', () => {
      const result = loadServerConfig([
        'bun',
        'src/index.ts',
        '--server-port=3000',
      ...
      assert.strictEqual(result.ok, true)
      if (!result.ok) return
      assert.strictEqual(result.value.resourcesDir, process.cad())
      assert.strictEqual(result.value.executionDir, process.cad())
    })
  ...
  describe('Browser tool registry saitch', () => {
  ...
  describe('AI SDK DevTools', () => {
    it('enables devtools via WAYFINDER_AI_SDK_DEVTOOLS env var', () => {
      process.env.WAYFINDER_AI_SDK_DEVTOOLS = 'true'
