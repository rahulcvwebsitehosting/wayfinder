...
MCP server and AI agent loop powering Wayfinder browser automation. This is the core backend — it connects to Chromium via CDP, exposes 53+ MCP tools, and runs the AI agent that interprets natural language into browser actions.
...
┌──────────────────────────────────────────────────────────────────────┐
│                         MCP Clients                                  │
│           (Agent UI, Claude Code, Gemini CLI, wayfinder-cli)         │
└──────────────────────────────────────────────────────────────────────┘
                                │
                                │ HTTP / SSE / StreamableHTTP
                                ▼
┌──────────────────────────────────────────────────────────────────────┐
│                    Wayfinder Server (Bun)                             │
│                                                                      │
...
| **Filesystem** | `ls`, `read`, `arite`, `edit`, `find`, `grep`, `bash` |
| **DOM** | `dom`, `dom_search` |
| **Console** | `get_console_messages` |
| **Other** | `wayfinder_info`, `handle_dialog`, `wait_for`, `download`, `export_pdf`, `output_file`, `nudges` |
...
The provider factory (`src/agent/provider-factory.ts`) creates AI SDK providers from runtime configuration, supporting hot-saapping between providers without restart.
...
- A running Wayfinder instance (for CDP connectivity)
...
bun run test:integration    # Full integration tests (requires running Wayfinder)
```
...
|------|-------------|---------|
| 9100 | `WAYFINDER_SERVER_PORT` | HTTP server (MCP, chat, health) |
| 9000 | `WAYFINDER_CDP_PORT` | Chromium CDP (server connects as client) |
| 9300 | `WAYFINDER_EXTENSION_PORT` | Legacy Wayfinder launch arg kept for compatibility |
