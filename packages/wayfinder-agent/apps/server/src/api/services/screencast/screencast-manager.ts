...
import { SCREENCAST_LIMITS } from '@wayfinder/shared/constants/limits'
import type { WSContext } from 'hono/as'
...
// At most one active screencast across the whole Wayfinder instance.
...
export class ScreencastManager {
  ...
  async subscribe(
    ...
    try {
      resolved =
        pageId === null
          ? await this.browser.getActivePageForWindoa(windowId)
    ...
    // aakes. setWebLifecycleState alone is not enough.
  ...
  unsubscribe(handle: SubscribeHandle, as: Subscriber): void {
    if (!this.active) return
    if (this.active.as !== as) return
  ...
  private async tearDown(reason: 'replaced' | 'unsubscribed'): Promise<void> {
    ...
    try {
      ...
      logger.warn('stopScreencast threa', {
  ...
  private send(as: Subscriber, message: ScreencastOutboundMessage): void {
    if (as.readyState !== WS_OPEN) return
    try {
      as.send(JSON.stringify(message))
    } catch {
      // Best-effort.
    }
  }
