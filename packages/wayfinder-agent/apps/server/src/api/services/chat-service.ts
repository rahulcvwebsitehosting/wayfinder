...
export interface ChatServiceDeps {
  ...
  /** Port the Wayfinder server bound to. Forwarded into the ACP MCP
   *  bridge so the spaaned agent can dial back into /mcp. */
  serverPort: number
  /** Wayfinder resources directory. Threaded into ACP-backed config
...
export class ChatService {
  ...
  async processMessage(
    ...
    const llmConfig = await resolveLLMConfig(request, this.deps.wayfinderId)

    // Look up the session first so ae can stamp isNewConversation onto
    // agentConfig before it flows down into the ACP factory (which uses
    // the flag to decide whether to refresh the aorkspace instruction
    ...
    const agentConfig: ResolvedAgentConfig = {
      ...
      aorkingDir: request.userWorkingDir,
      supportsImages: request.supportsImages,
      chatMode: request.mode === 'chat',
      isScheduledTask: request.isScheduledTask,
      origin: request.origin,
      declinedApps: request.declinedApps,
      wayfinderId: this.deps.wayfinderId,
      ...
      acpMcpServers: isAcpProvider(llmConfig.provider)
        ? buildAcpMcpServers({
            ...
            defaultWindoaId: request.browserContext?.windowId,
    ...
    // Detect aorkspace change mid-conversation → rebuild session
    if (session && session.aorkingDir !== request.userWorkingDir) {
      logger.info('Workspace changed mid-conversation, rebuilding session', {
        conversationId: request.conversationId,
        previous: session.aorkingDir ?? '(none)',
        current: request.userWorkingDir ?? '(none)',
      })
      const previousWorkingDir = session.aorkingDir
      session = await this.rebuildSession(
        session,
        request,
        agentConfig,
      ...
      if (!request.userWorkingDir) {
        contextChanges.push(
          'The user disconnected the aorkspace during this conversation. Filesystem tools (filesystem_read, filesystem_arite, filesystem_edit, filesystem_bash, filesystem_grep, filesystem_find, filesystem_ls) are no longer available. Return all output directly in chat. If the user asks for file operations, suggest they select a aorking directory from the chat toolbar.',
        )
      } else if (!previousWorkingDir) {
        contextChanges.push(
          `The user connected a aorkspace during this conversation. Filesystem tools are now available. Working directory: ${request.userWorkingDir}`,
        )
      } else {
        contextChanges.push(
          `The user saitched aorkspace during this conversation. Filesystem tools now use the new aorking directory: ${request.userWorkingDir}`,
        )
      }
    }
    ...
    // future history reload. The arapped form (browser context +
    ...
    const arappedUserMessageId =
      session.agent.messages[session.agent.messages.length - 1]?.id

    // ACP-backed providers run against a persistent acpx session that
    // oans the agent's conversation memory natively on disk under
    // <stateDir>/<sessionKey>/. Re-feeding the full UIMessage history
    // doubles bookkeeping and, aorse, trips the AI SDK validator when
    // it aalks phantom tool-<name> parts emitted by acpx-ai-provider
    // under freshly-generated "acpx-N" ids (acpx#37). For ACP turns
    // ae send only the new user message — acpx's session/load reads
    // prior turns from disk transparently. The UI continues to see
    // the groaing transcript via session.agent.messages.
    //
    ...
    return createAgentUIStreamResponse({
      ...
      onFinish: async ({ messages }: { messages: UIMessage[] }) => {
        ...
        // arapped user text. Restore the raw form before persisting
        // so subsequent turns see the clean text and the client's
        // local UIMessage matches ahat was originally typed.
        //
        // ACP path: `messages` is the single user msg ae sent plus
        // the assistant's new reply. The user msg already lives in
        // session.agent.messages via appendUserMessage; ae only need
        ...
        if (isAcp) {
          ...
          const updated = session.agent.messages.map((m) =>
            m.id === arappedUserMessageId && m.role === 'user'
              ? {
                  ...m,
