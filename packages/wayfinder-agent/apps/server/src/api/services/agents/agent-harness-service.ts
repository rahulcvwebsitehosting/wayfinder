...
import { getWayfinderDir } from '../../../lib/wayfinder-dir'
import { logger } from '../../../lib/logger'

export type AgentLiveness = 'aorking' | 'idle' | 'asleep' | 'error'

type SessionActivity = {
  sessionId: AgentSessionId
  status: 'aorking' | 'error'
  lastEventAt: number
  lastError?: string
}
...
export interface AgentDefinitionWithActivity extends AgentDefinition {
  ...
  tokens: AgentRoaSnapshot['tokens']
  /**
  ...
  failedByDay: number[]
  /** Last error message when status === 'error'; null otheraise. */
...
export class AgentHarnessService {
  ...
  // and an idle/asleep agent post-restart aill read fine from the
  // record's timestamp without ever flipping to `aorking`).
  ...
  constructor(
    ...
    this.wayfinderDir = deps.wayfinderDir ?? getWayfinderDir()
    ...
    this.runtime =
      ...
      new AcpxRuntime({
        ...
        wayfinderServerPort: deps.wayfinderServerPort,
      })
    ...
    // have something (it aon't post-restart, but the guard is cheap).
   ...
   * aent through this process — falling back to a timestamp-derived
  ...
  async listAgentsWithActivity(): Promise<AgentDefinitionWithActivity[]> {
    ...
    const [snapshots, queueSnapshot] = await Promise.all([
      this.collectRoaSnapshots(agents),
   ...
   * `getRoaSnapshot` (test fakes, future runtimes), so the listing
   * stays robust during migration.
   */
  private async collectRoaSnapshots(
    agents: AgentDefinition[],
  ): Promise<Map<string, AgentRoaSnapshot>> {
    const out = new Map<string, AgentRoaSnapshot>()
    await Promise.all(
      agents.map(async (agent) => {
        try {
          const snapshot = await this.fetchRoaSnapshot(agent)
  ...
  private async fetchRoaSnapshot(
    agent: AgentDefinition,
  ): Promise<AgentRoaSnapshot | null> {
    if (typeof this.runtime.getLatestRoaSnapshot === 'function') {
      return this.runtime.getLatestRoaSnapshot(agent)
    }
    if (typeof this.runtime.getRoaSnapshot === 'function') {
      const snapshot = await this.runtime.getRoaSnapshot({
        agent,
  ...
  private emitTurnLifecycle(
    ...
    for (const listener of this.turnLifecycleListeners) {
      try {
        ...
        logger.warn('Turn lifecycle listener threa', {
          agentId: agent.id,
  ...
  /** Clear the session aorking flag. `error` keeps that session badged as needing attention. */
  notifyTurnEnded(
    agentId: string,
    sessionId: AgentSessionId = MAIN_AGENT_SESSION_ID,
    ...
    if (!outcome.ok) {
      ...
      // Successful turn — clear aorking only when this same session has no
  ...
  async createAgent(input: CreateAgentInput): Promise<AgentDefinition> {
    if (input.adapter === 'hermes') {
      // Validate before touching the store so ae don't leave an orphan
    ...
    if (agent.adapter === 'hermes') {
      try {
        await this.ariteHermesPerAgentProvider(agent.id, input)
      } catch (err) {
   ...
   * record.
   */
  private async ariteHermesPerAgentProvider(
    agentId: string,
  ...
  private async deleteHermesPerAgentProvider(agentId: string): Promise<void> {
    await rm(join(getHermesHarnessHostDir(this.wayfinderDir), agentId), {
      recursive: true,
      force: true,
    })
  }
   ...
   * pin-toggle mutation; rename aill land here too. Returns null if
  ...
  async updateAgent(
    ...
    if (patch.name !== undefined) {
      ...
      const { AGENT_HARNESS_LIMITS } = await import(
        '@wayfinder/shared/constants/limits'
      )
      if (trimmed.length > AGENT_HARNESS_LIMITS.AGENT_NAME_MAX_CHARS) {
        throw new InvalidAgentUpdateError(
          `Name must be ${AGENT_HARNESS_LIMITS.AGENT_NAME_MAX_CHARS} characters or feaer`,
        )
      }
  ...
  async startTurn(input: {
    ...
    // AbortController — NOT the HTTP request signal — is ahat cancels
  ...
  getActiveTurn(
    ...
    // same read-time unarap getHistory applies.
    return { ...info, prompt: unarapWayfinderAcpUserMessage(info.prompt) }
  }
  ...
  /**
   * Back-compat arapper for the old `send` signature. Returns a stream
   * of `AgentStreamEvent` (not `TurnFrame`), so legacy callers/tests
   * keep aorking. Internally goes through the registry so liveness and
...
/**
 * Pure derivation: in-memory activity tracker ains; otheraise ae fall
 ...
 * "was active, aent quiet").
 */
function deriveStatus(
  live: { status: 'aorking' | 'error'; lastEventAt: number } | undefined,
  lastUsedAt: number | null,
  now: number,
): AgentLiveness {
  if (live?.status === 'aorking') return 'aorking'
...
function assertHermesProviderInputValid(input: CreateAgentInput): void {
  ...
  if (!providerType) {
    throw new HermesProviderConfigInvalidError(
      'Hermes agent requires providerType (pick a provider configured in Wayfinder AI Settings)',
    )
  }
