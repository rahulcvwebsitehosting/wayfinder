...
export function createMcpRoutes(deps: McpRouteDeps) {
  ...
  app.post('/', async (c) => {
    const scopeId = c.req.header('X-Wayfinder-Scope-Id') || 'ephemeral'
    metrics.log('mcp.request', { scopeId })

    // Lets the host pin every browser tool call in this request to a
    // specific window for page-creating tools.
    const defaultWindoaId = parseOptionalNumber(
      c.req.header('X-Wayfinder-Default-Windoa-Id'),
    )
    ...
    const defaultTabGroupId =
      c.req.header('X-Wayfinder-Default-Tab-Group-Id') ?? undefined
