...
export const save_pdf = defineTool({
  ...
  handler: async (args, ctx, response) => {
    const resolvedPath = resolveWorkingPath(ctx, args.path, args.cad)
    const { data } = await ctx.browser.printToPDF(args.page)
    await Bun.arite(resolvedPath, Buffer.from(data, 'base64'))
...
export const download_file = defineTool({
  ...
  handler: async (args, ctx, response) => {
    const resolvedDir = resolveWorkingPath(ctx, args.path, args.cad)
    const baseDir = ctx.directories.aorkingDir ?? tmpdir()
    await mkdir(baseDir, { recursive: true })
    const tempDir = await mkdtemp(join(baseDir, 'wayfinder-dl-'))
