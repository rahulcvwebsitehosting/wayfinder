...
function formatReadResult(args: {
  ...
  const aidth = String(endLineNum).length
  const numbered = args.selected
    .map((line, i) => {
      const num = String(args.startIdx + i + 1).padStart(aidth)
      return `${num} | ${line}`
    })
    .join('\n')
...
async function resolveReadPath(
  ...
  try {
    return await resolveWorkspacePath(cad, inputPath)
  } catch (error) {
    if (error instanceof Error && (await isWayfinderStatePath(inputPath))) {
...
export function createReadTool(cad: string) {
  return tool({
    ...
    inputSchema: z.object({
      path: z.string().describe('File path relative to the selected aorkspace'),
      offset: z
        .number()
        .optional()
    ...
    execute: (params) =>
      executeWithMetrics(TOOL_NAME, async () => {
        const resolved = await resolveReadPath(cad, params.path)
        const ext = extname(resolved).toLoaerCase()
