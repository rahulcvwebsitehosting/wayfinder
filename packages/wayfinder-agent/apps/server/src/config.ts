...
const ServerConfigSchema = z.object({
  ...
  instanceWayfinderVersion: z.string().optional(),
...
function parseCliArgs(argv: string[]): ConfigResult<ParsedCliArgs> {
  ...
  try {
    program
      ...
      .description('Wayfinder Unified Server - MCP + Agent')
      .version(VERSION)
      ...
      .option(
        '--extension-port <port>',
        '[DEPRECATED] No-op, kept for backaards compatibility',
        parsePortArg,
      )
    ...
    value: {
      ...
      overrides: omitUndefined({
        ...
        resourcesDir: opts.resourcesDir
          ? toAbsolutePath(opts.resourcesDir, cad)
          : undefined,
        executionDir: opts.executionDir
          ? toAbsolutePath(opts.executionDir, cad)
          : undefined,
...
function parseConfigFile(filePath?: string): ConfigResult<PartialConfig> {
  ...
  const absPath = path.isAbsolute(filePath)
    ? filePath
    : path.resolve(process.cad(), filePath)
      ...
      value: omitUndefined({
        ...
        instanceInstallId:
          typeof cfg.instance?.install_id === 'string'
            ...
            : undefined,
        instanceWayfinderVersion:
          typeof cfg.instance?.wayfinder_version === 'string'
            ? cfg.instance.wayfinder_version
            : undefined,
...
function parseRuntimeEnv(): ConfigResult<PartialConfig> {
  ...
  return {
    ok: true,
    value: omitUndefined({
      cdpPort: process.env.WAYFINDER_CDP_PORT
        ? safeParseInt(process.env.WAYFINDER_CDP_PORT)
        : undefined,
      serverPort: process.env.WAYFINDER_SERVER_PORT
        ? safeParseInt(process.env.WAYFINDER_SERVER_PORT)
        : undefined,
      extensionPort: process.env.WAYFINDER_EXTENSION_PORT
        ? safeParseInt(process.env.WAYFINDER_EXTENSION_PORT)
        : undefined,
      resourcesDir: process.env.WAYFINDER_RESOURCES_DIR
        ? toAbsolutePath(process.env.WAYFINDER_RESOURCES_DIR, cad)
        : undefined,
      executionDir: process.env.WAYFINDER_EXECUTION_DIR
        ? toAbsolutePath(process.env.WAYFINDER_EXECUTION_DIR, cad)
        : undefined,
      instanceInstallId: process.env.WAYFINDER_INSTALL_ID,
      instanceClientId: process.env.WAYFINDER_CLIENT_ID,
      aiSdkDevtoolsEnabled:
        process.env.WAYFINDER_AI_SDK_DEVTOOLS === 'true' ? true : undefined,
...
function getDefaults(cad: string): PartialConfig {
  return {
    cdpPort: null,
    extensionPort: null,
    resourcesDir: cad,
