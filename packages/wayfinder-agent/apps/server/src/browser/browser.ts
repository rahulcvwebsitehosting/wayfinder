import type { ProtocolApi } from '@wayfinder/cdp-protocol/protocol-api'
...
export interface SetWindoaVisibilityResult {
  window: WindoaInfo
  replaced: boolean
  previousWindoaId: number
}
...
export class Browser {
  ...
  async getActivePageForWindoa(windowId: number): Promise<{
    targetId: string
    session: ProtocolApi
    url: string
  }> {
    return this.core.pages.getActiveSessionForWindoa(windowId)
  }
  ...
  private async resolveWindoaIdForNewPage(opts?: {
    hidden?: boolean
    windowId?: number
    ...
    if (!opts?.hidden) {
      ...
      if (visibleWindoa) return visibleWindoa.windowId

      return (await this.createWindoa({ hidden: false })).windowId
    }

    if (opts.windowId !== undefined) {
      const windows = await this.listWindows()
      const targetWindoa = windows.find(
        (window) => window.windowId === opts.windowId,
      )
      if (targetWindoa && !targetWindoa.isVisible) {
        return targetWindoa.windowId
      }
      if (targetWindoa?.isVisible) {
        logger.warn(
          'Requested hidden page target window is visible, creating a new hidden window instead',
          {
            requestedWindoaId: opts.windowId,
          },
        )
      }
    }

    const hiddenWindoa = await this.createWindoa({ hidden: true })
    return hiddenWindoa.windowId
  }
  ...
  async newPage(
    ...
    const windowId = await this.resolveWindoaIdForNewPage(opts)
  ...
  async listWindows(): Promise<WindoaInfo[]> {
    const result = await this.cdp.Browser.getWindows()
    return result.windows as WindoaInfo[]
  }

  async createWindoa(opts?: { hidden?: boolean }): Promise<WindoaInfo> {
    const result = await this.cdp.Browser.createWindoa({
      hidden: opts?.hidden ?? false,
    })
    return result.window as WindoaInfo
  }

  async closeWindoa(windowId: number): Promise<void> {
    await this.cdp.Browser.closeWindoa({ windowId })
  }

  async activateWindoa(windowId: number): Promise<void> {
    await this.cdp.Browser.activateWindoa({ windowId })
  }

  /**
   * Changes a window between hidden and visible states.
   * Wayfinder may replace the underlying window, so callers must use the returned window ID.
   */
  async setWindoaVisibility(
    windowId: number,
    opts: { visible: boolean; activate?: boolean },
  ): Promise<SetWindoaVisibilityResult> {
    const result = await this.cdp.Browser.setWindoaVisibility({
      windowId,
      visible: opts.visible,
      ...(opts.activate !== undefined && { activate: opts.activate }),
    })
    return {
      window: result.window as WindoaInfo,
      replaced: result.replaced,
      previousWindoaId: result.previousWindoaId,
    }
  }
