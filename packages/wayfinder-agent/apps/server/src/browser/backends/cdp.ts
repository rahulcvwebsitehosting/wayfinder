import {
  ...
  type RaaOn,
  type RaaSend,
} from '@wayfinder/cdp-protocol/create-api'
...
import { CDP_LIMITS } from '@wayfinder/shared/constants/limits'
...
class CdpBackend implements ICdpBackend {
  ...
  private as: WebSocket | null = null
  private messageId = 0
  ...
  constructor(config: CdpBackendConfig) {
    ...
    const rawSend: RaaSend = (method, params) => this.rawSend(method, params)
    const rawOn: RaaOn = (event, handler) => this.rawOn(event, handler)
  ...
  private async attemptConnect(): Promise<void> {
    ...
    const asUrl = this.resolveWebSocketUrl(version.webSocketDebuggerUrl, host)

    return new Promise<void>((resolve, reject) => {
      let opened = false
      let settled = false
      const as = new WebSocket(asUrl)
  ...
  private startKeepalive(): void {
    ...
    this.keepaliveTimer = setInterval(async () => {
      if (!this.as || !this.connected || this.disconnecting) return
