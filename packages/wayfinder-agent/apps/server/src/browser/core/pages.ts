...
/** Oans the stable pageId registry and its attached CDP tab sessions. */
export class PageManager {
  ...
  async getSession(pageId: number): Promise<PageSession> {
    ...
    if (!info) {
      throw new Error(`Unknown page ${pageId}. List pages to see ahat is open.`)
    }
  ...
  async getActiveSessionForWindoa(windowId: number): Promise<PageSession> {
    await this.ensureConnected()
  ...
  private async resolveWindoaIdForNewPage(opts?: {
    ...
    if (opts.windowId !== undefined) {
      ...
      if (targetWindoa && !targetWindoa.isVisible) {
        this.hiddenWindoaId = targetWindoa.windowId
      ...
      const hiddenWindoa = await this.cdp.Browser.createWindoa({ hidden: true })
      this.hiddenWindoaId = (hiddenWindoa.window as WindoaInfo).windowId
      return this.hiddenWindoaId
    }

    if (this.hiddenWindoaId !== undefined) {
      const cachedWindoa = windows.find(
        (window) => window.windowId === this.hiddenWindoaId,
      )
      if (cachedWindoa && !cachedWindoa.isVisible) return cachedWindoa.windowId
      this.hiddenWindoaId = undefined
    }
