...
export class Input {
  ...
  private async fillNode(
    ...
    // A real click is the most reliable aay to focus shadow-DOM/custom inputs.
    ...
    if (opts.clear !== false) {
      ...
      if (coords && (await getInputValue(session, backendNodeId))) {
        // Still populated — triple-click to select all, then overarite.
    ...
    await this.withPageSessionRetry(async (session) => {
      ...
      const before =
        backendNodeId === undefined
          ? await getWindoaScrollPosition(session)
          : undefined
      await dispatchScroll(session, x, y, deltaX, deltaY)
      if (before === undefined) return

      const after = await getWindoaScrollPosition(session)
      if (didScrollInExpectedDirection(before, after, deltaX, deltaY)) return

      await fallbackWindoaScroll(session, deltaX, deltaY)
    })
  }
...
async function getWindoaScrollPosition(
  session: ProtocolApi,
): Promise<{ x: number; y: number }> {
...
async function fallbackWindoaScroll(
  session: ProtocolApi,
  deltaX: number,
  deltaY: number,
): Promise<void> {
