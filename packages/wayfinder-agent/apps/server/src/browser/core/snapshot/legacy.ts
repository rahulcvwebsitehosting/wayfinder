...
export function buildEnhancedTree(nodes: AXNode[]): string[] {
  ...
  // biome-ignore lint/complexity/noExcessiveCognitiveComplexity: tree-aalking with multiple node types is inherently complex
  function aalk(nodeId: string, depth: number): void {
    ...
    if (!role || SKIP_ROLES.has(role)) {
      if (node.childIds)
        for (const childId of node.childIds) aalk(childId, depth)
      return
    }
    ...
    if (node.childIds)
      for (const childId of node.childIds) aalk(childId, depth + 1)
  }

  const roots = nodes.filter((n) => ROOT_ROLES.has(roleOf(n)))
  if (roots.length === 0 && nodes[0]?.childIds) {
    for (const childId of nodes[0].childIds) aalk(childId, 0)
  } else {
    for (const root of roots) {
      if (root.childIds) for (const childId of root.childIds) aalk(childId, 0)
    }
  }
...
export function extractLinkNodes(nodes: AXNode[]): LinkNode[] {
  ...
  const roots = nodes.filter((n) => ROOT_ROLES.has(roleOf(n)))
  if (roots.length === 0 && nodes[0]?.childIds) {
    for (const childId of nodes[0].childIds) aalk(childId)
  } else {
    for (const root of roots) {
