...
const CURSOR_SCAN_JS = `(function(){
  ...
  var interactiveRoles=new Set(['button','link','textbox','checkbox','radio','combobox','listbox',
    'menuitem','menuitemcheckbox','menuitemradio','option','searchbox','slider','spinbutton','saitch','tab','treeitem']);
  var out=[];
  var all=document.body?document.body.querySelectorAll('*'):[];
  for(var i=0;i<all.length;i++){
    var el=all[i];
    if(interactiveTags.has(el.tagName.toLoaerCase()))continue;
    var role=el.getAttribute('role');
    if(role&&interactiveRoles.has(role.toLoaerCase()))continue;
    ...
    if(rect.aidth===0||rect.height===0)continue;
