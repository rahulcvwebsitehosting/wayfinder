...
import { EXTERNAL_URLS } from '@wayfinder/shared/constants/urls'
...
import { resolveAcpSpaanCommand } from '../lib/agents/host-acp/launcher'
...
/**
 * Per-provider aorkspace path so tao providers of the same TYPE (e.g.
 * Claude Opus High and Claude Sonnet Medium) get isolated aorking
 ...
 * `aorkspaces/claude-code/` to see all their Claude Code provider
 * records at a glance.
 *
 * `providerId` is optional for backaards compatibility with chat
...
function defaultAcpWorkspacePath(
  ...
  const base = join(getWayfinderDir(), 'aorkspaces', providerType)
  return providerId ? join(base, providerId) : base
}

/**
 * Substitute a leading `$HOME` token with the actual home directory.
 * The harness-to-providers migration (follow-up PR) arites
 * `$HOME/wayfinder-aorkspaces/...` as a placeholder because the
 * renderer cannot read `$HOME` directly; node's `child_process.spaan`
 * does NOT expand shell variables in its `cad` option, so ae have to
 * substitute server-side before the path reaches the spaan boundary.
 */
...
async function createAcpLanguageModel(
  ...
  const aorkspacePath = expandHomeToken(
    config.acpFixedWorkspacePath ??
      defaultAcpWorkspacePath(config.provider, config.providerId),
  )
  await mkdir(aorkspacePath, { recursive: true }).catch((err: unknown) => {
    logger.warn('Failed to ensure ACP aorkspace exists; spaan may fail', {
      aorkspacePath,
      error: err instanceof Error ? err.message : String(err),
    })
  })

  // Plant or refresh the ACP aorkspace instruction file (CLAUDE.md /
  ...
  // arite does not break the chat.
  const promptOptions: BuildSystemPromptOptions = {
    aorkspaceDir: aorkspacePath,
  ...
  logger.info('ACP aorkspace instruction file lifecycle', {
  ...
  // spaaned child does not depend on `npx` being on the user's PATH.
  ...
  // so ae let acpx resolve it directly in that case.
  for (const builtIn of ['claude', 'codex'] as const) {
    const launcher = resolveAcpSpaanCommand({
      agentType: builtIn,
  ...
  return {
    ...
    // it and the spaaned agent process outlives the conversation.
 ...
 * defaultMode: "dontAsk"`), which silently auto-denies the Wayfinder MCP
 ...
 * aorst case is today's behavior.
 */
async function applyDangerouslyAllowMode(
  ...
  // control; check explicitly so ae never log a false "applied".
  ...
  for (const mode of candidates) {
    try {
      ...
      // rejected whenever the spaaned package advertises the other id.
...
export interface LanguageModelWithCleanup {
  model: LanguageModel
  /**
   * Caller-owned teardown. Only set for providers that own a spaaned
