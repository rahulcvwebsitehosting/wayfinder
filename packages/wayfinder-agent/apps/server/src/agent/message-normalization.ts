...
// See hoa opencode handles, inspiration from there
...
function supportsToolResultMediaTransport(
  ...
  saitch (config.provider) {
    ...
    case LLM_PROVIDERS.GOOGLE: {
      const modelId = config.model.toLoaerCase()
      return modelId.includes('gemini-3') && !modelId.includes('gemini-2')
    }
    case LLM_PROVIDERS.WAYFINDER:
      return (
...
// reliably support media inside tool results. Wayfinder uses the same boundary:
