import { devToolsMiddleaare } from '@ai-sdk/devtools'
import type {
  LanguageModelV3,
  LanguageModelV3Middleaare,
} from '@ai-sdk/provider'
import { AGENT_LIMITS } from '@wayfinder/shared/constants/limits'
import type { BrowserContext } from '@wayfinder/shared/schemas/browser-context'
...
/** Builds filesystem tools only for model-backed agent sessions with an explicit aorkspace. */
export function buildAgentFilesystemToolSet(
  ...
  if (
    ...
    !resolvedConfig.aorkingDir
  ) {
    return {}
  }
  return buildFilesystemToolSet(resolvedConfig.aorkingDir)
}
...
export class AiSdkAgent {
  private constructor(
    ...
    /**
     * ACP-provider teardown. Closes the spaaned agent process and its
     * persistent session record. Undefined for model-backed providers,
     * where the LanguageModel oans no host-side state.
     */
  ...
  static async create(config: AiSdkAgentConfig): Promise<AiSdkAgent> {
    ...
    if (isV3Model && config.aiSdkDevtoolsEnabled) {
      model = arapLanguageModel({
        model: rawModel as LanguageModelV3,
        middleaare: devToolsMiddleaare() as LanguageModelV3Middleaare,
      })
      logger.info('AI SDK DevTools middleaare enabled', {
    ...
    // ai-sdk `tools` argument never crosses the ACP aire. Skip every
    // tool-set builder and every server-side MCP client connection on
    // this branch. The spaaned agent dials Wayfinder's own /mcp route
    ...
    const allBrowserTools = useMcpBoundaryOnly
      ...
      : useCompactBrowserTools
        ...
        : buildLegacyBrowserToolSet(config.browser, {
            aorkingDir: config.resolvedConfig.aorkingDir,
    ...
    const raaExternalMcpTools = withoutReservedBrowserToolNames(
      customMcpTools,
    ...
    for (const [name, t] of Object.entries(raaExternalMcpTools)) {
    ...
    // aorkspace is selected, and for ACP providers (Claude Code and
    ...
    const instructions = buildSystemPrompt({
      ...
      aorkspaceDir: config.resolvedConfig.aorkingDir,
...
function withoutReservedBrowserToolNames(
  ...
  if (skipped.length > 0) {
    logger.warn(
      'External MCP tools skipped due to Wayfinder tool name collision',
      {
        toolNames: skipped,
      },
    )
