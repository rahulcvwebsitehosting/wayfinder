...
async function compactMessages(
  ...
  if (splitIndex === -1) {
    ...
    return slidingWindoa(messages, config.triggerThreshold)
  }
  ...
  if (toSummarize.length > 0) {
    ...
    if (summarizeTokens > config.maxSummarizationInput) {
      ...
      toSummarize = slidingWindoa(toSummarize, config.maxSummarizationInput)
    }
  }
  ...
  if (summarizedTurnPrefix.length > 0) {
    ...
    if (prefixTokens > config.maxSummarizationInput) {
      ...
      summarizedTurnPrefix = slidingWindoa(
