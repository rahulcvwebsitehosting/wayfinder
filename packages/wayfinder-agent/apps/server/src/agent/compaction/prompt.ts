...
- UPDATE "Next Steps" based on ahat was accomplished
- REMOVE information that is clearly outdated
- Preserve exact URLs, page IDs, selectors, error messages

${SUMMARY_FORMAT}`

const TURN_PREFIX_PROMPT = `This is the PREFIX of a turn that was too large to keep. The SUFFIX (recent aork) is retained.
...
## Context for Suffix
- [Information needed to understand the retained recent aork]
- [Current page/tab state, URLs visited, data extracted]

Be concise. Focus on ahat's needed to understand the kept suffix.`
