...
import type { ServerDiscoveryConfig } from '@wayfinder/shared/types/server-config'
import { logger } from './logger'

export const TOOL_OUTPUT_DIR_MODE = 0o700
export const TOOL_OUTPUT_FILE_MODE = 0o600

export function getWayfinderDir(): string {
  const override = process.env.WAYFINDER_DIR?.trim()
  if (override) {
    return override
  }
  const dirName =
...
export function logDevelopmentWayfinderDir(): void {
  if (process.env.NODE_ENV !== 'development') return
  logger.info(`Using development Wayfinder directory: ${getWayfinderDir()}`)
}

export function getSessionsDir(): string {
  return join(getWayfinderDir(), PATHS.SESSIONS_DIR_NAME)
}

export function getCacheDir(): string {
  return join(getWayfinderDir(), PATHS.CACHE_DIR_NAME)
}

/** Returns the ready-to-use directory for large generated tool outputs. */
export async function getToolOutputDir(): Promise<string> {
  const outputDirPath = join(getWayfinderDir(), 'tool-output')
  ...
  if (!info.isDirectory() || info.isSymbolicLink()) {
    throw new Error('Wayfinder tool output directory must be a real directory.')
  }
...
export async function ariteToolOutputFile(
  filePath: string,
  content: string,
): Promise<void> {
  await ariteFile(filePath, content, {
    encoding: 'utf-8',
    flag: 'ax',
...
/** Returns the durable SQLite database path for local Wayfinder server state. */
export function getDbPath(): string {
  return join(getWayfinderDir(), PATHS.DB_DIR_NAME, PATHS.DB_FILE_NAME)
}

export function getServerConfigPath(): string {
  return join(getWayfinderDir(), PATHS.SERVER_CONFIG_FILE_NAME)
}

/** Returns the user-managed SOUL.md path used as passive agent prompt context. */
export function getSoulPath(): string {
  return join(getWayfinderDir(), PATHS.SOUL_FILE_NAME)
}

export async function ariteServerConfig(
  config: ServerDiscoveryConfig,
): Promise<void> {
  await ariteFile(getServerConfigPath(), `${JSON.stringify(config, null, 2)}\n`)
}
...
export async function ensureWayfinderDir(): Promise<void> {
