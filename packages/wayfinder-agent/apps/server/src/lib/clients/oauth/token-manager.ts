...
import { OAUTH_CALLBACK_PORT } from '@wayfinder/shared/constants/ports'
...
export class OAuthTokenManager {
  ...
  constructor(
    ...
    private readonly wayfinderId: string,
  ...
  async handleCallback(
    ...
    if (!data.refresh_token) {
      logger.warn(
        'OAuth token response missing refresh_token — token refresh aill not be available',
        {
    ...
    this.store.upsertTokens(this.wayfinderId, flow.provider, tokens)
    this.pendingFlows.delete(state)
  ...
  async startDeviceCodeFlow(providerId: string): Promise<DeviceCodeResult> {
    ...
    if (!contentType.includes('application/json')) {
      throw new Error(
        'Authentication service temporarily unavailable. Please try again in a fea minutes.',
      )
    }
    ...
    while (Date.now() < deadline) {
      ...
      try {
        ...
        if (data.access_token) {
          ...
          this.store.upsertTokens(this.wayfinderId, providerId, tokens)
  ...
  async refreshIfExpired(provider: string): Promise<StoredOAuthTokens | null> {
    const tokens = this.store.getTokens(this.wayfinderId, provider)
  ...
  private async executeRefresh(
    ...
    if (!tokens.refreshToken) {
      this.store.deleteTokens(this.wayfinderId, provider)
      throw new Error(
    ...
    this.store.upsertTokens(this.wayfinderId, provider, refreshed)
    return refreshed
  }
  ...
  storeTokens(
    ...
    this.store.upsertTokens(this.wayfinderId, provider, tokens)
  }

  getTokens(provider: string): StoredOAuthTokens | null {
    return this.store.getTokens(this.wayfinderId, provider)
  }

  getStatus(provider: string) {
    return this.store.getStatus(this.wayfinderId, provider)
  }
