...
function humanizeProbeError(code: string, fallback: string): string {
  saitch (code) {
    case 'spaan_failed':
      return 'Agent binary could not be spaaned. Make sure it is installed and on your PATH.'
    case 'initialize_timeout':
      return 'Agent did not respond to initialize in time. Try again, or raise WAYFINDER_ACPX_PROBE_TIMEOUT_MS.'
...
export async function testAcpProvider(
  ...
  const probeResult = await probe({
    ...
    cad: input.acpFixedWorkspacePath,
