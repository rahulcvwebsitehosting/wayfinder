 ...
 * extension's existing settings layout. The aire format `acpx-ai-provider`
 * wants is a flat `Record<string, string>` for env / headers, so ae
...
export interface BuildAcpxProviderOptions {
  ...
  /**
   * MCP servers to expose to the spaaned agent. The list is global to
   ...
   * `undefined`. Wayfinder ships ACP agents in `approve-all` so read +
   * arite tools run end-to-end without a UI gate; callers that want a
   ...
   * subdirectory under `getWayfinderDir()` so it sits next to the rest
   * of the Wayfinder state and gets cleaned up with the install.
   */
  stateDir?: string
}
...
function defaultStateDir(): string {
  return join(getWayfinderDir(), 'acpx-state')
}
 ...
 * `provider-factory.ts`. Lifecycle (spaan child, open ACP session,
...
export async function buildAcpxProvider(
  ...
  return createAcpxProvider({
    agent: opts.agentId,
    cad: opts.aorkspacePath ?? homedir(),
