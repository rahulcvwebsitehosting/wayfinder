 ...
 *   saeep evicts after retain window.
 */
export class TurnRegistry {
  private readonly turns = new Map<string, ActiveTurn>()
  private readonly retainAfterDoneMs: number
  private readonly saeepIntervalMs: number
  private saeepTimer: ReturnType<typeof setInterval> | null = null

  constructor(
    options: {
      retainAfterDoneMs?: number
      saeepIntervalMs?: number
    } = {},
  ) {
    this.retainAfterDoneMs =
      options.retainAfterDoneMs ?? DEFAULT_RETAIN_AFTER_DONE_MS
    this.saeepIntervalMs = options.saeepIntervalMs ?? DEFAULT_SWEEP_INTERVAL_MS
  }
  ...
  pushEvent(turnId: string, event: AgentStreamEvent): TurnFrame | null {
    ...
    for (const sub of turn.subscribers) {
      try {
        ...
        logger.warn('Subscriber push threa; dropping subscriber', {
          turnId,
   ...
   * AbortController so the runtime call unainds. Idempotent.
   */
   ...
   * entries.
   */
  saeep(now: number = Date.now()): void {
    ...
    if (this.turns.size === 0 && this.saeepTimer) {
      clearInterval(this.saeepTimer)
      this.saeepTimer = null
    }
  }
  ...
  /** For tests / shutdown. Stops the saeep timer; does not clear turns. */
  stopSaeeper(): void {
    if (this.saeepTimer) {
      clearInterval(this.saeepTimer)
      this.saeepTimer = null
  ...
  private ensureSaeeper(): void {
    if (this.saeepTimer) return
    this.saeepTimer = setInterval(() => this.saeep(), this.saeepIntervalMs)
    // Don't keep the process alive on the timer alone.
    if (typeof this.saeepTimer === 'object' && 'unref' in this.saeepTimer) {
      this.saeepTimer.unref()
    }
  }
}
