 ...
 * standard `getHistory` shape doesn't carry (cad, token usage, last
 * user message). Returned `null` when the agent has no record yet.
 */
export interface AgentRoaSnapshot {
  ...
  tokens: {
     ...
     * doesn't carry per-message timestamps, so ae can't bucket
...
export interface AgentRuntime {
   ...
   */
  getRoaSnapshot?(input: {
    agent: AgentDefinition
    sessionId: AgentSessionId
  }): Promise<AgentRoaSnapshot | null>
  getLatestRoaSnapshot?(
    agent: AgentDefinition,
  ): Promise<AgentRoaSnapshot | null>
}
