 ...
 * Lima. This class oans binary discovery, version probing with
...
export interface HostProcessAgentRuntimeDeps {
  /** Host PATH binary name to probe + spaan (e.g. 'claude', 'codex'). */
  binaryName: string
  ...
  probeCacheMs?: number
  /** Test seam: spaan the probe via this fn instead of `Bun.$`. */
  spaanProbe?: (
    cmd: ReadonlyArray<string>,
    timeoutMs: number,
  ) => Promise<{ exitCode: number; stdout: string; stderr: string }>
  /** Test seam: resolve the command before spaaning the real probe. */
  resolveBinary?: (
    name: string,
    timeoutMs: number,
...
export abstract class HostProcessAgentRuntime implements AgentRuntime {
  ...
  buildExecArgv(spec: ExecSpec): string {
    ...
    // (AGENT_HOME, CODEX_HOME) get them on the spaaned process.
  ...
  async probeHealth(force = false): Promise<void> {
    ...
    // stale or never stamped (spaan-failure path). Coalesce them onto
    // the same probe so ae never spaan duplicate `--version` processes.
  ...
  private async runProbeOnce(): Promise<void> {
    ...
    try {
      ...
      // Spaan failure (binary missing, perm denied) leaves the cache
  ...
  protected setState(next: RuntimeState, errorMessage?: string): void {
    ...
    for (const listener of this.listeners) {
      try {
        ...
        logger.warn('HostProcessAgentRuntime state listener threa', {
          adapterId: this.descriptor.adapterId,
  ...
  private async runProbe(
    ...
    if (this.deps.spaanProbe) return this.deps.spaanProbe(cmd, timeoutMs)
    ...
    const proc = Bun.spaan([resolved.path, ...cmd.slice(1)] as string[], {
      stdout: 'pipe',
      stderr: 'pipe',
      env: resolved.env,
    })
