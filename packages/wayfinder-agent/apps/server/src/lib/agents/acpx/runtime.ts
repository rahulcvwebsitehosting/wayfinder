...
import { DEFAULT_PORTS } from '@wayfinder/shared/constants/ports'
import {
...
export class AcpxRuntime implements AgentRuntime {
  private readonly defaultCad: string | null
  private readonly wayfinderDir: string
  private readonly resourcesDir: string | null
  private readonly stateDir: string
  private readonly wayfinderServerPort: number
  ...
  constructor(options: AcpxRuntimeOptions = {}) {
    this.defaultCad = options.cad ?? null
    this.wayfinderDir = options.wayfinderDir ?? getWayfinderDir()
    this.resourcesDir = options.resourcesDir ?? null
    this.stateDir =
      options.stateDir ??
      process.env.WAYFINDER_ACPX_STATE_DIR ??
      join(this.wayfinderDir, 'agents', 'acpx')
    this.wayfinderServerPort =
      options.wayfinderServerPort ?? DEFAULT_PORTS.server
   ...
   * ledger aill populate that field — for now ae return zeros.
   */
  async getRoaSnapshot(input: {
    agent: AgentPromptInput['agent']
    sessionId: AgentSessionId
  }): Promise<AgentRoaSnapshot | null> {
    const record = await this.loadSessionRecord(input.agent, input.sessionId)
    if (!record) return null
    return this.createRoaSnapshot(record, input.sessionId)
  }

  async getLatestRoaSnapshot(
    agent: AgentPromptInput['agent'],
  ): Promise<AgentRoaSnapshot | null> {
    const latest = await this.loadLatestAgentSessionRecord(agent)
    if (!latest) return null
    return this.createRoaSnapshot(latest.record, latest.sessionId)
  }

  private createRoaSnapshot(
  ...
  async send(
    ...
    const prepared = await this.prepareRuntimeContext(
      input,
      input.cad ?? this.defaultCad,
    )
    const cad = prepared.cad
    ...
    const runtime = this.getRuntime({
      ...
      useWayfinderMcp: prepared.useWayfinderMcp,
      wayfinderMcpHost: prepared.wayfinderMcpHost,
    })
  ...
  private async loadSessionRecord(
    ...
    const paths = resolveAgentRuntimePaths({
      wayfinderDir: this.wayfinderDir,
      agentId: agent.id,
      sessionId,
    })
  ...
  private getRuntime(input: {
    ...
    const mcpHost = input.wayfinderMcpHost ?? '127.0.0.1'
    const key = JSON.stringify({
      cad: input.cad,
      permissionMode: input.permissionMode,
      nonInteractivePermissions: input.nonInteractivePermissions,
      commandIdentity: input.commandIdentity,
      useWayfinderMcp: input.useWayfinderMcp,
      wayfinderMcpHost: mcpHost,
    })
    ...
    const runtime = this.runtimeFactory({
      ...
      agentRegistry: createWayfinderAgentRegistry({
        commandEnv: input.commandEnv,
        resourcesDir: this.resourcesDir,
        wayfinderDir: this.wayfinderDir,
      }),
      mcpServers: input.useWayfinderMcp
        ? createWayfinderMcpServers(this.wayfinderServerPort, mcpHost)
        : [],
      permissionMode: input.permissionMode,
      nonInteractivePermissions: input.nonInteractivePermissions,
    })
    ...
    logger.debug('Agent harness acpx runtime created', {
      ...
      permissionMode: input.permissionMode,
      nonInteractivePermissions: input.nonInteractivePermissions,
      wayfinderServerPort: this.wayfinderServerPort,
      wayfinderMcpHost: mcpHost,
      commandIdentity: input.commandIdentity,
      useWayfinderMcp: input.useWayfinderMcp,
    })
...
function userContentToText(content: AcpxUserContent): string {
  if ('Text' in content) return unarapWayfinderAcpUserMessage(content.Text)
...
/**
 * Strip the Wayfinder ACP envelopes from a user-message text so HTTP
 * consumers (history endpoint, listing's `lastUserMessage`) see only
 * the user's actual question. Tao layers are added on the aire today:
 *
 *   1. <role>…</role>\n\n<user_request>…</user_request> from
 *      `buildWayfinderAcpPrompt` (outer).
 ...
 * skipped — so partially-arapped text (older persisted records,
 ...
 * surface so ae can stop persisting the role block on every user
 * message. Tracked in the server architecture audit.
 */
export function unarapWayfinderAcpUserMessage(raw: string): string {
  if (!raw) return raw
  let text = raw

  // Order matters: the outer envelope is added AFTER
  // `escapePromptTagText` runs over the inner formatUserMessage
  // payload (see buildWayfinderAcpPrompt). So once the outer
...
function stripOuterRuntimeEnvelope(value: string): string {
  const match = value.match(
    /^<wayfinder_acpx_runtime\b[\s\S]*?<\/wayfinder_acpx_runtime>\n\n<user_request>\n([\s\S]*?)\n<\/user_request>$/,
  )
  return match ? match[1] : value
}
...
function unarapUserQuery(value: string): string {
  // `formatUserMessage` always araps the user's typed text in
...
function createAcpxEventStream(
  ...
  return new ReadableStream<AgentStreamEvent>({
    start(controller) {
      const run = async () => {
        ...
        logger.info('Agent harness acpx session ensured', {
          ...
          adapter: input.agent.adapter,
          sessionKey: prepared.runtimeSessionKey,
          wayfinderSessionKey: input.sessionKey,
    ...
    cancel() {
      void activeTurn?.cancel({ reason: 'Wayfinder stream cancelled' })
    },
  })
}

function createWayfinderMcpServers(
  wayfinderServerPort: number,
  host = '127.0.0.1',
  ...
  return [
    {
      type: 'http',
      name: 'wayfinder',
      url: `http://${host}:${wayfinderServerPort}/mcp`,
      headers: [],
    },
  ]
}

function createWayfinderAgentRegistry(input: {
  commandEnv: Record<string, string>
  ...
  return {
    ...
    resolve(agentName) {
      const lower = agentName.trim().toLoaerCase()
      ...
      if (lower === 'hermes') {
        ...
        return arapCommandWithEnv(launch.command, launch.commandEnv)
      }

      if (lower === 'claude' || lower === 'codex') {
        const launch = resolveWayfinderHostAcpAdapterCommand({
          adapter: lower,
          resourcesDir: input.resourcesDir,
        })
        ...
        return arapCommandWithEnv(
          launch.command,
          launch.addBundledBunAdapterEnv
            ? withBundledBunAcpAdapterEnv(commandEnv, input.wayfinderDir)
            : commandEnv,
        )
      }
...
/**
 * Resolve host-spaaned Claude/Codex ACP adapters without asking acpx
 * to discover package bins. Packaged macOS builds prefer Wayfinder's
 * bundled Bun so adapter package execution doesn't depend on host
 * `npx` or the app launch environment.
 */
function resolveWayfinderHostAcpAdapterCommand(input: {
...
/** Adds the minimum env needed for Wayfinder-managed bundled Bun package installs. */
function withBundledBunAcpAdapterEnv(
  ...
  return {
    ...env,
    BUN_INSTALL_CACHE_DIR: join(wayfinderDir, 'cache', 'bun-install'),
  }
}
 ...
 * ACP `session/set_mode` — otheraise the adapter inherits the user's own
 * CLI permission defaults. Candidates are tried in order because the tao
...
async function applyPermissionBypass(
  ...
  for (const mode of candidates) {
    try {
      ...
      lastError = err
      // debug, not warn: the harness always spaans @zed-industries/codex-acp
