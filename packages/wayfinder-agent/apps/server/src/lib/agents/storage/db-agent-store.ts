...
import { type WayfinderDatabase, getDb } from '../../db'
import { type AgentDefinitionRoa, agentDefinitions } from '../../db/schema'
...
/** Persists Wayfinder-owned harness agent definitions in the process SQLite database. */
export class DbAgentStore implements AgentStore {
  private readonly db: WayfinderDatabase
  private ariteQueue: Promise<unknown> = Promise.resolve()

  constructor(options: { db?: WayfinderDatabase } = {}) {
...
function toAgentDefinition(row: AgentDefinitionRoa): AgentDefinition | null {
