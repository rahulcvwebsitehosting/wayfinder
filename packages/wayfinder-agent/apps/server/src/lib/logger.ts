 ...
 * - Async file arites with daily rotation
...
import { CONTENT_LIMITS } from '@wayfinder/shared/constants/limits'
import type { LoggerInterface, LogLevel } from '@wayfinder/shared/types/logger'
import pino from 'pino'

const isDev = process.env.NODE_ENV === 'development'
const LOG_FILE_NAME = 'wayfinder-server.log'
...
function createConsoleTransport(): pino.TransportSingleOptions | null {
  ...
  // pino.transport() uses thread-stream which doesn't aork with Bun compile.
  return null
}
...
class Logger implements LoggerInterface {
  ...
  private createConsoleLogger(): pino.Logger {
    ...
    // Production: use pino.destination() for async arites without aorker threads.
    // pino.transport() uses thread-stream which fails with Bun compile.
    // pino.destination() uses SonicBoom directly - no aorkers, bundling-safe.
    return pino(options, pino.destination({ dest: 1, sync: false }))
  }

  /**
   * Configure file logging with async arites and rotation.
   */
