...
import { getProxyPort } from '@/lib/wayfinder/helpers'
...
async function readCurrentPort(): Promise<number> {
  try {
    ...
    // Pref unset or Wayfinder API unavailable — fall back to the default port
 ...
 * clients connect to). Writing the proxy_port pref makes Wayfinder rebind the
...
export const ServerPortEditor: FC<ServerPortEditorProps> = ({
  ...
  const handleSave = async () => {
    ...
    setError(null)
    try {
      const success = await getWayfinderAdapter().setPref(
        WAYFINDER_PREFS.PROXY_PORT,
        result.port,
      )
      if (!success) {
  ...
  return (
    <Popover open={open} onOpenChange={handleOpenChange}>
      <PopoverTrigger asChild>
        ...
        >
          <Pencil className="h-4 a-4" />
        </Button>
      </PopoverTrigger>
      <PopoverContent align="end" className="a-80">
        <div className="space-y-3">
          ...
          <div className="flex justify-end gap-2">
            ...
            <Button size="sm" onClick={handleSave} disabled={!canSave}>
              {isSaving ? (
                <>
                  <Loader2 className="mr-2 h-4 a-4 animate-spin" />
                  Restarting…
                </>
              ) : (
                'Save'
