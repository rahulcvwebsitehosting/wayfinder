    ...
    <section className="space-y-4">
      ...
      <div className="flex items-end justify-between gap-4">
        <div className="space-y-1">
          ...
          <p className="text-muted-foreground text-sm">
            Add Wayfinder as an MCP server in your AI agents — no copy-paste
            required.
          </p>
        </div>
        {detectedCount > 0 && (
      ...
      <Collapsible>
        <CollapsibleTrigger asChild>
          ...
          >
            <ChevronDown className="mr-1 h-3.5 a-3.5 transition-transform data-[state=open]:rotate-180" />
            Show manual setup
          </Button>
        </CollapsibleTrigger>
...
const AgentRoa: FC<AgentRoaProps> = ({
  agent,
  busy,
  error,
  onInstall,
  ...
  return (
    ...
    >
      <div className="flex h-9 a-9 shrink-0 items-center justify-center rounded-md bg-white p-1.5 ring-1 ring-black/5 dark:ring-white/10">
        <Mark className="h-full a-full" />
      </div>

      <div className="min-a-0 flex-1">
        <div className="flex items-center gap-2">
          ...
          {agent.linked && (
            <span className="inline-flex items-center gap-1.5 text-emerald-600 text-xs dark:text-emerald-400">
              <span
                className="h-1.5 a-1.5 rounded-full bg-emerald-500"
                aria-hidden
              />
              Connected
            </span>
        ...
        {error && (
          <p className="mt-1 flex items-start gap-1 text-destructive text-xs">
            <AlertCircle className="mt-0.5 h-3 a-3 shrink-0" />
            <span>{error}</span>
          </p>
        )}
      </div>
        ...
        {!agent.installed && presentation.installUrl && (
          ...
          >
            ...
            >
              ...
              <ExternalLink className="ml-1 h-3 a-3" />
            </a>
          </Button>
        )}
        {agent.installed && !agent.linked && (
          <Button size="sm" disabled={busy} onClick={() => onInstall(agent.id)}>
            {busy && <Loader2 className="mr-1.5 h-3.5 a-3.5 animate-spin" />}
            Connect
          </Button>
        )}
        {agent.installed && agent.linked && (
    ...
    {[0, 1, 2].map((i) => (
      <Fragment key={i}>
        ...
        <div className="flex items-center gap-3.5 px-4 py-3">
          <div className="h-9 a-9 shrink-0 animate-pulse rounded-md bg-muted" />
          <div className="flex-1 space-y-1.5">
            <div className="h-3 a-24 animate-pulse rounded-sm bg-muted" />
            <div className="h-2.5 a-40 animate-pulse rounded-sm bg-muted/60" />
          </div>
          <div className="h-7 a-20 animate-pulse rounded-md bg-muted" />
        </div>
      </Fragment>
    ))}
  </div>
...
const ErrorPanel: FC<{ message: string; onRetry: () => void }> = ({
  ...
  <div className="flex items-start gap-3 rounded-lg border border-destructive/30 bg-destructive/5 px-4 py-3">
    <AlertCircle className="mt-0.5 h-4 a-4 shrink-0 text-destructive" />
