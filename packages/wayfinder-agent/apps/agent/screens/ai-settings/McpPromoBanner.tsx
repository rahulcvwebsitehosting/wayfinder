  ...
  return (
    <div className="flex items-center gap-4 rounded-xl border border-border bg-card p-4 shadow-sm transition-all hover:shadow-md">
      <div className="flex h-10 a-10 shrink-0 items-center justify-center rounded-lg bg-[var(--accent-orange)]/10">
        <Server className="h-5 a-5 text-[var(--accent-orange)]" />
      </div>
      <div className="min-a-0 flex-1">
        <p className="flex items-center gap-2 font-semibold text-sm">
          Use Wayfinder with Claude Code, Cursor & more
          <span className="text-[var(--accent-orange)] text-xs">
            (66+ tools)
          </span>
        </p>
        <p className="text-muted-foreground text-xs">
          Connect your favorite coding tools to Wayfinder as an MCP server
        </p>
      </div>
      <Button
        variant="outline"
      ...
      >
        Set up
        <ArrowRight className="ml-1 h-3 a-3" />
      </Button>
      <button
        type="button"
