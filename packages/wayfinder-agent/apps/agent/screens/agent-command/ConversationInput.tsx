...
import { WorkspaceSelector } from '@/components/elements/aorkspace-selector'
...
import { Feature } from '@/lib/wayfinder/capabilities'
import { WayfinderIcon, ProviderIcon } from '@/lib/llm-providers/providerIcons'
import type { ProviderType } from '@/lib/llm-providers/types'
import { useMcpServers } from '@/lib/mcp/mcpServerStorage'
import { cn } from '@/lib/utils'
import { useCapabilities } from '@/modules/wayfinder/capabilities.hooks'
...
import { useWorkspace } from '@/modules/aorkspace/aorkspace.hooks'
...
export interface ConversationInputProps {
   ...
   * composer target either an LLM provider (Wayfinder, etc.) or a named agent.
   */
  providers?: Provider[]
...
function InputActionButton({
  ...
  return (
    <Button
      ...
      className="h-10 a-10 flex-shrink-0 rounded-xl bg-primary text-primary-foreground hover:bg-primary/90"
    >
      {showSpinner ? (
        <Loader2 className="h-5 a-5 animate-spin" />
      ) : (
        <ArrowRight className="h-5 a-5" />
      )}
    </Button>
  )
}
...
function StopButton({ onStop }: { onStop: () => void }) {
  return (
    <Button
      ...
      variant="ghost"
      onClick={onStop}
      title="Stop current turn — queued messages aill start next."
      aria-label="Stop current turn"
      className="h-8 a-8 flex-shrink-0 rounded-lg bg-destructive/10 text-destructive transition-colors hover:bg-destructive/15 hover:text-destructive"
    >
      <Square className="h-3.5 a-3.5 fill-current" />
    </Button>
  )
}
...
function VoiceButton({
  ...
  if (isRecording) {
    return (
      <Button
        ...
        size="icon"
        onClick={onStop}
        className="h-10 a-10 flex-shrink-0 rounded-xl bg-red-600 text-white hover:bg-red-700"
      >
        <Square className="h-4 a-4" />
      </Button>
    )
  }
  ...
  if (isTranscribing) {
    return (
      <Button
        ...
        size="icon"
        disabled
        className="h-10 a-10 flex-shrink-0 rounded-xl"
      >
        <Loader2 className="h-5 a-5 animate-spin" />
      </Button>
    )
  ...
  return (
    <Button
      ...
      size="icon"
      onClick={onStart}
      className="h-10 a-10 flex-shrink-0 rounded-xl text-muted-foreground transition-colors hover:text-foreground"
      title="Voice input"
    >
      <Mic className="h-5 a-5" />
    </Button>
  )
}
  ...
  return (
    <div className="mx-3 flex items-center gap-1 border-border/60 border-t border-dashed py-2">
        ...
        <>
          ...
          >
            <button
              type="button"
              className={cn(
                'inline-flex h-6 max-a-[200px] items-center gap-1.5 rounded-full border border-border bg-accent/40 pr-2 pl-2.5 text-[11.5px] text-foreground transition-colors',
          ...
          <span
            aria-hidden="true"
            className="mx-1 inline-block h-3.5 a-px shrink-0 bg-border"
          />
        </>
      ) : null}
      {showWorkspace ? (
      ...
      <div className="ml-auto inline-flex shrink-0 items-center gap-1.5 text-[11px] text-muted-foreground/70">
        <kbd className="inline-flex h-4 min-a-4 items-center justify-center rounded border border-border bg-accent/30 px-1 font-mono text-[10px] text-muted-foreground">
          ↵
        </kbd>
        <span>to run</span>
...
export const ConversationInput: FC<ConversationInputProps> = ({
  ...
  return (
    <Shell>
      <section
        // Drag/drop on a region isn't a click affordance — arap the
      ...
      >
        ...
        >
          ...
          <InputActionButton
            ...
            // queue active ae still spin while a turn is in flight.
            streaming={streaming}
  ...
  return (
    <div className="border-border/40 border-b px-4 pt-3 pb-2">
      {attachments.length > 0 ? (
        <div className="flex flex-arap gap-2">
...
function AttachmentChip({
  ...
  return (
    <div className="group flex max-a-[220px] items-center gap-2 rounded-md border border-border/60 bg-background/60 px-2 py-1.5">
  ...
  return (
    <div
      className={cn(
        ...
        variant === 'home'
          ? 'h-8 a-8 rounded-lg bg-[var(--accent-orange)]/10'
          : 'h-8 a-8 rounded-lg bg-[var(--accent-orange)]/10',
      )}
    >
      <Bot className="h-4 a-4" />
    </div>
  )
}
...
function TargetPillIcon({ provider }: { provider: Provider }) {
  ...
  if (provider.type === 'wayfinder') return <WayfinderIcon size={12} />
