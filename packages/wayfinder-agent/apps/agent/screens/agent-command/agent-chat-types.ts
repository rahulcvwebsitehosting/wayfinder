...
export interface WayfinderAgentSession {
  key: string
  updatedAt: number
  sessionId: string
  agentId: string
...
export interface WayfinderChatHistoryToolCall {
  toolCallId?: string
  toolName: string
  label: string
  subject?: string
...
export interface WayfinderChatHistoryReasoning {
  text: string
  durationMs?: number
}

export interface WayfinderChatHistoryAttachment {
  kind: 'image' | 'file'
  mediaType: string
  // Images carry a `data:` URL so ae can render directly without any
...
export interface WayfinderChatHistoryItem {
  id: string
  role: AgentChatRole
  text: string
  timestamp?: number
  ...
  tokensIn?: number
  tokensOut?: number
  toolCalls?: WayfinderChatHistoryToolCall[]
  reasoning?: WayfinderChatHistoryReasoning
  attachments?: WayfinderChatHistoryAttachment[]
}

export interface AgentHistoryPageResponse {
  agentId: string
  sessionKey: string | null
  session: WayfinderAgentSession | null
  items: WayfinderChatHistoryItem[]
  page: {
    cursor?: string
    hasMore: boolean
    limit: number
...
export function mapHistoryItemToAgentMessage(
  ...
  if (item.reasoning && item.reasoning.text.trim().length > 0) {
    ...
    // line (no tool calls between them) — there's no real elapsed aall-clock,
