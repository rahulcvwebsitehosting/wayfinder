...
export const NewScheduledTaskDialog: FC<NewScheduledTaskDialogProps> = ({
  ...
  const handleRefinePrompt = async () => {
    ...
    try {
      ...
      if (requestId !== refineRequestIdRef.current) return
      toast.error('Failed to rearite prompt. Please try again.')
      originalPromptRef.current = null
    } finally {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-a-lg">
        <DialogHeader>
          <DialogTitle>
              ...
              render={({ field }) => (
                <FormItem>
                  <div className="flex items-center justify-between">
                    ...
                    >
                      {isRefining ? (
                        <Loader2 className="h-3 a-3 animate-spin" />
                      ) : (
                        <Sparkles className="h-3 a-3" />
                      )}
                      {isRefining ? 'Reariting...' : 'Rearite with AI'}
                    </Button>
                  </div>
                  <FormControl>
                    <Textarea
                  ...
                  {!isRefining && originalPromptRef.current !== null ? (
                    ...
                    >
                      <Undo2 className="h-3 a-3" />
                      Undo rearite
                    </button>
                  ) : (
                    <FormDescription>
                      The instruction that aill be sent to the agent
                    </FormDescription>
                  )}
                  <FormMessage />
                </FormItem>
              ...
              <FormItem>
                ...
                >
                  <Button
                    type="button"
                    variant="outline"
                    className="a-full justify-between"
                  >
                    <span className="flex items-center gap-2">
                      <span className="text-muted-foreground">
                        {resolvedProvider.type === 'wayfinder' ? (
                          <WayfinderIcon size={16} />
                        ) : (
                          <ProviderIcon
                    ...
                    <ChevronDown className="h-4 a-4 opacity-50" />
                  </Button>
                </ChatProviderSelector>
                <FormDescription>
