...
export const SidebarBranding: FC<SidebarBrandingProps> = ({
  ...
  const headerIcon = isLoggedIn ? (
    ...
    <img src={ProductLogo} alt="Wayfinder" className="size-8" />
  )
  ...
  return (
    <div className="flex h-14 items-center justify-between border-b px-2">
      <DropdownMenu modal={false}>
        <DropdownMenuTrigger asChild>
          ...
          >
            ...
            <div
              className={cn(
                'flex min-a-0 flex-col gap-0.5 leading-none transition-opacity duration-200',
      ...
      >
        <ThemeToggle className="h-8 a-8" iconClassName="h-4 a-4" />
      </div>
    </div>
  )
}
