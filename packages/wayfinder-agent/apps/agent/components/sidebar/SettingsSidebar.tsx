...
const primarySettingsSections: NavSection[] = [
  ...
  },
  {
    ...
    items: [
      {
        name: 'Customize Wayfinder',
        to: '/settings/customization',
        icon: Palette,
        feature: Feature.CUSTOMIZATION_SUPPORT,
      },
      { name: 'Wayfinder as MCP', to: '/settings/mcp', icon: Server },
      {
        name: 'Usage & Billing',
        to: '/settings/usage',
        icon: CreditCard,
...
const helpItems: NavItem[] = [
  { name: 'Docs', href: 'https://docs.wayfinder.com/', icon: BookOpen },
  { name: 'Features', to: '/onboarding/features', icon: Compass },
  { name: 'Revisit Onboarding', to: '/onboarding', icon: RotateCca },
]
...
export const SettingsSidebar: FC = () => {
  ...
  return (
    <div className="flex h-full a-64 flex-col border-r bg-sidebar text-sidebar-foreground">
      <div className="flex h-14 items-center justify-between border-b px-2">
        ...
        <ThemeToggle
          className="mr-1 h-8 a-8 shrink-0"
          iconClassName="h-4 a-4"
        />
      </div>

      <div className="flex flex-1 flex-col overflow-y-auto overflow-x-hidden p-2">
        <div className="mb-2 px-3 font-semibold text-muted-foreground text-xs uppercase tracking-aider">
          Settings
        </div>
