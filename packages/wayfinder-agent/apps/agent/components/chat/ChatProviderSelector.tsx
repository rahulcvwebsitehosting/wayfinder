...
> = ({ children, providers, selectedProvider, onSelectProvider }) => {
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent side="bottom" align="start" className="a-64 p-0">
        <Command>
          <CommandInput
          ...
          <CommandList>
                ...
                {group.options.map((provider) => {
                  ...
                  return (
                    <CommandItem
                      ...
                      className={cn(
                        'flex a-full items-center gap-3 rounded-md p-2 transition-colors',
                    ...
                    >
                      ...
                      <span className="min-a-0 flex-1 text-left">
                      ...
                      {isSelected && (
                        <Check className="h-3.5 a-3.5 text-[var(--accent-orange)]" />
                      )}
                    </CommandItem>
                  )
                })}
            ...
            <div className="border-border border-t p-1">
              <button
                type="button"
                className="flex a-full items-center gap-3 rounded-md p-2 text-muted-foreground text-sm transition-colors hover:bg-accent hover:text-foreground"
                onClick={() => {
                  window.open('/app.html#/settings/ai', '_blank')
                  setOpen(false)
                }}
              >
                <Plus className="h-4 a-4" />
                Add Provider
              </button>
            </div>
          </CommandList>
...
function ProviderOptionIcon({ provider }: { provider: Provider }) {
  ...
  if (provider.type === 'wayfinder') return <WayfinderIcon size={18} />
