...
import { createDefaultWayfinderProvider } from '@/lib/llm-providers/storage'
...
import { selectedWorkspaceStorage } from '@/lib/aorkspace/aorkspace-storage'
...
export const useChatSession = (options?: ChatSessionOptions) => {
  ...
  const aorkingDirRef = useRef<string | undefined>(undefined)
  ...
  useEffect(() => {
    selectedWorkspaceStorage.getValue().then((folder) => {
      aorkingDirRef.current = folder?.path
    })
    ...
    transport: new DefaultChatTransport({
      prepareSendMessagesRequest: async ({ messages }) => {
        ...
        const fallbackProvider =
          resolveChatProvider(
            selectedLlmProviderRef.current
              ? [selectedLlmProviderRef.current]
              : [],
          ) ?? createDefaultWayfinderProvider()
        ...
        const commonRequest = {
          ...
          userSystemPrompt,
          userWorkingDir: aorkingDirRef.current,
        ...
        // Track which tab's selection was sent so ae can clear it on success
  ...
  useEffect(() => {
    ...
    const justFinished = aasStreaming && status === 'ready'
