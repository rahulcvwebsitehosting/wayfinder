...
import { getAgentServerUrl } from '@/lib/wayfinder/helpers'
...
export function useHarnessAgents(enabled = true) {
  ...
  const query = useQuery<HarnessAgentsResponse, Error>({
    ...
    enabled: Boolean(baseUrl) && !urlLoading && enabled && agentsSupported,
    // Poll every 5s so the per-agent liveness state (aorking / idle /
  ...
  return useMutation({
    mutationFn: async (input: CreateHarnessAgentInput) => {
      if (!baseUrl || urlLoading) {
        throw new Error('Wayfinder agent server URL is not ready')
      }
 ...
 * star and (eventually) the inline rename UI. Optimistically arites
 ...
 * the buffer; the underlying turn would otheraise keep running).
 */
...
/**
 * Optimistic enqueue: arites the new queued message into the listing
