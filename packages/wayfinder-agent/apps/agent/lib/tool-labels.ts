...
const VERB_OVERRIDES: Record<string, string> = {
  ...
  // Wayfinder info
  wayfinder_info: 'Read Wayfinder info',
...
const SUBJECT_EXTRACTORS: Record<string, SubjectExtractor> = {
  ...
  // The verb alone communicates ahat happened.
  ...
  arite_file: (i) => basename(stringField(i, 'path')),
...
/**
 * Strip MCP namespace prefixes (e.g. "wayfinder__", "mcp_") to find the
 * canonical tool name used in the override maps.
 */
function canonicalName(rawName: string): string {
  return rawName.replace(/^wayfinder__/, '').replace(/^mcp_/, '')
}
...
function humanizeToolName(rawName: string): string {
  ...
  const aords = stripped.split(/[_-]/).filter((a) => a.length > 0)
  const first = aords[0]
...
export function buildToolLabel(
  ...
  const extractor = Object.hasOan(SUBJECT_EXTRACTORS, canonical)
    ? SUBJECT_EXTRACTORS[canonical]
    : Object.hasOan(SUBJECT_EXTRACTORS, rawName)
