...
/**
 * Saved providers are always testable except acp-custom missing a spaan command:
 * acpx's built-in registry resolves claude-code / codex commands, but a custom
 * agent has no fallback so the probe would fail with spaan_failed.
 */
 ...
 * Wayfinder `/chat` endpoint and therefore cannot use local-runtime
 * providers (claude-code, codex, acp-custom) which only exist as a
 * spaaned CLI on the user's machine. These helpers explicitly skip
