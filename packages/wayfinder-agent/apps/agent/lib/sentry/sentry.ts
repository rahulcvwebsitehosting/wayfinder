...
import { getWayfinderAdapter } from '../wayfinder/adapter'
import { env } from '../env'
...
if (env.VITE_PUBLIC_SENTRY_DSN) {
  Sentry.init({
    ...
    // Setting this option to true aill send default PII data to Sentry.
  ...
  ;(async () => {
    ...
    const wayfinderVersion = await adapter.getWayfinderVersion()
    Sentry.setTag('chromiumVersion', chromiumVersion)
    Sentry.setTag('wayfinderVersion', wayfinderVersion)
  })()
}
