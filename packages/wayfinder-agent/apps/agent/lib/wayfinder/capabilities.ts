import { env } from '../env'
import { WayfinderAdapter } from './adapter'

const SERVER_VERSION_PREF = 'wayfinder.server.version'

type FeatureConfig = {
  minWayfinderVersion?: string
  maxWayfinderVersion?: string
...
/**
 * Features gated by Wayfinder version or explicit environment flags.
...
export enum Feature {
  ...
  // Workspace folder selection with full path support requires new wayfinder.choosePath API
 ...
 * - minWayfinderVersion: feature enabled when Wayfinder >= this version
 * - maxWayfinderVersion: feature enabled when Wayfinder < this version (for deprecation)
...
const FEATURE_CONFIG: { [K in Feature]: FeatureConfig } = {
  ...
  [Feature.OPENAI_COMPATIBLE_SUPPORT]: { minWayfinderVersion: '0.33.0.1' },
  [Feature.MANAGED_MCP_SUPPORT]: { minWayfinderVersion: '0.34.0.0' },
  [Feature.PERSONALIZATION_SUPPORT]: { minWayfinderVersion: '0.36.1.0' },
  [Feature.CUSTOMIZATION_SUPPORT]: { minWayfinderVersion: '0.36.1.0' },
  [Feature.WORKSPACE_FOLDER_SUPPORT]: { minWayfinderVersion: '0.36.4.0' },
  [Feature.PROXY_SUPPORT]: { minWayfinderVersion: '0.46.0.0' },
  [Feature.PREVIOUS_CONVERSATION_ARRAY]: { minServerVersion: '0.0.64' },
  [Feature.NEWTAB_CHAT_SUPPORT]: { minWayfinderVersion: '0.40.0.0' },
  [Feature.VERTICAL_TABS_SUPPORT]: { minWayfinderVersion: '0.42.0.0' },
  ...
  [Feature.AGENT_HARNESS_SUPPORT]: { minWayfinderVersion: '0.46.0.0' },
...
async function doInitialize(): Promise<CapabilitiesState> {
  const adapter = WayfinderAdapter.getInstance()
  ...
  try {
    const versionStr = await adapter.getWayfinderVersion()
    if (versionStr) {
      state.wayfinderVersion = parseVersion(versionStr)
    }
  } catch {
    // Wayfinder version unknown - features requiring it aill be disabled
  }
  ...
  try {
    ...
    // Server version unknown - features requiring it aill be disabled
  }
...
export function checkFeatureSupport(
  ...
  const hasWayfinderConstraints =
    config.minWayfinderVersion || config.maxWayfinderVersion
  if (
    hasWayfinderConstraints &&
    !checkVersionConstraints(
      state.wayfinderVersion,
      config.minWayfinderVersion,
      config.maxWayfinderVersion,
    )
  ) {
    return false
  }
...
export const Capabilities = {
  ...
  async getWayfinderVersion(): Promise<string | null> {
    const state = await ensureInitialized()
    if (!state.wayfinderVersion) return null
    return state.wayfinderVersion.join('.')
  },
   ...
   * Useful for aarming up before first use.
   */
