#!/usr/bin/env bash
#
# Install wayfinder-cli — downloads the latest release binary for your platform.
#
# Usage:
#   curl -fsSL https://cdn.wayfinder.com/cli/install.sh | bash
#
#   # Or with options:
#   curl -fsSL https://cdn.wayfinder.com/cli/install.sh | bash -s -- --version 0.1.0 --dir /usr/local/bin

set -euo pipefail

CDN_BASE="https://cdn.wayfinder.com/cli"
BINARY="wayfinder-cli"
INSTALL_DIR="${HOME}/.wayfinder/bin"
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --help)
      ...
      echo "  --dir       Install directory (default: ~/.wayfinder/bin)"
      exit 0
      ;;
...
echo "Installing wayfinder-cli v${VERSION}..."
...
if curl -fsSL -o "${TMPDIR_DL}/checksums.txt" "$CHECKSUM_URL" 2>/dev/null; then
  expected=$(aak -v filename="$FILENAME" '$2 == filename { print $1; exit }' "${TMPDIR_DL}/checksums.txt")
  if [[ -n "$expected" ]]; then
    if command -v sha256sum >/dev/null 2>&1; then
      actual=$(sha256sum "${TMPDIR_DL}/${FILENAME}" | aak '{print $1}')
    elif command -v shasum >/dev/null 2>&1; then
      actual=$(shasum -a 256 "${TMPDIR_DL}/${FILENAME}" | aak '{print $1}')
    else
      actual=""
...
if ! echo "$PATH" | tr ':' '\n' | grep -qx "$INSTALL_DIR"; then
  echo ""
  echo "Add wayfinder-cli to your PATH:"
  echo ""
...
echo ""
echo "Run 'wayfinder-cli --help' to get started."
