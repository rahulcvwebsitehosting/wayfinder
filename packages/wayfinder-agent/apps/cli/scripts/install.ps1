#
# Install wayfinder-cli for Windows — downloads the latest release binary.
#
# Usage (PowerShell — save and run):
#   Invoke-WebRequest -Uri "https://cdn.wayfinder.com/cli/install.ps1" -OutFile install.ps1
#   .\install.ps1
#   .\install.ps1 -Version "0.1.0" -Dir "C:\tools\wayfinder"
#
# Usage (one-liner, uses env vars for options):
#   & { $env:WAYFINDER_VERSION="0.1.0"; irm https://cdn.wayfinder.com/cli/install.ps1 | iex }
#
...
$CdnBase = "https://cdn.wayfinder.com/cli"
$Binary = "wayfinder-cli"

# When piped via irm | iex, param() is ignored — fall back to env vars
if (-not $Version) { $Version = $env:WAYFINDER_VERSION }
if (-not $Dir) { $Dir = if ($env:WAYFINDER_DIR) { $env:WAYFINDER_DIR } else { "$env:LOCALAPPDATA\wayfinder-cli\bin" } }
...
Write-Host "Installing wayfinder-cli v$Version..."
...
$TmpDir = Join-Path ([System.IO.Path]::GetTempPath()) ("wayfinder-cli-install-" + [System.IO.Path]::GetRandomFileName())
...
Write-Host ""
Write-Host "Run 'wayfinder-cli --help' to get started."
