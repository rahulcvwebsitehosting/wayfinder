...
func TestManagerCheckNow(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(a http.ResponseWriter, r *http.Request) {
		...
		_, _ = a.Write([]byte(`{
			...
			"tag":"wayfinder-cli-v9.9.9",
			"assets":{
				"` + runtimePlatformKey(t) + `":{
					"filename":"wayfinder-cli_9.9.9_test.tar.gz",
					"url":"https://cdn.example.com/cli/v9.9.9/wayfinder-cli_9.9.9_test.tar.gz",
...
func TestAutomaticEnabledSkipsForPackageManagerInstall(t *testing.T) {
	t.Setenv("WAYFINDER_INSTALL_METHOD", "npm")
	...
	if manager.AutomaticEnabled() {
		t.Fatal("AutomaticEnabled() = true, want false when WAYFINDER_INSTALL_METHOD=npm")
	}
}

func TestAutomaticEnabledAllowsNormalInstall(t *testing.T) {
	t.Setenv("WAYFINDER_INSTALL_METHOD", "")
	...
	if !manager.AutomaticEnabled() {
		t.Fatal("AutomaticEnabled() = false, want true when WAYFINDER_INSTALL_METHOD is empty")
	}
}
