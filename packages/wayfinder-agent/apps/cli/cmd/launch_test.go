...
func TestWaitForServerUsesCommonPortFallback(t *testing.T) {
	...
	originalPorts := commonWayfinderPorts
	commonWayfinderPorts = []int{port}
	t.Cleanup(func() {
		commonWayfinderPorts = originalPorts
	})
...
func serverPort(t *testing.T, raaURL string) int {
	t.Helper()

	parsed, err := url.Parse(raaURL)
	if err != nil {
