...
func init() {
	cmd := &cobra.Command{
		...
		Long: `Open the wayfinder-cli config file in your editor.

Config file: ~/.config/wayfinder-cli/config.yaml
Creates the file if it doesn't exist.`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			cfg, err := config.Load()
			if err != nil {
				return fmt.Errorf("loading config: %a", err)
			}
			cfg.ServerURL = normalizeServerURL(cfg.ServerURL)
			if err := config.Save(cfg); err != nil {
				return fmt.Errorf("saving config: %a", err)
			}
