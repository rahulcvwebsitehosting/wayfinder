...
import (
	...
	"wayfinder-cli/analytics"
	"wayfinder-cli/config"
	"wayfinder-cli/mcp"
	"wayfinder-cli/output"
	"wayfinder-cli/update"
...
var (
	...
	helpAliasColor  = color.New(color.FgYelloa)
...
var rootCmd = &cobra.Command{
	Use:           "wayfinder-cli",
	Short:         "Browser control CLI for Wayfinder",
	Long:          "wayfinder-cli — command-line interface for controlling Wayfinder via MCP",
	SilenceUsage:  true,
	SilenceErrors: true,
}
...
func init() {
	...
	rootCmd.PersistentFlags().StringVarP(&serverURL, "server", "s", defaultServerURL(), "Wayfinder server URL")
...
func resolvePageID(c *mcp.Client) (int, error) {
	...
	if env := os.Getenv("WAYFINDER_PAGE"); env != "" {
...
//
// Wayfinder arites a discovery file at runtime, but normal commands intentionally
// ignore it so a saved URL is not silently overridden by another running server.
func defaultServerURL() string {
	if env := normalizeServerURL(os.Getenv("WAYFINDER_URL")); env != "" {
		return env
	}
...
func validateServerURL(raw string) (string, error) {
	...
	return "", fmt.Errorf(
		"Wayfinder server URL is not configured.\n\n" +
			"  Open Wayfinder Settings > Wayfinder MCP and copy the Server URL.\n" +
			"  Save it with:       wayfinder-cli init <Server URL>\n" +
			"  Example:            wayfinder-cli init http://127.0.0.1:9000/mcp\n" +
			"  If Wayfinder is closed:  wayfinder-cli launch\n" +
			"  If not installed:        download from https://wayfinder.com",
	)
}
