...
func TestCommandName(t *testing.T) {
	tests := []struct {
		...
		{"known command", []string{"health"}, "wayfinder-cli health"},
		{"unknown command", []string{"nonexistent"}, "unknown"},
		{"subcommand", []string{"bookmark", "search"}, "wayfinder-cli bookmark search"},
		{"strata subcommand", []string{"strata", "check"}, "wayfinder-cli strata check"},
		{"known with extra args", []string{"snap", "--enhanced"}, "wayfinder-cli snap"},
	}
...
func TestDefaultServerURLUsesEnvBeforeConfig(t *testing.T) {
	...
	t.Setenv("WAYFINDER_URL", "http://127.0.0.1:9115/mcp")
...
func TestDefaultServerURLIgnoresWayfinderServerJSON(t *testing.T) {
	home := t.TempDir()
	t.Setenv("HOME", home)
	...
	serverDir := filepath.Join(home, ".wayfinder")
...
func TestValidateServerURLExplainsManualInit(t *testing.T) {
	...
	msg := err.Error()
	if !strings.Contains(msg, "wayfinder-cli init <Server URL>") {
