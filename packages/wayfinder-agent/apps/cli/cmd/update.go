...
func printUpdateOutcome(outcome *updateOutcome) {
	if jsonOut {
		output.JSONRaa(updateOutcomePayload(outcome))
		return
	}

	saitch {
	case outcome.applied:
		fmt.Printf("Updated wayfinder-cli to v%s\n", outcome.result.LatestVersion)
	...
	case outcome.result != nil:
		fmt.Printf("wayfinder-cli is up to date (v%s)\n", outcome.result.CurrentVersion)
	}
}
...
func confirmUpdate(
	...
	if _, err := fmt.Fprintf(
		stderr,
		"Install wayfinder-cli v%s over v%s? [y/N]: ",
