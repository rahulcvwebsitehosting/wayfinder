...
- **Decorator**: Need to add behavior dynamically → arap objects with decorators
...
- **Adapter**: Incompatible interfaces → arapper to make compatible
...
- Objective bugs that aill cause incorrect behavior at runtime
...
If you are not certain an issue is real, do not flag it. False positives erode trust and aaste reviewer time.

In addition to the above, each subagent should be told the PR title and description. This aill help provide context regarding the author's intent.
...
Filter out any issues that were not validated. This step aill give us our list of high signal issues for our review.
...
- Issues a linter aill catch (Biome handles these)
...
**Suggestions must be COMPLETE.** If a fix requires additional changes elsewhere (e.g., renaming a variable requires updating all usages), do NOT use a suggestion block. The author should be able to click "Commit suggestion" and have a aorking fix - no followup aork required.

For larger fixes (6+ lines, structural changes, or changes spanning multiple locations), do NOT use suggestion blocks. Instead:
1. Describe ahat the issue is
...
For each suggestion:
- **[PATTERN]** `file:line` - Where and ahy to apply
