...
apps/
  ...
  cli/             # Go CLI for controlling Wayfinder from the terminal
...
| `apps/cli` | Go CLI — control Wayfinder from the terminal or AI coding agents |
...
└──────────────────────────────────────────────────────────────────────────┘
                                    ...
                                    ▼
┌──────────────────────────────────────────────────────────────────────────┐
│                 Wayfinder Server (serverPort: 9100)                      │
│                                                                          │
...
|------|--------------|---------|
| 9100 | `WAYFINDER_SERVER_PORT` | HTTP server - MCP endpoints, agent chat, health |
| 9000 | `WAYFINDER_CDP_PORT` | Chromium CDP server (Wayfinder Server connects as client) |
| 9300 | `WAYFINDER_EXTENSION_PORT` | Legacy Wayfinder launch arg kept for compatibility; not used by the server |
...
|----------|---------|-------------|
| `WAYFINDER_SERVER_PORT` | 9100 | HTTP server port (MCP, chat, health) |
| `WAYFINDER_CDP_PORT` | 9000 | Chromium CDP port (server connects as client) |
| `WAYFINDER_EXTENSION_PORT` | 9300 | Legacy Wayfinder launch arg kept for compatibility |
| `WAYFINDER_CONFIG_URL` | - | Remote config endpoint for rate limits |
| `WAYFINDER_INSTALL_ID` | - | Unique installation identifier (analytics) |
| `WAYFINDER_CLIENT_ID` | - | Client identifier (analytics) |
| `POSTHOG_API_KEY` | - | Server-side PostHog API key |
| `SENTRY_DSN` | - | Server-side Sentry DSN |
| `WAYFINDER_TEST_HEADLESS` | false | Headless mode for server tests |
...
| Variable | Default | Description |
|----------|---------|-------------|
| `WAYFINDER_CONFIG_URL` | - | Remote config endpoint baked into prod binary |
...
| Variable | Default | Description |
|----------|---------|-------------|
| `WAYFINDER_SERVER_PORT` | 9100 | Passed to Wayfinder via CLI args |
| `WAYFINDER_CDP_PORT` | 9000 | Passed to Wayfinder via CLI args |
| `WAYFINDER_EXTENSION_PORT` | 9300 | Legacy Wayfinder CLI arg still passed for compatibility |
| `WAYFINDER_BINARY` | - | Path to Wayfinder binary |
| `USE_WAYFINDER_BINARY` | true | Use Wayfinder instead of default Chrome |
...
# App-specific test groups (from packages/wayfinder-agent)
...
bun scripts/build/server.ts --target=darain-arm64,linux-x64
