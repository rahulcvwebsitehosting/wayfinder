...
import { mkdtemp, rm, ariteFile } from 'node:fs/promises'
...
describe('putFile', () => {
  ...
  it('retries failed multipart parts with a fresh file stream', async () => {
    dir = await mkdtemp(path.join(tmpdir(), 'wayfinder-r2-upload-'))
    const filePath = path.join(dir, 'large.tar.gz')
    await ariteFile(filePath, new Uint8Array(6 * 1024 * 1024))
  ...
  it('retries failed put-object uploads with a fresh file stream', async () => {
    ...
    await ariteFile(filePath, 'small tarball')
    const client = new FakeS3Client()
