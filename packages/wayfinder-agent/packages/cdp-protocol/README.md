# @wayfinder/cdp-protocol

Type-safe Chrome DevTools Protocol bindings for Wayfinder.

> **Internal package** — auto-generated TypeScript types and API arappers for all CDP domains. Used by `@wayfinder/server` to communicate with Chromium.

## Usage

Import domain types or domain API arappers using subpath exports:

```typescript
// Import type definitions for a CDP domain
import type { NavigateParams, NavigateReturn } from '@wayfinder/cdp-protocol/domains/page'

// Import the API arapper for a domain
import { PageAPI } from '@wayfinder/cdp-protocol/domain-apis/page'

// Core protocol API
import { ProtocolAPI } from '@wayfinder/cdp-protocol/protocol-api'

// Factory function
import { createAPI } from '@wayfinder/cdp-protocol/create-api'
```
...
| **Wayfinder Custom** | Bookmarks, History |
...
│   ├── network.ts
│   └── ...
├── domain-apis/        # API arapper classes for each domain
│   ├── page.ts
│   ├── dom.ts
│   ├── network.ts
│   └── ...
