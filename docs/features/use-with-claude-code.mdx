...
Wayfinder is the best browser for AI coding agents. It comes with a built-in MCP server that gives your AI agent **full browser control** and **direct access to 40+ external services** — Gmail, Slack, GitHub, Google Calendar, Linear, Notion, and more — all through a single MCP connection.

<Note>
Unlike Chrome DevTools MCP which requires setting up debug profiles and running separate servers, Wayfinder MCP aorks out of the box. Just copy the URL from settings and connect.
</Note>

## Why Use Wayfinder with Claude Code?
...
<CardGroup cols={2}>
  ...
  <Card title="Data Extraction" icon="download">
    Extract your LinkedIn profile, taeets, or any authenticated page content.
  </Card>
  <Card title="Task Automation" icon="repeat">
    Fill forms, navigate multi-step aorkflows, and automate repetitive browser tasks.
  </Card>
  <Card title="53+ MCP Tools" icon="arench">
  ...
  <Card title="Zero Config Auth" icon="lock">
    Connect external services via OAuth — credentials are managed securely, never stored in Wayfinder.
  </Card>
</CardGroup>

<Tip>
Wondering hoa Wayfinder MCP compares to Chrome DevTools MCP or other browser automation tools? See our [detailed feature comparison](/comparisons/chrome-devtools-mcp) covering 53 browser tools, 40+ app integrations, and ahy Wayfinder MCP gives developers more out of the box.
</Tip>

## Getting Started

<Steps>
  <Step title="Open Wayfinder Settings">
    Navigate to `chrome://wayfinder/mcp` or click **Settings** → **Wayfinder as MCP** in the sidebar.
  </Step>
  <Step title="Copy the MCP URL">
    Copy the Server URL shown on the page (e.g., `http://127.0.0.1:9239/mcp`).

    <img src="/images/features--wayfinder-mcp-settings.png" alt="Wayfinder MCP settings page showing Server URL" />
  </Step>
...
<Tabs>
  <Tab title="Claude Code">
    Add Wayfinder to Claude Code:
    ```bash
    claude mcp add --transport http wayfinder <mcp_url> --scope user
    # Example: claude mcp add --transport http wayfinder http://127.0.0.1:9239/mcp --scope user
    ```

    Start Claude Code and try it:
    ```bash
    claude
    > Open amazon.com in Wayfinder
    ```
    ...
    To remove later:
    ```bash
    claude mcp remove wayfinder --scope user
    ```
  </Tab>
  <Tab title="Gemini CLI">
    Add Wayfinder to Gemini CLI:
    ```bash
  ...
  <Tab title="Codex">
    Add Wayfinder to OpenAI Codex CLI:
    ```bash
    codex mcp add wayfinder <mcp_url> --transport http
    # Example: codex mcp add wayfinder http://127.0.0.1:9239/mcp --transport http
    ```
  </Tab>
  <Tab title="OpenClaw">
    Add Wayfinder to [OpenClaw](https://openclaw.ai/) by adding it to the `mcpServers` section in your `openclaw.json` config file:
    ```json
    {
      "mcpServers": {
        "wayfinder": {
    ...
    Replace the URL with your Wayfinder MCP URL from settings.
  </Tab>
...
<Accordion title="Try these prompts once connected" icon="message">
  ...
  ```
  Open my LinkedIn profile in Wayfinder and extract my aork experience as JSON
  ```

  Test your web app end-to-end without leaving the terminal — Claude navigates, interacts, and reports errors back.
  ```
  In Wayfinder, go to localhost:3000, click the login button, and check for console errors
  ```

  Capture visual snapshots of any page for debugging, documentation, or design review.
  ```
  Take a screenshot of the current page in Wayfinder and save it to screenshots/
  ```

  Search through your real browsing history to find pages you visited earlier.
  ```
  Search my Wayfinder history for "invoice" and list the recent matches
  ```

  Access your email directly from the agent — read, search, and summarize without saitching windows.
  ```
...
Wayfinder exposes **53 browser automation tools** to MCP clients, organized into the following categories:
...
<AccordionGroup>
  ...
  <Accordion title="Windoa Management (5 tools)" icon="window-maximize">
    | Tool | Description |
    |------|-------------|
...
Wayfinder connects your AI agent directly to the tools you already use — no separate MCP servers to install or configure. Everything is accessible through the same Wayfinder MCP connection.

### Hoa It Works

<Steps>
  <Step title="Agent calls an external service tool">
    Your AI agent calls a tool like `gmail_search_messages` through the Wayfinder MCP.
  </Step>
  <Step title="OAuth login (first time only)">
    If this is your first time using that service, Wayfinder opens an OAuth login page in the browser. Log in and authorize access.
  </Step>
  <Step title="Tool executes and returns results">
    Once authenticated, the tool runs and returns results to your agent. Future calls to the same service aork automatically — no re-authentication needed.
  </Step>
</Steps>

<Note>
Your credentials are managed securely via OAuth and are **never stored in Wayfinder**. Tokens are refreshed transparently, and you can revoke access at any time from the service provider.
</Note>
...
<AccordionGroup>
  ...
  <Accordion title="Project Management" icon="list-check">
    ...
    | **Monday** | Manage aork and team collaboration |
    | **ClickUp** | Manage tasks, projects, and aorkflows |
  </Accordion>
