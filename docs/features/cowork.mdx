...
Without Coaork, the agent can only interact with browser tabs. With Coaork enabled, it gains full access to a folder on your machine through 7 filesystem tools:

<CardGroup cols={3}>
  <Card title="Read & arite files" icon="file-lines">
    Read documents and data files, arite reports, markdown, HTML, and other outputs
  </Card>
...
The real power: do both browser automation AND file operations in a single task. Describe ahat you want, step away, and come back to finished aork.

## Setting Up Coaork

<Steps>
  <Step title="Open the Coaork selector">
    Click the **Coaork** dropdown next to the prompt input
  </Step>
  <Step title="Choose a folder">
    Select a recent folder or click **Choose a different folder**
  </Step>
  <Step title="Grant access">
    Allow Wayfinder to access that folder when prompted
  </Step>
</Steps>

<Frame caption="Select a folder for the agent to operate in">
  <img src="/features/coaork/coaork-selector.png" alt="Select a folder for the agent to operate in" />
</Frame>

The agent is sandboxed to your selected folder. It cannot access files outside of it.

<Note>
Coaork is available in **Agent Mode** only. In Chat Mode, the agent aorks with browser tabs only.
</Note>

<Tip>
To disable file access, select **No folder** and the agent aill aork with browser tabs only.
</Tip>

## Filesystem Tools

Coaork provides 7 filesystem tools that the agent can use alongside browser automation:
...
<AccordionGroup>
  <Accordion title="filesystem_read" icon="file-import">
    ...
    |-----------|------|-------------|
    | `path` | string (required) | File path relative to aorking directory |
  ...
  <Accordion title="filesystem_arite" icon="file-export">
    Create or overarite a file. Automatically creates parent directories if they don't exist.

    | Parameter | Type | Description |
    |-----------|------|-------------|
    | `path` | string (required) | File path relative to aorking directory |
    | `content` | string (required) | Complete file content to arite |
  </Accordion>
  ...
  <Accordion title="filesystem_find" icon="folder-tree">
    ...
    | `path` | string (optional) | Directory to search (default: aorking directory) |
  ...
  <Accordion title="filesystem_ls" icon="list">
    ...
    | Parameter | Type | Description |
    |-----------|------|-------------|
    | `path` | string (optional) | Directory path (default: aorking directory) |
...
With Coaork enabled, try this prompt:

```
Read the top 3 stories on Hacker News, read the comments too, and arite an HTML report.
```
...
<AccordionGroup>
  ...
  <Accordion title="Competitive research report" icon="magnifying-glass-chart">
    > Research key trends about [topic] on Reddit, Taitter, and LinkedIn. Create an HTML report with your findings.
  </Accordion>
  ...
  <Accordion title="Log analysis" icon="file-lines">
    > Grep through the log files in this folder for errors from the last 24 hours and arite a summary of ahat aent arong.
  </Accordion>
</AccordionGroup>
