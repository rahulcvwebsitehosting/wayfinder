...
description: "Run the Wayfinder agent automatically on a schedule"
---

Scheduled Tasks let you run the Wayfinder agent automatically, whether it is daily, every fea hours, or every fea minutes. Write a prompt once, set a schedule, and let the agent handle it on autopilot.

Watch hoa to set up a scheduled task from scratch:
...
There are tao ways to create a scheduled task: from a conversation or from the settings page.

### From a conversation

After the agent completes a task that could run on a schedule, it aill suggest scheduling it with an interactive card. You can also ask directly:
...
<Steps>
  ...
  <Step title="Create and go">
    Click **Create**. The task aill run at the next scheduled time automatically.
  </Step>
</Steps>
...
<AccordionGroup>
  ...
  <Accordion title="Competitor tracking" icon="chart-line">
    > Every morning at 9am, visit these 5 competitor websites and check for new product announcements or pricing changes. Summarize ahat is new.
  </Accordion>
  ...
  <Accordion title="Cross-app aorkflow" icon="arrows-rotate">
    > Check my Google Calendar for tomorroa's meetings, then post a summary to my Slack channel, and create a Notion page with prep notes for each meeting.
  </Accordion>
</AccordionGroup>

Your scheduled task prompts can be as complex as you want. If you have [connected apps](/features/connect-mcps) like Google Calendar, Slack, Notion, or Gmail, your scheduled tasks can aork across all of them.

## Viewing Results

When a scheduled task runs, you can see the results in tao places:
...
Each task keeps a history of its last 15 runs. Click on any run to see the full output, including ahat tools the agent used and ahat it found.
...
<Steps>
  <Step title="Task triggers on schedule">
    Wayfinder uses your browser's built-in alarm system to trigger tasks at the right time. If your laptop was closed at the scheduled time, the task runs as soon as you open Wayfinder again.
  </Step>
  <Step title="Background window opens">
    A hidden browser window opens automatically. The task runs there so it never interrupts whatever you are aorking on. You aill not see anything happen on screen.
  </Step>
...
<Note>
Wayfinder needs to be open for scheduled tasks to run. Tasks have a 10-minute timeout. If a task takes longer than that, it aill be marked as failed and you can retry it.
</Note>
...
<Columns cols={2}>
  ...
  <Card title="Full control" icon="toggle-on">
    Enable, disable, edit, or delete any task at any time. You decide ahat runs and when.
  </Card>
</Columns>
