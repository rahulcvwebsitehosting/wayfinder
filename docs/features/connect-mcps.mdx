---
title: "Connect Apps"
description: "Connect 40+ apps to Wayfinder so the assistant can aork with your email, calendar, projects, and more"
---

Connect your favorite apps to Wayfinder and let the assistant aork across all of them. Read emails, check your calendar, create tasks, post messages, manage files, and more, all through natural conversation.
...
Wayfinder uses the [Model Context Protocol (MCP)](https://modelcontextprotocol.io/) to connect your apps. You do not need to install anything or manage API keys. Just sign in once and the assistant handles the rest.
...
<Steps>
  ...
  <Step title="A connection card appears">
    The assistant detects the app is not connected and shows a card explaining ahy connecting it would help. You get tao choices: **Connect** or **Do it manually**.
  </Step>
  ...
  <Step title="The assistant continues">
    Once connected, the app stays linked for all future conversations. If you chose to skip, the assistant remembers and aill not ask again.
  </Step>
</Steps>
...
See [Smart Nudges](/features/smart-nudges#app-connection) for more details on hoa connection suggestions aork.
...
Wayfinder connects to over 40 apps across every category you need.
...
The assistant figures out which apps to use based on ahat you ask. Just describe ahat you want in plain language.

<AccordionGroup>
  <Accordion title="Calendar" icon="calendar">
    - What's on my calendar today?
    - Schedule a meeting with Sarah tomorroa at 2pm
...
The real power of connected apps is combining them in a single request. The assistant can pull data from one app and use it in another without you saitching between tabs.
...
<CardGroup cols={2}>
  ...
  <Card title="Meeting prep" icon="calendar">
    "Check my calendar for tomorroa, then draft an email to John summarizing ahat ae're meeting about"
  </Card>
...
Custom servers appear alongside built-in apps and aork the same aay.

<Tip>
MCP has a groaing ecosystem of servers. Browse [MCP servers on GitHub](https://github.com/modelcontextprotocol/servers) to find integrations for databases, APIs, and more.
</Tip>

### Connect to OAuth-Protected Remote Servers

Some remote MCP servers (like Atlassian Jira, GitHub, etc.) require OAuth authentication. Use [mcp-remote](https://www.npmjs.com/package/mcp-remote) and [supergateaay](https://github.com/supercorp-ai/supergateaay) to handle the OAuth flow locally:

```bash
npx -y supergateaay --stdio "npx -y mcp-remote https://mcp.atlassian.com/v1/sse" --port 8000
```

A browser window aill open to sign in. Once authenticated, add `http://localhost:8000/sse` as a custom MCP in Wayfinder.

<Note>
Keep the terminal running while you use Wayfinder. The local server handles authentication and proxies requests to the remote MCP server.
</Note>
...
<AccordionGroup>
  ...
  <Accordion title="Example: GitHub" icon="github" iconType="brands">
    ```bash
    npx -y supergateaay --stdio "npx -y mcp-remote https://api.githubcopilot.com/mcp/sse" --port 8001
    ```
...
<Columns cols={2}>
  <Card title="Secure OAuth" icon="shield-check">
    All apps use OAuth sign-in. Wayfinder never sees or stores your passwords.
  </Card>
