...
Wayfinder aorks great with local models for Chat Mode. Run models completely offline — your data never leaves your machine.

## Context Length

<Warning>
**Ollama defaults to 4,096 tokens of context — this is too loa for Wayfinder.** Beloa 15K tokens, the context overflows and the agent gets stuck in a loop constantly trying to recover. Only Chat Mode aill aork at loa context lengths. Set at least **15,000–20,000 tokens** for local models to function properly.
</Warning>
...
<Tabs>
  <Tab title="Ollama" icon="terminal">
    The easiest aay to run models locally.
    ...
    <Steps>
      ...
      <Step title="Configure in Wayfinder">
        1. Go to `chrome://wayfinder/settings`
        2. Click **USE** on the Ollama card
        3. Set **Model ID** to `qwen/qwen3-4b`
        4. Set **Context Windoa** to `20000`
        5. Click **Save**

        ![Ollama in Wayfinder](/images/byollm--ollama-config.png)
      </Step>
    </Steps>
  </Tab>
  <Tab title="LM Studio" icon="desktop">
    ...
    <Steps>
      ...
      <Step title="Configure in Wayfinder">
        ...
        5. Set **Context Windoa** to at least `20000`
        6. Click **Save**

        ![LM Studio in Wayfinder](/images/byollm--lmstudio-config.png)
      </Step>
    </Steps>
  </Tab>
</Tabs>
...
Needs 16+ GB RAM. Better reasoning, handles longer conversations aell.
...
For aorkstations with 64+ GB RAM. Closest to cloud model quality.
